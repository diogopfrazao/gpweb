...
class FixedClassLoader
{
  ...
  public function loadClass($className)
  {
    ...
    require $this->getFileName($className);
   ...
   *
   * @return FixedClassLoader The <tt>ClassLoader</tt> for the class or NULL if no such <tt>ClassLoader</tt> exists.
   */
  public static function getClassLoader($className)
  {
    foreach (spl_autoload_functions() as $loader) {
      if (is_array($loader)
          ...
          && $classLoader instanceof FixedClassLoader
  ...
  private function getFileName($className){
    $filePath = substr($className, strlen($this->namespace)+1);

    $filePath = ($this->includePath !== null ? $this->includePath . DIRECTORY_SEPARATOR : '')
                . str_replace($this->namespaceSeparator, DIRECTORY_SEPARATOR, $filePath)
                . $this->fileExtension;
