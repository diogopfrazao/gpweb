...
function fecharPopupExtJS(){
	if(window.parent.gpwebApp){
		var popups = window.parent.gpwebApp.popups;
...
function __buildTooltip(){
    if(!__gpweb__toolTip) return;

    var spans = $$('span');
    if(spans){
        spans.each(function(span){
            if(span.getAttribute('title')){
                if(typeof span.$tmp === 'undefined' || span.$tmp === null ){
                    span.$tmp = {};
                }
