...
*/
//todo: revisar para questões de segurança
die('Esta funcionalidade esta temporariamente indisponível.');
...
if ($config['militar']==11){
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Segmento', 'Selecione o segmento de atuação d'.$config['genero_organizacao'].' '.$config['organizacao'].'.').'*Segmento:'.dicaF().'</td><td>'.selecionaVetor($segmentos, 'segmento_id', 'class="texto" size=1 style="width:300px;"').'</td></tr>';

	}



echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nome Completo', 'Escreva seu nome completo.').'Nome completo:'.dicaF().'</td><td colspan="2"><input type="text" class="texto" name="contato_nomecompleto" value="'.(isset($usuario['contato_nomecompleto']) ? $usuario['contato_nomecompleto']:'').'" maxlength="255" size="40" /></td></tr>';

echo '<tr><td align="right" style="white-space: nowrap">'.dica('Identidade', 'Escreva a identidade.').($config['id_usuario_identidade'] ? '*' : '').'Identidade:'.dicaF().'</td><td><input type="text" class="texto" name="contato_identidade" value="'.(isset($usuario['contato_identidade']) ? $usuario['contato_identidade'] : '').'" maxlength="25" size="40" /></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('CPF', 'Escreva o CPF.').'CPF:'.dicaF().'</td><td><input type="text" class="texto" name="contato_cpf" value="'.(isset($usuario['contato_cpf']) ? $usuario['contato_cpf'] : '').'" maxlength="14" size="40" /></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Matrícula', 'Escreva a matrícula d'.$config['genero_usuario'].' '.$config['usuario'].', caso exista.').'Matrícula:'.dicaF().'</td><td><input type="text" class="texto" name="contato_matricula" value="'.(isset($usuario['contato_matricula']) ? $usuario['contato_matricula'] : '').'" maxlength="100" size="40" /> </td></tr>';
echo '<tr><td align="right">'.dica('Telefone', 'Escreva seu telefone de contato.').'Telefone:'.dicaF().'</td><td colspan="2"><input type="text" class="texto" name="contato_tel" value="'.(isset($usuario['contato_tel']) ? $usuario['contato_tel']:'').'" maxlength="50" size="33" /></td></tr>';
...
function enviar(){
  ...
  if (form.usuario_login.value.length < <?php echo config('tam_min_login'); ?>) {
	...
	<?php	if ($config['ativar_criacao_externa_cia']) echo ' else if (form.cia_nome.value.length < 3) {alert("'.ucfirst($config['genero_organizacao']).' '.$config['organizacao'] .' deverá ser preenchid'.$config['genero_organizacao'].'");form.cia_nome.focus();}';?>
  ...
  <?php	if ($config['id_usuario_identidade']) { ?>
	...
	else if (form.contato_email.value.length < 3) {
	...
	else if (form.contato_funcao.value.length < 3) {
