...
//na criação externa usuário sempre inativo inicialmente e não admin
$_REQUEST['usuario_admin'] = 0;
$_REQUEST['usuario_ativo'] = 0;
$_REQUEST['usuario_especial'] = 0;
$_REQUEST['usuario_login2'] = null;
$_REQUEST['usuario_senha2'] = null;
$_REQUEST['usuario_contas'] = null;


foreach($_REQUEST as $chave => $valor){
	$_REQUEST[$chave]=($valor || $valor=='0' || $valor===0 ? previnirXSS($valor) : null);
	}
...
if(isset($_REQUEST['cia_nome']) && !empty($_REQUEST['cia_nome'])){
	$sql->adTabela('cias');
	$sql->adCampo('cia_id');
	...
	if(!$cia_id && $config['ativar_criacao_externa_cia']){
		$sql->adTabela('cias');
		$sql->adCampo('cia_id');
	...
	if(!$cia_id){
        ver2(mb_strtoupper($config['genero_organizacao']) .' '. $config['organizacao']. ' informada é inválida.');
        echo '<script>document.env.submit();</script>';
        exit();
	}

    $contato->contato_cia = $cia_id;
...
if (($msg = $contato->armazenar())){
	...
	if (($msg = $usuario->armazenar()))	$Aplic->setMsg($msg, UI_MSG_ERRO);
		...
		if ($ehNovoUsuario){
			notificarNovoUsuarioExterno($contato->contato_email, $contato->contato_posto.' '.$contato->contato_nomeguerra, $usuario->usuario_login, $_REQUEST['usuario_senha']);
        	}
...
if (!$config['externo_ativo']){
	ver2('O Administrador já foi avisado para lhe permitir acessar o sistema'.($config['email_ativo'] ? ' e uma mensagem de e-mail foi enviada para você' :''));
}
		...
		if ($config['email_ativo']) {
			foreach($gerentes as $gerente) {
				if ($email->EmailValido($gerente['contato_email'])) {
					$email->Para($gerente['contato_email'], true);
					$qnt++;
					}
				}
			$email->Assunto($titulo);
