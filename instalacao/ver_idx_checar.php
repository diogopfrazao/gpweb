...
echo '<tr><td  width="300"><li>PHP &gt;= 5.5</li></td><td align="left">'.(version_compare(phpversion(), '5.5', '<') ? '<b class="error">'.$falhaImg.' ('.phpversion().'): o '.(isset($config['gpweb']) ? $config['gpweb'] : 'gpweb').' poderá não funcionar. Por favor faça uma atualização!</b>' : '<b class="ok">'.$okImg.'</b><span > ('.phpversion().')</span>').'</td></tr>';
...
echo '<tr><td ><li>Suporte a multi byte string (mbstring)</li></td><td align="left">'.(extension_loaded('mbstring') ? '<b class="ok">'.$okImg.'</b>' : '<b class="error">'.$falhaImg.'</b> O sistema requer que a extensão mbstring esteja instalada.').'</td></tr>';
