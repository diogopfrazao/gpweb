...
class  Cpdf {
  ...
  public function  o_info( $id, $action, $options = '') {
    ...
    case  'new':
      ...
      $this->objects[$id] = array('t'=>'info', 'info'=>array('Creator'=>'R and OS php pdf writer, http://www.ros.co.nz', 'CreationDate'=>$data));
   ...
   */
  public function  marcarTodasHere() {
  }
   ...
   */
  public function  setCor( $r, $g, $b, $force =  0) {
  ...
  public function  setLineStyle( $width =  1, $cap =  '', $join =  '', $dash =  '', $phase =  0) {
    ...
    // this is quite inefficient in that it sets all the parameters whenever 1 is changed, but will fix another dia
    $string =  '';
  ...
  /**
   * a filled rectangle, note that it is the width and height of the rectangle which are the segundoary paramaters, not
  ...
  /**
   * draw a rectangle, note that it is the width and height of the rectangle which are the segundoary paramaters, not
   * the coordinates of the upper-right corner
   */
  ...
  public function  newPage( $insert =  0, $id =  0, $pos =  'after') {
    ...
    if  ($this->currentColour['r'] >=  0) {
      ...
      $this->setCor($this->currentColour['r'], $this->currentColour['g'], $this->currentColour['b'], 1);
    }
          ...
          if  ($k !==  false &&  $text[$j] ==  ':') {
            ...
            if  (!isset($func) ||  !strlen(trim($func))) {
              ...
              if  ($final) {
                ...
                // for simplicity, just take from the end, fix this another dia
                $this->nCallback--;
  ...
  public function  getTextWidth( $size, $text, $spacing =  0) {
    ...
    $armazenar_currentTextState =  $this->currentTextState;
    ...
    $this->currentTextState =  $armazenar_currentTextState;
  ...
  public function  saveState( $pageEnd =  0) {
    ...
    if  ($pageEnd) {
      ...
      $this->setCor($opt['col']['r'], $opt['col']['g'], $opt['col']['b'], 1);
  ...
  public function  addPngFromFile( $file, $x, $y, $w =  0, $h =  0) {
    ...
    if  ( ($data =  file_get_contents($file)) ===  false) {
      ...
      $erromsg =  'trouble opening file: '.$file;
    }
    ...
    if  (!$erro) {
      ...
      if  (substr($data, 0, 8) !=  $header) {
        ...
        $erromsg =  'this file does not have a valid header';
      }
    }
    ...
    if  (!$erro) {
      ...
      while  ($p < $len) {
        ...
        case  'IHDR':
          ...
          if  ($info['compressionMethod'] !=  0) {
            ...
            $erromsg =  'unsupported compression method';
          }
          ...
          if  ($info['filterMethod'] !=  0) {
            ...
            $erromsg =  'unsupported filter method';
          }
      ...
      if (!$haveHeader) {
        ...
        $erromsg =  'information header is missing';
      }
      ...
      if  (isset($info['interlaceMethod']) &&  $info['interlaceMethod']) {
        ...
        $erromsg =  'There appears to be no support for interlaced images in pdf.';
      }
    }


    if  (!$erro &&  $info['bitDepth'] > 8) {

      $erro =  1;

      $erromsg =  'only bit depth of 8 or less is supported';
    }
    ...
    if  (!$erro) {
      ...
      if  ($info['colorType'] !=  2 &&  $info['colorType'] !=  0 &&  $info['colorType'] !=  3) {
        ...
        $erromsg =  'transparancey alpha channel not supported, transparency only supported for palette images.';
      } else {
    ...
    if  ($erro) {
      ...
      $this->addMessage('PNG error - ('.$file.') '.$erromsg);
