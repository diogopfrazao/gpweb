...
class Style {
   ...
   * @param float       $ref_size  an absolute reference size to resolve percentagem lengths
   * @return float
   */
  ...
  /**
   * Set inherited properties in this style using values in $superior
   *
   * @param Style $superior
   */
  public function inherit( Style $superior) {

    // Set parent font size
    $this->_parent_font_size = $superior->get_font_size();
    
    foreach (self::$_inherited as $prop) {
      if ( !isset($this->_props[$prop]) && isset($superior->_props[$prop]) ) 
        $this->_props[$prop] = $superior->_props[$prop];
    }
      
    foreach (array_keys($this->_props) as $prop) {
      if ( $this->_props[$prop] == "inherit" )
        $this->$prop = $superior->$prop;
    }
 ...
 /**#@+
  ...
  public function set_list_style( $val) {
    ...
    $tipos = array("disc", "circle", "square", "decimal",
