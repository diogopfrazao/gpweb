...
class Table_Frame_Reflower extends Frame_Reflower {
  ...
  protected function _assign_widths() {
    ...
    if ( $width > $min_width ) {
      ...
      //
      // 3. Only percentagem widths have been specified.  In this case we
      // normalize the percentagem values and distribute any remaining % to
      // width: auto columns.  We then proceed to assign widths as fractions
      // of the table width.
      //
      // 4. Both absolute and percentagem widths have been specified.  This
      // is annoying.

      // Case 1:
      if ( $absolute_used == 0 && $percentual_used == 0 ) {
      ...
      if ( $absolute_used == 0 && $percentual_used > 0 ) {
        ...
        // Scale percent values if the total percentagem is > 100, or if all
        // values are specified as percentagems.
        if ( $percentual_used > 100 || count($auto) == 0)
          $scale = 100 / $percentual_used;
        else
          $scale = 1;
      ...
      if ( $absolute_used > 0 && $percentual_used > 0 ) {
        ...
        // Scale percent values if the total percentagem is > 100 or there
