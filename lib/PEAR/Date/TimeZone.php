...
class Date_TimeZone
{
    ...
    public $shortname;
    /**
     * true if this time zone observes dialight savings time
     * @var boolean
     */
    public $hasdst;
    /**
    ...
    public $dstshortname;
    /**
     * offset, in millisegundos, of this timezone
     * @var int
     */
    public $offset;
    ...
    {
        global $default;
        if($res = Date_TimeZone::ehIdValido($id)) {
            $default = $id;
        }
    }
     ...
     */
    public static function ehIdValido( $id)
    {
        global $_DATE_TIMEZONE_DATA;
    ...
    /**
     * Returns true if this zone observes dialight savings time
     *
     * Returns true if this zone observes dialight savings time
     *
     * @access public
     ...
     *
     * @access public
     * @param object Date $data the date/time to test
     * @return boolean true if this date is in DST for this time zone
     */
    public function inDaylightTime( $data)
    {
	return date('I', $data->getData(DATE_FORMAT_UNIXTIME));
    }

    /**
     * Get the DST offset for this time zone
     *
     * Returns the DST offset of this time zone, in millisegundos,
     * if the zone observes DST, zero otherwise.  Currently the
     * DST offset is hard-coded to one hora.
     *
     * @access public
     * @return int the DST offset, in millisegundos or zero if the zone does not observe DST
     */
     ...
     * account dialight savings time, if the time zone observes it and if
     * it is in effect.  Please see the WARNINGS on Date::TimeZone::inDaylightTime().
     *
     *
     * @access public
     * @param object Date $data the Date to test
     * @return int the corrected offset to UTC in millisegundos
     */
    public function getOffset( $data)
    {
        if($this->inDaylightTime($data)) {
     ...
     *
     * @access public
     * @return string the dialight savings time long name
     */
     ...
     *
     * @access public
     * @return string the dialight savings time short name
     */
    public function getDSTNomeCurto()
    {
     ...
     *
     * @access public
     * @return int the offset, in millisegundos
     */
...
if(isset($_DATE_TIMEZONE_DEFAULT) 
    && Date_TimeZone::ehIdValido($_DATE_TIMEZONE_DEFAULT)
) {
    $resultado = Date_TimeZone::setDefault($_DATE_TIMEZONE_DEFAULT);
} elseif (getenv('PHP_TZ') && Date_TimeZone::ehIdValido(getenv('PHP_TZ'))) {
    $resultado = Date_TimeZone::setDefault(getenv('PHP_TZ'));
} elseif (getenv('TZ') && Date_TimeZone::ehIdValido(getenv('TZ'))) {
    $resultado = Date_TimeZone::setDefault(getenv('TZ'));
} elseif ($res = Date_TimeZone::ehIdValido(date('T'))) {
    $resultado = Date_TimeZone::setDefault(date('T'));
} else {
    $resultado = Date_TimeZone::setDefault('UTC');
}
//
// END
?>
