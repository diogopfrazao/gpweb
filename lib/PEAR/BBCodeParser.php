...
// +----------------------------------------------------------------------+
//
// $Id: BBCodeParser.php 25 2008-01-26 16:00:46Z pedroix $
//
...
class HTML_BBCodeParser
{
    ...
    public $_options = array(
        ...
        'close'         => ']',
        'xmlclose'      => true,
        'filters'       => array('Basic','Extended','Email','Lists','Links')
    );
     ...
     */
    public function setOption( $name, $valor)
    {
        $this->_options[$name] = $valor;
    }
     ...
     * @param string filter
     */
    public function addFilter( $filtro)
    {
    	global $Aplic;
        $filtro = ucfirst($filtro);
        if (!array_key_exists($filtro, $this->_filters)) {
            $classe = 'HTML_BBCodeParser_Filter_'.$filtro;
			include_once($Aplic->getClasseBiblioteca('PEAR/HTML/BBCodeParser/Filter/'.$filtro));
            if (!class_exists($classe)) {
                echo ("Failed to load filter $filtro");
            }
            $this->_filters[$filtro] = new $classe;
            $this->_definedTags = array_merge(
                $this->_definedTags,
                $this->_filters[$filtro]->_definedTags
            );
        }
    }
     ...
     * @param string $filtro
     */
    public function removeFilter( $filtro)
    {
        $filtro = ucfirst(trim($filtro));
        if (!empty($filtro) && array_key_exists($filtro, $this->_filters)) {
            unset($this->_filters[$filtro]);
        }
        // also remove the related $this->_definedTags for this filter,
        // preserving the others
        $this->_definedTags = array();
        foreach (array_keys($this->_filters) as $filtro) {
            $this->_definedTags = array_merge(
                $this->_definedTags,
                $this->_filters[$filtro]->_definedTags
            );
     ...
     */
    public function addFilters( $filtros)
    {
    ...
    public function _preparse()
    {
        ...
        foreach ($this->_filters as $filtro) {
            $filtro->setText($this->_preparsed);
            $filtro->_preparse();
            $this->_preparsed = $filtro->getPreparsed();
        }
    }
        ...
        foreach ($this->_tagArray as $tag) {
            ...
            case 0:
                if (($child = $this->_childNeeded(end($openTags), 'text')) &&
                     ...
                     * with it until someone relatorios a valid bug.
                     */
                    $child !== true )
                {
                    if (trim($tag['text']) == '') {
            ...
            case 1:
                if (!$this->_isAllowed(end($openTags), $tag['tag']) ||
                   ($superior = $this->_parentNeeded(end($openTags), $tag['tag'])) === true ||
                   ($child  = $this->_childNeeded(end($openTags),  $tag['tag'])) === true) {
                    ...
                    if ($superior) {
                        ...
                        if ($tag['tag'] == end($openTags)){
                            ...
                            $newTagArray[] = $superior;
                            $openTags[] = $superior['tag'];
                        }
                    }
                    if ($child) {
                        $newTagArray[] = $child;
        ...
        foreach ($this->_tagArray as $tag) {
            ...
            case 1:
                ...
                foreach ($tag['attributes'] as $a => $v) {
                    //prevento XSS attacks. IMHO this is not enough, though...
