...
//EndSkipDebug
	...
	*/
	public function printJavascript( $sJsURI="", $aJsFiles=array())
	{
		if (0 < strlen($sJsURI))
	...
	*/
	public function getJavascript( $sJsURI='', $aJsFiles=array())
	{
		ob_start();
	...
	*/
	...
	public function _compressSelf( $sFolder=null)
	{
		if (null == $sFolder)
	...
	public function _detectURI() {
		...
		if (!empty($_SERVER['REQUEST_URI'])) {
			...
			$_SERVER['REQUEST_URI'] = str_replace(
				array('"',"'",'<','>'), 
				array('%22','%27','%3C','%3E'), 
				previnirXSS($_SERVER['REQUEST_URI'])
				);
				
			$aURL = parse_url(previnirXSS($_SERVER['REQUEST_URI']));
		}

		// Fill in the empty values
		if (empty($aURL['scheme'])) {
			if (!empty($_SERVER['HTTP_SCHEME'])) {
				$aURL['scheme'] = previnirXSS($_SERVER['HTTP_SCHEME']);
			} else {
				$aURL['scheme'] = 
					(!empty($_SERVER['HTTPS']) && strtolower(previnirXSS($_SERVER['HTTPS'])) != 'off') 
					? 'https' 
					: 'http';
			}
		}
		...
		if (empty($aURL['host'])) {
			if (!empty($_SERVER['HTTP_X_FORWARDED_HOST'])) {
				if (strpos($_SERVER['HTTP_X_FORWARDED_HOST'], ':') > 0) {
					list($aURL['host'], $aURL['port']) = explode(':', previnirXSS($_SERVER['HTTP_X_FORWARDED_HOST']));
				} else {
					$aURL['host'] = previnirXSS($_SERVER['HTTP_X_FORWARDED_HOST']);
				}
			} else if (!empty($_SERVER['HTTP_HOST'])) {
				if (strpos($_SERVER['HTTP_HOST'], ':') > 0) {
					list($aURL['host'], $aURL['port']) = explode(':', previnirXSS($_SERVER['HTTP_HOST']));
				} else {
					$aURL['host'] = previnirXSS($_SERVER['HTTP_HOST']);
				}
			} else if (!empty($_SERVER['SERVER_NAME'])) {
				$aURL['host'] = previnirXSS($_SERVER['SERVER_NAME']);
			} else {
		...
		if (empty($aURL['port']) && !empty($_SERVER['SERVER_PORT'])) {
			$aURL['port'] = previnirXSS($_SERVER['SERVER_PORT']);
		}
		...
		if (empty($aURL['path'])) {
			...
			if (!empty($_SERVER['PATH_INFO'])) {
				$sPath = parse_url(previnirXSS($_SERVER['PATH_INFO']));
			} else {
				$sPath = parse_url(previnirXSS($_SERVER['PHP_SELF']));
			}
			if (isset($sPath['path']))
		...
		if (empty($aURL['query']) && !empty($_SERVER['QUERY_STRING'])) {
			$aURL['query'] = previnirXSS($_SERVER['QUERY_STRING']);
		}
	...
	*/
	...
	/*
		...
		deprecated : This function will be removed in future versions.  Please
			use <xajax->configure> instead.
	*/
	public function setDefaultMode( $sDefaultMode)
	{
	...
	*/
	...
	/*
		...
		deprecated : This function will be removed in future versions.  Please
			use <xajax->configure> instead.
	*/
	public function setDefaultMethod( $sMethod)
	{
	...
	*/
	...
	/*
		...
		deprecated : This function will be removed in future versions.  Please
			use <xajax->getConfiguration> instead.
	*/
	public function getWrapperPrefix()
	{
	...
	*/
	...
	/*
		...
		mFunction - (string or array):  The string containing the function name
			...
			- (string) The function as it is found in the class passed in the segundo
				parameter.
	...
	*/
	public function registerFunction( $mFunction, $sIncludeFile=null)
	{
	...
	/*
		...
		deprecated : This function will be removed in future versions.  Please
			use <xajax->register> instead.
	*/
	public function registerCallableObject( &$oObject)
	{
		$mResult = false;
	...
	/*
		...
		deprecated : This function will be removed in future versions.  Please
			use <xajax->register> instead.
	*/
	public function registerEvent( $sEventName, $mCallback)
	{
...
{
	$erroReporting = error_reporting();
	if (($errno & $erroReporting) == 0) return;
