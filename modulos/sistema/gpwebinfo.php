...
Verifica as configurações do servidor para a instalação e exibe os resultados
...
$botoesTitulo = new CBlocoTitulo('Instalação do '.$config['gpweb'].'', 'administracao.png', $m, $m.'.'.$a);
...
echo '<tr><td  width="300"><li>PHP &gt;= 5.2</li></td><td align="left">'.(version_compare(phpversion(), '5.2', '<') ? '<b class="error">'.$falhaImg.' ('.phpversion().'): o '.$config['gpweb'].' poderá não funcionar. Por favor faça uma atualização!</b>' : '<b class="ok">'.$okImg.'</b><span > ('.phpversion().')</span>').'</td></tr>';
...
echo '<tr><td ><li>MySQL</li></td><td align="left">'.($bdOk ? '<b class="ok">'.$okImg.'</b><span > '.$bdInfo.'</span>' : '<span class="warning">'.$falhaImg.' Não disponível</span>').'</td></tr>';
...
echo '<tr><td >Servidor Web suportado?</td><td align="left">'.((stristr(previnirXSS($_SERVER['SERVER_SOFTWARE']), 'apache') != false) ? '<b class="ok">'.$okImg.'</b><span > ('.previnirXSS($_SERVER['SERVER_SOFTWARE']).')</span>' : '<b class="error">'.$falhaImg.'</b><span class="warning">Aparentemente você está utilizando um Servidor Web não suportado pelo aplicativo.  Somente o servidor Apache é 100% suportado pelo '.$config['gpweb'].'.</span>').'</td></tr>';
echo '<tr><td >Navegador Web padrão?</td><td align="left">'.((stristr(previnirXSS($_SERVER['HTTP_USER_AGENT']), 'msie') == false) ? '<b class="ok">'.$okImg.'</b><span > ('.previnirXSS($_SERVER['HTTP_USER_AGENT']).')</span>' : '<b class="error">'.$falhaImg.'</b><span class="warning">Aparentemente você está utilizando o Internet Explorer.  Este navegador é conhecido por inúmeros problemas de segurança e não utiliza padrões internacionais para visualização de páginas. Considere a ideia de utilizar o navegador Firefox.</span>').'</td></tr>';
echo '</table>';
