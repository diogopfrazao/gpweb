...
if (!$dialogo || $selecao){
	if (!$selecao) $Aplic->salvarPosicao();
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionada', 'A que área a agenda coletiva está relacionada.').'Relacionada:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	$botoesTitulo = new CBlocoTitulo('Lista de Agendas Coletivas', 'agenda.png', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$nova_calendario=($podeAdicionar || $Aplic->usuario_admin ? '<tr><td style="white-space: nowrap" align=right>'.dica('Nova Agenda Coletiva'.'', 'Criar uma nova agenda coletiva.').'<a href="javascript: void(0)" onclick="url_passar(0,\'m=calendario&a=calendario_editar\');" ><img src="'.acharImagem('agenda_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir '.'Agenda Coletiva'.'', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de agenda coletiva.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&u='.$u.'&dialogo=1&tab='.$tab.'\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$procuraBuffer.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.(!$selecao ? $nova_calendario.$imprimir : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
