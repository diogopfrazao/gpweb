...
$permissao_tipo= getParam($_REQUEST, 'permissao_tipo', '');
$permissao_tipo=explode(',', $permissao_tipo);
...
echo '<tr><td align="right" style="white-space: nowrap" width="60">'.dica('Nome', 'Nome do perfil de acesso.').'Nome:'.dicaF().'</td><td class="realce" width="100%">'.$obj->perfil_nome.'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Descrição', 'Descrição do perfil de acesso.').'Descrição:'.dicaF().'</td><td class="realce" width="100%">'.$obj->perfil_descricao.'</td></tr>';
...
$modulos['log']='Registros de ocorrências';
...
echo '<tr>
<th>'.dica('Módulo', 'O módulo para o qual foram definidas quais funcionalidades foram abilitadas.').'Módulo'.dicaF().'</th>
<th style="white-space: nowrap">'.dica('Submódulo', 'Para certos módulos pode-se permitir o acesso a parte das funcionalidades dos mesmos.').'Submódulo'.dicaF().'</th>
<th style="white-space: nowrap">'.dica('Ação', 'Premitir ou negar as opções da função.').'Ação'.dicaF().'</th>
<th style="white-space: nowrap">'.dica('Função', 'Quais as funcionalidades dentro do módulo para as quais a opção terá efeito.').'Função'.dicaF().'</th>
<th>&nbsp;</th></tr>';
...
foreach ($permissoes as $linha) {
	...
	echo '<td style="white-space: nowrap" width=50>'.(isset($leganda_submodulo[$linha['perfil_acesso_objeto']]) ? $leganda_submodulo[$linha['perfil_acesso_objeto']] : '&nbsp;').'</td>';
	echo '<td style="white-space: nowrap" width=50>'.($linha['perfil_acesso_negar'] ? 'Negar' : 'Permitir').'</td>';
	$opcoes=array();
	...
	echo '<td style="white-space: nowrap" width=50>'.implode('</br>',$opcoes).'</td>'; 	
	echo '<td width=16>'.dica('Excluir', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir esta configuração de acesso às funcionalidades do módulo.').'<a href="javascript:excluir('.$linha['perfil_acesso_id'].');" >'.imagem('icones/remover.png').'</a></td></tr>';
	}
echo '</table></td>';
...
if ($podeEditar) { 
	...
	echo '<tr><td colspan="2" align="center">'.dica('Adicionar Permissões', 'Escolha nas opções abaixo para qual módulo deseja permitir ou negar uma das seguintes opções:<ul><li>Acessar</li><li>Visualizar</li><li>Adicionar</li><li>Editar</li><li>Excluir</li></ul>').'<b>Adicionar Permissões</b>'.dicaF().'</td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Módulo', 'Cada uma das funcionalidades do sistema está contida em um módulo.').'Módulo:'.dicaF().'</td><td width="100%">'.selecionaVetor($modulos, 'permissao_modulo', 'size="1" style="width:200px;" class="texto" onchange="mudar_submodulo(this.value);"', $permissao_modulo).'</td></tr>';
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Submódulo', 'Para certos módulos pode-se permitir o acesso a parte das funcionalidades dos mesmos.').'Submódulo:'.dicaF().'</td><td width="100%"><div id="combo_submodulo">'.selecionaVetor($vetor, 'permissao_submodulo', 'size="1" style="width:200px;" class="texto"', $permissao_submodulo).'</div></td></tr>';
	$vetor1=array(0 => 'Permitir', 1 => 'Negar');
	
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Ação', 'As opções abaixo são para serem peritidas ou negadas.').'Ação:'.dicaF().'</td><td width="100%">'.selecionaVetor($vetor1, 'perfil_acesso_negar', 'size="1" style="width:200px;" class="texto"', $perfil_acesso_negar).'</td></tr>';
	
	foreach ($permissao_lista as $perm_id => $perm_nome) echo '<tr><td align="right" style="white-space: nowrap">'.dica($perm_nome, 'Marque a caixa à direita caso deseje configurar a opção de '.$perm_nome.' dentro do módulo selecionado.').$perm_nome.':'.dicaF().'</td><td><input type="checkbox" name="permissao_tipo[]" value="'.$perm_id.'" '.(in_array($perm_id, $permissao_tipo) ? 'checked="checked"' : '').'/></td></tr>';
