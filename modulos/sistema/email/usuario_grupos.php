...
if ($usuario_id){
	...
	$sql->adCampo('grupo_usuario_grupo');
	$sql->adOnde('grupo_usuario_usuario='.$usuario_id);
	$grupos_pertencentes = $sql->ListaChave('grupo_usuario_grupo');
	$sql->limpar();
	
	$sql->adTabela('grupo');
	$sql->adTabela('usuarios');
	$sql->esqUnir('grupo_usuario', 'grupo_usuario', 'grupo_usuario_usuario = usuarios.usuario_id AND grupo.grupo_id=grupo_usuario_grupo');
	
	$sql->esqUnir('grupo_permissao', 'grupo_permissao', 'grupo_permissao_usuario = usuarios.usuario_id AND grupo.grupo_id=grupo_permissao_grupo');
	$sql->esqUnir('cias', 'cias', 'grupo_cia = cias.cia_id');
	$sql->adCampo('DISTINCT grupo.grupo_id, grupo_descricao, (SELECT COUNT(grupo_permissao_usuario) FROM grupo_permissao AS gp1 WHERE gp1.grupo_permissao_grupo=grupo.grupo_id) AS protegido, (SELECT COUNT(grupo_permissao_usuario) FROM grupo_permissao AS gp2 WHERE gp2.grupo_permissao_grupo=grupo.grupo_id AND gp2.grupo_permissao_usuario='.$usuario_id.') AS pertence, grupo_cia, cia_nome');
	$sql->adOnde('grupo_usuario=0 OR grupo_usuario IS NULL');
	$sql->adOnde('grupo_usuario_usuario='.$usuario_id.' OR grupo_permissao_usuario='.$usuario_id);
