...
$html_tipos = array('textinput' => 'Linha de texto', 'textarea' => 'Área de texto', 'checkbox' => 'Caixa de seleção', 'selecionar' => 'Lista de seleção', 'label' => 'Legenda', 'separator' => 'linha horizontal', 'href' => 'Link para Web');
...
$edit_campo_id=getParam($_REQUEST, 'campo_customizado_id', null);
...
$modulos[]=array('mod_nome' => ucfirst($config['patrocinador']), 'mod_diretorio' => 'patrocinadores');
...
if ($Aplic->profissional) $modulos[]=array('mod_nome' => 'Slideshow de composições', 'mod_diretorio' => 'painel_slideshow');
...
$modulos[]=array('mod_nome' => ucfirst($config['licao']), 'mod_diretorio' => 'licao_aprendida');
...
foreach ($modulos as $modulo) {
	...
	$sql->adOnde('campo_customizado_modulo = \''.strtolower($modulo['mod_diretorio']).'\'');
	...
	if (count($campos_customizados)) {
		...
		echo '<th>Tipo</th>';
		echo '<th>Posição</th>';
		echo '<th>Habilitado</th></tr>';
		foreach ($campos_customizados as $f) {
			echo '<tr align="center"><td width="32">'.dica ('Editar Campo Customizado','Clique neste ícone '.imagem('icones/editar.gif').'para editar este campo customizado').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=campo_customizado_editar&modulo='.$modulo['mod_diretorio'].'&nome_modulo='.$modulo['mod_nome'].'&campo_customizado_id='.$f['campo_customizado_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF();
			echo dica ('Excluir','Clique neste icone '.imagem('icones/remover.png').' para excluir este campo customizado').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=campo_customizado_editar&campo_customizado_id='.$f['campo_customizado_id'].'&excluir=1\');">'.imagem('icones/remover.png').'</a></td>';
			echo '<td>'.stripslashes($f['campo_customizado_nome']).'</td>';
			echo '<td>'.stripslashes($f['campo_customizado_descricao']).'</td>';
			echo '<td>'.$html_tipos[$f['campo_customizado_tipo_html']].'</td>';
			echo '<td width=50>'.stripslashes($f['campo_customizado_ordem']).'</td>';
			
			
			
			echo '<td width=50><input type="checkbox" value="1" name="'.$f['campo_customizado_id'].'" '.($f['campo_customizado_habilitado'] ? 'checked="checked"' : '').' onchange="marcar(this);" /></td>';
...
function marcar(obj){
	xajax_marcar(obj.name, obj.checked);	
	}	
