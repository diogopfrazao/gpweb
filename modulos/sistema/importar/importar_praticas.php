...
if ($importar){
    ...
    if(!$abortar){
        ...
        if ($lista_plano_gestao){		
            ...
            $sql->adOnde('pg_swot_pg IN ('.$lista_plano_gestao.')');
            $sql->exec();
            $sql->limpar(); 
        ...
        if($lista_tarefas){
            ...
            $sql->setExcluir('log');
            $sql->adOnde('log_tarefa IN ('.$lista_tarefas.')');
            $sql->exec();
            $sql->limpar();	
        }
...
if (!$abortar && $importar_parte1){
  ...
  while ($buffer !=FALSE) {
	...
	importar_tabela('pg_swot', 'pg_swot_id', 'pg_swot_pg','pg_swot_usuario');	
  ...
  while ($buffer !=FALSE) {
	...
	if (isset($lista_tabelas['praticas'])){
		foreach($lista_tabelas['praticas'] as $linha_tabela){
			$sql->adTabela('praticas');
			foreach($linha_tabela as $chave => $valor) if ($chave!='pratica_id'  && $chave!='pratica_cia') $sql->adInserir($chave, $valor);
...
if (!$abortar && $importar_parte3){
  ...
  while ($buffer !=FALSE) {
	...
	importar_tabela('recurso_tarefa', '', '', '', '', '', '', '', '', 'recurso_tarefa_tarefa', 'recurso_tarefa_recurso');
	...
	importar_tabela('log', 'log_id', '', 'log_criador', '', '', '', '', '', 'log_tarefa');
