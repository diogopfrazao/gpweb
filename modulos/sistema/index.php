...
if($Aplic->profissional && $Aplic->usuario_super_admin) echo '<tr><td align="left">'.dica('Priorização', 'Abre interface para criação de perguntas para priorização de objeto do sistema.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=priorizacao_pro\');">Priorização</a>'.dicaF().'</td></tr>';
...
if($Aplic->profissional && $Aplic->usuario_super_admin) echo '<tr><td align="left">'.dica('Importar Contatos do LDAP', 'Interface para importar contatos de um servidor LDAP.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=contatos_ldap_pro\');">Importar contatos via LDAP</a>'.dicaF().'</td></tr>';
if($Aplic->profissional && $Aplic->usuario_super_admin) echo '<tr><td align="left">'.dica('Importar Usuários do LDAP', 'Interface para importar usuários de um servidor LDAP.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=usuarios_ldap_pro\');">Importar usuários via LDAP</a>'.dicaF().'</td></tr>';
if($Aplic->usuario_super_admin) echo '<tr><td align="left">'.dica('Importar do MS Project ou WBS Chart Pro', 'Abre uma janela onde se pode importar projeto do MS Project ou do WBS Chart Pro.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=index&u=importar\');">Importar do MS Project ou WBS Chart Pro</a>'.dicaF().'</td></tr>';
...
if($Aplic->usuario_super_admin){
	echo '<tr><td width="42" valign="top">'.imagem('projeto.png','','',1).'</td><td><table><tr><td align="left" style="color:#666;margin-bottom:0;	margin-top:15px; font:700 12px verdana, arial, helvetica, sans-serif">Artefatos Gerais & '.ucfirst($config['projeto']).'</td></tr>';
	...
	if ($Aplic->profissional) {
		...
		echo '<tr><td align="left">'.dica('Salvar Baselines', 'Criação automática de baseline de todos os projetos ativos.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&u=projeto&a=salvar_baselines_pro\');">Salvar baselines</a>'.dicaF().'</td></tr>';
...
if($Aplic->profissional && $Aplic->usuario_super_admin) echo '<tr><td align="left">'.dica('Limpar Caixa de Entrada', 'Limpar caixa de entrada de mensagens do sistema.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&u=email&a=limpar_email_pro\');">Limpar caixa de entrada</a>'.dicaF().'</td></tr>';

echo '</table></td></tr>';


echo '<tr><td width="42" valign="top">'.imagem('agenda.png','','',1).'</td><td><table><tr><td align="left" style="color:#666;margin-bottom:0;	margin-top:15px; font:700 12px verdana, arial, helvetica, sans-serif">Agenda</td></tr>';
...
if ($Aplic->profissional && $Aplic->usuario_super_admin && $Aplic->modulo_ativo('financeiro')){
	
	//checar se é sema
	$sql = new BDConsulta;
	$sql->adTabela('financeiro_config');
	$sql->adCampo('financeiro_config_campo, financeiro_config_valor');
	$configuracao = $sql->listaVetorChave('financeiro_config_campo','financeiro_config_valor');
	$sql->limpar();
	if ($configuracao['organizacao']=='sema_mt'){
		echo '<tr><td width="42" valign="top">'.imagem('../../../modulos/financeiro/imagens/financeiro.png','','',1).'</td><td><table><tr><td align="left" style="color:#666;margin-bottom:0;	margin-top:15px; font:700 12px verdana, arial, helvetica, sans-serif">'.$config['siafi'].'</td></tr>';
		echo '<tr><td align="left">'.dica('Importação Automática d'.$config['genero_siafi'].' '.$config['siafi'], 'Importação automática d'.$config['genero_siafi'].' '.$config['siafi'].' de  notas de empenho, notas de liquidação e ordens bancárias.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=importacao_fiplan_pro\');">Importação automática d'.$config['genero_siafi'].' '.$config['siafi'].'</a>'.dicaF().'</td></tr>';
		echo '<tr><td align="left">'.dica('Inconsistências de Importações n'.$config['genero_projeto'].'s '.ucfirst($config['projetos']), 'Relatório de inconsistências de importações advindas d'.$config['genero_siafi'].' '.$config['siafi'].' n'.$config['genero_projeto'].'s '.$config['projetos'].'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=problem_fiplan_pro\');">Inconsistência de importações n'.$config['genero_projeto'].'s '.$config['projetos'].'</a>'.dicaF().'</td></tr>';
		
		echo '<tr><td align="left">'.dica('Super Alocação de Notas de Empenho em '.ucfirst($config['projetos']), 'Relatório de super alocação de notas de empenho em '.ucfirst($config['projetos']).'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=solucionar_problema_fiplan_pro&modulo=projeto&tipo=ne\');">Super alocação de notas de empenho em '.strtolower($config['projetos']).'</a>'.dicaF().'</td></tr>';
		echo '<tr><td align="left">'.dica('Super Alocação de Notas de Liquidação em '.ucfirst($config['projetos']), 'Relatório de super alocação de notas de liquidação em '.ucfirst($config['projetos']).'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=solucionar_problema_fiplan_pro&modulo=projeto&tipo=ns\');">Super alocação de notas de liquidação em '.strtolower($config['projetos']).'</a>'.dicaF().'</td></tr>';
		echo '<tr><td align="left">'.dica('Super Alocação de Notas de Ordens Bancarias em '.ucfirst($config['projetos']), 'Relatório de super alocação de notas de ordem bancária em '.ucfirst($config['projetos']).'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=solucionar_problema_fiplan_pro&modulo=projeto&tipo=ob\');">Super alocação de notas de ordem bancária em '.strtolower($config['projetos']).'</a>'.dicaF().'</td></tr>';
		
		echo '<tr><td align="left">'.dica('Super Alocação de Notas de Empenho em '.ucfirst($config['instrumentos']), 'Relatório de super alocação de notas de empenho em '.ucfirst($config['instrumentos']).'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=solucionar_problema_fiplan_pro&modulo=instrumento&tipo=ne\');">Super alocação de notas de empenho em '.strtolower($config['instrumentos']).'</a>'.dicaF().'</td></tr>';
		echo '<tr><td align="left">'.dica('Super Alocação de Notas de Liquidação em '.ucfirst($config['instrumentos']), 'Relatório de super alocação de notas de liquidação em '.ucfirst($config['instrumentos']).'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=solucionar_problema_fiplan_pro&modulo=instrumento&tipo=ns\');">Super alocação de notas de liquidação em '.strtolower($config['instrumentos']).'</a>'.dicaF().'</td></tr>';
		echo '<tr><td align="left">'.dica('Super Alocação de Notas de Ordens Bancarias em '.ucfirst($config['instrumentos']), 'Relatório de super alocação de notas de ordem bancária em '.ucfirst($config['instrumentos']).'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=solucionar_problema_fiplan_pro&modulo=instrumento&tipo=ob\');">Super alocação de notas de ordem bancária em '.strtolower($config['instrumentos']).'</a>'.dicaF().'</td></tr>';
		
		echo '<tr><td align="left">'.dica('Super Alocação de Notas de Empenho em '.ucfirst($config['trs']), 'Relatório de super alocação de notas de empenho em '.ucfirst($config['trs']).'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=solucionar_problema_fiplan_pro&modulo=tr&tipo=ne\');">Super alocação de notas de empenho em '.strtolower($config['trs']).'</a>'.dicaF().'</td></tr>';
		echo '<tr><td align="left">'.dica('Super Alocação de Notas de Liquidação em '.ucfirst($config['trs']), 'Relatório de super alocação de notas de liquidação em '.ucfirst($config['trs']).'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=solucionar_problema_fiplan_pro&modulo=tr&tipo=ns\');">Super alocação de notas de liquidação em '.strtolower($config['trs']).'</a>'.dicaF().'</td></tr>';
		echo '<tr><td align="left">'.dica('Super Alocação de Notas de Ordens Bancarias em '.ucfirst($config['trs']), 'Relatório de super alocação de notas de ordem bancária em '.ucfirst($config['trs']).'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=solucionar_problema_fiplan_pro&modulo=tr&tipo=ob\');">Super alocação de notas de ordem bancária em '.strtolower($config['trs']).'</a>'.dicaF().'</td></tr>';
		
		echo '<tr><td align="left">'.dica('Super Alocação de Notas de Empenho em '.ucfirst($config['oss']), 'Relatório de super alocação de notas de empenho em '.ucfirst($config['oss']).'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=solucionar_problema_fiplan_pro&modulo=os&tipo=ne\');">Super alocação de notas de empenho em '.strtolower($config['oss']).'</a>'.dicaF().'</td></tr>';
		echo '<tr><td align="left">'.dica('Super Alocação de Notas de Liquidação em '.ucfirst($config['oss']), 'Relatório de super alocação de notas de liquidação em '.ucfirst($config['oss']).'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=solucionar_problema_fiplan_pro&modulo=os&tipo=ns\');">Super alocação de notas de liquidação em '.strtolower($config['oss']).'</a>'.dicaF().'</td></tr>';
		echo '<tr><td align="left">'.dica('Super Alocação de Notas de Ordens Bancarias em '.ucfirst($config['oss']), 'Relatório de super alocação de notas de ordem bancária em '.ucfirst($config['oss']).'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&a=solucionar_problema_fiplan_pro&modulo=os&tipo=ob\');">Super alocação de notas de ordem bancária em '.strtolower($config['oss']).'</a>'.dicaF().'</td></tr>';
...
//historico
if ($Aplic->profissional && $Aplic->usuario_super_admin && $config['registrar_mudancas']){
	echo '<tr><td width="42" valign="top">'.imagem('historico.png','','',1).'</td><td><table><tr><td align="left" style="color:#666;margin-bottom:0;	margin-top:15px; font:700 12px verdana, arial, helvetica, sans-serif">Historico</td></tr>';
	echo '<tr><td align="left">'.dica('Limpar Histórico', 'Entrar na tela de escolha de período fo histórico a ser excluído.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=sistema&u=historico&a=historico_limpar_pro\');">Limpar Histórico</a>'.dicaF().'</td></tr>';
	echo '</table></td></tr>';
	}
