...
if (!$dialogo){
	$botoesTitulo = new CBlocoTitulo('Compromissos Diário', 'compromisso.png', $m, $m.'.'.$a);
	$botoesTitulo->adicionaBotao('m=email&a=ver_ano&data='.$este_dia->format('%Y%m%d'), 'visão anual','','Visão Anual','Visualizar o ano inteiro.');
	$botoesTitulo->adicionaBotao('m=email&a=ver_mes&data='.$este_dia->format('%Y%m%d'), 'visão mensal','','Visão Mensal','Visualizar o mês inteiro.');
	...
	echo '<tr><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=email&a=ver_dia&data='.$dia_ant->format('%Y%m%d').'\');">'.imagem('icones/'.($estilo_interface=='metro' ? 'navAnterior_metro.png' :'anterior.gif'), 'Dia Anterior', 'Clique neste ícone '.imagem('icones/'.($estilo_interface=='metro' ? 'navAnterior_metro.png' :'anterior.gif')).' para exibir o dia anterior.').'</a></td><th width="100%">'.$titulo.'</th><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=email&a=ver_dia&data='.$dia_prox->format('%Y%m%d').'\');">'.imagem('icones/'.($estilo_interface=='metro' ? 'navProximo_metro.png' :'proximo.gif'), 'Próximo Dia', 'Clique neste ícone '.imagem('icones/'.($estilo_interface=='metro' ? 'navProximo_metro.png' :'proximo.gif')).' para exibir o próximo dia.').'</a></td></tr>';
	echo '</table>';
	
	$caixaTab = new CTabBox('m=email&a=ver_dia&data='.$este_dia->format('%Y%m%d'), BASE_DIR.'/modulos/email/', $tab);
