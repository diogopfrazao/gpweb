...
echo '<tr>';
echo '<td width="20" align="center" style="white-space: nowrap;font-size:10pt;font-weight:Bold; padding-left: 5px; padding-right: 5px;'.($config['msg_precedencia'] ? '' : ' display:none;').'">'.dica('Precedência','A precedência diz respeito à urgencia na leitura d'.$config['genero_mensagem'].' '.$config['mensagem'].'.').'Precedência'.dicaF().'</td>';
echo '<td width="20" align="center" style="white-space: nowrap;font-size:10pt;font-weight:Bold; padding-left: 5px; padding-right: 5px;'.($config['msg_class_sigilosa'] ? '' : ' display:none;').'">'.dica('Classificação Sigilosa','A classificação sigilosa serve para restringir quais destinatários podem ler '.$config['genero_mensagem'].' '.$config['mensagem'].'.').'Class&nbsp;Sigilosa'.dicaF().'</td>';
...
if (count($lista)) {
	echo '<tr><td style="font-size:10pt; padding-left: 5px; padding-right: 5px;"><b>Relacionamento:</b></td><td colspan=20>';
...
foreach ($anexos as $rs_anexo){ 
	...
	if ($quantidade_anexos==1) {
		echo '<table rules="ALL" border="1" cellspacing=0 width=100% cellpadding=0 align="center"><tr><td>';
		echo '<table align="center" cellspacing=0 width=100% cellpadding=0>';
...
function formata_destinatario($rs_para=array()){
	...
	if ($rs_para['copia_oculta'] !=1 || $rs_para['de_id']==$Aplic->usuario_id || $rs_para['para_id']==$Aplic->usuario_id  || $Aplic->usuario_acesso_email > 3){
		if (!$rs_para['datahora_leitura']) $saida.= ' - não lida'; 
