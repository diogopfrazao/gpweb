...
if ($modelo_id){
	...
	$sql->esqUnir('usuarios','usuarios','usuarios.usuario_id=modelo_leitura.usuario_id');
	...
	$sql->esqUnir('usuarios','usuarios','usuarios.usuario_id=anexo_leitura.usuario_id');
	$sql->esqUnir('contatos', 'contatos', 'contato_id = usuario_contato');
	$sql->adCampo(($config['militar'] < 10 ? 'concatenar_tres(contato_posto, \' \', contato_nomeguerra)' : 'contato_nomeguerra').' AS nome_usuario, datahora_leitura');
	$sql->adOnde('anexo_id = '.$anexo_id);	
	$sql->adOnde('download=1');
	}
$linhas = $sql->Lista();
