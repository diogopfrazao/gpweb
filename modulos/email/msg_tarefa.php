...
global $m, $a, $u, $estilo_interface, $localidade_tipo_caract, $onde, $dialogo, $tab;
...
mostrarBarraNav($xtotalregistros, $xtamanhoPagina, $xtotal_paginas, $pagina, ucfirst($config['mensagem']), ucfirst($config['mensagens']),'','&ordenar='.$ordenar.'&ordem='.$ordem, ($estilo_interface=='classico' ? 'a6a6a6' : '006fc2'));

echo '<table cellpadding=2 cellspacing=0 width="100%" class="tbl1">';
echo '<tr>';
echo '<th style="white-space: nowrap">'.dica('Marcar Todos', 'Clique nesta caixa de opção para marcar todos os problemas da lista abaixo.').'<input type="checkbox" value="1" name="todos" id="todos" onclick="marcar_todos();" />'.dicaF().'</th>';
echo '<th align="left"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar='.($tab==1 || $tab==3 || $tab==5 ? 'ignorar_para' : 'ignorar_de').'&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar==($tab==1 || $tab==3 || $tab==5 ? 'ignorar_para' : 'ignorar_de') ? imagem('icones/'.$seta[$ordem]) : '').dica('Ignorada', 'Clique para ordenar '.$config['genero_mensagem'].'s '.$config['mensagens'].' que foram ignoradas pelo '.($tab==1 || $tab==3 || $tab==5 ? 'destinatário' : 'remetente').'.').'<b>I</b>'.dicaF().'</a></th>';

echo '<th align="left"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=datahora&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='datahora' ? imagem('icones/'.$seta[$ordem]) : '').dica('Data de Envio', 'Clique para ordenar '.$config['genero_mensagem'].'s '.$config['mensagens'].' do tipo atividade pela data em que foram enviad'.$config['genero_mensagem'].'s.').'<b>Data</b>'.dicaF().'</a></th>';
echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=data_limite&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='data_limite' ? imagem('icones/'.$seta[$ordem]) : '').dica('Prazo', 'Clique para ordenar '.$config['genero_mensagem'].'s '.$config['mensagens'].' do tipo atividade pelo prazo limite de execução.').'<b>Prazo</b>'.dicaF().'</a></th>';
echo ($tab==1 || $tab==3 || $tab==5 ? '<th><b>Destinatários</b></th>' : '<th><b>Remetente</b></th>' );
echo '<th><a class="hdr" href="index.php?m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=msg.msg_id&ordem='.($ordem ? '0' : '1').'">'.($ordenar=='msg.msg_id' ? imagem('icones/'.$seta[$ordem]) : '').dica('Número d'.$config['genero_mensagem'].' '.ucfirst($config['mensagem']), 'Clique para ordenar '.$config['genero_mensagem'].'s '.$config['mensagens'].' do tipo atividade pelo número d'.$config['genero_mensagem'].'s mesm'.$config['genero_mensagem'].'s.').'<b>Nr</b>'.dicaF().'</a></th>';
echo '<th><a class="hdr" href="index.php?m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=referencia&ordem='.($ordem ? '0' : '1').'">'.($ordenar=='referencia' ? imagem('icones/'.$seta[$ordem]) : '').dica(''.ucfirst($config['mensagem']), 'Clique para ordenar '.$config['genero_mensagem'].'s '.$config['mensagens'].' do tipo atividade pelo assunto d'.$config['genero_mensagem'].'s '.$config['mensagens'].'.').'<b>'.ucfirst($config['mensagem']).'</b>'.dicaF().'</a></th>';
echo '<th><a class="hdr" href="index.php?m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=tarefa_progresso&ordem='.($ordem ? '0' : '1').'">'.($ordenar=='tarefa_progresso' ? imagem('icones/'.$seta[$ordem]) : '').dica('Percentagem', 'Clique para ordenar '.$config['genero_mensagem'].'s '.$config['mensagens'].' do tipo atividade pela percentagem executada.').'<b>Percentagem</b>'.dicaF().'</a></th>';
echo '<th></th>';
echo '</tr>';
$qnt=0;
$agora=date('Y-m-d');
for ($i = 0; $i < count($tarefas); $i++) {
	...
	$qnt++;
	echo '<tr>';
	echo '<td style="white-space: nowrap" width="20">'.dica('Marcar para Exportar', 'Clique nesta caixa para marcar este lote para nova exportação.').'<input type="checkbox" value="'.$linha['msg_usuario_id'].'" name="ignorado[]" /></td>';
...
$legenda.='<td style="white-space: nowrap;border-style:solid;border-width:1px; background: #ffffff;">&nbsp; &nbsp;</td><td style="white-space: nowrap">'.dica('Atividade Iniciada e Dentro do Prazo', 'Atividade que a data limite ainda ainda não passou e já se encontra em execução.').'&nbsp;Iniciada e dentro do prazo'.dicaF().'</td><td>&nbsp;&nbsp;&nbsp;&nbsp;</td>';
$legenda.='<td style="white-space: nowrap;border-style:solid;border-width:1px; background: #ffeebb;">&nbsp; &nbsp;</td><td style="white-space: nowrap">'.dica('Atividade que Deveria ter Iniciada', 'Atividade que ainda se encontra em 0% executada.').'&nbsp;Deveria ter iniciada'.dicaF().'</td><td>&nbsp;&nbsp;&nbsp;&nbsp;</td>';
$legenda.='<td style="white-space: nowrap;border-style:solid;border-width:1px; background: #cc6666;">&nbsp; &nbsp;</td><td style="white-space: nowrap">'.dica('Atividade Atrasada', 'Atividade que a data de término da mesma já ocorreu, entretanto ainda não se encontra em 100% executada.').'&nbsp;Atrasada'.dicaF().'</td><td>&nbsp;&nbsp;&nbsp;&nbsp;</td>';
