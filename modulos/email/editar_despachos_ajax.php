...
function atualizar_despachos($despacho_usuario){
	...
	if (count($despachos)) {
		...
		foreach ($despachos as $despacho_id => $linha) {
			...
			$saida.= '<td width=32><a href="javascript: void(0);" onclick="editar_despacho('.$linha['despacho_id'].');">'.imagem('icones/editar.gif', 'Editar', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar a despacho.').'</a>';
			$saida.= '<a href="javascript: void(0);" onclick="if (confirm(\'Tem certeza que deseja excluir esta despacho?\')) {excluir_despacho('.$linha['despacho_id'].');}">'.imagem('icones/remover.png', 'Excluir', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir esta despacho.').'</a></td>';
			$saida.= '</tr>';
			}
		$saida.= '</table>';
		}
