...
if ($enviar_email){
	...
	foreach ((array)$vetor_msg_usuario as $chave => $valor){
		...
		if (false && function_exists("curl_init")) {
			...
			curl_setopt ($sessao_Curl[$qnt], CURLOPT_POSTFIELDS, 'destinatarios='.implode(',',$destinatarios).'&de_id='.$Aplic->usuario_id.'&msg_usuario_id='.$valor);
			...
			$sql->adOnde('msg_usuario_id = '.(int)$valor);
			$titulo = $sql->Resultado();
			$sql->limpar();
			$texto=texto_msg_email($valor, $status, $Aplic->usuario_id);
...
echo '<input type=hidden name="m" value="email">';
echo '<input type=hidden name="a" value="envia_email">';
...
echo '<tr><td align="right" width="60" size="2"><b>De:&nbsp;</b></td><td>'.nome_funcao($Aplic->usuario_nome, '', $Aplic->usuario_funcao).'</td></tr>';
...
foreach ($sql_resultados as $rs){			
		if (!($rs['contato_email'] || $rs['contato_email2'])) $aviso[]=$rs['nome_usuario'];	
			...
			echo nome_funcao($rs['nome_usuario'], '', $rs['contato_funcao']).'['.$rs['contato_email'].']<br>';	
			$qnt++;
			}
		};
if (!$qnt) echo 'Não há '.$config['usuarios'].' com e-mail externo cadastrado';		
echo '<br></td></tr>';
...
if (count($vetor_msg_usuario)) echo '<tr><td colspan=2 align="center">Encaminhamento por e-mail para '.relacao_mensagens().'</td></tr><tr><td colspan=2 align="center">&nbsp</td></tr>';
echo '</table>';
