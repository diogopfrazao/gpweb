...
foreach ($convites as $convite) {
	...
	echo '<td>'.link_usuario($convite['agenda_dono']).'</td>';
	...
	echo '<td style="white-space: nowrap">'.$recorrencia[$convite['agenda_recorrencias']].($convite['agenda_recorrencias'] ? ' ('.$convite['agenda_nr_recorrencias'].' vez'.((int)$convite['agenda_nr_recorrencias'] > 1 ? 'es':''). ')' : '').'</td>';
	echo '</tr>';
	}
