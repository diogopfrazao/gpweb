...
$botoesTitulo = new CBlocoTitulo('Selecionar Destinatários', 'documento.png', $m, $m.'.'.$a);
...
$titulo='';
if ($destino=='modelo_grava_encaminha') echo '<td width=99%>'.dica('Encaminhar','Clique neste botão para encaminhar.').'<a class="botao" href="javascript:btRemeter()"><span><b>encaminhar</b></span></a>'.dicaF().'</td></tr><tr><td>&nbsp;</td><td>'.($recebido_enviado ? dica('Encaminhar e Arquivar','Clique nesta opção para encaminhar e arquivar.<BR><BR>'.ucfirst($config['genero_mensagem']).' '.$config['mensagem'].' será enviad'.$config['genero_mensagem'].' para a caixa d'.$config['genero_mensagem'].'s arquivad'.$config['genero_mensagem'].'s').'<a class="botao" href="javascript:btRemeter_arquivar()"><span><b>enc. e arquivar</b></span></a>'.dicaF() : '').'</td></tr><tr><td>&nbsp;</td><td>'.($recebido_enviado ? dica('Encaminhar e Pender','Clique nesta opção para encaminhar e pender.<BR><BR>'.ucfirst($config['genero_mensagem']).' '.$config['mensagem'].' será enviad'.$config['genero_mensagem'].' para a caixa d'.$config['genero_mensagem'].'s pendentes').'<a class="botao" href="javascript:btRemeter_pender()"><span><b>enc. e pender</b></span></a>'.dicaF() : '').'</td>';				
else if ($destino=='modelo_envia_email')	echo '<td width=99%>'.dica('Encaminhar por e-mail','Clique neste botão para encaminhar por e-mail.').'<a class="botao" href="javascript:btRemeter()"><span><b>encaminhar por e-mail</b></span></a>'.dicaF().'</td>';
else if ($destino=='modelo_envia_msg')	echo '<td width=99%>'.dica('Avançar','Clique neste botão para escrever '.$config['genero_mensagem'].' '.$config['mensagem'].'.').'<a class="botao" href="javascript:btRemeter()"><span><b>avançar</b></span></a>'.dicaF().'</td>';
else echo '<td width=99%>'.dica('Despacho','Clique nesta botão para escrever o despacho.').'<a class="botao" href="javascript:btRemeter()"><span><b>despacho</b></span></a></td>';


echo '<td align="right">'.dica('Voltar','Clique neste botão para voltar à tela anterior.').'<a class="botao" href="javascript:void(0);" onclick="javascript:env.a.value=\''.$retornar.'\'; env.submit();"><span><b>voltar</b></span></a></td>';
