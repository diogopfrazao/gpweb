...
if ($protocolar_sequencial){
	...
	foreach($modeloID as $modelo){
		...
		$modelo_protocolo=$prefixo.$cia_qnt_nr.$sufixo;
		$sql->adTabela('modelos');
		$sql->adAtualizar('modelo_protocolo',$modelo_protocolo);
		$sql->adAtualizar('modelo_protocolista', $Aplic->usuario_id);
		$sql->adAtualizar('modelo_data_protocolo', date('Y-m-d H:i:s'));
		$sql->adOnde('modelo_id ='.$modelo);
		$sql->exec();
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data de Início', 'Digite ou escolha no calendário a data de início da pesquisa.').'De:'.dicaF().'</td><td style="white-space: nowrap"><input type="hidden" name="pesquisa_inicio" id="pesquisa_inicio" value="'.$pesquisa_inicio.'" /><input type="text" name="data_inicio" style="width:70px;" id="data_inicio" onchange="setData(\'env\', \'data_inicio\', \'pesquisa_inicio\');" value="'.($pesquisa_inicio ? retorna_data($pesquisa_inicio, false): '').'" class="texto" />'.dica('Data Inicial', 'Clique neste ícone '.imagem('icones/calendario.gif').' para abrir um calendário onde poderá selecionar a data de início da pesquisa.').'<a href="javascript: void(0);" ><img id="f_btn1" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" alt="Agenda"" border=0 /></a>'.dicaF().dica('Data de Término', 'Digite ou escolha no calendário a data de término da pesquisa.').'&nbsp;&nbsp;a&nbsp;&nbsp;'.dicaF().'<input type="hidden" name="pesquisa_fim" id="pesquisa_fim" value="'.$pesquisa_fim.'" /><input type="text" name="data_fim" id="data_fim" style="width:70px;" onchange="setData(\'env\', \'data_fim\', \'pesquisa_fim\');" value="'.($pesquisa_fim ? retorna_data($pesquisa_fim, false): '').'" class="texto" />'.dica('Data de Término', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data limite desta psquisa.').'<a href="javascript: void(0);" ><img id="f_btn2" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" alt="Agenda"" border=0 /></a>'.dicaF().'&nbsp;&nbsp;&nbsp;&nbsp;'.selecionaVetor($tipos_tempo, 'tipo_tempo', 'size="1" class="texto"', $tipo_tempo).'</td></tr>';
echo'<tr><td align="right" style="white-space: nowrap">'.dica('Tipo de Documento', 'Escolha qual o modelo utilizado.').'Tipo de documento:'.dicaF().'</td><td style="white-space: nowrap">'.selecionaVetor($modelos, 'tipo_documento', 'size="1" class="texto"', $tipo_documento).'</td></tr>';
echo'<tr><td align="right" style="white-space: nowrap">'.dica('Estado de Documento', 'Escolha em qual estado o mesmo se encontra.').'Estado do documento:'.dicaF().'</td><td style="white-space: nowrap">'.selecionaVetor($estado_documentos, 'estado_documento', 'size="1" class="texto"', $estado_documento).'</td></tr>';
...
if($item_menu=='pesquisar'){
  ...
  $sql->adCampo('modelos.modelo_id, modelo_data_aprovado, modelos.class_sigilosa, modelo_tipo_nome, modelo_assunto, modelo_assunto');
	...
	if ($modelo_protocolo) $sql->adOnde('modelo_protocolo=\''.$modelo_protocolo.'\'');
...
echo '<td style="white-space: nowrap"><a href="javascript:void(0);" onclick="javascript:ordenar(\'aprovou\');">'.dica('Aprovou', 'Neste campo fica o responsável por a provar o documento.').($campo_ordenar=='aprovou' ? imagem('icones/'.$seta[$sentido]) : '').'<b>Aprovou</b>'.dicaF().'</td>';
...
for ($i = ($pagina - 1) * $xpg_tamanhoPagina; $i < $pagina * $xpg_tamanhoPagina && $i < $xpg_totalregistros; $i++) {
	...
	echo '<td><a href="javascript:void(0);" onclick="window.open(\'?m=email&a=modelo_ver&retornar=modelo_protocolar&modelo_id='.$rs['modelo_id'].(true ? '&dialogo=1\', \'Documento\', \'left=0,top=0,height=1000,width=850,scrollbars=yes, resizable=yes\'' : '\', \'_self\'').')">'.$icone_anexar.$rs['modelo_assunto'].'</a></td>';
