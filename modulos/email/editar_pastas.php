...
echo '<tr><td colspan=20 align=left><div id="pastas">';
if (count($pastas)) {
	...
	foreach ($pastas as $pasta_id => $linha) {
		...
		echo '<td style="white-space: nowrap" width="32"><a href="javascript: void(0);" onclick="editar_pasta('.$linha['pasta_id'].');">'.imagem('icones/editar.gif', 'Editar', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar a pasta.').'</a>';
		echo '<a href="javascript: void(0);" onclick="if (confirm(\'Tem certeza que deseja excluir esta pasta?\')) {excluir_pasta('.$linha['pasta_id'].');}">'.imagem('icones/remover.png', 'Excluir', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir esta pasta.').'</a></td>';
