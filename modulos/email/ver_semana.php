...
$primeiraData = new CData(Data_calc::beginOfWeek($dd, $mm, $aa, '%Y%m%d', (defined(localidade_PRIMEIRO_DIA) ? localidade_PRIMEIRO_DIA : 1)));
$primeiraData->setTime(0, 0, 0);
$ultimaData = new CData(Data_calc::endOfWeek($dd, $mm, $aa, '%Y%m%d', (defined(localidade_PRIMEIRO_DIA) ? localidade_PRIMEIRO_DIA : 1)));
$ultimaData->setTime(23, 59, 59);
$semana_ant = new CData(Data_calc::beginOfPrevWeek($dd, $mm, $aa, '%Y%m%d', (defined(localidade_PRIMEIRO_DIA) ? localidade_PRIMEIRO_DIA : 1)));
$proxSemana = new CData(Data_calc::beginOfNextWeek($dd, $mm, $aa, '%Y%m%d', (defined(localidade_PRIMEIRO_DIA) ? localidade_PRIMEIRO_DIA : 1)));
$links = array();
...
if (!$dialogo){
	$botoesTitulo = new CBlocoTitulo('Compromissos Semanal', 'compromisso.png', $m, "$m.$a");
	$botoesTitulo->adicionaBotao('m=email&a=ver_mes&data='.$esta_semana->format('%Y%m%d'), 'visão mensal','','Visão Mensal','Visualizar o mês inteiro.');
...
echo '<tr><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=email&a=ver_semana&data='.$semana_ant->format('%Y%m%d').'\');">'.imagem('icones/'.($estilo_interface=='metro' ? 'navAnterior_metro.png' :'anterior.gif'), 'Semana Anterior', 'Clique neste ícone '.imagem('icones/'.($estilo_interface=='metro' ? 'navAnterior_metro.png' :'anterior.gif')).' para exibir a semana anterior.').'</a></td><td width="100%" align="center"><b><span style="font-size:12pt">'.$primeiraData->format('%U').'ª Semana - '.$nome_meses[$primeiraData->format('%m')].' de '.$primeiraData->format('%Y').'</span></b></td><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=email&a=ver_semana&data='.$proxSemana->format('%Y%m%d').'\');">'.imagem('icones/'.($estilo_interface=='metro' ? 'navProximo_metro.png' :'proximo.gif'), 'Próxima Semana', 'Clique neste ícone '.imagem('icones/'.($estilo_interface=='metro' ? 'navProximo_metro.png' :'proximo.gif')).' para exibir a próxima semana.').'</a></td></tr>';
echo '</table>';
...
echo '<tr>';
for ($i = 0; $i < 7; $i++) {
	$diaFormato = $mostrar_dia->format('%Y%m%d');
	
	$dia = $mostrar_dia->getDay();
	$href = 'm=email&a=ver_dia&data='.$diaFormato.'&tab=0';
	$diadasemana = intval($mostrar_dia->format('%w'));	
	
	$horas=(isset($expediente[$diaFormato])? $expediente[$diaFormato] : 8);
	if ($horas==4) $classe='meio_expediente';
...
if (!$dialogo) echo '<tr><td colspan="20" align="right" bgcolor="#f2f0ec"><table width="100%"><tr><td width="20%"></td><td valign="top" align="center" width="60%"><table align="center"><tr>
<td style="border-style:solid;border-width:1px; background-color:#ffffff;">&nbsp;&nbsp;</td><td style="white-space: nowrap">Expediente normal</td><td>&nbsp;</td>
<td style="border-style:solid;border-width:1px; background-color: #fefeed;">&nbsp;&nbsp;</td><td style="white-space: nowrap">Meio expediente</td><td>&nbsp;</td>
<td style="border-style:solid;border-width:1px; background-color:#dffefc;">&nbsp;&nbsp;</td><td style="white-space: nowrap">Expediente alterntivo</td><td>&nbsp;</td>
<td style="border-style:solid;border-width:1px; background-color: #f0e8e8;">&nbsp;&nbsp;</td><td style="white-space: nowrap">Sem expediente</td>
</tr></table></td><td width="20%" align=right><a href="javascript:void(0);" onclick="url_passar(0, \'m=email&a=ver_dia&tab=0\');">'.dica('Compromissos para Hoje', 'Clique para exibir os compromissos para a data atual.').'hoje'.dicaF().'</a></td></tr></table></td></tr>';
...
function popAgenda() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Agenda', 1000, 700, 'm=publico&a=selecionar_multiplo&dialogo=1&chamar_volta=setAgenda&tabela=agenda_tipo&valores=<?php echo $agenda_tipo_id ?>', window.setAgenda, window);
	else window.open('./index.php?m=publico&a=selecionar_multiplo&dialogo=1&chamar_volta=setAgenda&tabela=agenda_tipo&valores=<?php echo $agenda_tipo_id ?>', 'Agenda','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
	}
