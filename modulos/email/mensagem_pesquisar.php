...
$sql = new BDConsulta;
/*   NÃO ESTÁ FAZENDO SENTIDO. FALA DE UMA COLUNA QUE NÃO EXISTE msg_id_original
if ($anexar_msg){
...
if($pesquisar){
  ...
  $sql->adCampo('class_sigilosa, msg.de_id as criador, texto, msg_usuario_id, msg_usuario.datahora, referencia, msg_usuario.datahora, msg.msg_id, msg_usuario.cor, msg_usuario.nota, contatos.contato_funcao, msg_usuario.status, msg_usuario.de_id, msg_usuario.nome_de, msg_usuario.funcao_de');
	...
	if ($pesquisa_inicio){
		if ($tipo_tempo=='data_criacao')  $sql->adOnde('msg.data_envio >= \''.$data_inicio->format('%Y-%m-%d %H:%M:%S').'\'');
		elseif ($tipo_tempo=='data_envio')  $sql->adOnde('msg_usuario.datahora >= \''.$data_inicio->format('%Y-%m-%d %H:%M:%S').'\'');
		}
	if ($pesquisa_fim){
		if ($tipo_tempo=='data_criacao')  $sql->adOnde('msg.data_envio <= \''.$data_fim->format('%Y-%m-%d %H:%M:%S').'\'');
		elseif ($tipo_tempo=='data_envio')  $sql->adOnde('msg_usuario.datahora <= \''.$data_fim->format('%Y-%m-%d %H:%M:%S').'\'');
		}
...
for ($i = ($pagina - 1) * $xpg_tamanhoPagina; $i < $pagina * $xpg_tamanhoPagina && $i < $xpg_totalregistros; $i++) {
  ...
  $sql->adCampo('anexo_modelo, anexo_msg, anexo_nome, anexo_caminho, anexo_usuario, anexo_tipo_doc, anexo_doc_nr, anexo_nome_de, anexo_funcao_de, anexo_data_envio, contatos.contato_funcao');
  $sql->adCampo(($config['militar'] < 10 ? 'concatenar_tres(contato_posto, \' \', contato_nomeguerra)' : 'contato_nomeguerra').' AS nome_usuario');
	$sql->esqUnir('contatos', 'contatos', 'contato_id = usuario_contato');
  ...
  foreach((array)$sql_resultadosc as $rs_anexo){
		...
		$texto_anexo.='&nbsp;&nbsp;'.$rs_anexo['anexo_nome'].' - '.($Aplic->usuario_prefs['nomefuncao'] ? ($rs_anexo['anexo_nome_de'] ? $rs_anexo['anexo_nome_de'] : $rs_anexo['nome_usuario']) : ($rs_anexo['anexo_funcao_de'] ? $rs_anexo['anexo_funcao_de'] : $rs_anexo['contato_funcao']) ).($rs_anexo['anexo_data_envio']? ' - '.retorna_data($rs_anexo['anexo_data_envio']) : '').'<br>';
		}
	...
	echo '<td style="white-space: nowrap">'.dica('Detalhes do '.ucfirst($config['usuario']), $dentro).'<a href="javascript:void(0);" onclick="javascript:visualizar_usuario('.$rs['de_id'].');">'.($Aplic->usuario_prefs['nomefuncao'] ? ($rs['nome_de'] ? $rs['nome_de'] : $rs['nome_usuario']) : ($rs['funcao_de'] ? $rs['funcao_de'] : $rs['contato_funcao'])).'</a>'.dicaF().'</td>';	
	...
	echo '<td style="white-space: nowrap" width="120">'.retorna_data($rs['datahora']).'</td>';
