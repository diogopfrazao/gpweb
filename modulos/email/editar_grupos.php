...
echo '<td id="adicionar_grupo" style="display:"><a href="javascript: void(0);" onclick="incluir_grupo();">'.imagem('icones/adicionar_g.png','Incluir','Clique neste ícone '.imagem('icones/adicionar.png').' para incluir o grupo.').'</a></td>';
...
if (count($grupos_cadastrados)) {
	...
	foreach ($grupos_cadastrados as $grupo_id => $linha) {
		...
		echo '<td width=32><a href="javascript: void(0);" onclick="editar_grupo('.$linha['grupo_id'].');">'.imagem('icones/editar.gif', 'Editar', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o grupo.').'</a>';
		echo '<a href="javascript: void(0);" onclick="if (confirm(\'Tem certeza que deseja excluir esto grupo?\')) {excluir_grupo('.$linha['grupo_id'].');}">'.imagem('icones/remover.png', 'Excluir', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir esto grupo.').'</a></td>';
		echo '</tr>';
		}
	echo '</table>';
	}
