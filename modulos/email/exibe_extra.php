...
echo '<table align="center" style="font-size:10pt; padding-left: 5px; padding-right: 5px; background-color: #'.($dialogo ? 'FFFFFF' : $cor['cor_encamihamentos']).'" cellspacing=0 width="100%" cellpadding=0>';
...
foreach ($sql_resultadosb as $rs_anot){ 
	if ($rs_anot['tipo'] == 1 ) { 
		...
		$saida = '<table rules="ALL" border="1" cellspacing=0 cellpadding=0 align="center" width=100%><tr><td>';
		$saida.= '<table align="center" cellspacing=0 width=100% cellpadding=0>';
...
if (count($sql_resultadosb) && !$dialogo) echo estiloFundoCaixa(); 
...
if (($sql_resultadosf && count($sql_resultadosf)) || count($destinatarios_extras)){
	echo '<table rules="ALL" border="1" cellspacing=0 cellpadding=0 align="center" width="100%"><tr><td><table align="center" style="font-size:10pt; padding-left: 5px; padding-right: 5px; background-color: #'.($dialogo ? 'FFFFFF' : $cor['cor_encamihamentos']).'" cellspacing=0 width="100%" cellpadding=0>';
	...
	foreach ($sql_resultadosf as $rs_enc){ 
	  if (($rs_enc['copia_oculta'] !=1) || ($rs_enc['de_id']==$Aplic->usuario_id || $rs_enc['para_id']==$Aplic->usuario_id )) {
	    ...
	    echo "<td nowrap='nowrap' style='font-size:7pt; padding-left: 2px; padding-right: 2px;'>";
			...
			else echo retorna_data($rs_enc['datahora_leitura']);
			echo '</td>';
	...
	foreach ($destinatarios_extras as $extra){ 
		...
		echo '<td style="white-space: nowrap;font-size:7pt; padding-left: 2px; padding-right: 2px;">'.retorna_data($extra['datahora']).'</td>';
...
if(count($porcentagens)){
	...
	echo '<table rules="ALL" border="1" cellspacing=0 cellpadding=0 align="center" width=100%><tr><td><table align="center" style="font-size:10pt; padding-left: 5px; padding-right: 5px; background-color: #'.($dialogo ? 'FFFFFF' : $cor['cor_encamihamentos']).'" cellspacing=1 width=100% cellpadding=0>';
