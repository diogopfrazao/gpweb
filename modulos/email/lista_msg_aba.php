...
if ($status == 3 ){
  ...
  else if ($pasta == 0) $sql->adOnde('msg_usuario.pasta_id IS NULL OR msg_usuario.pasta_id = 0'); 
  
  $sql_total->adOnde('msg_usuario.para_id '.($coletivo ? 'IN ('.$Aplic->usuario_lista_grupo.')' : '='.$usuario_id));
	$sql_total->adOnde('msg_usuario.status = 3');
  if ($pasta > 0) $sql_total->adOnde('msg_usuario.pasta_id ='.$pasta);
  else if ($pasta == 0) $sql_total->adOnde('msg_usuario.pasta_id IS NULL OR msg_usuario.pasta_id = 0'); 
	}
...
if ($status == 10) { // pesquisa
  ...
  if ($data_fim)  $sql->adOnde('data_envio <= \''.date_format($data_fim, 'Y-m-d H:i:s').'\'');
	if ($pasta > 0)  $sql->adOnde('msg_usuario.pasta_id = '.$pasta); 
	elseif ($pasta == 0) $sql->adOnde('msg_usuario.pasta_id IS NULL OR msg_usuario.pasta_id = 0'); 
  ...
  if ($data_fim)  $sql_total->adOnde('data_envio <= \''.date_format($data_fim, 'Y-m-d H:i:s').'\'');
	if ($pasta > 0)  $sql_total->adOnde('msg_usuario.pasta_id = '.$pasta); 
	elseif ($pasta == 0) $sql_total->adOnde('msg_usuario.pasta_id IS NULL OR msg_usuario.pasta_id = 0'); 
	}
...
mostrarBarraNav2($xpg_totalregistros, $xpg_tamanhoPagina, $xpg_total_paginas, $pagina, 'mensagem', 'mensagens', '', 'env',($estilo_interface=='classico' ? 'a6a6a6' : '006fc2'));
...
foreach($sql_resultados as $rs) {
  ...
  foreach ($sql_resultadosc as $rs_anexo){
	...
	foreach($lista as $linha) {	
		...
		elseif ($linha['msg_gestao_patrocinador']) $icone.='<a href="javascript:void(0);" onclick="url_passar(0,\'m=patrocinadores&a=patrocinador_ver&patrocinador_id='.$linha['msg_gestao_patrocinador'].'\');">'.imagem('icones/patrocinador_p.gif','Patrocinador','Clique neste ícone '.imagem('icones/patrocinador_p.gif').' para ver o patrocinador que '.($config['genero_mensagem']=='o' ? 'este' : 'esta').' '.$config['mensagem'].' está vinculad'.$config['genero_mensagem'].'.').'</a>';
		...
		elseif ($linha['msg_gestao_ssti']) $icone.='<a href="javascript:void(0);" onclick="url_passar(0,\'m=ssti&a=ssti_ver&ssti_id='.$linha['msg_gestao_ssti'].'\');">'.imagem('icones/ssti_p.png',ucfirst($config['ssti']),'Clique neste ícone '.imagem('icones/ssti_p.png').' para ver '.$config['genero_ssti'].' '.$config['ssti'].' que '.($config['genero_mensagem']=='o' ? 'este' : 'esta').' '.$config['mensagem'].' está vinculad'.$config['genero_mensagem'].'.').'</a>';
		elseif ($linha['msg_gestao_laudo']) $icone.='<a href="javascript:void(0);" onclick="url_passar(0,\'m=ssti&a=laudo_ver&laudo_id='.$linha['msg_gestao_laudo'].'\');">'.imagem('icones/laudo_p.png',ucfirst($config['laudo']),'Clique neste ícone '.imagem('icones/laudo_p.png').' para ver '.$config['genero_laudo'].' '.$config['laudo'].' que '.($config['genero_mensagem']=='o' ? 'este' : 'esta').' '.$config['mensagem'].' está vinculad'.$config['genero_mensagem'].'.').'</a>';
		elseif ($linha['msg_gestao_trelo']) $icone.='<a href="javascript:void(0);" onclick="url_passar(0,\'m=trelo&a=trelo_ver&trelo_id='.$linha['msg_gestao_trelo'].'\');">'.imagem('icones/trelo_p.png',ucfirst($config['trelo']),'Clique neste ícone '.imagem('icones/trelo_p.png').' para ver '.$config['genero_trelo'].' '.$config['trelo'].' que '.($config['genero_mensagem']=='o' ? 'este' : 'esta').' '.$config['mensagem'].' está vinculad'.$config['genero_mensagem'].'.').'</a>';
		elseif ($linha['msg_gestao_trelo_cartao']) $icone.='<a href="javascript:void(0);" onclick="url_passar(0,\'m=trelo&a=trelo_cartao_ver&trelo_cartao_id='.$linha['msg_gestao_trelo_cartao'].'\');">'.imagem('icones/trelo_cartao_p.png',ucfirst($config['trelo_cartao']),'Clique neste ícone '.imagem('icones/trelo_cartao_p.png').' para ver '.$config['genero_trelo_cartao'].' '.$config['trelo_cartao'].' que '.($config['genero_mensagem']=='o' ? 'este' : 'esta').' '.$config['mensagem'].' está vinculad'.$config['genero_mensagem'].'.').'</a>';
		elseif ($linha['msg_gestao_pdcl']) $icone.='<a href="javascript:void(0);" onclick="url_passar(0,\'m=pdcl&a=pdcl_ver&pdcl_id='.$linha['msg_gestao_pdcl'].'\');">'.imagem('icones/pdcl_p.png',ucfirst($config['pdcl']),'Clique neste ícone '.imagem('icones/pdcl_p.png').' para ver '.$config['genero_pdcl'].' '.$config['pdcl'].' que '.($config['genero_mensagem']=='o' ? 'este' : 'esta').' '.$config['mensagem'].' está vinculad'.$config['genero_mensagem'].'.').'</a>';
		...
		elseif ($linha['msg_gestao_os']) $icone.='<a href="javascript:void(0);" onclick="url_passar(0,\'m=os&a=os_ver&os_id='.$linha['msg_gestao_os'].'\');">'.imagem('icones/os_p.png',ucfirst($config['os']),'Clique neste ícone '.imagem('icones/os_p.png').' para ver '.$config['genero_os'].' '.$config['os'].' que '.($config['genero_mensagem']=='o' ? 'este' : 'esta').' '.$config['mensagem'].' está vinculad'.$config['genero_mensagem'].'.').'</a>';
	...
	if ($status == 5) echo '<td style="white-space: nowrap" width="130">'.dica('Detalhes do '.ucfirst($config['usuario']), 'Clique para ver detalhes deste usuário').'<a href="javascript:void(0);" onclick="javascript:url_passar(0, \'m=admin&a=ver_usuario&tab=3&usuario_id='.$rs['para_id'].'\');">'.$rs['funcao_para'].'</a>'.dicaF().'</td>';				
	...
	if ($config['msg_precedencia']) echo '<td style="white-space: nowrap; color:'.(isset($cor_prioridade[$rs['precedencia']]) ? $cor_prioridade[$rs['precedencia']] : '#000').(isset($rs['precedencia']) && $rs['precedencia'] ? ';font-weight: bold;' :';').'">'.(isset($precedencia[$rs['precedencia']]) ? $precedencia[$rs['precedencia']] : '').'</td>';
	...
	if ($status == 1 || $status == 10 ){ 
		echo '<td style="white-space: nowrap">'.(isset($tipos_status[$rs['status']]) ? $tipos_status[$rs['status']] : '&nbsp;').'</td>';
		$passou=1;
		}
...
if ($status != 1 && !$outro_usuario && $tem_pasta && $status != 5) {
	$vetor_pasta=array(0 => 'fora das pastas', -1 =>'tod'.$config['genero_mensagem'].'s '.$config['mensagens']);
	$sql->adTabela('pasta');
  $sql->adCampo('pasta_id, nome');
	$sql->adOnde('pasta.usuario_id='.(int)$usuario_id);
	$vetor_pasta+=$sql->listaVetorChave('pasta_id', 'nome');
	$sql->limpar();
	echo '<td>'.dica('Caixa de Seleção de Pasta','Selecione na caixa de opção em qual pasta deseja entrar para ver '.$config['genero_mensagem'].'s '.$config['mensagens'].' armazenad'.$config['genero_mensagem'].'s.<BR><BR>Para mover '.($config['genero_mensagem']=='a' ? 'uma': 'um').' '.$config['mensagem'].' para a pasta, selecione a mesma e utilize a próxima caixa de seleção, do lado direito.').'<a>'.selecionaVetor($vetor_pasta, 'codigo_pasta', 'class="texto" onchange="resulta_combo('.$status.');"', $pasta).'</a>'.dicaF().'</td>'; 
	}
