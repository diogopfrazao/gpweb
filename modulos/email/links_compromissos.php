...
function getCompromissoLinks($periodo_todo=false, $inicioPeriodo, $fimPeriodo, $links, $strMaxLarg, $minutoiCal = false, $usuario_id=0, $agenda_tipo_id=0) {
	...
	foreach ($compromissos as $linha) {
		...
		for ($i = 0, $i_cmp = $inicio->dataDiferenca($fim); $i <= $i_cmp; $i++) {
			...
			if ($minutoiCal) $link = array('texto_mini' => $texto);
				...
				$link['texto'] = '<tr><td nowrap=nowrap><a href="javascript:void(0);" onclick="url_passar(0, \'m=email&a=ver_compromisso&agenda_id='.$linha['agenda_id'].'\');">'.$inicio_fim.'</a></td><td valign=middle>'.link_agenda($linha['agenda_id'],'',true).'</td></tr>';
	...
	$db_inicio= $inicioPeriodo->format('%Y-%m-%d');
	$db_fim = $fimPeriodo->format('%Y-%m-%d');	
	$q = new BDConsulta;
