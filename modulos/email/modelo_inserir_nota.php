...
$item_menu=getParam($_REQUEST, 'item_menu', 'entrada');
...
if ($alterar){
	...
	if (!$sql->exec()) die('Não foi possivel alterar os valores da tabela modelo_usuario!'.$bd->stderr(true));
	$sql->limpar();
	}
...
echo '<input type=hidden name="a" id="a" value="modelo_inserir_nota">';
...
if ($excluir || $alterar) echo '<script>env.a.value="modelo_pesquisar"; env.submit();</script>';

$sql->adTabela('modelo_usuario');
$sql->adCampo('cor, nota, modelo_id');
$sql->adOnde('modelo_usuario_id='.$modelo_usuario_id);
$linha = $sql->Linha();
$sql->limpar();


$botoesTitulo = new CBlocoTitulo('Anotação em Documento', 'documento.png', $m, $m.'.'.$a);
$botoesTitulo->mostrar();


echo estiloTopoCaixa();
echo '<table width=100% class="std" align="center" border=0 cellspacing=0 cellpadding=0 >';
echo '<tr><td align="center" colspan=2><h1>Documento Nº '.$linha['modelo_id'].'<h1></td></tr>';
...
if (strlen($linha['nota'])) echo '<td width=98%>'.dica('Alterar','Clique neste botão para alterar a anotação.').'<a class="botao" href="javascript:void(0);" onclick="inserir_alterar();"><span><b>alterar</b></span></a>'.dicaF().'</td>';
else echo '<td width=98%>'.dica('Inserir','Clique neste botão para inserir uma anotação.').'<a class="botao" href="javascript:void(0);" onclick="inserir_alterar();"><span><b>inserir</b></span></a>'.dicaF().'</td>';
...
echo '<td>'.dica('Cancelar','Clique neste botão para cancelar esta operação.').'<a class="botao" href="javascript:void(0);" onclick="javascript:env.a.value=\''.$retornar.'\'; env.submit();"><span><b>cancelar</b></span></a>'.dicaF().'</td></tr></table>';
