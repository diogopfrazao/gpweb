...
if (!$dialogo && $Aplic->profissional){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=historico&a=index&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$imprimir.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo) {
	...
	$botoesTitulo->adicionaCelula('<table cellpadding=0 cellspacing=0><tr><td align="right" style="white-space: nowrap">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=historico&a=index&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr></table>');
	$botoesTitulo->mostrar();
...
mostrarBarraNav($xtotalregistros, $xtamanhoPagina, $xtotal_paginas, $pagina, 'Registro', 'Registros','','',($estilo_interface=='classico' ? 'a6a6a6' : '006fc2'));
...
foreach($historico as $linha){
	...
	echo '<td style="white-space: nowrap">'.$linha['nome'].'</td>';
