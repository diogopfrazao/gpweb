...
for ($i = 0; $i < 12; $i++) {
	...
	$s .= '<td width="8%"><a href="javascript:void(0);" onclick="url_passar(0, \'m=publico&a=expediente&dialogo=1&chamar_volta='.$chamarVolta.'&data='.$este_mes->format('%Y%m%d').'&uts='.$dataAnt.'\');" class="">'.substr($este_mes->format('%b'), 0, 1).'</a></td>';
	}
echo $s;
echo '</tr><tr>';
echo '<td colspan="6" align="left">';
echo '<a href="javascript:void(0);" onclick="url_passar(0, \'m=publico&a=expediente&dialogo=1&chamar_volta='.$chamarVolta.'&data='.$cal->anoAnterior->format('%Y%m%d').'&uts='.$dataAnt.'\');" class="">'.$cal->anoAnterior->getYear().'</a>';
echo '</td><td colspan="6" align="right">';
echo '<a href="javascript:void(0);" onclick="url_passar(0, \'m=publico&a=expediente&dialogo=1&chamar_volta='.$chamarVolta.'&data='.$cal->anoProximo->format('%Y%m%d').'&uts='.$dataAnt.'\');" class="">'.$cal->anoProximo->getYear().'</a>';
echo '</td>';
echo '</tr></table>';
?>
