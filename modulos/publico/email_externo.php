...
if (getParam($_REQUEST, 'enviado', 0)){
	...
	if ($ok) echo '<script>alert("Mensagem enviada.");</script>';
	else echo '<script>alert("Não foi possível enviar a mensagem.");</script>';
    //não fechar janela em modo depuração
	if(!config('email_debug', false)){
    echo '<script>if(parent && parent.gpwebApp) parent.gpwebApp._popupCallback(null, ""); else {window.opener = window;	window.close();}</script>';
    }
	}
...
echo '<table width="100%" cellspacing=0 cellpadding=1 class="std">';
echo '<tr><td><table width="100%"><tr><td width="25%"></td><td align="center" width="50%"><b>Mensagem</b></td><td width="25%" align=right>'.botao('enviar', '', '','','envia()').'</td></tr></table></td></tr>';
