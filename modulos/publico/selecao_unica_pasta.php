...
if ($Aplic->profissional && $arquivo){
	...
	AND arquivo_pasta_gestao_me IS NULL
	AND arquivo_pasta_gestao_acao_item IS NULL
	AND arquivo_pasta_gestao_beneficio IS NULL
	AND arquivo_pasta_gestao_painel_slideshow IS NULL
	AND arquivo_pasta_gestao_projeto_viabilidade IS NULL
	AND arquivo_pasta_gestao_projeto_abertura IS NULL
	AND arquivo_pasta_gestao_plano_gestao IS NULL
	AND arquivo_pasta_gestao_ssti IS NULL
	AND arquivo_pasta_gestao_laudo IS NULL
	AND arquivo_pasta_gestao_trelo IS NULL
	AND arquivo_pasta_gestao_trelo_cartao IS NULL
	AND arquivo_pasta_gestao_pdcl IS NULL
	AND arquivo_pasta_gestao_pdcl_item IS NULL
	AND arquivo_pasta_gestao_os IS NULL
	AND arquivo_pasta_gestao_usuario IS NULL)';
	
	foreach($lista as $linha){
		if ($linha['arquivo_gestao_tarefa']) {
			$tem_tarefa++;
			$vetor[]='arquivo_pasta_gestao_tarefa='.(int)$linha['arquivo_gestao_tarefa'].' OR tarefas2.tarefa_id='.(int)$linha['arquivo_gestao_tarefa'];
			}
		...
		elseif ($linha['arquivo_gestao_me']) $vetor[]='arquivo_pasta_gestao_me='.(int)$linha['arquivo_gestao_me'];
		elseif ($linha['arquivo_gestao_acao_item']) $vetor[]='arquivo_pasta_gestao_acao_item='.(int)$linha['arquivo_gestao_acao_item'];
		elseif ($linha['arquivo_gestao_beneficio']) $vetor[]='arquivo_pasta_gestao_beneficio='.(int)$linha['arquivo_gestao_beneficio'];
		elseif ($linha['arquivo_gestao_painel_slideshow']) $vetor[]='arquivo_pasta_gestao_painel_slideshow='.(int)$linha['arquivo_gestao_painel_slideshow'];
		elseif ($linha['arquivo_gestao_projeto_viabilidade']) $vetor[]='arquivo_pasta_gestao_projeto_viabilidade='.(int)$linha['arquivo_gestao_projeto_viabilidade'];
		elseif ($linha['arquivo_gestao_projeto_abertura']) $vetor[]='arquivo_pasta_gestao_projeto_abertura='.(int)$linha['arquivo_gestao_projeto_abertura'];
		elseif ($linha['arquivo_gestao_plano_gestao']) $vetor[]='arquivo_pasta_gestao_plano_gestao='.(int)$linha['arquivo_gestao_plano_gestao'];
		elseif ($linha['arquivo_gestao_ssti']) $vetor[]='arquivo_pasta_gestao_ssti='.(int)$linha['arquivo_gestao_ssti'];
		elseif ($linha['arquivo_gestao_laudo']) $vetor[]='arquivo_pasta_gestao_laudo='.(int)$linha['arquivo_gestao_laudo'];
		elseif ($linha['arquivo_gestao_trelo']) $vetor[]='arquivo_pasta_gestao_trelo='.(int)$linha['arquivo_gestao_trelo'];
		elseif ($linha['arquivo_gestao_trelo_cartao']) $vetor[]='arquivo_pasta_gestao_trelo_cartao='.(int)$linha['arquivo_gestao_trelo_cartao'];
		elseif ($linha['arquivo_gestao_pdcl']) $vetor[]='arquivo_pasta_gestao_pdcl='.(int)$linha['arquivo_gestao_pdcl'];
		elseif ($linha['arquivo_gestao_pdcl_item']) $vetor[]='arquivo_pasta_gestao_pdcl_item='.(int)$linha['arquivo_gestao_pdcl_item'];
		elseif ($linha['arquivo_gestao_os']) $vetor[]='arquivo_pasta_gestao_os='.(int)$linha['arquivo_gestao_os'];
	...
	elseif ($me_id) $filtro='arquivo_pasta_gestao_me='.(int)$me_id;
	elseif ($plano_acao_item_id) $filtro='arquivo_pasta_gestao_acao_item='.(int)$plano_acao_item_id;
	elseif ($beneficio_id) $filtro='arquivo_pasta_gestao_beneficio='.(int)$beneficio_id;
	elseif ($painel_slideshow_id) $filtro='arquivo_pasta_gestao_painel_slideshow='.(int)$painel_slideshow_id;
	elseif ($projeto_viabilidade_id) $filtro='arquivo_pasta_gestao_projeto_viabilidade='.(int)$projeto_viabilidade_id;
	elseif ($projeto_abertura_id) $filtro='arquivo_pasta_gestao_projeto_abertura='.(int)$projeto_abertura_id;
	elseif ($pg_id) $filtro='arquivo_pasta_gestao_plano_gestao='.(int)$pg_id;
	elseif ($ssti_id) $filtro='arquivo_pasta_gestao_ssti='.(int)$ssti_id;
	elseif ($laudo_id) $filtro='arquivo_pasta_gestao_laudo='.(int)$laudo_id;
	elseif ($trelo_id) $filtro='arquivo_pasta_gestao_trelo='.(int)$trelo_id;
	elseif ($trelo_cartao_id) $filtro='arquivo_pasta_gestao_trelo_cartao='.(int)$trelo_cartao_id;
	elseif ($pdcl_id) $filtro='arquivo_pasta_gestao_pdcl='.(int)$pdcl_id;
	elseif ($pdcl_item_id) $filtro='arquivo_pasta_gestao_pdcl_item='.(int)$pdcl_item_id;
	elseif ($os_id) $filtro='arquivo_pasta_gestao_os='.(int)$os_id;
...
if ($Aplic->profissional){
	...
	if ($tem_tarefa) {
		$sql->esqUnir('projetos','projetos2', 'projetos2.projeto_id=arquivo_pasta_gestao_projeto');
		$sql->esqUnir('tarefas','tarefas2', 'tarefas2.tarefa_projeto=projetos2.projeto_id');
		}
	if (!$nenhum_filtro && $filtro) $sql->adOnde($filtro);
	elseif ($nenhum_filtro && $sem_nada) $sql->adOnde($sem_nada.($filtro ? ' OR '.$filtro : ''));
	}
else {
	if ($tarefa_id) $sql->adOnde('arquivo_pasta_tarefa = '.$tarefa_id);
	else if ($projeto_id) $sql->adOnde('arquivo_pasta_projeto = '.$projeto_id);
...
function subniveis($pasta_id, $subnivel){
	global $Aplic, $edicao, $arquivo_pasta_id, $chamarVolta, $campo, $sql, $sem_nada, $filtro, $nenhum_filtro, $tem_tarefa,
	$usuario,
	$tarefa_id,
	$projeto_id,
	$pg_perspectiva_id,
