...
$notificar_responsavel=getParam($_REQUEST, 'log_notificar_responsavel', 0);

$del=getParam($_REQUEST, 'del', 0);
$nao_eh_novo=getParam($_REQUEST, 'log_id', null);

if ($del && !$Aplic->checarModulo('log', 'excluir')) $Aplic->redirecionar('m=publico&a=acesso_negado');
elseif ($nao_eh_novo && !$Aplic->checarModulo('log', 'editar')) $Aplic->redirecionar('m=publico&a=acesso_negado');
elseif (!$nao_eh_novo && !$Aplic->checarModulo('log', 'adicionar')) $Aplic->redirecionar('m=publico&a=acesso_negado');
$obj = new CTarefaLog();


if ($obj->log_data) {
	$data = new CData($obj->log_data.date('Hi'));
	$obj->log_data = $data->format('%Y-%m-%d %H:%M:%S');
	}
$dot = strpos($obj->log_horas, ':');
if ($dot > 0) {
	$log_duracao_minutos = sprintf('%.3f', substr($obj->log_horas, $dot + 1) / 60.0);
	$obj->log_horas = floor($obj->log_horas) + $log_duracao_minutos;
	}
$obj->log_horas = round($obj->log_horas, 3);
...
if ($del) {
	...
	$obj->log_nd = limparTexto($obj->log_nd);
	$obj->log_custo = limparTexto($obj->log_custo);
	if (($msg = $obj->armazenar())) {
		$Aplic->setMsg($msg, UI_MSG_ERRO);
		$Aplic->redirecionar('m=tarefas&a=ver&tab=0&tarefa_id='.$tarefa_id);
		} 
	else $Aplic->setMsg($_REQUEST['log_id'] ? 'atualizado' : 'inserido', UI_MSG_OK, true);
	}

$tarefa = new CTarefa();
$tarefa->load($obj->log_tarefa);
...
$Aplic->redirecionar('m=tarefas&a=ver&tarefa_id='.$obj->log_tarefa.'&tab=0&tarefalog='.$obj->log_id);
