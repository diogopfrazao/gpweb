...
global $m, $a, $u, $historico_ativo, $envolvimento, $projeto_id, $f, $ver_min, $texto_consulta, $tipoDuracao, $cia_id, $dept_id, $lista_depts, $lista_cias, $mostrar_marcada, $inativo,
...
$sql->esqUnir(($baseline_id ? 'baseline_' : '').'projetos', 'projetos', 'projeto_id = tarefa_projeto'.($baseline_id ? ' AND projetos.baseline_id='.(int)$baseline_id : ''));
...
if ($dept_id && !$lista_depts) {
	$sql->esqUnir(($baseline_id ? 'baseline_' : '').'tarefa_depts','tarefa_depts', 'tarefa_depts.tarefa_id=tarefas.tarefa_id'.($baseline_id ? ' AND tarefa_depts.baseline_id='.(int)$baseline_id : ''));
...
elseif (!$envolvimento && $Aplic->profissional && ($cia_id || $lista_cias)) {
	$sql->esqUnir(($baseline_id ? 'baseline_' : '').'tarefa_cia', 'tarefa_cia', 'tarefas.tarefa_id=tarefa_cia_tarefa'.($baseline_id ? ' AND tarefa_cia.baseline_id='.(int)$baseline_id : ''));
...
$sql->adTabela(($baseline_id ? 'baseline_' : '').'tarefas', 'tarefas', ($baseline_id ? 'tarefas.baseline_id='.(int)$baseline_id : ''));
$sql->esqUnir(($baseline_id ? 'baseline_' : '').'projetos', 'p', 'p.projeto_id = tarefa_projeto'.($baseline_id ? ' AND p.baseline_id='.(int)$baseline_id : ''));
$sql->adCampo('tarefas.tarefa_id, tarefa_acao, tarefa_acesso, tarefa_superior, tarefa_nome, tarefa_inicio, tarefa_fim, tarefa_dinamica, tarefa_prioridade, tarefa_percentagem, tarefa_duracao, tarefa_duracao_tipo, tarefa_projeto, tarefa_descricao, tarefa_dono, tarefa_status, tarefa_marco');
...
switch ($f) {
	...
	case 'subordinadaProfunda':
		...
		break;
	case 'meu':
		$sql->esqUnir(($baseline_id ? 'baseline_' : '').'tarefa_designados','tarefa_designados','tarefa_designados.tarefa_id = tarefas.tarefa_id'.($baseline_id ? ' AND tarefa_designados.baseline_id='.(int)$baseline_id : ''));
	...
	case 'todasIncompletas':
		...
		break;
	case 'semDesignado':
		$sql->esqUnir(($baseline_id ? 'baseline_' : '').'tarefa_designados', 'vazio', 'tarefas.tarefa_id = vazio.tarefa_id'.($baseline_id ? ' AND vazio.baseline_id='.(int)$baseline_id : ''));
...
$titulos .= '<th style="white-space: nowrap"><b>'.dica('Responsável', 'Responsável pel'.$config['genero_tarefa'].' '.$config['tarefa'].'.').'Responsável'.dicaF().'</b></th>';
$titulos .= '<th style="white-space: nowrap"><b>'.dica(ucfirst($config['usuarios']).' Designados', 'Nos campos abaixo são mostrados '.$config['genero_usuario'].'s '.$config['usuarios'].' que foram designados para cad'.$config['genero_tarefa'].' '.$config['tarefa'].', assim como o grau de comprometimento dos mesmos com '.$config['genero_tarefa'].'s '.$config['tarefas'].'.').'Designados'.dicaF().'</b></th>';
$titulos .= '<th style="white-space: nowrap" width="120"><b>'.dica('Data de Início', 'Data de início tarefas.').'Início'.dicaF().'</b></th>';
$titulos .= '<th style="white-space: nowrap"><b>'.dica('Duração', 'Duração d'.$config['genero_tarefa'].'s '.$config['tarefas'].' em dias uteis de '.config('horas_trab_diario').' horas.<br>No caso d'.$config['genero_tarefa'].'s '.$config['tarefas'].' diâmic'.$config['genero_tarefa'].'s será o somatório da duração d'.$config['genero_tarefa'].'s '.$config['tarefas'].' filh'.$config['genero_tarefa'].'s.').'Dur.'.dicaF().'</b></th>';
$titulos .= '<th style="white-space: nowrap" width="120"><b>'.dica('Data de Término', 'Término d'.$config['genero_tarefa'].'s '.$config['tarefas'].'.').'Término'.dicaF().'</b></th>';
$titulos .= '<th style="white-space: nowrap"><b>'.dica('Dias', 'Número de dias entre o início e término d'.$config['genero_tarefa'].'s '.$config['tarefas'].'.').'Dias'.dicaF().'</b></th>';
		...
		if ($tnums > 0 || $projeto_id == $p['projeto_id']) {
			...
			if (!$ver_min) {
			  ...
			  echo '<td colspan="'.($config['editar_designado_diretamente'] ? $cols - 4 : $cols - 1).'"  style="background: #f2f0ec">';
				...
				echo '<td style="background-color:#'.$p['projeto_cor'].';color:#'.melhorCor($p['projeto_cor']).';  white-space: nowrap;">'.link_projeto($k, true).'</a></td>';
			...
			if ($tnums && !$ver_min) {
			  ...
			  echo '<tr><td>'.botao('gráfico Gantt', 'Gráfico Gantt','Visualizar o gráfico Gantt d'.$config['genero_projeto'].' '.$config['projeto'].' dest'.$config['genero_tarefa'].'s '.$config['tarefas'].'.','','ver_gantt('.$k.');','','',0).'</td></tr>';
...
elseif (!$qnt) echo '<table width="100%" border=0 style="background: #ffffff"><tr><td><p>Não tem autorização para visualizar nenhum d'.$config['genero_tarefa'].'s '.$config['tarefa'].'.</p></td><td align="right"></td></tr></table>';
	...
	echo '<td style="white-space: nowrap; border-style:solid;border-width:1px" bgcolor="#ffffff">&nbsp; &nbsp;</td><td style="white-space: nowrap">'.dica(ucfirst($config['tarefa']).' Futur'.$config['genero_tarefa'], ucfirst($config['tarefa']).' futur'.$config['genero_tarefa'].' é '.$config['genero_tarefa'].' que a data de ínicio ainda não ocorreu.').ucfirst($config['tarefa']).' futur'.$config['genero_tarefa'].dicaF().'</td><td>&nbsp; &nbsp;</td>';
	echo '<td style="white-space: nowrap; border-style:solid;border-width:1px" bgcolor="#e6eedd">&nbsp; &nbsp;</td><td style="white-space: nowrap">'.dica(ucfirst($config['tarefa']).' Iniciad'.$config['genero_tarefa'].' e Dentro do Prazo', ucfirst($config['tarefa']).' iniciad'.$config['genero_tarefa'].' e dentro do prazo é '.$config['genero_tarefa'].' que a data de ínicio já ocorreu, e já está acima de 0% executada, entretanto ainda não se chegou na data de término.').'Iniciada e dentro do prazo'.dicaF().'</td><td>&nbsp; &nbsp;</td>';
	echo '<td style="white-space: nowrap; border-style:solid;border-width:1px" bgcolor="#ffeebb">&nbsp; &nbsp;</td><td style="white-space: nowrap">'.dica(ucfirst($config['tarefa']).' que Deveria ter Iniciad'.$config['genero_tarefa'], ucfirst($config['tarefa']).' deveria ter iniciad'.$config['genero_tarefa'].' é '.$config['genero_tarefa'].' que a data de ínicio d'.$config['genero_tarefa'].' mesm'.$config['genero_tarefa'].' já ocorreu, entretanto ainda se encontra em 0% executad'.$config['genero_tarefa'].'.').'Deveria ter iniciad'.$config['genero_tarefa'].''.dicaF().'</td><td>&nbsp; &nbsp;</td>';
	echo '<td style="white-space: nowrap; border-style:solid;border-width:1px" bgcolor="#cc6666">&nbsp; &nbsp;</td><td style="white-space: nowrap">'.dica(ucfirst($config['tarefa']).' Atrasad'.$config['genero_tarefa'], ucfirst($config['tarefa']).' em atraso é '.$config['genero_tarefa'].' que a data de término d'.$config['genero_tarefa'].' mesm'.$config['genero_tarefa'].' já ocorreu, entretanto ainda não se encontra em 100% executad'.$config['genero_tarefa'].'.').'Em atraso'.dicaF().'</td>';
	echo '<td style="border-style:solid;border-width:1px;  white-space: nowrap;" bgcolor="#aaddaa">&nbsp;&nbsp;&nbsp;</td><td style="white-space: nowrap">'.dica(ucfirst($config['tarefa']).' Feit'.$config['genero_tarefa'], ucfirst($config['tarefa']).' feit'.$config['genero_tarefa'].' é '.$config['genero_tarefa'].' em que se encontra 100% executad'.$config['genero_tarefa'].'.').'Feit'.$config['genero_tarefa'].dicaF().'</td>';
...
function ver_gantt(projeto_id){
	if (window.parent && window.parent.gpwebApp) window.parent.gpwebApp.popUp('Gantt',1024, 762,'m=tarefas&a=ver_gantt_pro&dialogo=1&projeto_id='+projeto_id, null, window);
	else  window.open('./index.php?m=tarefas&a=ver_gantt&projeto_id='+projeto_id, 'Gantt','height=762,width=1024,resizable,scrollbars=no');
	}
...
function retornoLog(update){
  if(update) url_passar(false,'m=tarefas&a=index'); 
  }
