...
$sq = new BDConsulta;
$sq->adTabela(($baseline_id ? 'baseline_' : '').'tarefas', 'starefas', ($baseline_id ? 'starefas.baseline_id='.(int)$baseline_id : ''));
...
$sql->esqUnir(($baseline_id ? 'baseline_' : '').'tarefas', 'tarefas', 'tarefa_designados.tarefa_id=tarefas.tarefa_id'.($baseline_id ? ' AND tarefas.baseline_id='.(int)$baseline_id : ''));
...
echo '<th style="white-space: nowrap"><b>'.dica('Responsável', 'Responsável pel'.$config['genero_tarefa'].' '.$config['tarefa'].'.').'Responsável'.dicaF().'</b></th>';
echo '<th style="white-space: nowrap"><b>'.dica(ucfirst($config['usuarios']).' Designados', 'Nos campos abaixo são mostrados '.$config['genero_usuario'].'s '.$config['usuarios'].' que foram designados para cad'.$config['genero_tarefa'].' '.$config['tarefa'].', assim como o grau de comprometimento dos mesmos com '.$config['genero_tarefa'].'s '.$config['tarefas'].'.').'Designados'.dicaF().'</b></th>';
echo '<th style="white-space: nowrap" width="120"><b>'.dica('Data de Início', 'Data de início tarefas.').'Início'.dicaF().'</b></th>';
echo '<th style="white-space: nowrap"><b>'.dica('Duração', 'Duração d'.$config['genero_tarefa'].'s '.$config['tarefas'].' em dias uteis de '.config('horas_trab_diario').' horas.<br>No caso d'.$config['genero_tarefa'].'s '.$config['tarefas'].' diâmic'.$config['genero_tarefa'].'s será o somatório da duração d'.$config['genero_tarefa'].'s '.$config['tarefas'].' filh'.$config['genero_tarefa'].'s.').'Dur.'.dicaF().'</b></th>';
echo '<th style="white-space: nowrap" width="120"><b>'.dica('Data de Término', 'Término d'.$config['genero_tarefa'].'s '.$config['tarefas'].'.').'Término'.dicaF().'</b></th>';
echo '<th style="white-space: nowrap"><b>'.dica('Dias', 'Número de dias entre o início e término d'.$config['genero_tarefa'].'s '.$config['tarefas'].'.').'Dias'.dicaF().'</b></th>';
...
function mover_tarefa(){
	if (numero_tarefas_selecionado() == 0) alert("Selecione ao menos <?php echo ($config['genero_tarefa']=='a' ? 'uma' : 'um').' '.$config['tarefa'] ?>");
		...
		window.open('./index.php?m=publico&a=selecionar&dialogo=1&edicao=1&chamar_volta=ProjetoMoverTarefas&tabela=projetos&cia_id='+<?php echo $obj->projeto_cia ?>, 'Projetos','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
		}
	}	
...
function clonar_tarefa(){
	if (numero_tarefas_selecionado() == 0) alert("Selecione ao menos <?php echo ($config['genero_tarefa']=='a' ? 'uma' : 'um').' '.$config['tarefa'] ?>");
		...
		window.open('./index.php?m=publico&a=selecionar&dialogo=1&edicao=1&chamar_volta=ProjetoClonarTarefas&tabela=projetos&cia_id='+<?php echo $obj->projeto_cia ?>, 'Projetos','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
		}
	}	
