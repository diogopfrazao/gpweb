...
if ($acao!='inserir' && $acao!='editar'){
	...
	$sql->adTabela(($baseline_id ? 'baseline_' : '').'tarefa_custos', 'tc', ($baseline_id ? 'tc.baseline_id='.(int)$baseline_id : ''));
	$sql->adCampo('tc.*, ((tarefa_custos_quantidade*tarefa_custos_custo*tarefa_custos_cotacao)*((100+tarefa_custos_bdi)/100)) AS valor ');
	$sql->adOnde('tc.tarefa_custos_tarefa ='.$tarefa_id);
	$sql->adOrdem('tarefa_custos_ordem');
	$linhas= $sql->Lista();
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nome', 'Escreva o nome deste item.').'Nome:'.dicaF().'</td><td><input type="text" class="texto" name="tarefa_custos_nome" value="'.(isset($atual['tarefa_custos_nome']) ? $atual['tarefa_custos_nome']:'').'" maxlength="255" size="40" /></td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Unidade de Medida', 'Escolha a unidade de medida deste item.').'Unidade de medida:'.dicaF().'</td><td>'.selecionaVetor($unidade, 'tarefa_custos_tipo', 'class=texto size=1', (isset($atual['tarefa_custos_tipo']) ? $atual['tarefa_custos_tipo']:'')).'</td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Quantidade', 'Insira a quantidade deste item.').'Quantidade:'.dicaF().'</td><td><input type="text" class="texto" onkeypress="return entradaNumerica(event, this, true, true);" onchange="javascript:valor();" onclick="javascript:valor();"name="tarefa_custos_quantidade" id="tarefa_custos_quantidade" value="'.(isset($atual['tarefa_custos_quantidade']) ? number_format($atual['tarefa_custos_quantidade'], 2, ',', '.'):'').'" maxlength="255" size="10" /></td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Valor', 'Insira o valor deste item.').'Valor:'.dicaF().'</td><td><input type="text" class="texto" onkeypress="return entradaNumerica(event, this, true, true);" onchange="javascript:valor();" onclick="javascript:valor();" name="tarefa_custos_custo" id="tarefa_custos_custo" value="'.(isset($atual['tarefa_custos_custo']) ? number_format($atual['tarefa_custos_custo'], 2, ',', '.'):'').'" size="40" /></td></tr>';
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Grupo de Despesa', 'Escolha o grupo de despesa deste item.').'Grupo de despesa:'.dicaF().'</td><td>'.selecionaVetor($GrupoND, 'tarefa_custos_grupo_despesa', 'class=texto size=1 style="width:395px;"  onchange="env.tarefa_custos_nd.value=\'\'; mudar_nd();"',(isset($atual['tarefa_custos_grupo_despesa']) ?  $atual['tarefa_custos_grupo_despesa'] : '')).'</td></tr>';
	
	$ModalidadeAplicacao=array(''=>'')+getSisValor('ModalidadeAplicacao');
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Modalidade de Aplicação', 'Escolha a modalidade de aplicação deste item.').'Modalidade de aplicação:'.dicaF().'</td><td>'.selecionaVetor($ModalidadeAplicacao, 'tarefa_custos_modalidade_aplicacao', 'class=texto size=1 style="width:395px;"  onchange="env.tarefa_custos_nd.value=\'\'; mudar_nd();"', (isset($atual['tarefa_custos_modalidade_aplicacao']) ?  $atual['tarefa_custos_modalidade_aplicacao'] : '')).'</td></tr>';
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Elemento de Despesa', 'Escolha o elemento de despesa (ED) deste item.').'Elemento de despesa:'.dicaF().'</td><td><div id="combo_nd">'.selecionaVetor($nd, 'tarefa_custos_nd', 'class=texto size=1 style="width:395px;" onchange="mudar_nd();"', (isset($atual['tarefa_custos_nd']) ?  $atual['tarefa_custos_nd'] : '')).'</div></td></tr>';
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Descrição', 'Insira a descrição deste item.').'Descrição:'.dicaF().'</td><td><textarea cols="70" rows="5" class="textarea" name="tarefa_custos_descricao">'.(isset($atual['tarefa_custos_descricao']) ? $atual['tarefa_custos_descricao']:'').'</textarea></td></tr>';

	if (count($pi)>1) echo '<tr><td align="right" style="white-space: nowrap">'.dica('PI', 'Escolha o PI deste item.').'PI:'.dicaF().'</td><td>'.selecionaVetor($pi, 'tarefa_custos_pi', 'class=texto size=1 style="width:395px;"', (isset($atual['tarefa_custos_pi']) ?  $atual['tarefa_custos_pi'] : '')).'</td></tr>';
	...
	echo '<tr><td align="right">'.dica('Data','Data limite para o recebimento do ítem.').'Data:</td><td><table cellpadding=0 cellspacing=0><tr><td><td><input type="hidden" name="tarefa_custos_data_limite" id="tarefa_custos_data_limite" value="'.($data_texto ? $data_texto->format('%Y%m%d') : '').'" /><input type="text" name="data_texto"  id="data_texto" style="width:70px;" onchange="setData(\'env\', \'data_texto\', \'tarefa_custos_data_limite\');" value="'.($data_texto ? $data_texto->format('%d/%m/%Y') : '').'" class="texto" />'.dica('Data Limite', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data limite para o recebimento do ítem.').'<a href="javascript: void(0);" ><img id="f_btn1" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 /></a>'.dicaF().'</td></tr></table></td></tr>';
...
else if ($imprimindo && !($Aplic->usuario_nomeguerra=='Visitante' && $Aplic->usuario_id=1)) echo '<script language=Javascript>self.print();</script>';
?>
