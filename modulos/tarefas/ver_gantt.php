...
echo '</td><td align="left" style="white-space: nowrap">';
...
echo '&nbsp;&nbsp;'.dica('Data Final', 'O Gráfico Gantt será mostrado até a data à direita.').'Até:'.dicaF().'<input type="hidden" name="projeto_data_fim" id="projeto_data_fim" value="'.($data_fim ? $data_fim->format('%Y%m%d') : '').'" /><input type="text" style="width:70px;" name="data_fim" id="data_fim" onchange="setData(\'frmEditar\', \'data_fim\');" value="'.($data_fim ? $data_fim->format('%d/%m/%Y') : '').'" class="texto" /><a href="javascript: void(0);">'.dica('Escolher a Data Final', 'Clique neste ícone '.imagem('icones/calendario.gif').' para escolher a data final em que o gráfico Gantt será visualizado.').'<img id="f_btn2" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" alt="" border=0 />'.dicaF().'</a>';
