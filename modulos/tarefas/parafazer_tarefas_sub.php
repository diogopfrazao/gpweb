...
global $perms, $usuario_id, $podeEditar, $tab, $podeExcluir, $podeEditar, $m, $a, $u;
...
echo '<tr>';
echo '<th width="10" >&nbsp;</th>';
echo '<th width="10">'.dica('Registrar Ocorrência', 'Clique  abaixo para registrar ocorrência n'.$config['genero_tarefa'].' '.$config['tarefa'].'.').'R'.dicaF().'</th>';
echo '<th width="20">'.dica('Porcentual d'.$config['genero_tarefa'].' '.ucfirst($config['tarefa']).' Realizada', 'Clique para ordenar '.$config['genero_tarefa'].'s '.$config['tarefas'].' pelos percentuais realizados.').ordenar_por_item_titulo('Feito', 'tarefa_percentagem', SORT_NUMERIC, '&a=parafazer').dicaF().'</th>';
...
echo '<th style="white-space: nowrap" width="140">'.dica('Data de Início', 'Clique para ordenar '.$config['genero_tarefa'].'s '.$config['tarefas'].' abaixo pela data de início das mesmas.').ordenar_por_item_titulo('Data de Início', 'tarefa_inicio', SORT_NUMERIC, '&a=parafazer').dicaF().'</th>';
echo '<th style="white-space: nowrap">'.dica('Duração', 'Clique para ordenar '.$config['genero_tarefa'].'s '.$config['tarefas'].' abaixo pela duração das mesmas ').ordenar_por_item_titulo('Duração', 'tarefa_duracao', SORT_NUMERIC, '&a=parafazer').dicaF().'</th>';
echo '<th style="white-space: nowrap" width="140">'.dica('Término', 'Clique para ordenar '.$config['genero_tarefa'].'s '.$config['tarefas'].' abaixo pelo término das mesmas.').ordenar_por_item_titulo('Término', 'tarefa_fim', SORT_NUMERIC, '&a=parafazer').dicaF().'</th>';
echo '<th style="white-space: nowrap">'.dica('Fazer em', 'Clique para ordenar '.$config['genero_tarefa'].'s '.$config['tarefas'].' abaixo pelo tempo necessário para realiza-las.').ordenar_por_item_titulo('Fazer em', 'tarefa_fazer_em', SORT_NUMERIC, '&a=parafazer').dicaF().'</th>';
echo '<th style="white-space: nowrap">'.dica('Dias', 'Clique para ordenar '.$config['genero_tarefa'].'s '.$config['tarefas'].' abaixo pela duração em dias.').ordenar_por_item_titulo('Dias', 'dias', SORT_NUMERIC, '&a=parafazer').dicaF().'</th>';
