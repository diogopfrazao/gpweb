...
$nova_tarefa=($podeEditar && $projeto_id ? '<tr><td style="white-space: nowrap">'.dica('Nov'.$config['genero_tarefa'].' '.ucfirst($config['tarefa']), 'Criar uma nov'.$config['genero_tarefa'].' '.$config['tarefa'].' n'.($config['genero_projeto']=='o' ? 'este' : 'esta').' '.$config['projeto'].'.').'<a href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=tarefas&a=editar&tarefa_projeto='.$projeto_id.'\');" ><img src="'.acharImagem('tarefa_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr><tr><td style="white-space: nowrap"></td></tr>' : '');
...
$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$pesquisar.$tipo_filtro.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$nova_tarefa.'</table></td></tr></table>';
$saida.= '</div></div>';
