 ...
 public function ata_acao( $baseline_id=false, $indicador_id=false){
	...
	public function email_log( &$log, $designados, $tarefa_contatos, $projeto_contatos, $outros, $extras) {
	  ...
	  $sem_email_interno=0;
		...
		$corpo .= '<b>Sumário:</b> '.$log->log_nome.'<br>';
		$corpo .= '<b>Descrição:</b> '.$log->log_descricao;
		...
		foreach($usuarios as $usuario){
			if (!isset($usado[$usuario['usuario_id']]) && !isset($usado[$usuario['contato_email']])){
				...
				if ($Aplic->profissional){
						...
						if ($obj->tarefa_dono) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;">Responsável:</td><td>'.nome_usuario($obj->tarefa_dono,'','','esquerda').'</td></tr>';
						...
						if ($data_inicio) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;">Início:</td><td width="300">'.$data_inicio->format('%d/%m/%Y %H:%M').'</td></tr>';
						if ($data_fim) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;">Término:</td><td width="300">'.$data_fim->format('%d/%m/%Y %H:%M').'</td></tr>';
						...
						if ($obj->tarefa_emprego_obra) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;">Empregos durante a execução:</td><td width="300">'.$obj->tarefa_emprego_obra.'</td></tr>';
						if ($obj->tarefa_emprego_direto) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;">Empregos diretos após conclusão:</td><td width="300">'.$obj->tarefa_emprego_direto.'</td></tr>';
						if ($obj->tarefa_emprego_indireto) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;">Empregos indiretos após conclusão:</td><td width="300">'.$obj->tarefa_emprego_indireto.'</td></tr>';
						if ($obj->tarefa_forma_implantacao) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;">Forma de implantação:</td><td width="300">'.$obj->tarefa_forma_implantacao.'</td></tr>';
						if ($obj->tarefa_populacao_atendida) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;">População atendida:</td><td width="300">'.$obj->tarefa_populacao_atendida.'</td></tr>';
						...
						if ($obj->tarefa_url_relacionada) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;">Endereço URL:</td><td width="300"><a href="'.$obj->tarefa_url_relacionada.'" target="tarefa'.$post['log_tarefa'].'">'.$obj->tarefa_url_relacionada.'</a></td></tr>';
						...
						if ($obj->tarefa_percentagem!=100 && $provavel!=0) $corpo.='<tr><td align="right" style="white-space: nowrap;width:110px;">Custo provável:</td><td width="300" '.($provavel > $custo_estimado ? 'style="color:#FF0000"' : '').'>'.$config['simbolo_moeda'].' '.number_format($provavel, 2, ',', '.').'</td></tr>';
						...
						if (!$obj->tarefa_marco){
							$corpo.='<tr><td align="right" style="white-space: nowrap">Físico Previsto:</td><td width="100%">'.number_format($obj->fisico_previsto(date('Y-m-d H:i:s')), 2, ',', '.').'%</td></tr>';
							$corpo.='<tr><td align="right" style="white-space: nowrap">Velocidade do físico:</td><td width="100%">'.number_format($obj->fisico_velocidade(date('Y-m-d H:i:s')), 2, ',', '.').'</td></tr>';
							}
...
function mostrarTarefa($arr, $nivel = 0, $aberta = true, $visao_hoje = false, $esconderAbrirFecharLink = false, $permitirRepitir = false, $baseline_id=false) {
	...
	if ($Aplic->usuario_super_admin || permiteAcessar($arr['tarefa_acesso'], $arr['tarefa_projeto'], $arr['tarefa_id'])){
		...
		if ($visao_hoje) echo '<td id="ignore_td_'.$arr['tarefa_id'].'" style="white-space: nowrap" align="center" style="'.$estilo.'">'.($arr['tarefa_fazer_em'] > 0 ? $arr['tarefa_fazer_em'] : dica('Prazo Expirou', 'Est'.($config['genero_tarefa']=='a' ?  'a' : 'e').' '.$config['tarefa'].' deveria ter sido completada há '.($arr['tarefa_fazer_em']*-1). ' dias atrás.').'expirou'.dicaF()).'</td>';
