...
if (isset($_REQUEST['tarefa_duracao'])) $_REQUEST['tarefa_duracao']=float_americano(getParam($_REQUEST, 'tarefa_duracao', null))*$config['horas_trab_diario'];
...
$email_tarefa_responsavel=getParam($_REQUEST, 'email_tarefa_responsavel', 0);
$email_tarefa_designados=getParam($_REQUEST, 'email_tarefa_designados', 0);
$email_tarefa_designados_novos=getParam($_REQUEST, 'email_tarefa_designados_novos', 0);
$email_tarefa_contatos=getParam($_REQUEST, 'email_tarefa_contatos', 0);

$email_projeto_responsavel=getParam($_REQUEST, 'email_projeto_responsavel', 0);
$email_projeto_supervisor=getParam($_REQUEST, 'email_projeto_supervisor', 0);
$email_projeto_autoridade=getParam($_REQUEST, 'email_projeto_autoridade', 0);
$email_projeto_cliente=getParam($_REQUEST, 'email_projeto_cliente', 0);
$email_projeto_designados=getParam($_REQUEST, 'email_projeto_designados', 0);
$email_projeto_stakeholder=getParam($_REQUEST, 'email_projeto_stakeholder', 0);
$email_projeto_contatos=getParam($_REQUEST, 'email_projeto_contatos', 0);

$email_contatos=getParam($_REQUEST, 'email_contatos', null);
$email_extras=getParam($_REQUEST, 'email_extras', null);

$email_texto=getParam($_REQUEST, 'email_texto', '');
...
if ($email_tarefa_designados_novos && $tarefa_id){
...
$tarefa_superior_anterior = $obj->tarefa_superior;
...
if ($obj->tarefa_inicio) {
	...
	$obj->tarefa_inicio = $data->format('%Y-%m-%d %H:%M:%S');
	}
...
if ($obj->tarefa_fim){
	...
	$obj->tarefa_fim = $data_fim->format('%Y-%m-%d %H:%M:%S');
	}
...
if ($tarefa_superior_anterior != $obj->tarefa_superior){
	if(isset($cache)) $cache->calcularTarefaSuperior($tarefa_superior_anterior);
	else calcular_superior($tarefa_superior_anterior);
	}
...
if ($email_projeto_responsavel) {
	if ($msg = $obj->notificarResponsavelProjeto($nao_eh_novo,'gerente', $email_texto))	$Aplic->setMsg($msg, UI_MSG_ERRO);
	}		
if ($email_projeto_supervisor) {
	if ($msg = $obj->notificarResponsavelProjeto($nao_eh_novo,'supervisor', $email_texto))	$Aplic->setMsg($msg, UI_MSG_ERRO);
	}
if ($email_projeto_autoridade) {
	if ($msg = $obj->notificarResponsavelProjeto($nao_eh_novo,'autoridade', $email_texto))	$Aplic->setMsg($msg, UI_MSG_ERRO);
	}		
if ($email_projeto_cliente) {
	if ($msg = $obj->notificarResponsavelProjeto($nao_eh_novo,'cliente', $email_texto))	$Aplic->setMsg($msg, UI_MSG_ERRO);
	}	
if ($email_projeto_designados) {
	if ($msg = $obj->notificarProjeto($nao_eh_novo,'designados', $email_texto)) $Aplic->setMsg($msg, UI_MSG_ERRO);
	}
if ($email_projeto_stakeholder) {
	if ($msg = $obj->notificarProjeto($nao_eh_novo,'stakeholders', $email_texto))	$Aplic->setMsg($msg, UI_MSG_ERRO);
	}		
if ($email_projeto_contatos) {
	if ($msg = $obj->notificarProjeto($nao_eh_novo,'contatos', $email_texto)) $Aplic->setMsg($msg, UI_MSG_ERRO);
	}
if ($email_contatos) {
	if ($msg = $obj->notificarProjeto($nao_eh_novo,'outros', $email_texto, $email_contatos)) $Aplic->setMsg($msg, UI_MSG_ERRO);
	}			
if ($email_extras) {
	if ($msg = $obj->notificarProjeto($nao_eh_novo,'extras', $email_texto, $email_extras)) $Aplic->setMsg($msg, UI_MSG_ERRO);
	}		
	

	

if ($email_tarefa_designados) {
	if ($msg = $obj->notificar($email_texto, $nao_eh_novo)) $Aplic->setMsg($msg, UI_MSG_ERRO);
	}
	
if ($email_tarefa_designados_novos) {
	if ($msg = $obj->notificar_novos($email_texto, $nao_eh_novo, $lista_designados_antigo)) $Aplic->setMsg($msg, UI_MSG_ERRO);
	}
if ($email_tarefa_contatos) {
	if ($msg = $obj->notificarContatos($email_texto, $nao_eh_novo)) $Aplic->setMsg($msg, UI_MSG_ERRO);
	}
	
if ($email_tarefa_responsavel) {
	if ($msg = $obj->notificarResponsavel($email_texto, $nao_eh_novo)) $Aplic->setMsg($msg, UI_MSG_ERRO);
	}	
