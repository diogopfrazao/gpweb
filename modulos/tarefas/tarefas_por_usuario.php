...
if ($Aplic->profissional){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$filtro_projeto.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
function mostrarTarefa_por_usuario($lista, $tarefa, $usuario_id) {
	global $Aplic, $config, $tipoDuracao, $log_filtroUsuario_usuarios, $agora, $prioridade, $sistema_usuarios, $z, $zi, $x, $usuarioDesig, $projetos, $projeto_id;
	$tmp ='';
	if (permiteAcessar($tarefa->tarefa_acesso, $tarefa->tarefa_projeto, $tarefa->tarefa_id)){
		...
		$tmp .= '<td align="left" style="white-space: nowrap">';
		...
		$tmp .= '<td align="center" style="white-space: nowrap">'.$tarefa->tarefa_duracao.'&nbsp;'.substr($tipoDuracao[$tarefa->tarefa_duracao_tipo],0,1).'</td>';
		$tmp .= '<td align="center" style="white-space: nowrap">';
		...
		$s = '<td align="left" style="white-space: nowrap">&nbsp;'.link_usuario($lista_usuarios[0]['usuario_id'],'','','esquerda').' ('.$usuarios[$lista_usuarios[0]['usuario_id']]['perc_designado'].'%)&nbsp;';
		if ($total> 1) {
			$lista='';
...
function datasSemana($mostrar_horas_alocadas, $doPeriodo, $atePeriodo) {
	...
	for ($i = $sw; $i <= $ew; $i++) {
		...
		$linha .= '<th style="white-space: nowrap">'.dica($wn.'ª Semana de '.date('Y',$ts_start),'De '.date('d/m/Y',$ts_start).' Dom à '.date('d/m/Y',$ts_end).' Sab.').$wn.dicaF().'</th>';
...
function mostrarSemanas($lista, $tarefa, $nivel, $doPeriodo, $atePeriodo) {
	...
	for ($i = $sw; $i <= $ew; $i++) {
		if ($i >= $semanaInicioTarefa and $i < $semanaFimTarefa) {
			...
			$linha .= '<td  id="ignore_td_" style="white-space: nowrap" bgcolor="'.$cor.'">';
			}
		else $linha .= '<td style="white-space: nowrap">';
		$linha .= '&#160&#160</td>';
		}
	return $linha;
	}
