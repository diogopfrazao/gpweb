...
if (!$dialogo || $selecao){
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionado','A qual parte do sistema os patrocinadores estão relacionados.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	$botoesTitulo = new CBlocoTitulo('Lista de '.ucfirst($config['patrocinadores']), '../../../modulos/patrocinadores/imagens/patrocinador.gif', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$novo_patrocinador=($podeAdicionar ? '<tr><td style="white-space: nowrap">'.dica('Nov'.$config['genero_patrocinador'].' '.ucfirst($config['patrocinador']), 'Criar nov'.$config['genero_patrocinador'].' '.$config['patrocinador'].'.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=patrocinadores&a=patrocinador_editar\');" >'.imagem('../../../modulos/patrocinadores/imagens/patrocinador_novo.png').'</a>'.dicaF().'</td></tr><tr><td style="white-space: nowrap"></td></tr>' : '');
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['patrocinadores'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&u='.$u.'&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
	
	$botao_favorito=($Aplic->profissional ? '<tr><td style="white-space: nowrap"><a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&patrocinador=1\');">'.imagem('icones/favorito_p.png','Favoritos', 'Clique neste ícone '.imagem('icones/favorito_p.png').' para criar ou editar um grupo de favoritos, para uma rápida filtragem.').'</a></td></tr>' : '');
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$procuraBuffer.$favoritos.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.(!$selecao ? $novo_patrocinador.$imprimir.$botao_prioridade.$botao_favorito.$botao_trava.$botao_campos : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
...
if (!$dialogo || $selecao){
	$caixaTab->adicionar(BASE_DIR.'/modulos/patrocinadores/patrocinador_ver_idx', 'Ativ'.$config['genero_patrocinador'].'s',null,null,'Ativ'.$config['genero_patrocinador'].'s','Visualizar '.$config['genero_patrocinador'].'s '.$config['patrocinadores'].' ativ'.$config['genero_patrocinador'].'s.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/patrocinadores/patrocinador_ver_idx', 'Inativ'.$config['genero_patrocinador'].'s',null,null,'Inativ'.$config['genero_patrocinador'].'s','Visualizar '.$config['genero_patrocinador'].'s '.$config['patrocinadores'].' inativ'.$config['genero_patrocinador'].'s.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/patrocinadores/patrocinador_ver_idx', 'Tod'.$config['genero_patrocinador'].'s',null,null,'Tod'.$config['genero_patrocinador'].'s','Visualizar todos '.$config['genero_patrocinador'].'s '.$config['patrocinadores'].'.');
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=patrocinadores', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=patrocinadores&a=index');
	}	
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&patrocinador=1&filtro_prioridade='+env.filtro_prioridade_patrocinador.value, window.setFiltroPriorizacao, window);
	}
