...
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($patrocinador_id ? '&patrocinador_id='.$patrocinador_id  : '').($tab ? '&tab='.$tab : '').'&ordenar=recurso_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenarPor=='recurso_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome', 'Neste campo fica um nome para identificação do recurso.').'Nome'.dicaF().'</th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($patrocinador_id ? '&patrocinador_id='.$patrocinador_id  : '').($tab ? '&tab='.$tab : '').'&ordenar=recurso_responsavel&ordem='.($ordem ? '0' : '1').'\');">'.($ordenarPor=='recurso_responsavel' ? imagem('icones/'.$seta[$ordem]) : '').dica('Responsável', 'Neste campo fica o responsável pelo recurso.').'Responsável'.dicaF().'</th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($patrocinador_id ? '&patrocinador_id='.$patrocinador_id  : '').($tab ? '&tab='.$tab : '').'&ordenar=recurso_quantidade&ordem='.($ordem ? '0' : '1').'\');">'.($ordenarPor=='recurso_quantidade' ? imagem('icones/'.$seta[$ordem]) : '').dica('Quantidade', 'Neste campo fica a quantidade total deste recurso.').'Quantidade'.dicaF().'</a></th>';
...
for ($i = ($pagina - 1) * $xpg_tamanhoPagina; $i < $pagina * $xpg_tamanhoPagina && $i < $xpg_totalregistros; $i++) {
	...
	if (permiteAcessarRecurso($linha['recurso_nivel_acesso'], $linha['recurso_id'])){	
		...
		echo '<tr>';
		echo '<td style="white-space: nowrap" width="16">'.($editar ? dica('Editar instrumento', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o instrumento.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=recursos&a=editar&recurso_id='.$linha['recurso_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
		echo '<td style="white-space: nowrap">'.dica($linha['recurso_nome'], 'Clique para visualizar os detalhes deste instrumento.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=recursos&a=ver&recurso_id='.$linha['recurso_id'].'\');">'.$linha['recurso_nome'].'</a>'.dicaF().'</td>';
		echo '<td>'.link_usuario($linha['recurso_responsavel'],'','','esquerda').'</td>';
		echo '<td style="white-space: nowrap" align="center">'.number_format($linha['recurso_quantidade'], 2, ',', '.').'</td>';
		echo '</tr>';
		}
	}
