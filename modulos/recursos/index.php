...
$identificador='<tr id="identificador" '.($recurso_tipo!=5 ? 'style="display:none"' : '').' ><td align="right" style="white-space: nowrap">'.dica('Identificador de Uso', 'O uso deste recurso.').'Idt:'.dicaF().'</td><td>'.selecionaVetor($IdentificadorUso, 'recurso_identificador_uso', 'class=texto size=1 style="width:250px;"', $recurso_identificador_uso).'</td></tr>';
$credito_adicional='<tr id="credito_adicional" '.($recurso_tipo!=5 ? 'style="display:none"' : '').' ><td align="right" style="white-space: nowrap">'.dica('Crédito Adicional', 'Caso seja monetário, seleciona o crédito adicional deste recurso, se for o caso.').'Crédito adicional:'.dicaF().'</td><td>'.selecionaVetor($CreditoAdicional, 'recurso_credito_adicional', 'style="width:250px;" class="texto"', $recurso_credito_adicional).'</td></tr>';
$movimentacao='<tr id="movimentacao" '.($recurso_tipo!=5 ? 'style="display:none"' : '').' ><td align="right" style="white-space: nowrap">'.dica('Movimentação Orcamentária', 'Caso seja monetário, seleciona a movimentação orcamentária deste recurso, se for o caso.').'Movimentação:'.dicaF().'</td><td>'.selecionaVetor($MovimentacaoOrcamentaria, 'recurso_movimentacao_orcamentaria', 'style="width:250px;" class="texto"', $recurso_movimentacao_orcamentaria).'</td></tr>';
$ptres='<tr id="ptres" '.($recurso_tipo!=5 ? 'style="display:none"' : '').' ><td align="right" style="white-space: nowrap">'.dica('Plano de Trabalho Resumido', 'Insira o plano de trabalho resumido deste recurso.').'PTRES:'.dicaF().'</td><td>'.selecionaVetor($listaPtres, 'recurso_ptres', 'style="width:250px;" class="texto"', $recurso_ptres).'</td></tr>';
...
$botao_campos=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="popCamposExibir();">'.imagem('icones/campos_p.gif', 'Campos' , 'Clique neste ícone '.imagem('campos_p.gif').' para escolha quais campos dos recursos deseja exibir.').'</a>'.dicaF().'</td></tr>' : '');
...
if (!$dialogo || $selecao){
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionado','A qual parte do sistema os monitoramentos estão relacionados.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$identificador.$tipo.$pesquisa.$credito_adicional.$movimentacao.$ano.$ptres.$ugrs.$favoritos.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$filtro.(!$selecao ? $novo.$botao_prioridade.$botao_favorito.$botao_trava.$botao_campos : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
	$botoesTitulo->adicionaCelula($saida);
	$botoesTitulo->mostrar();
	}
elseif ($Aplic->profissional && !$selecao){
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&recurso=1&filtro_prioridade='+env.filtro_prioridade_recurso.value, window.setFiltroPriorizacao, window);
	}
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=recursos', window.setCamposExibir, window);
	}

function setCamposExibir(){
	url_passar(0, 'm=recursos&a=index');
	}
