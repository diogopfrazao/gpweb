...
if (1 == 1) {
	...
	// Campos 31 - 35
	$texto .= sprintf("%s", "\"Telefone Comercial\",\"Telefone comercial 2\",\"Retorno de chamada\",\"Telefone do carro\",");
	// Campos 36 - 40
	$texto .= sprintf("%s", "\"Telefone principal da empresa\",\"Fax residencial\",\"Telefone residencial\",\"Telefone residencial 2\",\"ISDN\",");
	// Campos 41 - 45
	$texto .= sprintf("%s", "\"Telefone celular\",\"Outro telefone\",\"Pager\",\"Telefone principal\",");
	// Campos 46 - 50
	...
	$q = new BDConsulta;
	$q->adTabela('campo_customizado');
	$q->adOnde('campo_customizado_modulo = \'contatos\'');
	$q->adOrdem('campo_customizado_ordem');
	$campos_customizados = $q->Lista();
	foreach ($campos_customizados as $f) $texto .= sprintf("%s", "\"$f[campo_customizado_descricao]\",");
	$texto .= sprintf("%s\r\n", "");
	$q->limpar();
	...
	foreach ($contatos as $linha) {
	  ...
	  // Campos 6 - 10
		...
		$texto .= sprintf(",,,,,");
		// Campos 31 - 35
		$texto .= sprintf("\"%s\",\"%s\",,,,", $linha['contato_tel']);
		// Campos 36 - 40
