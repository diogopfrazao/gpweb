			...
			while (isset($ci['TEL'][++$i])){
				if (in_array('VOICE', $ci['TEL'][$i]['param']['TYPE'])){
					if (in_array('CELL', $ci['TEL'][$i]['param']['TYPE'])){
						$contatoValores['contato_cel'] = $ci['TEL'][$i]['valor'][0][0];
						} 
					elseif (in_array('HOME', $ci['TEL'][$i]['param']['TYPE'])){
						$contatoValores['contato_tel2'] = $ci['TEL'][$i]['valor'][0][0];
						}
					elseif (in_array('WORK', $ci['TEL'][$i]['param']['TYPE'])){
						$contatoValores['contato_tel'] = $ci['TEL'][$i]['valor'][0][0];
						} 
					}
				}
...
elseif (isset($_REQUEST['dialogo']) && (getParam($_REQUEST, 'dialogo', null) == '0')) { 
	...
	echo '<tr><td align="center" style="white-space: nowrap">'.dica('Arquivo vCard', 'Selecione o arquivo no formato vCard a ser importado.').'Arquivo vCard:'.dicaF().'<input name="vcf" type="File" style="width:280px" accept="text/x-vcard"></td></tr>';
	echo '<tr><td style="white-space: nowrap">'.botao('salvar', 'Salvar', 'Salvar a importação.','','vcfFrm.submit()').'</td><td align="right" align="left">'.botao('cancelar', 'Cancelar', 'Cancelar e retornar a tela anterior.','','url_passar(0, \''.$Aplic->getPosicao().'\');').'</td></tr>';
	echo '</table></form>';
