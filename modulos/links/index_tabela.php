...
if ($exibir['link_cor']) echo '<th width=16><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=link_cor&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='link_cor' ? imagem('icones/'.$seta[$ordem]) : '').dica('Cor', 'Neste campo fica a cor de identificação do link.').'Cor'.dicaF().'</a></th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=link_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='link_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome', 'Neste campo fica um nome para identificação do link.').'Nome'.dicaF().'</th>';
...
if ($filtro_prioridade_link) echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=link_url&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='link_url' ? imagem('icones/'.$seta[$ordem]) : '').dica('Endereço Eletrônico da Referência', 'Caso exista um link para página ou arquivo na rede que faça referência ao registro.').'URL'.dicaF().'</a></th>';
if ($exibir['link_descricao']) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=link_descricao&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='link_descricao' ? imagem('icones/'.$seta[$ordem]) : '').dica('Descrição', 'Neste campo fica a descrição pormenorizada do link.').'Descrição'.dicaF().'</th>';
if ($exibir['link_relacionado']) echo '<th style="white-space: nowrap">'.dica('Relacionado', 'Neste campo fica a quais áreas do sistema o link está relacionado.').'Relacionado'.dicaF().'</a></th>';
...
if ($exibir['link_dept']) echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=link_dept&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica(ucfirst($config['departamento']), strtoupper($config['genero_dept']).' '.strtolower($config['departamento']).' responsável.').($ordenar=='link_dept' ? imagem('icones/'.$seta[$ordem]) : '').ucfirst($config['departamento']).dicaF().'</a></th>';
...
if ($exibir['link_dono']) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=link_dono&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='link_dono' ? imagem('icones/'.$seta[$ordem]) : '').dica('Responsável', 'Neste campo fica o nome d'.$config['genero_usuario'].' '.$config['usuario'].' responsável pela inserção do link.').'Responsável'.dicaF().'</th>';
...
if(!$from_lista) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=link_ativo&ordem='.($ordem ? '0' : '1').$extra.'\');">'.($ordenar==='link_ativo' ? imagem('icones/'.$seta[$ordem]) : '').dica('Ativo', 'Neste campo consta se o link está ativo.').'At.'.dicaF().'</a></th>';
...
foreach ($linhas as $linha) {
	if (permiteAcessarLink($linha['link_acesso'],$linha['link_id'])) {
		...
		if (!$dialogo) echo '<td style="white-space: nowrap" width="16">'.($editar && $podeEditar && !$bloquear ? dica('Editar Link', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o link.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=links&a=editar&link_id='.$linha['link_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
		...
		if ($Aplic->profissional && $exibir['link_aprovado']) echo '<td style="width: 30px; text-align: center">'.($linha['link_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
		...
		else echo '<td align="center" valign="middle" width=16>'.dica('Link', 'Clique neste ícone '.imagem('icones/link.png').' para  acessar:<ul><li>'.$linha['link_url'].'</ul>').'<a href="'.$linha['link_url'].'" target="_blank">'.imagem('icones/link.png').'</a>'.dicaF().'</td>';
        ...
        if (!$from_lista) echo '<td style="width: 30px; text-align: center">'.($linha['link_ativo'] ? 'Sim' : 'Não').'</td>';
