...
if (isset($_REQUEST['link_id'])) $Aplic->setEstado('link_id', getParam($_REQUEST, 'link_id', null), $m, $a, $u);
$link_id = $Aplic->getEstado('link_id', null, $m, $a, $u);
...
$obj = new CLink();
$obj->load($link_id);
if (!($podeAcessar && permiteAcessarLink($obj->link_acesso, $link_id))) $Aplic->redirecionar('m=publico&a=acesso_negado');

$editar=permiteEditarLink($obj->link_acesso, $link_id);

$podeEditarTudo=($obj->link_acesso>=5 ? $editar && (in_array($obj->link_dono, $Aplic->usuario_lista_grupo_vetor) || $Aplic->usuario_super_admin || $Aplic->usuario_admin) : $editar);
...
if (!$dialogo){	
	...
	if ($podeAdicionar)	$km->Add("inserir","inserir_objeto",dica('Novo Link', 'Criar um novo link.').'Novo Link'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=links&a=editar\");");
	if ($podeEditar && $editar) {	
		$km->Add("inserir","inserir_registro",dica('Registro de Ocorrência','Inserir um novo registro de ocorrência.').'Registro de ocorrência'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=log_editar&link_id=".$link_id."\");");
		if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('eventos', 'adicionar')) $km->Add("inserir","inserir_evento",dica('Novo Evento', 'Criar um novo evento relacionado.').'Evento'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=calendario&a=editar&evento_link=".$link_id."\");");
		...
		if ($Aplic->checarModulo('projetos', 'adicionar', null, 'demanda')) $km->Add("inserir","inserir_demanda",dica('Nova Demanda', 'Inserir uma nova demanda relacionada.').'Demanda'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=demanda_editar&demanda_link=".$link_id."\");");
		...
		if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'adicionar')) $km->Add("diverso","inserir_link",dica('Novo Link', 'Inserir um novo link relacionado.').'Link'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=links&a=editar&link_link=".$link_id."\");");
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("diverso","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_link=".$link_id."\");");
	...
	if ($podeEditarTudo && $podeEditar && !$bloquear) $km->Add("acao","acao_editar",dica('Editar Link','Editar os detalhes deste link.').'Editar Link'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=links&a=editar&link_id=".(int)$link_id."\");");
	if ($podeEditarTudo && $podeExcluir && !$bloquear) $km->Add("acao","acao_excluir",dica('Excluir','Excluir este link.').'Excluir Link'.dicaF(), "javascript: void(0);' onclick='excluir()");
...
echo '<tr><td style="border: outset #d1d1cd 1px;background-color:#'.$obj->link_cor.'" colspan="2"><font color="'.melhorCor($obj->link_cor).'"><b>'.$obj->link_nome.'<b></font></td></tr>';


echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nome', 'Neste campo deve constar um nome para identificação deste link.').'Nome:'.dicaF().'</td><td align="left" class="realce">'.$obj->link_nome.'</td></tr>';

if ($obj->link_cia) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', ucfirst($config['genero_organizacao']).' '.$config['organizacao'].' responsável.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_cia($obj->link_cia).'</td></tr>';
if ($Aplic->profissional){
...
if ($obj->link_dept) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']).' Responsável', ucfirst($config['genero_dept']).' '.$config['departamento'].' responsável por este link.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_dept($obj->link_dept).'</td></tr>';
$saida_depts='';
...
if ($obj->link_dono) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Responsável', ucfirst($config['usuario']).' responsável por gerenciar o link.').'Responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($obj->link_dono, '','','esquerda').'</td></tr>';		
...
if (count($lista)) {
	echo '<tr><td align="right" style="white-space: nowrap" valign="middle">'.dica('Relacionado','A qual parte do sistema o link está relacionado.').'Relacionado:'.dicaF().'</td></td><td class="realce">';	
...
if (isset($tipo[$obj->link_categoria])) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Categoria', 'A categoria à qual este link pertence.').'Categoria:'.dicaF().'</td><td align="left" class="realce">'.$tipo[$obj->link_categoria].'</tr>';

echo '<tr><td align="right" style="white-space: nowrap">'.dica('URL', 'A parte mais importante do do link é o URL do mesmo. Neste campo deve constar o endereço tal como visualizado na tela no Navegador Web caso seje uma página da Internet.').'URL do Link:'.dicaF().'</td><td align="left" class="realce"><a href="javascript: void(0);" onclick="window.open(\''.get_protocol().$obj->link_url.'\',\'_blank\',\'\')">'.get_protocol().$obj->link_url.'</a></td></tr>';
if ($obj->link_descricao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Descrição', 'Ao se enviar um link, pode-se escrever um texto explicativo para facilitar a compreensão do link e facilitar futuras pesquisas.').'Descrição:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->link_descricao.'</td>';


if ($obj->link_principal_indicador) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Indicador Principal', 'Dentre os indicadores relacionados, o mais representativo da situação geral.').'Indicador principal:'.dicaF().'</td><td width="100%" class="realce">'.link_indicador($obj->link_principal_indicador).'</td></tr>';
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	if (isset($moedas[$obj->link_moeda])) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Moeda', 'A moeda padrão utilizada na meta.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->link_moeda].'</td></tr>';	
	}
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Aprovado', 'Se o link se encontra aprovado.').'Aprovado:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->link_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Ativo', 'Se o link se encontra ativo.').'Ativo:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.($obj->link_ativo ? 'Sim' : 'Não').'</td></tr>';



require_once ($Aplic->getClasseSistema('CampoCustomizados'));
$campos_customizados = new CampoCustomizados('links', $link_id, 'ver');
...
if ($Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao_item')) {
	...
	$sql->adOnde('plano_acao_item_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('projetos', 'acesso', null, 'demanda')) {
	...
	$sql->adOnde('demanda_gestao_link = '.(int)$link_id);
...
if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'acesso')) {
		...
		$sql->adOnde('link_gestao_semelhante = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco_resposta')) {		
	...
	$sql->adOnde('risco_resposta_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('recursos', 'acesso', null, null)) {
	...
	$sql->adOnde('recurso_gestao_link = '.(int)$link_id);
...
if ($Aplic->modulo_ativo('patrocinadores') && $Aplic->checarModulo('patrocinadores', 'acesso', null, null)) {
	...
	$sql->adOnde('patrocinador_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'programa')) {
	...
	$sql->adOnde('programa_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'beneficio')) {
	...
	$sql->adOnde('beneficio_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('projeto', 'acesso', 'licao')) {
	...
	$sql->adOnde('licao_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('praticas', 'acesso', null, 'pratica')) {
	...
	$sql->adOnde('pratica_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->modulo_ativo('tr') && $Aplic->checarModulo('tr', 'acesso', null, null)) {
	...
	$sql->adOnde('tr_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('praticas', 'acesso', null, 'brainstorm')) {
	...
	$sql->adOnde('brainstorm_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('praticas', 'acesso', null, 'gut')) {
	...
	$sql->adOnde('gut_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('praticas', 'acesso', null, 'causa_efeito')) {
	...
	$sql->adOnde('causa_efeito_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'tgn')) {
	...
	$sql->adOnde('tgn_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'canvas')) {
	...
	$sql->adOnde('canvas_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->modulo_ativo('swot') && $Aplic->checarModulo('swot', 'acesso', null, null)) {
	...
	$sql->adOnde('mswot_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->modulo_ativo('swot') && $Aplic->checarModulo('swot', 'acesso', null, null)) {
	...
	$sql->adOnde('swot_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->modulo_ativo('operativo') && $Aplic->checarModulo('operativo', 'acesso', null, null)) {
	...
	$sql->adOnde('operativo_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'monitoramento')) {
	...
	$sql->adOnde('monitoramento_gestao_link = '.(int)$link_id);
	...
	$sql->adOnde('avaliacao_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('praticas', 'acesso', null, 'checklist')) {
	...
	$sql->adOnde('checklist_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional) {
	...
	$sql->adOnde('agenda_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->modulo_ativo('agrupamento') && $Aplic->checarModulo('agrupamento', 'acesso', null, null)) {
	...
	$sql->adOnde('agrupamento_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'modelo')) {
	...
	$sql->adOnde('template_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'painel_indicador')) {
	...
	$sql->adOnde('painel_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'odometro_indicador')) {
	...
	$sql->adOnde('painel_odometro_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'composicao_painel')) {
	...
	$sql->adOnde('painel_composicao_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'slideshow_painel')) {
	...
	$sql->adOnde('painel_slideshow_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->checarModulo('agenda', 'acesso', null, null)) {
	...
	$sql->adOnde('calendario_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('praticas', 'acesso', null, 'perspectiva')) {
	...
	$sql->adOnde('perspectiva_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('praticas', 'acesso', null, 'tema')) {
	...
	$sql->adOnde('tema_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('praticas', 'acesso', null, 'objetivo')) {
	...
	$sql->adOnde('objetivo_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('praticas', 'acesso', null, 'me')) {
	...
	$sql->adOnde('me_gestao_link = '.(int)$link_id);
...
if ($config['exibe_fator'] && $Aplic->checarModulo('praticas', 'acesso', null, 'fator')) {
	...
	$sql->adOnde('fator_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('praticas', 'acesso', null, 'iniciativa')) {
	...
	$sql->adOnde('estrategia_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('praticas', 'acesso', null, 'meta')) {
	...
	$sql->adOnde('meta_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('praticas', 'acesso', null, 'planejamento')) {
	...
	$sql->adOnde('plano_gestao_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('projetos', 'acesso', null, 'abertura')) {
	...
	$sql->adOnde('projeto_abertura_gestao_link = '.(int)$link_id);
...
if ($Aplic->checarModulo('projetos', 'acesso', null, 'viabilidade')) {
	...
	$sql->adOnde('projeto_viabilidade_gestao_link = '.(int)$link_id);
...
if ($Aplic->profissional && $Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso')) {
		...
		$sql->adOnde('ssti_gestao_link = '.(int)$link_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso',null, 'laudo')) {
		...
		$sql->adOnde('laudo_gestao_link = '.(int)$link_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'acesso')) {
		...
		$sql->adOnde('trelo_gestao_link = '.(int)$link_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'acesso')) {
		...
		$sql->adOnde('trelo_cartao_gestao_link = '.(int)$link_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso')) {
		...
		$sql->adOnde('pdcl_gestao_link = '.(int)$link_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso')) {
		...
		$sql->adOnde('pdcl_item_gestao_link = '.(int)$link_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('os') && $Aplic->checarModulo('os', 'acesso')) {
		...
		$sql->adOnde('os_gestao_link = '.(int)$link_id);
