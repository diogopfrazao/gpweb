...
if (getParam($_REQUEST, 'inserir', 0)){
	...
	for ($i=$data1; $i->format('%Y%m%d')<=$data2->format('%Y%m%d'); $i->adDias(1)){
		if (!$dias_semana || in_array($conversao[$i->format("%w")] , $dias_semana)){
			$sql->adTabela('jornada_excessao');
			$sql->adInserir('jornada_excessao_data', $i->format('%Y%m%d'));
...
echo '<tr>';
echo '<td align="left" style="white-space: nowrap" width="120" ><table cellspacing=0 cellpadding=0><tr><td>&nbsp;'.dica('Data Inícial', 'Digite ou escolha no calendário a data inícial do intervalo de tempo em que serão editados os expedientes.').'Data Inicial'.dicaF().'</td></tr><tr><td style="white-space: nowrap"><input type="hidden" name="periodo_inicial" id="periodo_inicial" value="'.$periodo_inicial.'" /><input type="text" size="9" name="data_inicial" id="data_inicial" onchange="setData(\'env\', \'inicial\');" value="'.($periodo_inicial ? $data->format('%d/%m/%Y') : '').'" class="texto" />'.dica('Data Inicial', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data inícial do intervalo de tempo em que serão editados os expedientes.').'<a href="javascript: void(0);" ><img src="'.acharImagem('calendario.gif').'" id="f_btn1" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 />'.dicaF().'</a></td></tr>';
if ($periodo_final) $data = new CData($periodo_final);
echo '<tr><td>&nbsp;</td></tr><tr><td>&nbsp;'.dica('Data Final', 'Digite ou escolha no calendário a data final do intervalo de tempo em que serão editados os expedientes.').'Data Final</td></tr><tr><td style="white-space: nowrap"><input type="hidden" name="periodo_final" id="periodo_final" value="'.$periodo_final.'" /><input type="text" size="9" name="data_final" id="data_final" onchange="setData(\'env\', \'final\');" value="'.($periodo_final ? $data->format('%d/%m/%Y') : '').'" class="texto" /><a href="javascript: void(0);" >'.dica('Data Final', 'Clique neste ícone '.imagem('icones/calendario.gif').' para abrir um calendário onde poderá selecionar a data final do intervalo de tempo em que serão editados os expedientes.').'<img id="f_btn2" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 />'.dicaF().'</a></td></tr></table></td>';
echo '<td align="left" style="white-space: nowrap" width="100"><table cellspacing=0 cellpadding=0>';
