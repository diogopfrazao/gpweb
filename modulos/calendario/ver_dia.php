...
$aa = $este_dia->getYear();
$esta_semana = Data_calc::beginOfWeek($dd, $mm, $aa, '%Y%m%d', (defined(localidade_PRIMEIRO_DIA) ? localidade_PRIMEIRO_DIA : 1));
...
$dia_ant = new CData(Data_calc::diaAnterior($dd, $mm, $aa, '%Y%m%d'));
$dia_prox = new CData(Data_calc::proxDia($dd, $mm, $aa, '%Y%m%d'));
...
if (!$dialogo){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$novo_evento=($podeAdicionar ? '<tr><td style="white-space: nowrap">'.dica('Novo Evento', 'Criar um novo evento.<br><br>Os eventos são atividades com data e hora específicas podendo estar relacionados com '.$config['projetos'].', '.$config['tarefas'].' e '.$config['usuarios'].' específicos.').'<a href="javascript: void(0)" onclick="javascript:env.a.value=\'editar\'; env.submit();" >'.imagem('calendario_novo.png').'</a>'.dicaF().'</td></tr><tr><td style="white-space: nowrap"></td></tr>' : '');
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$procurar_filtro.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$novo_evento.$imprimir.$icone_periodo_todo.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
$titulo=dia_semana($este_dia->format('%A')).', '.$este_dia->format('%d/%m/%Y');
echo '<tr><td valign="top"><table border=0 cellspacing=0 cellpadding=0 width="100%" class="motitulo">';
echo '<tr><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=calendario&a=ver_dia&data='.$dia_ant->format('%Y%m%d').'\');">'.imagem('icones/'.($estilo_interface=='metro' ? 'navAnterior_metro.png' :'anterior.gif'), 'Dia Anterior', 'Clique neste ícone '.imagem('icones/'.($estilo_interface=='metro' ? 'navAnterior_metro.png' :'anterior.gif')).' para exibir o dia anterior.').'</a></td><td width="100%" align=center><b>'.$titulo.'</b></td><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=calendario&a=ver_dia&data='.$dia_prox->format('%Y%m%d').'\');">'.imagem('icones/'.($estilo_interface=='metro' ? 'navProximo_metro.png' :'proximo.gif'), 'Próximo Dia', 'Clique neste ícone '.imagem('icones/'.($estilo_interface=='metro' ? 'navProximo_metro.png' :'proximo.gif')).' para exibir o próximo dia.').'</a></td></tr>';
echo '</table>';


if (!$dialogo){
	$caixaTab = new CTabBox('m=calendario&a=ver_dia&data='.$este_dia->format('%Y%m%d'), '', $tab);
	...
	if ($Aplic->modulo_ativo('atas')) $caixaTab->adicionar(BASE_DIR.'/modulos/calendario/ver_atas_dia', 'Atas de Reunião',null,null,'Atas de Reunião','Visualizar as ações de atas de reunião em execução neste dia.');
...
if (!$dialogo && $config['cal_ver_dia_mostrar_minical']) {
  echo '<td valign="top" width="175">';
	...
	$pdcl_item_id,
	$os_id);
	elseif ($tab==3) $links=getAcaoLinks($periodo_todo, $primeiraData, $ultimaData, $links, $tamanho, true, $usuario_id, $envolvimento, $cia_id, $dept_id, 
		$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$pdcl_item_id,
	$os_id);
	elseif ($tab==4) $links=getAtaLinks($periodo_todo, $primeiraData, $ultimaData, $links, $tamanho, true, $usuario_id, $envolvimento,$cia_id, $dept_id, 
		$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
