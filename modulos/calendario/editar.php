...
echo '<tr><td align="right" style="white-space: nowrap;width:150px">'.dica('Nome', 'Qual o nome do evento.Cada evento deve ter um nome que facilite a compreensão do mesmo').'Nome:'.dicaF().'</td><td><input type="text" class="texto" style="width:400px;" name="evento_titulo" value="'.$obj->evento_titulo.'" maxlength="255" />*</td></tr>';
...
echo '<tr '.($evento_os ? '' : 'style="display:none"').' id="os" ><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['os']), 'Caso seja específico de '.($config['genero_os']=='o' ? 'um' : 'uma').' '.$config['os'].', neste campo deverá constar o nome d'.$config['genero_os'].' '.$config['os'].'.').ucfirst($config['os']).':'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="evento_os" value="'.$evento_os.'" /><input type="text" id="os_nome" name="os_nome" value="'.nome_os($evento_os).'" style="width:400px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="pop_os();">'.imagem('icones/os_p.png','Selecionar '.ucfirst($config['os']),'Clique neste ícone '.imagem('icones/os_p.png').' para selecionar '.($config['genero_os']=='o' ? 'um' : 'uma').' '.$config['os'].'.').'</a></td></tr></table></td></tr>';
...
if ($exibir['evento_descricao']) echo '<tr><td align="right" valign="middle" style="white-space: nowrap">'.dica('Descrição', 'Um resumo sobre o evento.').'Descrição:'.dicaF().'</td><td><textarea class="textarea" name="evento_descricao" data-gpweb-cmp="ckeditor" rows="5" cols="45">'.$obj->evento_descricao.'</textarea></td></tr>';
...
echo '<tr><td align=right width="100" >'.dica('Data de Início', 'A data de início do evento.').'Início:'.dicaF().'</td><td><input type="hidden" id="oculto_data_inicio" name="oculto_data_inicio"  value="'.$data_inicio->format('%Y-%m-%d').'" /><input type="text" onchange="setData(\'env\', \'data_inicio\'); data_ajax();" class="texto" style="width:70px;" id="data_inicio" name="data_inicio" value="'.$data_inicio->format('%d/%m/%Y').'" />'.dica('Data Inicial', 'Clique neste ícone '.imagem('icones/calendario.gif').' para abrir um calendário onde poderá selecionar a data de início.').'<a href="javascript: void(0);" ><img id="f_btn1" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário"" border=0 /></a>'.dicaF().dica('Hora do Início', 'Selecione na caixa de seleção a hora do ínicio do evento.'). selecionaVetor($horas, 'inicio_hora', 'size="1" onchange="CompararDatas(); data_ajax();" class="texto"', $data_inicio->getHour()).' : '.dica('Minutos do Início', 'Selecione na caixa de seleção os minutos do ínicio do evento.').selecionaVetor($minutos, 'inicio_minuto', 'size="1" class="texto" onchange="CompararDatas(); data_ajax();"', $data_inicio->getMinute()).'</td></tr>';
echo '<tr><td align=right>'.dica('Data de Término', 'A data de término do evento.').'Término:'.dicaF().'</td><td><input type="hidden" id="oculto_data_fim" name="oculto_data_fim" value="'.($data_fim ? $data_fim->format('%Y-%m-%d') : '').'" /><input type="text" onchange="setData(\'env\', \'data_fim\'); horas_ajax();" class="texto" style="width:70px;" id="data_fim" name="data_fim" value="'.($data_fim ? $data_fim->format('%d/%m/%Y') : '').'" />'.dica('Data de Término', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data de término.').'<a href="javascript: void(0);" ><img id="f_btn2" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário"" border=0 /></a>'.dicaF().dica('Hora do Término', 'Selecione na caixa de seleção a hora do término.</p>Caso não saiba a hora provável de término, deixe em branco este campo e clique no botão <b>Data de Término</b>').selecionaVetor($horas, 'fim_hora', 'size="1" onchange="CompararDatas(); horas_ajax();" class="texto"', $data_fim ? $data_fim->getHour() : $fim).' : '.dica('Minutos do Término', 'Selecione na caixa de seleção os minutos do término. </p>Caso não saiba os minutos prováveis de término, deixe em branco este campo e clique no botão <b>Data de Término</b>').selecionaVetor($minutos, 'fim_minuto', 'size="1" class="texto" onchange="CompararDatas(); horas_ajax();"', $data_fim ? $data_fim->getMinute() : '00').'</td></tr>';
echo '<tr><td align="right" nowrap="nowrap">'.dica('A duração do evento em dias.').'Duração:'.dicaF().'</td><td nowrap="nowrap"><input type="text" onchange="data_ajax();" onkeypress="return somenteFloat(event)" class="texto" name="evento_duracao" id="evento_duracao" maxlength="30" size="2" value="'.float_brasileiro((float)$obj->evento_duracao/($config['horas_trab_diario'] ? $config['horas_trab_diario'] : 8)).'" />&nbsp;dias</td></tr>';

echo '<tr><td align="right" style="white-space: nowrap">'.dica('Recorrência', 'De quanto em quanto tempo este evento se repete.').'Recorrência:'.dicaF().'</td><td>'.selecionaVetor($recorrencia, 'evento_recorrencias', 'size="1" class="texto"', $obj->evento_recorrencias).dica('Número de Recorrencias', 'Escolha o número de vezes que a faixa de tempo escolhida repetirá.').'x'.dicaF().'<input type="text" class="texto" name="evento_nr_recorrencias" value="'.((isset($obj->evento_nr_recorrencias)) ? ($obj->evento_nr_recorrencias) : '1').'" maxlength="2" size="3" />'.dica('Número de Recorrencias', 'Escolha o número de vezes que a faixa de tempo escolhida repetirá.').'vezes'.dicaF().'</td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('De 2ª a 6ª feira', 'Marque esta caixa para que a faixa de tempo do evento não inclua os fins de semana.').'<label for="evento_diautil">De 2ª a 6ª feira:</label>'.dicaF().'</td><td><input type="checkbox" value="1" name="evento_diautil" id="evento_diautil" '.($obj->evento_diautil ? 'checked="checked"' : '').' /></td></tr>';
...
if ($Aplic->profissional){
	...
	$sql->limpar();
	if (count($indicadores)>1) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Indicador Principal', 'Escolha dentre os indicadores relacionados mais representativo da situação geral.').'Indicador principal:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($indicadores, 'evento_principal_indicador', 'class="texto" style="width:284px;"', $obj->evento_principal_indicador).'</td></tr>';
	}
	
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nível de Acesso', 'O evento pode ter cinco níveis de acesso:<ul><li><b>Público</b> - Todos podem ver e editar.</li><li><b>Protegido I</b> - Todos podem ver, porem apenas o responsável e os designados podem editar.</li><li><b>Protegido II</b> - Todos podem ver, porem apenas o responsável pode editar tudo e os designados editarem os objetos relacionados.</li><li><b>Protegido III</b> - Todos podem ver, porem apenas o responsável pode editar.</li><li><b>Participante I</b> - Somente o responsável, participantes e designados podem ver e responsável e designados editar</li><li><b>Participantes II</b> - Somente o responsável, participantes e os designados podem ver e apenas o responsável pode editar tudo e os designados editarem os objetos relacionados.</li><li><b>Participantes III</b> - Somente o responsável, participantes e os designados podem ver e o responsável editar.</li></ul>').'Nível de acesso:'.dicaF().'</td><td colspan="2">'.selecionaVetor($niveis_acesso, 'evento_acesso', 'class="texto"', ($evento_id ? $obj->evento_acesso : $config['nivel_acesso_padrao'])).'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Cor', 'Cor selecionada dentre as 16 milhões possíveis. Pode-se escrever diretamente o hexadecinal na cor ou utilizar a interface que se abre ao clicar na caixa de inserção do valor.').'Cor:'.dicaF().'</td><td align="left" style="white-space: nowrap"><input class="jscolor" name="evento_cor" value="'.($obj->evento_cor ? $obj->evento_cor : 'DDDDDD').'" '.($config['selecao_cor_restrita'] ? 'readonly="readonly" ' : '').'size="6" maxlength="6" style="width:57px;" /></td></tr>';
echo '<tr><td align="right" width="100">'.dica('Ativo', 'Caso o evento ainda esteja ativo deverá estar marcado este campo.').'Ativo:'.dicaF().'</td><td><input type="checkbox" value="1" name="evento_ativo" '.($obj->evento_ativo || !$evento_id ? 'checked="checked"' : '').' /></td></tr>';
...
if ($exibir['evento_oque']) echo '<tr><td align="right" style="white-space: nowrap;width:150px">'.dica('O Que Fazer', 'Sumário sobre o que se trata este evento.').'O Que:'.dicaF().'</td><td colspan="2"><textarea name="evento_oque" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->evento_oque.'</textarea></td></tr>';
...
if ($exibir['evento_quando']) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Quando Fazer', 'Quando o evento é executado.').'Quando:'.dicaF().'</td><td colspan="2"><textarea name="evento_quando" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->evento_quando.'</textarea></td></tr>';
if ($exibir['evento_onde']) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Onde Fazer', 'Onde o evento é executado.').'Onde:'.dicaF().'</td><td colspan="2"><textarea name="evento_onde" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->evento_onde.'</textarea></td></tr>';
if ($exibir['evento_porque']) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Por Que Fazer', 'Por que o evento será executado.').'Por que:'.dicaF().'</td><td colspan="2"><textarea name="evento_porque" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->evento_porque.'</textarea></td></tr>';
if ($exibir['evento_como']) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Como Fazer', 'Como o evento é executado.').'Como:'.dicaF().'</td><td colspan="2"><textarea name="evento_como" data-gpweb-cmp="ckeditor" cols="60" rows="2" class="textarea">'.$obj->evento_como.'</textarea></td></tr>';
...
echo '<tr><td align=right style="white-space: nowrap;width:150px">'.dica('Exibição', 'Forma de apresentar os ').'Exibição:'.dicaF().'</td><td><input type="radio" name="modo_exibicao" value="dept" id="dept" onChange="mudar_usuarios_designados()" checked>'.ucfirst($config['departamento']).'<input type="radio" name="modo_exibicao" value="simples" id="simples" onChange="mudar_usuarios_designados();">Lista simples</td></tr>';
$sql->adTabela('grupo');
...
echo '<tr><td valign="middle" align="right" style="white-space: nowrap;width:150px">'.dica('Notificar', 'Marque esta caixa para avisar da '.($evento_id > 0 ? 'modificação' : 'criação').' do evento.').'Notificar:'.dicaF().'</td><td>';
echo '<input type="checkbox" name="email_responsavel" id="email_responsavel" '.($Aplic->getPref('informa_responsavel') ? "checked='checked'": '').' />'.dica('Responsável', 'Ao selecionar esta opção, o responsável pelo evento será informado '.($evento_id > 0 ? 'das alterações realizadas.' : 'da criação.')).'Responsável'.dicaF().'<br>';
echo '<input type="checkbox" name="email_designados" id="email_designados" '.($Aplic->getPref('informa_designados') ? "checked='checked'": '').' />'.dica('Designados', 'Ao selecionar esta opção, os designados para o evento serão informados '.($evento_id > 0 ? 'das alterações realizadas.' : 'da criação.')).'Designados'.dicaF().'<br>';
echo '<input type="checkbox" name="email_designados_novos" id="email_designados_novos" />'.dica('Designados Novos', 'Ao selecionar esta opção, os novos designados para o evento serão informados '.($evento_id > 0 ? 'das alterações realizadas.' : 'da criação.')).'Designados novos'.dicaF().'<br>';

echo '<input type="checkbox" name="email_convidados" id="email_convidados" '.($Aplic->getPref('informa_designados') ? "checked='checked'": '').' />'.dica('Convidados', 'Ao selecionar esta opção, os convidados do evento serão informado '.($evento_id > 0 ? 'das alterações realizadas.' : 'da criação.')).'Convidados'.dicaF().'<br>';
echo '<input type="checkbox" name="email_convidados_novos" id="email_convidados_novos" '.($Aplic->getPref('informa_responsavel') ? "checked='checked'" : '').' />'.dica('Convidados Novos', 'Ao selecionar esta opção, os novos convidados do evento serão informado '.($evento_id > 0 ? 'das alterações realizadas.' : 'da criação.')).'Convidados novos'.dicaF().'&nbsp;&nbsp;';
echo '</td></tr>';
...
function popAta() {
	parent.gpwebApp.popUp('Ata de Reunião', 1000, 700, 'm=atas&a=ata_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setAta&tabela=ata&cia_id='+document.getElementById('evento_cia').value, window.setAta, window);
	}
...
function popSWOT() {
	parent.gpwebApp.popUp('Camçpo SWOT', 1000, 700, 'm=swot&a=swot_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setSWOT&tabela=swot&cia_id='+document.getElementById('evento_cia').value, window.setSWOT, window);
	}
...
function popInstrumento() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Instrumento Jurídico', 1000, 700, 'm=instrumento&a=instrumento_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setInstrumento&tabela=instrumento&cia_id='+document.getElementById('evento_cia').value, window.setInstrumento, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&edicao=1&chamar_volta=setInstrumento&tabela=instrumento&cia_id='+document.getElementById('evento_cia').value, 'Instrumento Jurídico','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
	}
...
function popAvaliacao() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Avaliação', 1000, 700, 'm=praticas&a=avaliacao_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setAvaliacao&tabela=avaliacao&cia_id='+document.getElementById('evento_cia').value, window.setAvaliacao, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&edicao=1&chamar_volta=setAvaliacao&tabela=avaliacao&cia_id='+document.getElementById('evento_cia').value, 'Avaliação','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
	}
...
function popForum() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Fórum', 1000, 700, 'm=foruns&a=index&dialogo=1&edicao=1&selecao=1&chamarVolta=setForum&tabela=foruns&cia_id='+document.getElementById('evento_cia').value, window.setForum, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&edicao=1&chamar_volta=setForum&tabela=foruns&cia_id='+document.getElementById('evento_cia').value, 'Fórum','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
	}
...
function popOdometro() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Odômetro', 1000, 700, 'm=praticas&a=odometro_pro_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setOdometro&tabela=painel_odometro&cia_id='+document.getElementById('evento_cia').value, window.setOdometro, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&edicao=1&chamar_volta=setOdometro&tabela=painel_odometro&cia_id='+document.getElementById('evento_cia').value, 'Odômetro','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
	}
...
function popComposicaoPaineis() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Composição de Painéis', 1000, 700, 'm=praticas&a=painel_composicao_pro_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setComposicaoPaineis&tabela=painel_composicao&cia_id='+document.getElementById('evento_cia').value, window.setComposicaoPaineis, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&edicao=1&chamar_volta=setComposicaoPaineis&tabela=painel_composicao&cia_id='+document.getElementById('evento_cia').value, 'Composição de Painéis','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
	}
...
function popSlideshow() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Slideshow de Composições', 1000, 700, 'm=praticas&a=painel_slideshow_pro_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setSlideshow&tabela=painel_slideshow&cia_id='+document.getElementById('evento_cia').value, window.setSlideshow, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&edicao=1&chamar_volta=setSlideshow&tabela=painel_slideshow&cia_id='+document.getElementById('evento_cia').value, 'Slideshow de Composições','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
	}
...
function popPlanejamento() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Planejamento Estratégico', 1000, 700, 'm=praticas&u=gestao&a=gestao_lista&dialogo=1&edicao=1&selecao=1&chamarVolta=setPlanejamento&tabela=plano_gestao&cia_id='+document.getElementById('evento_cia').value, window.setPlanejamento, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&edicao=1&chamar_volta=setPlanejamento&tabela=plano_gestao&cia_id='+document.getElementById('evento_cia').value, 'Planejamento Estratégico','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
	}
