...
if ($Aplic->profissional){
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao'))	$exibir_vetor['acao']= 'Ações de '.ucfirst($config['acoes']);
	if ($Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'acesso', null, null)) {
		$exibir_vetor['ata']='Atas de Reunião';
		$exibir_vetor['ata_acao']='Ações de Atas de Reunião';
		}
...
if (!$dialogo){
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionado','A qual parte do sistema os eventos estão relacionados.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
...
if (($Aplic->profissional && in_array('evento', $exibir_calendario)) || (!$Aplic->profissional && ($exibir_calendario==1 || $exibir_calendario==3 || $exibir_calendario==5 || $exibir_calendario==7))) 
$links=getEventoLinks($evento_filtro, $periodo_todo, $primeiraData, $ultimaData, $links, $tamanho, false, $usuario_id, $envolvimento, ($lista_cias ? $lista_cias : $cia_id), ($lista_depts ? $lista_depts : $dept_id),
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
...
echo '<tr><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=calendario&a=ver_semana&data='.$semana_ant->format('%Y%m%d').'\');">'.imagem('icones/'.($estilo_interface=='metro' ? 'navAnterior_metro.png' :'anterior.gif'), 'Semana Anterior', 'Clique neste ícone '.imagem('icones/'.($estilo_interface=='metro' ? 'navAnterior_metro.png' :'anterior.gif')).' para exibir a semana anterior.').'</a></td><td width="100%" align="center"><b><span style="font-size:12pt">'.$primeiraData->format('%U').'ª Semana - '.$nome_meses[$primeiraData->format('%m')].' de '.$primeiraData->format('%Y').'</span></b></td><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=calendario&a=ver_semana&data='.$proxSemana->format('%Y%m%d').'\');">'.imagem('icones/'.($estilo_interface=='metro' ? 'navProximo_metro.png' :'proximo.gif'), 'Próxima Semana', 'Clique neste ícone '.imagem('icones/'.($estilo_interface=='metro' ? 'navProximo_metro.png' :'proximo.gif')).' para exibir a próxima semana.').'</a></td></tr>';
echo '</table>';
...
for ($i = 0; $i < 7; $i++) {
	...
	echo '<td class="'.$classe.'" style="width:14.29%;" '.($podeAdicionar ? 'ondblclick="url_passar(0, \'m=calendario&a=editar&data='.$diaFormato.'&cia_id='.$cia_id.'&dept_id='.$dept_id.'\');"' : '').'>';
...
if (!$dialogo) {
	...
	echo '<td style="border-style:solid;border-width:1px; background-color:#ffffff;">&nbsp;&nbsp;</td><td style="white-space: nowrap">Expediente normal</td><td>&nbsp;</td>';
	echo '<td style="border-style:solid;border-width:1px; background-color: #fefeed;">&nbsp;&nbsp;</td><td style="white-space: nowrap">Meio expediente</td><td>&nbsp;</td>';
	echo '<td style="border-style:solid;border-width:1px; background-color:#dffefc;">&nbsp;&nbsp;</td><td style="white-space: nowrap">Expediente alterntivo</td><td>&nbsp;</td>';
	echo '<td style="border-style:solid;border-width:1px; background-color: #f0e8e8;">&nbsp;&nbsp;</td><td style="white-space: nowrap">Sem expediente</td>';
