...
global $estilo_interface, $perms, $ver_subordinadas, $dialogo;
...
$xpg_tamanhoPagina = ($dialogo ? 9000 : $config['qnt_usuarios']);
...
mostrarBarraNav($xpg_totalregistros, $xpg_tamanhoPagina, $xpg_total_paginas, $pagina, $config['usuario'], $config['usuarios'],'','',($estilo_interface=='classico' ? 'a6a6a6' : '006fc2'));

echo '<table cellpadding="2" cellspacing=0 border=0 width="100%" class="tbl1"><tr>';
if (!$dialogo) echo '<th>&nbsp;</th>';
if (!$dialogo && !getParam($_REQUEST, 'tab', 0)) echo '<th>'.dica('Histórico de Acesso', 'O tempo em que '.$config['genero_usuario'].' '.$config['usuario'].' permaneceu conectado e desconectado do Sistema.').'Histórico de Acesso'.dicaF().'</th>';
...
if ($ver_subordinadas) echo '<th><a href="javascript:void(0);" onclick="url_passar(0, \'m=admin&a=index&ordem='.$ordem.'&ordemPor=cia_nome\');" class="hdr">'.dica(ucfirst($config['organizacao']), 'Clique para ordenar pel'.$config['genero_organizacao'].' '.$config['organizacao'].'.').($ordemPor=='cia_nome' ? imagem('icones/'.$seta[$ordem]) : '').ucfirst($config['organizacao']).dicaF().'</a></th>';
...
echo '<th><a href="javascript:void(0);" onclick="url_passar(0, \'m=admin&a=index&ordem='.$ordem.'&ordemPor=contato_hora_custo\');" class="hdr">'.dica('Custo Hora', 'Clique para ordenar pelo custo hora d'.$config['genero_usuario'].' '.$config['usuario'].'.').($ordemPor=='contato_hora_custo' ? imagem('icones/'.$seta[$ordem]) : '').'Custo Hora'.dicaF().'</a></th>';



echo '<th>'.dica('Perfil de '.ucfirst($config['usuario']), 'O perfil de acesso de '.$config['usuarios'].'.').'Perfil'.dicaF().'</a></th>';
echo '<th width=80><a href="javascript:void(0);" onclick="url_passar(0, \'m=admin&a=index&ordem='.$ordem.'&ordemPor=usuario_observador\');" class="hdr">'.dica('Observador'.($config['genero_usuario']=='a' ? 'a' : ''), 'Clique para ordenar pel'.$config['genero_usuario'].'s  '.$config['usuarios'].' observador'.($config['genero_usuario']=='a' ? 'a' : 'e').'s').($ordemPor=='usuario_observador' ? imagem('icones/'.$seta[$ordem]) : '').'Observador'.($config['genero_usuario']=='a' ? 'a' : '').dicaF().'</a></th>';
...
for ($i = ($pagina - 1) * $xpg_tamanhoPagina; $i < $pagina * $xpg_tamanhoPagina && $i < $xpg_totalregistros; $i++) {
	...
	if (!$dialogo){
		...
		echo '<tr><td width="48" align="center" style="white-space: nowrap">';
		if ($podeEditar) { 
		...
		if ($ver_subordinadas) echo '<td>'.($linha['contato_cia']? link_cia($linha['contato_cia']) : '&nbsp;').'</td>';
		echo '<td>'.($linha['dept_id'] ? link_dept($linha['dept_id']): '&nbsp;').'</td>';
		
		echo '<td align=right>'.($linha['contato_hora_custo']  > 0 ?  number_format($linha['contato_hora_custo'], 2, ',', '.') : '&nbsp;').'</td>';
		...
		echo '<td align="center">'.($linha['usuario_observador']? 'X':'&nbsp;').'</td>';
