...
echo '<table width="100%" align="center" class="std"><tr><td>';
	...
	echo '<tr><td align="right" width="45%" >'.dica('Data Inícial', 'Digite ou escolha no calendário a data inícial da pesquisa.').'Data Inicial:'.dicaF().'</td><td width="55%" align="left"><input type="hidden" name="reg_data_inicio" id="reg_data_inicio" value="'.($data_inicio ? $data_inicio->format('%Y%m%d') : '').'" /><input type="text" name="data_inicio" style="width:70px;" id="data_inicio" onchange="setData(\'frmData\', \'data_inicio\');" value="'.($data_inicio ? $data_inicio->format('%d/%m/%Y') : '').'" class="texto" />'.dica('Data Inicial', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data inícial da pesquisa.').'<a href="javascript: void(0);" ><img id="f_btn1" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 /></a>'.dicaF().'</td></tr>';
	echo '<tr align="center"><td align="right" width="45%">'.dica('Data Final', 'Digite ou escolha no calendário a data final da pesquisa.').'Data Final:'.dicaF().'</td><td width="55%" align="left"><input type="hidden" name="reg_data_fim" id="reg_data_fim" value="'.($data_fim ? $data_fim->format('%Y%m%d') : '').'" /><input type="text" name="data_fim" id="data_fim" style="width:70px;" onchange="setData(\'frmData\', \'data_fim\');" value="'.($data_fim ? $data_fim->format('%d/%m/%Y') : '').'" class="texto" />'.dica('Data Final', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data final da pesquisa.').'<a href="javascript: void(0);" ><img id="f_btn2" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 /></a>'.dicaF().'</td></tr>';
...
if (getParam($_REQUEST, 'mostrarDetalhes', 0)) {
	...
	$q->adCampo('usuario_reg_acesso.usuario_id, usuario_ip, formatar_data(entrou, "%d/%m/%Y %H:%i:%s") AS inicio, formatar_data(saiu, "%d/%m/%Y %H:%i:%s") AS fim, ultima_atividade');
	...
	echo '<tr>';
	echo '<th style="white-space: nowrap" >'.dica(ucfirst($config['usuario']), 'Nome d'.$config['genero_usuario'].' '.$config['usuario'].' à qual se está verificando as datas e horas de entrada e saída do Sistema.').ucfirst($config['usuario']).dicaF().'</th>';
	echo '<th style="white-space: nowrap" >'.dica('Endereço IP', 'O IP (Internet Protocol) da máquina que foi utilizada pel'.$config['genero_usuario'].' '.$config['usuario'].' para entrar no Sistema.').'Endereço IP'.dicaF().'</th>';
	echo '<th style="white-space: nowrap" >'.dica('Data/Hora Entrou', 'A Data e hora de entrada d'.$config['genero_usuario'].' '.$config['usuario'].' no Sistema.').'Data/Hora Entrou'.dicaF().'</th>';
	echo '<th style="white-space: nowrap" >'.dica('Data/Hora Saiu', 'A Data e hora de saída d'.$config['genero_usuario'].' '.$config['usuario'].' no Sistema.').'Data/Hora Saiu'.dicaF().'</th>';
	echo '</tr>';
	$qnt=0;
	foreach ($logs as $detalhe) {
		...
		echo '<td align="center">'.($detalhe['inicio'] ? $detalhe['inicio'] : '&nbsp;').'</td>';
		echo '<td align="center">'.($detalhe['fim'] ? $detalhe['fim'] : '&nbsp;').'</td>';
		echo '</tr>';
		}
