...
if ($usuario_id && !$podeEditar)	$Aplic->redirecionar('m=publico&a=acesso_negado');
elseif (!$usuario_id && !$podeAdicionar)	$Aplic->redirecionar('m=publico&a=acesso_negado');
...
if (!$usuario && $usuario_id) {
	...
	echo '<input type="hidden" name="usuario_pode_oculta" value="'.(isset($usuario['usuario_pode_oculta']) && $usuario['usuario_pode_oculta'] ? 1 : 0).'" />';
	echo '<input type="hidden" name="usuario_assinatura_nome" value="'.(isset($usuario['usuario_assinatura_nome']) ? $usuario['usuario_assinatura_nome'] : '').'" />';
	echo '<input type="hidden" name="usuario_assinatura_local" value="'.(isset($usuario['usuario_assinatura_local']) ? $usuario['usuario_assinatura_local'] : '').'" />';
	...
	echo '<input type="hidden" name="qnt_permitida" id="qnt_permitida" value="" />';
	...
	$sql->limpar();
	if ($podeEditar && !count($usuario_perfis)) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Perfil de Acesso', 'Escolha na caixa de seleção à direita qual o perfil de acesso.<br><br>Cada perfil tem permissões de acesso e edição diferentes nos diveros módulos do sistema.').'Perfil Acesso:'.dicaF().'</td><td>'.selecionaVetor($perfis_arr, 'usuario_perfil', 'style="width:260px;" size="1" class="texto"', $config['ldap_perfil']).'</td></tr>';
	if (!isset($usuario['usuario_login']) || !$usuario['usuario_id']) {
		echo '<tr><td align="right" style="white-space: nowrap">'.dica('Senha', 'Escreva a senha de acesso ao sistema.').'Senha:'.dicaF().'</td><td><input type="password" class="texto" name="usuario_senha" value="'.(isset($usuario['usuario_senha']) ? $usuario['usuario_senha'] : '').'" maxlength="32" style="width:260px;" /> </td></tr>';
		echo '<tr><td align="right" style="white-space: nowrap">'.dica('Confirmar a Senha', 'Escreva novamente a senha de acesso ao sistema.').'Confirmar a Senha:'.dicaF().'</td><td><input type="password" class="texto" name="senha_checar" value="'.(isset($usuario['usuario_senha']) ? $usuario['usuario_senha'] : '').'" maxlength="32" style="width:260px;" /></td></tr>';
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.($config['militar'] < 10 ? dica('Posto/Grad e Nome de Guerra', 'Selecione o posto/graduação e escreva o nome de guerra.').'Posto/Grad e Nome Guerra:' : dica('Pron. Trat. e Nome', 'Selecione o pronome de tratamento e escreva o nome d'.$config['genero_usuario'].' '.$config['usuario'].'.').'Pron. Trat. e Nome:').dicaF().'</td><td>'.selecionaVetor($posto, 'contato_posto','class="texto" size=1 style="width:70px;"' , (isset($usuario['contato_posto']) ? $usuario['contato_posto']: ''), true).'<input type="text" class="texto" name="contato_nomeguerra" value="'.(isset($usuario['contato_nomeguerra']) ? $usuario['contato_nomeguerra'] : '').'" maxlength="30" style="width:190px;" /></td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nome Completo', 'Escreva o nome completo.').'Nome completo:'.dicaF().'</td><td><input type="text" class="texto" name="contato_nomecompleto" value="'.(isset($usuario['contato_nomecompleto']) ? $usuario['contato_nomecompleto'] : '').'" maxlength="255" style="width:260px;" /> </td></tr>';
	if ($config['militar'] < 10) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Arma/Quadro/Sv', 'Escolha na caixa de seleção à direita qual a Arma/Quadro/Sv.').'Arma/Quadro/Sv:'.dicaF().'</td><td>'.selecionaVetor($arma, 'contato_arma', 'style="width:260px;" class="texto" size=1', (isset($usuario['contato_arma']) ? $usuario['contato_arma'] : ''), true).'</td></tr>';
	if ($podeEditar) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['organizacao']), 'Escolha na caixa de seleção à direita qual '.$config['genero_organizacao'].' '.$config['organizacao'].'.').ucfirst($config['organizacao']).':'.dicaF().'</td><td>'.($Aplic->checarModulo('cias', 'acesso') ? '<div id="combo_cia">'.selecionar_om((isset($usuario['contato_cia']) ? $usuario['contato_cia'] : ''), 'contato_cia', 'class=texto size=1 style="width:260px;" onchange="javascript:mudar_om();"').'</div>' : '<input type="hidden" name="contato_dept" id="contato_dept" value="'.(isset($usuario['contato_cia']) ? $usuario['contato_cia'] : '').'"><input type="text" class="texto" name="dept_nome" style="width:260px;" READONLY value="'. nome_cia((isset($usuario['contato_cia']) ? $usuario['contato_cia'] : '')).'" />').'</td></tr>';
	if ($podeEditar && $Aplic->modulo_ativo('depts')) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']), 'Escolha pressionando o botão <b>selecionar</b> à direita qual '.$config['genero_dept'].' '.$config['dept'].' d'.$config['genero_usuario'].' '.$config['usuario'].'.').ucfirst($config['departamento']).':'.dicaF().'</td><td><input type="hidden" name="contato_dept" id="contato_dept" value="'.(isset($usuario['contato_dept']) ? $usuario['contato_dept'] : '').'" /><input type="text" class="texto" name="dept_nome" value="'.nome_dept((isset($usuario['contato_dept']) ? $usuario['contato_dept'] : '')).'" style="width:260px;" READONLY />'.($Aplic->checarModulo('depts', 'acesso') ? botao_icone('secoes_p.gif','Selecionar', 'selecionar '.$config['departamento'],'popDept()') : '').'</td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Função n'.$config['genero_organizacao'].' '.$config['organizacao'], 'Escreva a função d'.$config['genero_usuario'].' '.$config['usuario'].' dentro d'.$config['genero_organizacao'].' '.$config['organizacao'].'. Embora não tenha impacto no funcionamento do Sistema facilita a distinção d'.$config['genero_usuario'].'s '.$config['usuarios'].'.').'Função n'.$config['genero_organizacao'].' '.$config['organizacao'].':'.dicaF().'</td><td><input type="text" class="texto" name="contato_funcao" value="'.(isset($usuario['contato_funcao']) ? $usuario['contato_funcao'] : '').'" maxlength="255" style="width:260px;" /> </td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('E-mail Principal', 'Escreva o e-mail principal.').'E-mail principal:'.dicaF().'</td><td><input type="text" class="texto" name="contato_email" value="'.(isset($usuario['contato_email']) ? $usuario['contato_email'] : '').'" maxlength="255" style="width:260px;" /> </td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('E-mail Secundário', 'Escreva o e-mail secundário.').'E-mail secundario:'.dicaF().'</td><td><input type="text" class="texto" name="contato_email2" value="'.(isset($usuario['contato_email2']) ? $usuario['contato_email2'] : '').'" maxlength="255" style="width:260px;" /> </td></tr>';
	echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Rodapé do e-mail', 'Escreva o texto que vai no rodapé d'.$config['genero_mensagem'].'s '.$config['mensagens'].' que poderá enviar pelo Sistema.').'Rodapé do e-mail:'.dicaF().'</td><td><textarea class="texto" style="width:260px;" name="usuario_rodape" data-gpweb-cmp="ckeditor" style="height: 50px">'.(isset($usuario['usuario_rodape']) ? $usuario['usuario_rodape'] : '').'</textarea></td></tr>';
	...
	echo '<tr><td align="right" style="white-space: nowrap" width=208>'.dica('Identidade', 'Escreva a identidade.').($config['id_usuario_identidade'] ? '' : '').'Identidade:'.dicaF().'</td><td><input type="text" class="texto" name="contato_identidade" id="contato_identidade" value="'.(isset($usuario['contato_identidade']) ? $usuario['contato_identidade'] : '').'" maxlength="25" style="width:260px;" /> </td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('CPF', 'Escreva o CPF.').'CPF:'.dicaF().'</td><td><input type="text" class="texto" name="contato_cpf" value="'.(isset($usuario['contato_cpf']) ? $usuario['contato_cpf'] : '').'" maxlength="14" style="width:260px;" /> </td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Matrícula', 'Escreva a matrícula d'.$config['genero_usuario'].' '.$config['usuario'].', caso exista.').'Matrícula:'.dicaF().'</td><td><input type="text" class="texto" name="contato_matricula" value="'.(isset($usuario['contato_matricula']) ? $usuario['contato_matricula'] : '').'" maxlength="100" style="width:260px;" /> </td></tr>';
	if ($Aplic->checarModulo('usuarios', 'editar', $Aplic->usuario_id, 'hora_custo')) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Custo da Hora', 'O custo da hora de trabalho d'.$config['genero_usuario'].' '.$config['usuario'].'.').'Custo hora:'.dicaF().'</td><td><input type="text" class="texto" onkeypress="return entradaNumerica(event, this, true, true);" name="contato_hora_custo" value="'.(isset($usuario['contato_hora_custo']) ? number_format($usuario['contato_hora_custo'], 2, ',', '.') : '').'" maxlength="100" style="width:260px;" /> </td></tr>';
	...
	if ($Aplic->usuario_super_admin || $Aplic->usuario_admin) {
		...
		echo '<tr><td align="right" style="white-space: nowrap" width=208>'.dica(ucfirst($config['departamentos']).' Envolvid'.$config['genero_dept'].'s', 'Quais '.strtolower($config['departamentos']).' estão envolvid'.$config['genero_dept'].'s.').ucfirst($config['departamentos']).' envolvid'.$config['genero_dept'].'s:'.dicaF().'</td><td><table cellpadding=0 cellspacing=0><tr><td style="width:288px;"><div id="combo_depts">'.$saida_depts.'</div></td><td>'.botao_icone('secoes_p.gif','Selecionar', 'selecionar '.$config['departamentos'],'popDepts()').'</td></tr></table></td></tr>';
		}
	echo '</table></td></tr>';
	...
	if ($Aplic->usuario_super_admin || $Aplic->usuario_admin) {
		echo '<tr><td width="210" align="right" style="white-space: nowrap">'.dica(ucfirst($config['usuario']).' Ativ'.$config['genero_usuario'], 'Escolha na caixa de seleção à direita se '.$config['genero_usuario'].' '.$config['usuario'].' está ativ'.$config['genero_usuario'].'.').'Ativ'.$config['genero_usuario'].':'.dicaF().'</td><td><input type="checkbox" name="usuario_ativo" id="usuario_ativo" value="1" '.((isset($usuario['usuario_ativo']) && $usuario['usuario_ativo']) || !$usuario_id ? 'checked="checked"': '').' /></td></tr>';
		echo '<tr><td align="right" style="white-space: nowrap">'.dica('Administrador de '.ucfirst($config['usuario']), 'Marque esta opção caso seja administrador de '.$config['usuario'].'. Nesta condição poderá incluir, alterar ou excluir '.$config['usuarios'].'.').'Administrador de '.$config['usuario'].':'.dicaF().'</td><td><input type="checkbox" name="usuario_admin" id="usuario_admin" value="1" '.(isset($usuario['usuario_admin']) && $usuario['usuario_admin'] ? 'checked="checked"': '').' /></td></tr>';
		echo '<tr><td align="right" style="white-space: nowrap">'.dica('Observador', 'Marque esta opção caso seja '.$config['usuario'].' com perfil limitado a apenas observador dos pincipais objetos do sistema.').'Observador:'.dicaF().'</td><td><input type="checkbox" name="usuario_observador" id="usuario_observador" value="1" '.(isset($usuario['usuario_observador']) && $usuario['usuario_observador'] ? 'checked="checked"': '').' /></td></tr>';
		...
		echo '<input type="hidden" name="usuario_observador" id="usuario_observador" value="'.(isset($usuario['usuario_observador']) && $usuario['usuario_observador'] ? 1 : 0).'" />';
		}
	...
	echo '<tr><td colspan=2 style="white-space: nowrap" align="right">'.(($podeEditar && !$usuario_id) ? '<label for="send_usuario_mail">'.dica('Enviar para o e-mail', 'Receber no seu e-mail os detalhes desta conta.').'Enviar para o e-mail'.dicaF().'</label><input type="checkbox" value="1" name="send_usuario_mail" id="send_usuario_mail" />' : '').'</td></tr>';
...
function enviarDados(){
	...
	<?php if($Aplic->usuario_super_admin || $Aplic->usuario_admin) { ?>
	xajax_qnt_permitida(document.getElementById('usuario_id').value, document.getElementById('usuario_observador').checked);
	<?php } else { ?>
	xajax_qnt_permitida(document.getElementById('usuario_id').value, document.getElementById('usuario_observador').value);
	<?php } ?>
	
	if (form.qnt_permitida.value==-1) {
		alert("O número máximo de <?php echo $config['usuarios']?> já foi atingido!");
		}	
	else if (form.qnt_permitida.value==-2) {
		alert('O número máximo de observadores já foi atingido!');
		}
	else if (form.usuario_login.value.length < <?php echo $config['tam_min_login']; ?> && form.usuario_login.value != '<?php echo $config["nome_administrador"]; ?>') {
	...
	<?php } ?>
	...
	else if (form.contato_nomeguerra.value.length < 3) {
		...
		form.usuario_grupo_dept.value =(form.usuarios.value || form.depts.value ? 1 : 0);
		form.submit();
		}
	}
