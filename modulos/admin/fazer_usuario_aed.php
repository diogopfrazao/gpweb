...
$sql = new BDConsulta;
if ($del && !($Aplic->checarModulo('usuarios', 'excluir') || $Aplic->usuario_admin)){
    $Aplic->redirecionar('m=publico&a=acesso_negado');
}
elseif ($ehNovoUsuario && !($Aplic->checarModulo('usuarios', 'adicionar') || $Aplic->usuario_admin)){
    $Aplic->redirecionar('m=publico&a=acesso_negado');
}
elseif (!$ehNovoUsuario && $usuario_id != $Aplic->usuario_id && !$Aplic->usuario_admin){
    $Aplic->redirecionar('m=publico&a=acesso_negado');
}
elseif (!$ehNovoUsuario && !$Aplic->checarModulo('usuarios', 'editar') && !$Aplic->usuario_admin){
    $Aplic->redirecionar('m=publico&a=acesso_negado');
}
...
if (($msg = $contato->armazenar(true))) $Aplic->setMsg($msg, UI_MSG_ERRO);
	...
	//atualizar o estado ativo de contato	
	$sql->adTabela('contatos');
	$sql->adAtualizar('contato_ativo',$obj->usuario_ativo);
	$sql->adOnde('contato_id = '.(int)$obj->usuario_contato);
	$sql->exec();
	$sql->limpar();	
