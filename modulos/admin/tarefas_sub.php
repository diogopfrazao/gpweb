...
echo '<tr>';
	...
	echo '<th style="white-space: nowrap" width="140">'.dica('Início', 'Clique para ordenar '.$config['genero_tarefa'].'s '.$config['tarefas'].' abaixo pela data de início das mesmas.');ordenar_por_titulo('Data de Início', 'tarefa_inicio', SORT_NUMERIC, 'admin', 'a=ver_usuario', $usuario_id).dicaF().'</th>';
	echo '<th style="white-space: nowrap">'.dica('Duração', 'Clique para ordenar '.$config['genero_tarefa'].'s '.$config['tarefas'].' abaixo pela duração das mesmas ');ordenar_por_titulo('Duração', 'tarefa_duracao', SORT_NUMERIC,  'admin', 'a=ver_usuario', $usuario_id).dicaF().'</th>';
	echo '<th style="white-space: nowrap" width="140">'.dica('Término', 'Clique para ordenar '.$config['genero_tarefa'].'s '.$config['tarefas'].' abaixo pelo término das mesmas.');ordenar_por_titulo('Término', 'tarefa_fim', SORT_NUMERIC, 'admin', 'a=ver_usuario', $usuario_id).dicaF().'</th>';
	echo '<th style="white-space: nowrap">'.dica('Fazer em', 'Clique para ordenar '.$config['genero_tarefa'].'s '.$config['tarefas'].' abaixo pelo tempo necessário para realiza-las.');ordenar_por_titulo('Fazer em', 'tarefa_fazer_em', SORT_NUMERIC,  'admin', 'a=ver_usuario', $usuario_id).dicaF().'</th>';
