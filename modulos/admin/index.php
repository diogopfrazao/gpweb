...
if (!$dialogo){
	...
	$novo_usuario=($podeAdicionar ? '<tr><td style="white-space: nowrap">'.dica('Nov'.$config['genero_usuario'].' '.ucfirst($config['usuario']), 'Adicionar '.($config['genero_usuario']=='o' ? 'um' : 'uma').' '.$config['usuario'].' no Sistema.').'<a href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=admin&a=editar_usuario\');" >'.imagem('icones/membros_novo.png').'</a>'.dicaF().'</td></tr>' : '');
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['iniciativas'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&u='.$u.'&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';

	$cha_botao=($Aplic->profissional ? '<tr><td align="right" style="white-space: nowrap"><a href="javascript:void(0);" onclick="if (document.getElementById(\'cha\').style.display) document.getElementById(\'cha\').style.display=\'\'; else document.getElementById(\'cha\').style.display=\'none\';"">'.imagem('icones/opcoes_filtro.gif','Filtros Extras','Clique neste ícone '.imagem('icones/opcoes_filtro.gif').' para aparecer outras opções de filtragem.').'</a></td></tr>' : '');
	$pesquisa_botao='<tr><td align="right" style="white-space: nowrap">'.dica('Pesquisar', 'Pesquisar '.$config['genero_usuario'].'s '.$config['usuarios'].' baseados no texto inserido à esquerda.').'Pesquisar:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="text" name="onde" class="texto" style="width:247px;" value="'.$onde.'" /></td><td><a href="javascript:void(0);" onclick="frm_filtro.onde.value=\'\'; frm_filtro.submit();">'.imagem('icones/limpar_p.gif','Limpar Pesquisa', 'Clique neste ícone '.imagem('icones/limpar_p.gif').' para limpar a caixa texto de pesquisa.').'</a></td></tr></table></td></tr>';
	...
	if ($Aplic->profissional) include_once BASE_DIR.'/modulos/admin/filtro_extra_pro.php';
	else $saida_cha='';
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$pesquisa_botao.$alfabeto.$saida_cha.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$novo_usuario.$imprimir.$cha_botao.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$tab) require BASE_DIR.'/modulos/admin/ver_usuarios_ativos.php';
else require BASE_DIR.'/modulos/admin/ver_usuarios_inativos.php';
