...
$instrumento_id = intval(getParam($_REQUEST, 'instrumento_id', null));
...
if($instrumento_id && !($podeEditar && permiteEditarInstrumento($obj->instrumento_acesso, $instrumento_id))) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if ($desfazer){
	...
	if ($tipo=='ne'){
		...
		if ($linha['financeiro_estorno_ne_valor'] < $linha['financeiro_estorno_ne_valor_original']){
			...
			$sql->adInserir('financeiro_rel_ne_instrumento', $instrumento_id);
	...
	elseif ($tipo=='ns'){
		...
		if ($linha['financeiro_estorno_ns_valor'] < $linha['financeiro_estorno_ns_valor_original']){
			...
			$sql->adInserir('financeiro_rel_ns_instrumento', $instrumento_id);
		...
		if ($linha['financeiro_estorno_ob_valor'] < $linha['financeiro_estorno_ob_valor_original']){
			...
			$sql->adInserir('financeiro_rel_ob_instrumento', $instrumento_id);
...
if (getParam($_REQUEST, 'processar', null)){
	...
	$sql->adInserir('financeiro_estorno_instrumento', $instrumento_id);
...
$botoesTitulo = new CBlocoTitulo('Estornar '.$tiponome.' n'.$config['genero_instrumento'].' '.ucfirst($config['instrumento']), '../../../modulos/instrumento/imagens/instrumento.png', $m, $m.'.'.$a);
...
if ($tipo=='ne'){
	...
	$sql->adOnde('financeiro_estorno_instrumento ='.(int)$instrumento_id);
...
if (count($financeiro_rel)) echo '<tr><td colspan=20><table cellspacing=0 cellpadding=0 width="100%"><tr><td>'.botao('estornar', 'Estornar', 'Estornar os valores definidos.','','salvarDados();').'</td><td align="right">'.botao('retornar', 'Retornar', 'Retornar aos detalhes d'.$config['genero_instrumento'].' '.$config['instrumento'].'.','','if(confirm(\'Tem certeza que deseja retornar?\')){url_passar(0, \''.$Aplic->getPosicao().'\');}').'</td></tr></table></td></tr>';	
else echo '<tr><td colspan=20><table cellspacing=0 cellpadding=0 class="tbl1" align=left width="100%"><tr><td colspan=20 align="right">'.botao('retornar', 'Retornar', 'Retornar aos detalhes d'.$config['genero_instrumento'].' '.$config['instrumento'].'.','','url_passar(0, \''.$Aplic->getPosicao().'\');').'</td></tr></table></td></tr>';	
