...
if (isset($_REQUEST['instrumento_campo']) || $houve_filtro) $Aplic->setEstado('instrumento_campo', getParam($_REQUEST, 'instrumento_campo', null));
$instrumento_campo = ($Aplic->getEstado('instrumento_campo') ? $Aplic->getEstado('instrumento_campo') : ($houve_filtro ? null : -1));

if (isset($_REQUEST['filtro_situacao']) || $houve_filtro) $Aplic->setEstado('filtro_situacao', getParam($_REQUEST, 'filtro_situacao', null));
$instrumento_situacao = ($Aplic->getEstado('filtro_situacao') ? $Aplic->getEstado('filtro_situacao') : ($houve_filtro ? null : -1));
...
if($Aplic->profissional){
  ...
  $sql->adCampo('*');
  $sql->adOnde("campo_customizado_modulo = 'instrumento'");
...
//campos customizados no formulario
if($Aplic->profissional){
...
if (!$dialogo || $selecao){	
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['instrumentos'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1,\'m='.$m.'&a='.$a.'&u='.$u.'&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
	...
	$botao_campo_customizado=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="javascript:popFiltroCamposCustomizados();">'.imagem('icones/custom_field_search.png', 'Campo Customizado', 'Clique neste ícone '.imagem('icones/custom_field_search.png').' para filtrar '.$config['trs'].' utilizando os campos customizados.').'</a></td></tr>' : '<tr><td></td></tr>');


	$novo=($podeEditar ? '<tr><td style="white-space: nowrap">'.dica('Nov'.$config['genero_instrumento'].' '.ucfirst($config['instrumento']), 'Criar '.($config['genero_instrumento']=='o' ? 'um' : 'uma').' nov'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'<a href="javascript: void(0)" onclick="javascript:criar_instrumento();" ><img src="'.acharImagem('instrumento_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' :'');
	...
	$lista_anos = $sql->carregarColuna();
	$sql->limpar();
	$anos=array(''=>null);
	foreach($lista_anos as $valor) $anos[$valor]=$valor;

	$procurar_ano='<tr><td align="right" style="white-space: nowrap">'.dica('Ano', 'Preencha neste campo o ano d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Ano:'.dicaF().'</td><td align="left">'.selecionaVetor($anos, 'instrumento_ano', 'class="texto" style="width:250px" multiple', $instrumento_ano).'</td></tr>';
	...
	$procurar_tipo='<tr><td align="right" style="white-space: nowrap">'.dica('Tipo', 'Selecione qual o tipo de '.$config['instrumento'].'.').'Tipo:'.dicaF().'</td><td align="left" >'.selecionaVetor($TipoInstrumento, 'instrumento_campo', 'class="texto" style="width:250px" multiple', $instrumento_campo).'</td></tr>';
	
	$procurar_situacao='<tr><td align="right" style="white-space: nowrap">'.dica('Situação', 'Selecione qual a situação d'.$config['genero_instrumento'].' '.$config['instrumento'].'.').'Situação:'.dicaF().'</td><td align="left" >'.selecionaVetor(getSisValor('SituacaoInstrumento'), 'filtro_situacao', 'class="texto" style="width:250px" multiple', $instrumento_situacao).'</td></tr>';
	...
	$botao_favorito=($Aplic->profissional ? '<tr><td style="white-space: nowrap"><a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&instrumento=1\');">'.imagem('icones/favorito_p.png','Favoritos', 'Clique neste ícone '.imagem('icones/favorito_p.png').' para criar ou editar um grupo de favoritos, para uma rápida filtragem.').'</a></td></tr>' : '');
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procuraBuffer.$procurar_usuario.$procurar_ano.$procurar_tipo.$procurar_situacao.$favoritos.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$botao_filtrar.(!$selecao ? $novo.$imprimir.$botao_prioridade.$botao_excel.$botao_favorito.$botao_trava.$botao_campos.$botao_campo_customizado : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
...
if (is_array($instrumento_campo)) $instrumento_campo=implode(',', $instrumento_campo);
...
echo '  $jq("#instrumento_campo").multiSelect();';
echo '  $jq("#filtro_situacao").multiSelect();';
echo '});';
echo '</script>';  
...
if (!$dialogo || $selecao){
	$caixaTab->adicionar(BASE_DIR.'/modulos/instrumento/instrumento_lista_idx', 'Em Andamento',null,null,'Em Andamento','Visualizar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' em andamento.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/instrumento/instrumento_lista_idx', 'Concluíd'.$config['genero_instrumento'].'s',null,null,'Concluíd'.$config['genero_instrumento'].'s','Visualizar '.$config['genero_instrumento'].'s '.$config['instrumentos'].' concluíd'.$config['genero_instrumento'].'s.');
	...
	$caixaTab->adicionar(BASE_DIR.'/modulos/instrumento/instrumento_lista_idx', 'Tod'.$config['genero_instrumento'].'s',null,null,'Tod'.$config['genero_instrumento'].'s','Visualizar tod'.$config['genero_instrumento'].'s '.$config['genero_instrumento'].'s '.$config['instrumentos'].'.');
...
function popFiltroCamposCustomizados(){
  ...
  for(key in campos){
    if(campos.hasOwnProperty(key)){
      ...
      if(fld) campos[key]['campo_customizado_valor_atual']=fld.value; 
    	}
  	}
		...
		w.on('salvar', function(w, fields){
			for(key in fields){
				if(fields.hasOwnProperty(key)){
					...
					if(fld)	fld.value = cmp['campo_customizado_valor_atual'];
					}
				}
			});
		}
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=instrumentos', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=instrumento&a=instrumento_lista');
	}	
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&instrumento=1&filtro_prioridade='+env.filtro_prioridade_instrumento.value, window.setFiltroPriorizacao, window);
	}


function criar_instrumento() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp("Criar", 400, 300, 'm=instrumento&a=instrumento_selecao_pro&chamar_volta=setCriar_instrumento', window.setCriar_instrumento, window);
	else url_passar(0, 'm=instrumento&a=instrumento_editar&instrumento_campo=1');
	}
	
function setCriar_instrumento(tipo){
	url_passar(0, 'm=instrumento&a=instrumento_editar&instrumento_campo='+tipo);
	}
