...
echo '<form name="formulario" id="formulario" action="index.php" target="_self" method="post" >';
echo '<input type="hidden" name="m" id="m" value="relatorios" />';
echo '<input type="hidden" name="a" id="a" value="pdf" />';
echo '<input type="hidden" name="sem_cabecalho" id="sem_cabecalho" value="1" />';
...
foreach ($mensagens as $linha) {
	...
	$pdf.='<tr><td>'.($linha['mensagem_data'] ? $data->format('%d/%m/%Y %H:%M') : '&nbsp;').'</td><td>'.($config['militar'] < 10 ? $linha['contato_posto'].' '.$linha['contato_nomeguerra'] : $linha['contato_nomeguerra']).'</td><td><b>'.$linha['mensagem_titulo'].'</b><br>'.$linha['mensagem_texto'].'</td></tr>';
	}
$pdf.='</table>';	
echo $pdf;
