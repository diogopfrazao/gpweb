...
foreach ($topicos as $linha) {
	...
	echo '</td>';
	echo '<td bgcolor="#dddddd" style="white-space: nowrap">'.link_contato($linha['contato_id'],'','','esquerda').'</td>';
	echo '<td align="center" width="40">'.$linha['respostas'].'</td>';
	echo '<td bgcolor="#dddddd" width="110" style="white-space: nowrap">';
	if ($linha['ultima_resposta']) {
			echo $ultimo->format('%d/%m/%Y %H:%M').'<br /><font color="#999966">(';
