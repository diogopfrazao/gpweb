...
foreach ($mensagens as $linha) {
	...
	if ($vertipo == 'normal') {
		$s .= '<tr>';
		$s .= '<td valign="top" style="'.$estilo.'" style="white-space: nowrap">';
		...
		$s .= '</tr><tr>';
		$s .= '<td valign="top" style="'.$estilo.'" style="white-space: nowrap">';
		$s .= '<img src="'.acharImagem('icones/postagem.gif', $m).'" alt="data da postagem" border=0 width="14" height="11">'.$data->format('%d/%m/%Y %H:%M').'</td>';
		
		$s .= '</tr>';
		} 
	elseif ($vertipo == 'curta') {
		$s .= '<tr><td valign="top" style="'.$estilo.'" ><font size="2">'.link_contato($linha['contato_id'],'','','esquerda').'</font>&nbsp;';
		$s .= ' ('.$data->format('%d/%m/%Y %H:%M').') ';
	...
	elseif ($vertipo == 'sozinha') {
		...
		$s .= $data->format('%d/%m/%Y %H:%M').' - <font size="2">'.link_contato($linha['contato_id'],'','','esquerda').'</font>&nbsp;&nbsp;';
...
if (!$dialogo){
	echo '<tr><td colspan="2"><table border=0 cellpadding="2" cellspacing="1" width="100%"><tr><td align="left" style="white-space: nowrap">';
