...
if ($del && !($podeExcluir && ($Aplic->usuario_id == $_REQUEST['mensagem_autor'] || $Aplic->usuario_super_admin))) $Aplic->redirecionar('m=publico&a=acesso_negado'); 
elseif ($nao_eh_novo && !($podeEditar && ($Aplic->usuario_id == $_REQUEST['mensagem_autor'] || $Aplic->usuario_super_admin))) $Aplic->redirecionar('m=publico&a=acesso_negado');
elseif (!$nao_eh_novo && !($podeEditar && ($Aplic->usuario_id == $_REQUEST['mensagem_autor'] || $Aplic->usuario_super_admin))) $Aplic->redirecionar('m=publico&a=acesso_negado');
