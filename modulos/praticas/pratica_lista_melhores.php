...
if(!$Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'melhor_pratica')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if (!$dialogo){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprir '.ucfirst($config['praticas']), 'Imprimir a lista de '.$config['praticas'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=praticas&a=pratica_lista_melhores&dialogo=1\')">'.imagem('imprimir_p.png').'</a>'. dicaF().'</td></tr>';


	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$procuraBuffer.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$imprimir.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=justificativa&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='justificativa' ? imagem('icones/'.$seta[$ordem]) : '').dica('Justificativa d'.$config['genero_pratica'].' '.ucfirst($config['pratica']), 'Neste campo fica a justificativa para '.$config['genero_pratica'].' '.$config['pratica'].' pertencer ao pool das melhores '.$config['praticas'].'.').'Justificativa'.dicaF().'</a></th>';
echo '</th>';
...
foreach($praticas as $linha){
	...
	if ($ver) echo '<td style="white-space: nowrap">'.link_pratica($linha['pratica_id']).'</td><td>'.$linha['justificativa'].'</td>';
	else echo '<td style="white-space: nowrap" colspan=2>'.dica($linha['pratica_nome'], 'Não tem permissão para ver os detalhes '.($config['genero_pratica']=='a' ? 'desta ': 'deste ').$config['pratica'].'.').'<i>'.$linha['pratica_nome'].'</i>'.dicaF().'</td>';
	echo '</tr>';
	}
