...
if ($exibir['gut_cor']) echo '<th width=16><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=gut_cor&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='gut_cor' ? imagem('icones/'.$seta[$ordem]) : '').dica('Cor', 'Neste campo fica a cor de identificação da matriz GUT').'Cor'.dicaF().'</a></th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=gut_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='gut_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome', 'Neste campo fica um nome para identificação da matriz GUT').'Nome'.dicaF().'</a></th>';
...
if ($filtro_prioridade_gut) echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=priorizacao&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica('Priorização', 'Clique para ordenar pela priorização.').($ordenar=='priorizacao' ? imagem('icones/'.$seta[$ordem]) : '').'Prio.'.dicaF().'</a></th>';

if ($exibir['gut_descricao']) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=gut_descricao&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='gut_descricao' ? imagem('icones/'.$seta[$ordem]) : '').dica('Descrição', 'Neste campo fica a descrição da matriz GUT.').'Descrição'.dicaF().'</a></th>';
...
if ($exibir['gut_relacionado']) echo '<th style="white-space: nowrap">'.dica('Relacionada', 'A quais áreas do sistema está relacionada.').'Relacionada'.dicaF().'</th>';
if ($exibir['gut_cia']) echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=gut_cia&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica(ucfirst($config['organizacao']), 'Clique para ordenar pel'.$config['genero_organizacao'].' '.$config['organizacao'].' responsável.').($ordenar=='gut_cia' ? imagem('icones/'.$seta[$ordem]) : '').ucfirst($config['organizacao']).dicaF().'</a></th>';
...
if ($exibir['gut_dept']) echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=gut_dept&ordem='.($ordem ? '0' : '1').'\');" class="hdr">'.dica(ucfirst($config['departamento']), strtoupper($config['genero_dept']).' '.strtolower($config['departamento']).' responsável.').($ordenar=='gut_dept' ? imagem('icones/'.$seta[$ordem]) : '').ucfirst($config['departamento']).dicaF().'</a></th>';
...
if ($exibir['gut_responsavel']) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=gut_responsavel&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='gut_responsavel' ? imagem('icones/'.$seta[$ordem]) : '').dica('Responsável', 'O '.$config['usuario'].' responsável pela matriz GUT.').'Responsável'.dicaF().'</a></th>';
...
if(!$from_lista && !$from_para_fazer) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=gut_ativo&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar==='gut_ativo' ? imagem('icones/'.$seta[$ordem]) : '').dica('Ativo', 'Neste campo consta se a matriz G.U.T está ativa.').'At.'.dicaF().'</a></th>';
...
foreach ($guts as $linha) {
	...
	if (permiteAcessarGut($linha['gut_acesso'],$linha['gut_id'])){
		...
		if (!$dialogo) echo '<td style="white-space: nowrap" width="16">'.($editar && $podeEditar && !$bloquear ? dica('Editar '.ucfirst($config['acao']), 'Clique neste ícone '.imagem('icones/editar.gif').' para editar as matrizes GUT.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=gut_editar&gut_id='.$linha['gut_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
		...
		if ($exibir['gut_aprovado'] && $Aplic->profissional) echo '<td style="width: 30px; text-align: center">'.($linha['gut_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
        ...
        if (!$from_lista && !$from_para_fazer) echo '<td style="width: 30px; text-align: center">'.($linha['gut_ativo'] ? 'Sim' : 'Não').'</td>';
		
		echo '</tr>';
		}
	}
if (!count($guts)) echo '<tr><td colspan="20"><p>Nenhuma matriz GUT encontrada.</p></td></tr>';
elseif(count($guts) && !$qnt) echo '<tr><td colspan="20"><p>Não teve permissão de visualizar qualquer das causas-efeitos.</p></td></tr>';
