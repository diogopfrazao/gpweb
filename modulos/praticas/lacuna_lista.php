...
if(!$Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'lacuna_indicador')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if (!$dialogo){
	...
	$nova_lacuna=($Aplic->checarModulo('praticas', 'adicionar', null, 'lacuna_indicador') ? '<tr><td style="white-space: nowrap">'.dica('Nova Lacuna de Indicador', 'Criar uma nova lacuna de indicador.').'<a href="javascript: void(0)" onclick="url_passar(0,\'m=praticas&a=lacuna_editar\');"><img src="'.acharImagem('lacuna_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$procura_pauta.$procuraBuffer.$procurar_ano.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$nova_lacuna.$imprimir.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
echo '<tr>';
if (!$impressao) echo '<th style="white-space: nowrap">&nbsp;</th>';
echo '<th width=16><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').(isset($projeto_id) ? '&projeto_id='.$projeto_id : '').(isset($tarefa_id) ? '&tarefa_id='.$tarefa_id : '').'&ordenar=indicador_lacuna_cor&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='indicador_lacuna_cor' ? imagem('icones/'.$seta[$ordem]) : '').dica('Cor do Indicador', 'Neste campo fica a cor de identificação do indicador.').'Cor'.dicaF().'</th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').(isset($projeto_id) ? '&projeto_id='.$projeto_id : '').(isset($tarefa_id) ? '&tarefa_id='.$tarefa_id : '').'&ordenar=indicador_lacuna_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='indicador_lacuna_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome do Indicador', 'Neste campo fica o nome para identificação do indicador.').'Nome'.dicaF().'</th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').(isset($projeto_id) ? '&projeto_id='.$projeto_id : '').(isset($tarefa_id) ? '&tarefa_id='.$tarefa_id : '').'&ordenar=indicador_lacuna_descricao&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='indicador_lacuna_descricao' ? imagem('icones/'.$seta[$ordem]) : '').dica('Descrição', 'Detalhes sobre do que se trata o indicador.').'Descrição'.dicaF().'</a></th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').(isset($projeto_id) ? '&projeto_id='.$projeto_id : '').(isset($tarefa_id) ? '&tarefa_id='.$tarefa_id : '').'&ordenar=indicador_lacuna_responsavel&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='indicador_lacuna_responsavel' ? imagem('icones/'.$seta[$ordem]) : '').dica(ucfirst($config['usuario']), 'O '.$config['usuario'].' responsável pelo indicador.').ucfirst($config['usuario']).dicaF().'</a></th>';
if (!isset($detalhe_projeto)) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=qnt_marcador&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='qnt_marcador' ? imagem('icones/'.$seta[$ordem]) : '').dica('Quantidade de Marcadores', 'A quantidade de marcadores relacionados à pauta selecionada nesta lacuna de indicador.').'<b>Qnt</b>'.dicaF().'</a></th>';
echo '</tr>';
$fp = -1;
$id = 0;
$qnt=0;
...
for ($i = ($pagina - 1) * $xpg_tamanhoPagina; $i < $pagina * $xpg_tamanhoPagina && $i < $xpg_totalregistros; $i++) {
	...
	echo '<tr>';
	if ($ver){
		if (!$impressao) echo '<td style="white-space: nowrap" width="16">'.($editar ? '<a href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=lacuna_editar&indicador_lacuna_id='.$linha['indicador_lacuna_id'].'\');">'.imagem('icones/editar.gif', 'Editar', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar esta lacuna de indicador.').dicaF().'</a>' : '').'</td>';
