...
function atualizar_causas($pratica_indicador_avaliacao_indicador=null){
	...
	if (count($causas)) {
		...
		foreach ($causas as $causa) {
			...
			$saida.= '<td style="white-space: nowrap" width="32"><a href="javascript: void(0);" onclick="editar_causa('.$causa['pratica_indicador_avaliacao_id'].');">'.imagem('icones/editar.gif', 'Editar Integrante', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o contato causa com '.$config['genero_projeto'].' '.$config['projeto'].'.').'</a>';
