...
$pauta='<tr><td align="right" style="white-space: nowrap">'.dica('Pauta de Pontuação', 'Utilize esta opção para filtrar '.$config['genero_marcador'].'s '.$config['marcadores'].' pela pauta de pontuação de sua preferência.').'Pauta de Pontuação:'.dicaF().'</td><td>'.selecionaVetor($modelos, 'pratica_modelo_id', 'onchange="document.env.submit()" class="texto" style="width:150px;"', $pratica_modelo_id).'</td></tr>';
$tipo='<tr><td align="right" style="white-space: nowrap">'.dica('Tipo de Exibição', 'Utilize esta opção escolher a forma de exibição.').'Tipo de Exibição:'.dicaF().'</td><td>'.selecionaVetor($tipos, 'tipo_modelo', 'onchange="document.env.submit()" class="texto" style="width:150px;"', $tipo_modelo).'</td></tr>';
...
foreach($marcadores as $dado){
	if ($dado['pratica_criterio_id']!=$criterio_atual){
		...
		$dentro .= '</table>';
		echo '<tr><td align="left" colspan=2 style="white-space: nowrap"><b>'.dica('Dados Sobre o Critério', $dentro).'<a href="javascript: void(0);" onclick="expandir_colapsar(\'criterio_'.$criterio_atual.'\')">'.$criterios[$dado['pratica_criterio_id']]['pratica_criterio_numero'].'.'.$criterios[$dado['pratica_criterio_id']]['pratica_criterio_nome'].'</a>'.dicaF().'</b></td></tr>';
	...
	if ($dado['pratica_item_id']!=$item_atual){
		...
		if (!$itens[$dado['pratica_item_id']]['pratica_item_oculto']) {
			echo '<tr><td align="left" colspan=2 style="white-space: nowrap"><b>'.($tipo_modelo!='completo' ? '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' : '').dica('Dados Sobre o Ítem', $dentro).$criterios[$dado['pratica_criterio_id']]['pratica_criterio_numero'].'.'.$itens[$dado['pratica_item_id']]['pratica_item_numero'].'.'.$itens[$dado['pratica_item_id']]['pratica_item_nome'].dicaF().'</b></td></tr>';
	...
	if ($tipo_modelo=='completo') {
		...
		echo '<tr><td align="left" style="white-space: nowrap" valign="top">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>'.$dado['pratica_marcador_letra'].'.&nbsp;</b></td><td id="caixa_'.$dado['pratica_marcador_id'].'" width="100%" valign="middle">'.$dado['pratica_marcador_texto'].'</td></tr>';
		if ($dado['pratica_marcador_extra']) echo '<tr><td align="left" style="white-space: nowrap" valign="top">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td width="100%" valign="middle">'.$dado['pratica_marcador_extra'].'</td></tr>';
		}
	}
