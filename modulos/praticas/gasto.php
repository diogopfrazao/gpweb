...
if ($acao!='inserir' && $acao!='editar'){
	...
	foreach ($linhas as $linha) {
		...
		echo '<td align="left" style="white-space: nowrap">'.link_usuario($linha['plano_acao_item_gastos_usuario'],'','','esquerda').'</td>';
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nome', 'Escreva o nome deste item.').'Nome:'.dicaF().'</td><td><input type="text" class="texto" name="plano_acao_item_gastos_nome" value="'.(isset($atual['plano_acao_item_gastos_nome']) ? $atual['plano_acao_item_gastos_nome']:'').'" maxlength="255" size="40" /></td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Unidade de Medida', 'Escolha a unidade de medida deste item.').'Unidade de medida:'.dicaF().'</td><td>'.selecionaVetor($unidade, 'plano_acao_item_gastos_tipo', 'class=texto size=1', (isset($atual['plano_acao_item_gastos_tipo']) ? $atual['plano_acao_item_gastos_tipo']:'')).'</td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Quantidade', 'Insira a quantidade deste item.').'Quantidade:'.dicaF().'</td><td><input type="text" class="texto" onkeypress="return entradaNumerica(event, this, true, true);" onchange="javascript:valor();" onclick="javascript:valor();"name="plano_acao_item_gastos_quantidade" id="plano_acao_item_gastos_quantidade" value="'.(isset($atual['plano_acao_item_gastos_quantidade']) ? number_format($atual['plano_acao_item_gastos_quantidade'], 2, ',', '.'):'').'" maxlength="255" size="10" /></td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Valor', 'Insira o valor deste item.').'Valor:'.dicaF().'</td><td><input type="text" class="texto" onkeypress="return entradaNumerica(event, this, true, true);" onchange="javascript:valor();" onclick="javascript:valor();" name="plano_acao_item_gastos_custo" id="plano_acao_item_gastos_custo" value="'.(isset($atual['plano_acao_item_gastos_custo']) ? number_format($atual['plano_acao_item_gastos_custo'], 2, ',', '.'):'').'" size="40" /></td></tr>';
	
	$categoria_economica=array(''=>'')+getSisValor('CategoriaEconomica');
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Categoria Econômica', 'Escolha a categoria econômica deste item.').'Categoria econômica:'.dicaF().'</td><td>'.selecionaVetor($categoria_economica, 'plano_acao_item_gastos_categoria_economica', 'class=texto size=1 style="width:395px;" onchange="frm.plano_acao_item_gastos_nd.value=\'\'; mudar_nd();"', (isset($atual['plano_acao_item_gastos_categoria_economica']) ? $atual['plano_acao_item_gastos_categoria_economica']:'')).'</td></tr>';
	
	$GrupoND=array(''=>'')+getSisValor('GrupoND');
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Grupo de Despesa', 'Escolha o grupo de despesa deste item.').'Grupo de despesa:'.dicaF().'</td><td>'.selecionaVetor($GrupoND, 'plano_acao_item_gastos_grupo_despesa', 'class=texto size=1 style="width:395px;" onchange="frm.plano_acao_item_gastos_nd.value=\'\'; mudar_nd();"', (isset($atual['plano_acao_item_gastos_grupo_despesa']) ? $atual['plano_acao_item_gastos_grupo_despesa']:'')).'</td></tr>';
	
	$ModalidadeAplicacao=array(''=>'')+getSisValor('ModalidadeAplicacao');
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Modalidade de Aplicação', 'Escolha a modalidade de aplicação deste item.').'Modalidade de aplicação:'.dicaF().'</td><td>'.selecionaVetor($ModalidadeAplicacao, 'plano_acao_item_gastos_modalidade_aplicacao', 'class=texto size=1 style="width:395px;" onchange="frm.plano_acao_item_gastos_nd.value=\'\'; mudar_nd();"', (isset($atual['plano_acao_item_gastos_modalidade_aplicacao']) ? $atual['plano_acao_item_gastos_modalidade_aplicacao']:'')).'</td></tr>';
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Elemento de Despesa', 'Escolha o elemento de despesa (ED) deste item.').'Elemento de despesa:'.dicaF().'</td><td><div id="combo_nd">'.selecionaVetor($nd, 'plano_acao_item_gastos_nd', 'class=texto size=1 style="width:395px;" onchange="mudar_nd();"', (isset($atual['plano_acao_item_gastos_nd']) ? $atual['plano_acao_item_gastos_nd']: '')).'</div></td></tr>';
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Descrição', 'Insira a descrição deste item.').'Descrição:'.dicaF().'</td><td><textarea cols="70" rows="5" class="textarea" name="plano_acao_item_gastos_descricao">'.(isset($atual['plano_acao_item_gastos_descricao']) ? $atual['plano_acao_item_gastos_descricao']:'').'</textarea></td></tr>';
	...
	echo '<tr><td align="right">'.dica('Data','Data limite para o recebimento do ítem.').'Data:</td><td><table cellpadding=0 cellspacing=0><tr><td><td><input type="hidden" name="plano_acao_item_gastos_data_recebido" id="plano_acao_item_gastos_data_recebido" value="'.($data_texto ? $data_texto->format('%Y%m%d') : '').'" /><input type="text" name="data_texto"  id="data_texto" style="width:70px;" onchange="setData(\'env\', \'data_texto\', \'plano_acao_item_gastos_data_recebido\');" value="'.($data_texto ? $data_texto->format('%d/%m/%Y') : '').'" class="texto" />'.dica('Data Limite', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data limite para o recebimento do ítem.').'<a href="javascript: void(0);" ><img id="f_btn1" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 /></a>'.dicaF().'</td></tr></table></td></tr>';
