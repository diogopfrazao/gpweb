...
if(!$Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'checklist')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if (!$dialogo || $selecao){
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionado', 'A que área o checklist está relacionado.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$procuraBuffer = '<tr><td align=right style="white-space: nowrap">'.dica('Pesquisar', 'Pesquisar pelo nome e campos de descrição').'Pesquisar:'.dicaF().'</td><td><input type="text" class="texto" style="width:250px;" name="checklisttextobusca" onChange="document.env.submit();" value="'.$pesquisar_texto.'"/></td><td><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.'&tab='.$tab.'&checklisttextobusca=\');">'.imagem('icones/limpar_p.gif','Limpar Pesquisa', 'Clique neste ícone '.imagem('icones/limpar_p.gif').' para limpar a caixa texto de pesquisa.').'</a></td></tr>';


	$novo_checklist=($Aplic->checarModulo('praticas', 'adicionar', null, 'checklist') ? '<tr><td style="white-space: nowrap">'.dica('Novo Checklist', 'Criar um novo checklist.').'<a href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=praticas&a=checklist_editar\');" ><img src="'.acharImagem('todo_list_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	$botao_favorito=($Aplic->profissional ? '<tr><td style="white-space: nowrap">'.dica('Favoritos', 'Criar ou editar um grupo de checklist favoritos, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&checklist=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir a Lista de Checklist', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de checklist.').'<a href="javascript: void(0);" onclick ="url_passar(1,\'m='.$m.'&a='.$a.'&u='.$u.'&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=checklists', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=praticas&a=checklist_lista');
	}		
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&checklist=1&filtro_prioridade='+env.filtro_prioridade_checklist.value, window.setFiltroPriorizacao, window);
	}
