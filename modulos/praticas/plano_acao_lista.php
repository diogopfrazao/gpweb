...
if (!$dialogo || $selecao){
	...
	$botoesTitulo = new CBlocoTitulo('Lista de '.ucfirst($config['acoes']), 'plano_acao.png', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$procuraBuffer.$procuraAno.$popFiltro.$filtros.$favoritos.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.(!$selecao ? $novo_plano.$imprimir.$botao_prioridade.$botao_favorito.$botao_trava.$botao_campos : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
...
if (!$dialogo || $selecao){
	...
	include_once(BASE_DIR.'/modulos/praticas/plano_acao_ver_idx.php');
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=acoes', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=praticas&a=plano_acao_lista');
	}		
