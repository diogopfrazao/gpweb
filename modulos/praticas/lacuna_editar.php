...
if($indicador_lacuna_id && !(permiteEditarLacuna($acesso,$indicador_lacuna_id) && $Aplic->checarModulo('praticas', 'editar', $Aplic->usuario_id, 'lacuna_indicador'))) $Aplic->redirecionar('m=publico&a=acesso_negado');
elseif (!$indicador_lacuna_id && !$indicador_lacuna_id && !$Aplic->checarModulo('praticas', 'adicionar', $Aplic->usuario_id, 'lacuna_indicador'))$Aplic->redirecionar('m=publico&a=acesso_negado');
...
echo '<tr><td align="right">'.dica('Nome da Lacuna de Indicador', 'Toda lacuna de indicador necessita ter um nome para identificação.').'Nome:'.dicaF().'</td><td><input type="text" name="indicador_lacuna_nome" value="'.$obj->indicador_lacuna_nome.'" style="width:400px;" maxlength="512" class="texto" />*</td></tr>';



echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['organizacao']).' da Lacuna de Indicador', 'A qual '.$config['organizacao'].' pertence esta lacuna de indicador.').ucfirst($config['organizacao']).':'.dicaF().'</td><td width="100%" style="white-space: nowrap" colspan="2"><div id="combo_cia">'.selecionar_om(($obj->indicador_lacuna_cia ? $obj->indicador_lacuna_cia : $cia_id), 'indicador_lacuna_cia', 'class=texto size=1 style="width:400px;" onchange="javascript:mudar_om();"').'</div></td></tr>';
...
if ($Aplic->profissional) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']).' Responsável', 'Escolha pressionando o ícone à direita qual '.$config['genero_dept'].' '.$config['dept'].' responsável por esta lacuna de indicador.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td><input type="hidden" name="indicador_lacuna_dept" id="indicador_lacuna_dept" value="'.($indicador_lacuna_id ? $obj->indicador_lacuna_dept : ($Aplic->getEstado('dept_id') !== null ? ($Aplic->getEstado('dept_id') ? $Aplic->getEstado('dept_id') : null) : $Aplic->usuario_dept)).'" /><input type="text" class="texto" name="dept_nome" id="dept_nome" value="'.nome_dept(($indicador_lacuna_id ? $obj->indicador_lacuna_dept : ($Aplic->getEstado('dept_id') !== null ? ($Aplic->getEstado('dept_id') ? $Aplic->getEstado('dept_id') : null) : $Aplic->usuario_dept))).'" style="width:400px;" READONLY />'.botao_icone('secoes_p.gif','Selecionar', 'selecionar '.$config['departamento'],'popDept()').'</td></tr>';
$saida_depts='';
...
echo '<tr><td align="right" style="white-space: nowrap" width=100>'.dica('Responsável', 'Toda lacuna de indicador deve ter um responsável.').'Responsável:'.dicaF().'</td><td colspan="2"><input type="hidden" id="indicador_lacuna_responsavel" name="indicador_lacuna_responsavel" value="'.($obj->indicador_lacuna_responsavel ? $obj->indicador_lacuna_responsavel : $Aplic->usuario_id).'" /><input type="text" id="nome_gerente" name="nome_gerente" value="'.nome_om(($obj->indicador_lacuna_responsavel ? $obj->indicador_lacuna_responsavel : $Aplic->usuario_id),$Aplic->getPref('om_usuario')).'" style="width:400px;" class="texto" READONLY /><a href="javascript: void(0);" onclick="popGerente();">'.imagem('icones/usuarios.gif','Selecionar '.ucfirst($config['usuario']),'Clique neste ícone '.imagem('icones/usuarios.gif').' para selecionar '.($config['genero_usuario']=='o' ? 'um' : 'uma').' '.$config['usuario'].'.').'</a></td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap" >'.dica('Descrição', 'Descrição sobre esta lacuna de indicador.').'Descrição:'.dicaF().'</td><td width="100%" colspan="2"><textarea data-gpweb-cmp="ckeditor" name="indicador_lacuna_descricao" style="width:400px;" rows="2" class="textarea">'.$obj->indicador_lacuna_descricao.'</textarea></td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nível de Acesso', 'Os indicadores podem ter cinco níveis de acesso:<ul><li><b>Público</b> - Todos podem ver e editar a lacuna de indicador.</li><li><b>Protegido I</b> - Todos podem ver, porem apenas o responsável e os designados para a lacuna de indicador podem editar.</li><li><b>Protegido II</b> - Todos podem ver, porem apenas o responsável pode editar tudo e os designados editarem os objetos relacionados.</li><li><b>Protegido III</b> - Todos podem ver, porem apenas o responsável pode editar.</li><li><b>Participante I</b> - Somente o responsável e os designados para a lacuna de indicador podem ver e editar o mesmo</li><li><b>Participantes II</b> - Somente o responsável e os designados podem ver e apenas o responsável pode editar tudo e os designados editarem os objetos relacionados.</li><li><b>Participantes III</b> - Somente o responsável e os designados para a lacuna de indicador podem ver o mesmo, e o responsável editar.</li></ul>').'Nível de acesso:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($indicador_lacuna_acesso, 'indicador_lacuna_acesso', 'class="texto"', ($indicador_lacuna_id ? $obj->indicador_lacuna_acesso : $config['nivel_acesso_padrao'])).'</td></tr>';

echo '<tr><td align="right" style="white-space: nowrap">'.dica('Cor', 'Cor selecionada dentre as 16 milhões possíveis. Pode-se escrever diretamente o hexadecinal na cor ou utilizar a interface que se abre ao clicar na caixa de inserção do valor.').'Cor:'.dicaF().'</td><td align="left" style="white-space: nowrap"><input class="jscolor" name="indicador_lacuna_cor" value="'.($obj->indicador_lacuna_cor ? $obj->indicador_lacuna_cor : 'FFFFFF').'" '.($config['selecao_cor_restrita'] ? 'readonly="readonly" ' : '').'size="6" maxlength="6" style="width:57px;" /></td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Notificar por e-mail', 'Um aviso da '.($indicador_lacuna_id > 0 ? 'modificação' : 'criação').' desta lacuna de indicador poderá ser enviados por e-mail').'Notificar por e-mail:'.dicaF().'</td><td width="100%" colspan="2" valign="top">';
...
function mudar_om(){
	...
	xajax_selecionar_om_ajax(indicador_lacuna_cia,'indicador_lacuna_cia','combo_cia', 'class="texto" size=1 style="width:400px;" onchange="javascript:mudar_om();"');
	}
