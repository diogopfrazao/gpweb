...
echo '<tr><td style="border: outset #d1d1cd 1px;background-color:#'.$checklist['checklist_cor'].'" colspan="2"><font color="'.melhorCor($checklist['checklist_cor']).'"><b>'.$checklist['checklist_nome'].'<b></font>'.(!$impressao ? '<a href="javascript: void(0);" onclick ="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.'&dialogo=1&impressao=1&checklist_dados_id='.$checklist_dados_id.'&pratica_indicador_id='.$pratica_indicador_id.'\');">'.imagem('impressora_p.gif','Imprir', 'Clique neste ícone '.imagem('impressora_p.gif').' para imprimir este resulrado do checklist.').'</a>'.dicaF() : '').'</td></tr>';

if ($checklist['checklist_descricao']) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Descrição', 'Descrição do checklist.').'Descrição:'.dicaF().'</td><td class="realce">'.$checklist['checklist_descricao'].'</td></tr>';
...
if ($campos_salvos['pratica_indicador_valor_data']) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Data', 'A data de preenchimento do checklist.').'Data:'.dicaF().'</td><td class="realce" width="100%">'.retorna_data($campos_salvos['pratica_indicador_valor_data'], false).'</td></tr>';		
if ($campos_salvos['checklist_dados_obs']) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Descrição', 'Descrição do checklist.').'Observação:'.dicaF().'</td><td class="realce">'.$campos_salvos['checklist_dados_obs'].'</td></tr>';
if ($campos_salvos['checklist_dados_responsavel']) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Responsável', ucfirst($config['usuario']).' responsável por marcar o checklist.').'Responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($campos_salvos['checklist_dados_responsavel'], '','','esquerda').'</td></tr>';		
...
else if ($impressao && !($Aplic->usuario_nomeguerra=='Visitante' && $Aplic->usuario_id=1)) echo '<script language=Javascript>self.print();</script>';
