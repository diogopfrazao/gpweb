...
if(!$Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'indicador')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
$pratica_indicador_tipo = $Aplic->getEstado('pratica_indicador_tipo') !== null ? $Aplic->getEstado('pratica_indicador_tipo') : '';
...
if (!$dialogo || $selecao){
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionado', 'A que área o indicador está relacionado.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$pesquisa_pauta='<tr><td align="right" style="white-space: nowrap">'.dica('Seleção de Pauta de Pontuação', 'Utilize esta opção para filtrar '.$config['genero_marcador'].'s '.$config['marcadores'].' pela pauta de pontuação de sua preferência.').'Pauta:'.dicaF().'</td><td align="left" style="white-space: nowrap">'.selecionaVetor($modelos, 'pratica_modelo_id', 'onchange="document.env.submit()" class="texto" style="width:250px;"', $pratica_modelo_id).'</td></tr>';

	$tipo='<tr><td align="right" style="white-space: nowrap">'.dica('Tipo', 'Definir o tipo de indicador.').'Tipo:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><div id="combo_indicador_tipo">'.selecionaVetor($indicadores_tipos, 'pratica_indicador_tipo', 'class="texto" size=1 style="width:250px;" onchange="mudar_indicador_tipo();"', $pratica_indicador_tipo).'</div></td></tr></table></td></tr>';
	...
	$novo_indicador=($Aplic->checarModulo('praticas', 'adicionar', null, 'indicador') ? '<tr><td align=right>'.dica('Novo Indicador', 'Criar um novo indicador.').'<a href="javascript: void(0)" onclick="url_passar(0,\'m=praticas&a=indicador_editar\');"><img src="'.acharImagem('indicador_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	
	$botao_favorito=($Aplic->profissional ? '<tr><td style="white-space: nowrap" align=right>'.dica('Favoritos', 'Criar ou editar um grupo de indicadores favoritos, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&indicador=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$pesquisa_pauta.$procuraBuffer.$selecao_ano.$tipo.$popFiltro.$filtros.$favoritos.'</table></td><td align=right valign=top><table cellspacing=0 cellpadding=0>'.$filtrar.(!$selecao ? $novo_indicador.$botao_imprimir.$botao_prioridade.$filtro_expandido.$botao_favorito.$botao_trava.$botao_excel.$botao_campos : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
...
foreach ((array)$praticas_criterios as $chave => $criterio) {
	...
	elseif ($ssti_id) $sql->adOnde('pratica_indicador_gestao_ssti IN ('.$ssti_id.')');
	elseif ($laudo_id) $sql->adOnde('pratica_indicador_gestao_laudo IN ('.$laudo_id.')');
	elseif ($trelo_id) $sql->adOnde('pratica_indicador_gestao_trelo IN ('.$trelo_id.')');
	elseif ($trelo_cartao_id) $sql->adOnde('pratica_indicador_gestao_trelo_cartao IN ('.$trelo_cartao_id.')');
	elseif ($pdcl_id) $sql->adOnde('pratica_indicador_gestao_pdcl IN ('.$pdcl_id.')');
	elseif ($pdcl_item_id) $sql->adOnde('pratica_indicador_gestao_pdcl_item IN ('.$pdcl_item_id.')');		
	elseif ($os_id) $sql->adOnde('pratica_indicador_gestao_os IN ('.$os_id.')');		
...
if ($pratica_modelo_id && isset($praticas_criterios[$tab-2]['pratica_criterio_resultado']) && !$praticas_criterios[$tab-2]['pratica_criterio_resultado']){
	...
	if ($a!='indicador_lista' && !$indicador_expandido) $sql->adOnde('pratica_indicador_ativo=1');
	elseif ($tab !=1 && !$indicador_expandido) $sql->adOnde('pratica_indicador_ativo=1');
	elseif (!$indicador_expandido) $sql->adOnde('pratica_indicador_ativo=0');
...
elseif ($pratica_modelo_id && isset($praticas_criterios[$tab-2]['pratica_criterio_resultado']) && $praticas_criterios[$tab-2]['pratica_criterio_resultado']){
	...
	$sql->adCampo('pratica_indicador.pratica_indicador_id, pratica_indicador_acesso, pratica_indicador_nome, pratica_indicador_cor, pratica_indicador_sentido, pratica_indicador_periodo_anterior, pratica_indicador_tolerancia, pratica_indicador_responsavel, pratica_indicador_cia, pratica_indicador_dept, pratica_indicador_codigo, ('.$q->prepare().') AS qnt_marcador, pratica_indicador_formula, pratica_indicador_formula_simples, pratica_indicador_composicao, pratica_indicador_checklist, pratica_indicador_campo_projeto, pratica_indicador_campo_tarefa, pratica_indicador_campo_acao, pratica_indicador_externo, pratica_indicador_agrupar, pratica_indicador_acumulacao, pratica_indicador_unidade, pratica_indicador_aprovado');
	$sql->adCampo('(SELECT formatar_data(MAX(pratica_indicador_valor_data), \'%d/%m/%Y\') FROM pratica_indicador_valor WHERE pratica_indicador_valor_indicador=pratica_indicador.pratica_indicador_id) AS ultima_data');
	$sql->adCampo('pratica_indicador_requisito_descricao, pratica_indicador_requisito_oque, pratica_indicador_requisito_onde, pratica_indicador_requisito_quando, pratica_indicador_requisito_como, pratica_indicador_requisito_porque,
	pratica_indicador_requisito_quanto, pratica_indicador_requisito_quem, pratica_indicador_requisito_melhorias');
	if ($indicador_expandido) $sql->adCampo($indicador_expandido.'=pratica_indicador.pratica_indicador_id AS pai');
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=indicadores', window.setCamposExibir, window);
	}
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&indicador=1&filtro_prioridade='+env.filtro_prioridade_indicador.value, window.setFiltroPriorizacao, window);
	}
...
function exportar_excel() {
    if(window.parent.gpwebApp){
        parent.gpwebApp.exportarListaIndicadoresExcel("<?php echo ucfirst($config['organizacao']) ?>", "<?php echo ucfirst($config['departamento']) ?>");
    }
    else{
