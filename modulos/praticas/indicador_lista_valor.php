...
global $estilo_interface, $pratica_indicador_id, $pratica_indicador, $editar, $podeEditarTudo, $podeValor, $podeEditar;
...
mostrarBarraNav($xpg_totalregistros, $xpg_tamanhoPagina, $xpg_total_paginas, $pagina, 'valor', 'valores','','&pratica_indicador_id='.$pratica_indicador_id,($estilo_interface=='classico' ? 'a6a6a6' : '006fc2'));

echo '<table cellpadding=0 cellspacing=0 width="100%" class="tbl1">';
echo '<tr>';
if ((($podeEditar && $podeEditarTudo) || $podeValor) && !$pratica_indicador['pratica_indicador_externo']) echo '<th width="16"><a href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=indicador_editar_valor&pratica_indicador_id='.$pratica_indicador_id.'\');">'.imagem('icones/adicionar.png','Inserir','Clique neste ícone '.imagem('icones/adicionar.png').' para inserir um novo valor.').'</a></th>';
...
for ($i = ($pagina - 1) * $xpg_tamanhoPagina; $i < $pagina * $xpg_tamanhoPagina && $i < $xpg_totalregistros; $i++) {
	...
	if (($podeEditar || $podeValor) && $podeEditarTudo  && !$pratica_indicador['pratica_indicador_externo']) {
		echo '<td>';
		if (!$Aplic->profissional || ($Aplic->profissional  && !$tem_prazo) || ($Aplic->profissional  && $tem_prazo && $valor['pratica_indicador_valor_data']>=$faixa['inicio'] && $valor['pratica_indicador_valor_data']<=$faixa['fim'])) echo '<a href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=indicador_editar_valor&pratica_indicador_valor_id='.$valor['pratica_indicador_valor_id'].'\');">'.imagem('icones/editar.gif','Editar','Clique neste ícone '.imagem('icones/editar.gif').' para editar o valor.').'</a>';
		echo '</td>';
		}
	elseif (($podeEditar || $podeValor) && $podeEditarTudo  && $pratica_indicador['pratica_indicador_externo']) {
		echo '<td>';
		if (!$Aplic->profissional || ($Aplic->profissional  && !$tem_prazo) || ($Aplic->profissional  && $tem_prazo && $valor['pratica_indicador_valor_data']>=$faixa['inicio'] && $valor['pratica_indicador_valor_data']<=$faixa['fim'])) echo '<a href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=indicador_importar_valor_pro&pratica_indicador_id='.$pratica_indicador_id.'&inicio='.$valor['pratica_indicador_valor_data'].'&fim='.$valor['pratica_indicador_valor_data'].'\');">'.imagem('icones/editar.gif','Importar','Clique neste ícone '.imagem('icones/editar.gif').' para importar valor na mesma data.').'</a>';
		echo '</td>';
		}
	...
	echo '<td  width="60" style="white-space: nowrap" align=center>'.retorna_data($valor['pratica_indicador_valor_data'], false).'</td>';
	echo '<td style="white-space: nowrap" align=right>'.number_format($valor['pratica_indicador_valor_valor'], $config['casas_decimais'], ',', '.').$icone.'</td>';
