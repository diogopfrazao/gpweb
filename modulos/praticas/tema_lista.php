...
if(!$Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'tema')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if (isset($_REQUEST['filtro_pg_perspectiva_id'])) $Aplic->setEstado('filtro_pg_perspectiva_id', getParam($_REQUEST, 'filtro_pg_perspectiva_id', null));
$filtro_pg_perspectiva_id = $Aplic->getEstado('filtro_pg_perspectiva_id') !== null ? $Aplic->getEstado('filtro_pg_perspectiva_id') : 0;
...
if (isset($_REQUEST['filtro_pg_id'])) $Aplic->setEstado('filtro_pg_id', getParam($_REQUEST, 'filtro_pg_id', null));
$filtro_pg_id = ($Aplic->getEstado('filtro_pg_id') !== null ? $Aplic->getEstado('filtro_pg_id') : null);
...
if ($filtro_pg_id){
	...
	$sql->adOnde('plano_gestao_perspectivas.pg_id='.(int)$filtro_pg_id);
	}
$sql->adOnde('pg_perspectiva_ativo = 1');
$sql->adCampo('perspectivas.pg_perspectiva_id, pg_perspectiva_nome');
...
if (!$dialogo || $selecao){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionad'.$config['genero_tema'], 'A que área '.$config['genero_tema'].' '.$config['tema'].' está relacionad'.$config['genero_tema'].'.').'Relacionad'.$config['genero_tema'].':'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	$filtra_perspectiva='<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['perspectiva']), 'Filtrar '.$config['temas'].' pel'.$config['genero_perspectiva'].' '.$config['perspectiva'].' dos mesmos.').ucfirst($config['perspectiva']).':'.dicaF().'</td><td>'.	selecionaVetor($perspectivas, 'filtro_pg_perspectiva_id', 'class="texto" style="width:250px;" onchange="env.submit();"', $filtro_pg_perspectiva_id,'','',$cor).'</td></tr>';
	...
	$selecionar_planejamento='<tr><td align="right" style="white-space: nowrap">'.dica('Planejamentos Estratégico', 'Utilize esta opção para filtrar pelo planejamentos estratégico que está vinculad'.$config['genero_fator'].'.').'Planejamentos Estratégico:</td><td>'.dicaF().selecionaVetor($planos, 'filtro_pg_id', 'onchange="env.submit();" style="width:250px;" class="texto"', $filtro_pg_id).'</td></tr>';
	...
	$novo_tema=($Aplic->checarModulo('praticas', 'adicionar', null, 'tema') ? '<tr><td style="white-space: nowrap" align=right>'.dica('Nov'.$config['genero_tema'].' '.ucfirst($config['tema']).'', 'Criar um nov'.$config['genero_tema'].' '.$config['tema'].'.').'<a href="javascript: void(0)" onclick="url_passar(0,\'m=praticas&a=tema_editar\');"><img src="'.acharImagem('tema_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir '.ucfirst($config['temas']), 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['temas'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&u='.$u.'&dialogo=1&tab='.$tab.'\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';

	$botao_favorito=($Aplic->profissional ? '<tr><td style="white-space: nowrap">'.dica('Favoritos', 'Criar ou editar um grupo de planos de ação favoritos, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&tema=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');	
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$selecionar_planejamento.$procuraBuffer.$filtra_perspectiva.$favoritos.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.(!$selecao ? $novo_tema.$imprimir.$botao_prioridade.$botao_favorito.$botao_trava.$botao_campos : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=temas', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=praticas&a=tema_lista');
	}	
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&tema=1&filtro_prioridade='+env.filtro_prioridade_tema.value, window.setFiltroPriorizacao, window);
	}
