...
if (isset($_REQUEST['tema_id'])) $Aplic->setEstado('tema_id', getParam($_REQUEST, 'tema_id', null), $m, $a, $u);
$tema_id = $Aplic->getEstado('tema_id', null, $m, $a, $u);
...
if(!(permiteAcessarTema($obj->tema_acesso,$tema_id) && $Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'tema'))) $Aplic->redirecionar('m=publico&a=acesso_negado');

$podeEditar=$Aplic->checarModulo('praticas', 'editar', $Aplic->usuario_id, 'tema');
$podeAdicionar=$Aplic->checarModulo('praticas', 'adicionar', $Aplic->usuario_id, 'tema');
$podeExcluir=$Aplic->checarModulo('praticas', 'excluir', $Aplic->usuario_id, 'tema');



$editar=permiteEditarTema($obj->tema_acesso,$tema_id);

$podeEditarTudo=($obj->tema_acesso>=5 ? $editar && (in_array($obj->tema_usuario, $Aplic->usuario_lista_grupo_vetor) || $Aplic->usuario_super_admin || $Aplic->usuario_admin) : $editar);
...
if (!$dialogo){	
	...
	if ($podeAdicionar)	$km->Add("inserir","inserir_objeto",dica('Nov'.$config['genero_tema'].' '.ucfirst($config['tema']), 'Criar um nov'.$config['genero_tema'].' '.$config['tema'].'.').'Nov'.$config['genero_tema'].' '.ucfirst($config['tema']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=tema_editar\");");
	if ($podeEditar && $editar) {
		$km->Add("inserir","inserir_registro",dica('Registro de Ocorrência','Inserir um novo registro de ocorrência.').'Registro de ocorrência'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=log_editar&tema_id=".$tema_id."\");");
		if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('eventos', 'adicionar')) $km->Add("inserir","inserir_evento",dica('Novo Evento', 'Criar um novo evento relacionado.').'Evento'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=calendario&a=editar&evento_tema=".$tema_id."\");");
		...
		if ($Aplic->checarModulo('projetos', 'adicionar', null, 'demanda')) $km->Add("inserir","inserir_demanda",dica('Nova Demanda', 'Inserir uma nova demanda relacionada.').'Demanda'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=demanda_editar&demanda_tema=".$tema_id."\");");
		...
		if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'adicionar')) $km->Add("diverso","inserir_link",dica('Novo Link', 'Inserir um novo link relacionado.').'Link'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=links&a=editar&link_tema=".$tema_id."\");");
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("diverso","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_tema=".$tema_id."\");");
		...
		if ($Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'adicionar', null, null)) {
			$km->Add("diverso","inserir_trelo",dica('Nov'.$config['genero_trelo'].' '.ucfirst($config['trelo']), 'Inserir um'.($config['genero_trelo']=='a' ? 'a' : '').' nov'.$config['genero_trelo'].' '.$config['trelo'].' relacionad'.$config['genero_trelo'].'.').ucfirst($config['trelo']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=trelo&a=trelo_editar&trelo_tema=".$tema_id."\");");
	...
	if ($podeEditarTudo && $podeEditar && !$bloquear) $km->Add("acao","acao_editar",dica('Editar '.ucfirst($config['tema']),'Editar os detalhes d'.($config['genero_tema']=='a' ? 'esta' : 'este').' '.$config['tema'].'.').'Editar '.ucfirst($config['tema']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=tema_editar&tema_id=".$tema_id."\");");
	if ($podeEditarTudo && $podeExcluir && !$bloquear) $km->Add("acao","acao_excluir",dica('Excluir','Excluir '.($config['genero_tema']=='a' ? 'esta' : 'este').' '.$config['tema'].'.').'Excluir '.ucfirst($config['tema']).dicaF(), "javascript: void(0);' onclick='excluir()");
	...
	if ($Aplic->profissional) $km->Add("acao_imprimir","dashboard_geral",dica('Grau de Consecução','Dashboard do grau de consecução d'.($config['genero_tema']=='a' ? 'esta' : 'este').' '.$config['tema'].'.').'Grau de Consecução'.dicaF(), "javascript: void(0);' onclick='url_passar(\"dashboard_geral_".$tema_id."\", \"m=praticas&a=tema_deshboard_geral_pro&jquery=1&dialogo=1&tema_id=".$tema_id."\");");
...
echo '<tr><td style="border: outset #d1d1cd 1px;background-color:#'.$obj->tema_cor.'" colspan="2"><font color="'.melhorCor($obj->tema_cor).'"><b>'.$obj->tema_nome.'<b></font>'.$cor_indicador.'</td></tr>';
...
if ($obj->tema_descricao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Descrição', 'Descrição d'.$config['genero_tema'].' '.$config['tema'].'.').'Descrição:'.dicaF().'</td><td class="realce" width="100%">'.$obj->tema_descricao.'</td></tr>';
if ($obj->tema_cia) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', ucfirst($config['genero_organizacao']).' '.$config['organizacao'].' responsável.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_cia($obj->tema_cia).'</td></tr>';
if ($Aplic->profissional){
...
if ($obj->tema_dept) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']).' Responsável', ucfirst($config['genero_dept']).' '.$config['departamento'].' responsável por '.($config['genero_tema']=='a' ? 'esta' : 'este').' '.$config['tema'].'.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_dept($obj->tema_dept).'</td></tr>';
$saida_depts='';
...
if ($saida_depts) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica(ucfirst($config['departamentos']), 'Qual '.strtolower($config['departamento']).' está envolvid'.$config['genero_dept'].' com '.($config['genero_tema']=='a' ? 'esta ': 'este ').$config['tema'].'.').ucfirst($config['departamentos']).' envolvid'.$config['genero_dept'].'s:'.dicaF().'</td><td width="100%" colspan="2" class="realce">'.$saida_depts.'</td></tr>';

if ($obj->tema_usuario) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Responsável pel'.$config['genero_tema'].' '.ucfirst($config['tema']).'', ucfirst($config['usuario']).' responsável por gerenciar '.$config['genero_tema'].' '.$config['tema'].'.').'Responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($obj->tema_usuario, '','','esquerda').'</td></tr>';		
...
if ($Aplic->profissional){
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Sistema de Percentagem', 'Qual forma a percentagem d'.$config['genero_tema'].' '.$config['tema'].' é calculada.').'Sistema de percentagem:'.dicaF().'</td><td class="realce">'.(isset($tipo_pontuacao[$obj->tema_tipo_pontuacao]) ? $tipo_pontuacao[$obj->tema_tipo_pontuacao] : '').'</td></tr>';
	if ($obj->tema_tipo_pontuacao && $obj->tema_tipo_pontuacao!='indicador'){
		...
		if (is_array($medias) && count($medias)){
			...
			foreach ($medias as $contato_id => $media) {
				...
				if ($media['tema_media_projeto']) echo '<td align="left">'.imagem('icones/projeto_p.gif').link_projeto($media['tema_media_projeto']).'</td>';
				elseif ($media['tema_media_acao']) echo '<td align="left">'.imagem('icones/plano_acao_p.gif').link_acao($media['tema_media_acao']).'</td>';
				elseif ($media['tema_media_objetivo']) echo '<td align="left">'.imagem('icones/obj_estrategicos_p.gif').link_objetivo($media['tema_media_objetivo']).'</td>';
				elseif ($media['tema_media_estrategia']) echo '<td align="left">'.imagem('icones/estrategia_p.gif').link_estrategia($media['tema_media_estrategia']).'</td>';
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Progresso', 'O progresso pode ir de 0% (não iniciado) até 100% (completado).').'Progresso:'.dicaF().'</td><td class="realce">'.number_format((float)$obj->tema_percentagem, 2, ',', '.').'%</td></tr>';
	
	if($obj->tema_tipo_pontuacao){
		...
		$velocidade_fisico=($fisico_previsto > 0 ? $obj->tema_percentagem/$fisico_previsto : 0);
...
$sql->adOnde('tema_gestao_tema ='.(int)$tema_id);	
...
if (count($lista)) {
	echo '<tr><td align="right" style="white-space: nowrap" valign="middle">'.dica('Relacionad'.$config['genero_tema'], 'A que área '.$config['genero_tema'].' '.$config['tema'].' está relacionad'.$config['genero_tema'].'.').'Relacionad'.$config['genero_tema'].':'.dicaF().'</td></td><td class="realce">';	
...
if ($obj->tema_principal_indicador) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Indicador Principal', 'O indicador d'.$config['genero_tema'].' '.$config['tema'].' mais representativo da situação geral d'.$config['genero_tema'].' mesm'.$config['genero_tema'].'.').'Indicador principal:'.dicaF().'</td><td width="100%" class="realce">'.link_indicador($obj->tema_principal_indicador).'</td></tr>';

if ($obj->tema_oque) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('O Que Fazer', 'Sumário sobre o que se trata '.($config['genero_tema']=='a' ? 'esta ': 'este ').'tema'.'.').'O Que:'.dicaF().'</td><td class="realce" width="100%">'.$obj->tema_oque.'</td></tr>';
if ($obj->tema_porque) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Por Que Fazer', 'Por que '.'o'.' '.'tema'.' será executad'.'o'.'.').'Por que:'.dicaF().'</td><td class="realce" width="100%">'.$obj->tema_porque.'</td></tr>';
if ($obj->tema_onde) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Onde Fazer', 'Onde '.'o'.' '.'tema'.' é executad'.'o'.'.').'Onde:'.dicaF().'</td><td class="realce" width="100%">'.$obj->tema_onde.'</td></tr>';
if ($obj->tema_quando) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Quando Fazer', 'Quando '.'o'.' '.'tema'.' é executad'.'o'.'.').'Quando:'.dicaF().'</td><td class="realce" width="100%">'.$obj->tema_quando.'</td></tr>';
if ($obj->tema_como) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Como Fazer', 'Como '.'o'.' '.'tema'.' é executad'.'o'.'.').'Como:'.dicaF().'</td><td class="realce" width="100%">'.$obj->tema_como.'</td></tr>';
if ($obj->tema_quanto) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Quanto Custa', 'Custo para executar '.$config['genero_tema'].' '.$config['tema'].'.').'Quanto:'.dicaF().'</td><td class="realce" width="100%">'.$obj->tema_quanto.'</td></tr>';
if ($obj->tema_quem) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Quem', 'Quais '.$config['usuarios'].' estarão executando '.($config['genero_tema']=='a' ? 'esta ': 'este ').'tema'.'.').'Quem:'.dicaF().'</td><td class="realce" width="100%">'.$obj->tema_quem.'</td></tr>';


$sql->adTabela('objetivo_gestao');
$sql->adCampo('objetivo_gestao_objetivo');
$sql->adOnde('objetivo_gestao_tema ='.(int)$tema_id);
...
if ($saida_objetivo) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['objetivo']).' Relacionad'.$config['genero_objetivo'], 'Qual '.$config['objetivo'].' está relacionad'.$config['genero_objetivo'].' a '.($config['genero_tema']=='o' ? 'este' : 'esta').' '.$config['tema'].'.').ucfirst($config['objetivo']).' relacionad'.$config['genero_objetivo'].':'.dicaF().'</td><td width="100%" colspan="2" class="realce">'.$saida_objetivo.'</td></tr>';





if ($obj->tema_desde_quando) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Desde Quando é Feita', 'Desde quando '.'o'.' '.'tema'.' é executad'.'o'.'.').'Desde quando:'.dicaF().'</td><td class="realce" width="100%">'.$obj->tema_desde_quando.'</td></tr>';
if ($obj->tema_controle) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Método de Controle', 'Como '.'o'.' '.'tema'.' é controlad'.'o'.'.').'Controle:'.dicaF().'</td><td class="realce" width="100%">'.$obj->tema_controle.'</td></tr>';
if ($obj->tema_metodo_aprendizado) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Método de Aprendizado', 'Como é realizado o aprendizado d'.$config['genero_tema'].' '.$config['tema'].'.').'Aprendizado:'.dicaF().'</td><td class="realce" width="100%">'.$obj->tema_metodo_aprendizado.'</td></tr>';
if ($obj->tema_melhorias) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Melhorias Efetuadas n'.$config['genero_tema'].' '.ucfirst($config['tema']).'', 'Quais as melhorias realizadas n'.$config['genero_tema'].' '.$config['tema'].' após girar o círculo PDCA.').'Melhorias:'.dicaF().'</td><td class="realce" width="100%">'.$obj->tema_melhorias.'</td></tr>';
...
if ($Aplic->profissional && $exibir['moeda']) {
	...
	$sql->limpar();
	if (isset($moedas[$obj->tema_moeda])) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Moeda', 'A moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->tema_moeda].'</td></tr>';
	}

if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Aprovado', 'Se a meta se encontra aprovada.').'Aprovada:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->tema_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';
...
if (!$dialogo) {
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao_item')) {
		...
		$sql->adOnde('plano_acao_item_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'demanda')) {
		...
		$sql->adOnde('demanda_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco_resposta')) {		
		...
		$sql->adOnde('risco_resposta_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->checarModulo('recursos', 'acesso', null, null)) {
		...
		$sql->adOnde('recurso_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->modulo_ativo('patrocinadores') && $Aplic->checarModulo('patrocinadores', 'acesso', null, null)) {
		...
		$sql->adOnde('patrocinador_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'programa')) {
		...
		$sql->adOnde('programa_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'beneficio')) {
		...
		$sql->adOnde('beneficio_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->checarModulo('projeto', 'acesso', 'licao')) {
		...
		$sql->adOnde('licao_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'pratica')) {
		...
		$sql->adOnde('pratica_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('tr') && $Aplic->checarModulo('tr', 'acesso', null, null)) {
		...
		$sql->adOnde('tr_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'tgn')) {
		...
		$sql->adOnde('tgn_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'canvas')) {
		...
		$sql->adOnde('canvas_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('swot') && $Aplic->checarModulo('swot', 'acesso', null, null)) {
		...
		$sql->adOnde('mswot_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('swot') && $Aplic->checarModulo('swot', 'acesso', null, null)) {
		...
		$sql->adOnde('swot_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('operativo') && $Aplic->checarModulo('operativo', 'acesso', null, null)) {
		...
		$sql->adOnde('operativo_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'monitoramento')) {
		...
		$sql->adOnde('monitoramento_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'avaliacao_indicador')) {
		...
		$sql->adOnde('avaliacao_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'checklist')) {
		...
		$sql->adOnde('checklist_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional) {
		...
		$sql->adOnde('agenda_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('agrupamento') && $Aplic->checarModulo('agrupamento', 'acesso', null, null)) {
		...
		$sql->adOnde('agrupamento_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'modelo')) {
		...
		$sql->adOnde('template_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'painel_indicador')) {
		...
		$sql->adOnde('painel_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'odometro_indicador')) {
		...
		$sql->adOnde('painel_odometro_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'composicao_painel')) {
		...
		$sql->adOnde('painel_composicao_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'slideshow_painel')) {
		...
		$sql->adOnde('painel_slideshow_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('agenda', 'acesso', null, null)) {
		...
		$sql->adOnde('calendario_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'perspectiva')) {
		...
		$sql->adOnde('perspectiva_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'tema')) {
		$sql->adTabela('tema_gestao');
		$sql->adOnde('tema_gestao_semelhante = '.(int)$tema_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'me')) {
		...
		$sql->adOnde('me_gestao_tema = '.(int)$tema_id);
	...
	if ($config['exibe_fator'] && $Aplic->checarModulo('praticas', 'acesso', null, 'fator')) {
		...
		$sql->adOnde('fator_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'iniciativa')) {
		...
		$sql->adOnde('estrategia_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'meta')) {
		...
		$sql->adOnde('meta_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'planejamento')) {
		...
		$sql->adOnde('plano_gestao_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'abertura')) {
		...
		$sql->adOnde('projeto_abertura_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'viabilidade')) {
		...
		$sql->adOnde('projeto_viabilidade_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso')) {
		...
		$sql->adOnde('ssti_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso',null, 'laudo')) {
		...
		$sql->adOnde('laudo_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'acesso')) {
		...
		$sql->adOnde('trelo_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'acesso')) {
		...
		$sql->adOnde('trelo_cartao_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso')) {
		...
		$sql->adOnde('pdcl_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso')) {
		...
		$sql->adOnde('pdcl_item_gestao_tema = '.(int)$tema_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('os') && $Aplic->checarModulo('os', 'acesso')) {
		...
		$sql->adOnde('os_gestao_tema = '.(int)$tema_id);
