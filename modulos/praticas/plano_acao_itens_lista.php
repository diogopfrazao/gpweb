...
if(!$Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'plano_acao')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if (isset($_REQUEST['ata_filtro'])) $Aplic->setEstado('IdxFiltro', getParam($_REQUEST, 'ata_filtro', null));
$ata_filtro = $Aplic->getEstado('IdxFiltro' , $Aplic->usuario_prefs['filtroevento']);
...
$status=array(''=>'')+getSisValor('StatusAcaoPlano');
...
if (isset($_REQUEST['plano_acao_item_pesquisa'])) $Aplic->setEstado('plano_acao_item_pesquisa', getParam($_REQUEST, 'plano_acao_item_pesquisa', null));
$plano_acao_item_pesquisa = ($Aplic->getEstado('plano_acao_item_pesquisa') !== null ? $Aplic->getEstado('plano_acao_item_pesquisa') : '');

if (isset($_REQUEST['acao_status'])) $Aplic->setEstado('acao_status', getParam($_REQUEST, 'acao_status', null));
$acao_status = ($Aplic->getEstado('acao_status') !== null ? $Aplic->getEstado('acao_status') : '');
...
if (!$dialogo || $selecao){
	...
	$pesquisa='<tr><tr><td align="right" style="white-space: nowrap">'.dica('Pesquisa', 'Pesquisar pelo nome e campos de descrição').'Pesquisar:'.dicaF().'</td><td><input type="text" class="texto" style="width:250px;" name="plano_acao_item_pesquisa" onChange="document.env.submit();" value="'.$plano_acao_item_pesquisa.'" /></td><td><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.'&plano_acao_item_pesquisa=\');">'.imagem('icones/limpar_p.gif','Limpar Pesquisa', 'Clique neste ícone '.imagem('icones/limpar_p.gif').' para limpar a caixa texto de pesquisa.').'</a></td></tr>';
	
	$filtro_status='<tr><tr><td align="right" style="white-space: nowrap">'.dica('Status', 'Filtre pelo status das ações.').'Status:'.dicaF().'</td><td>'.selecionaVetor($status, 'acao_status', 'size="1" style="width:250px;" class="texto" onchange="document.env.submit();"', $acao_status).'</td></tr>';
	...
	$botoesTitulo = new CBlocoTitulo('Controle das Ações de '.ucfirst($config['acoes']), 'plano_acao.png', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionao', 'A que área o plano de ação está relacionado.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	$imprimir='<tr><td>'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para abrir uma nova janela onde poderá imprimir a lista ações d'.$config['genero_acao'].'s '.$config['acoes'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1,\'m=atas&a=ata_lista&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';

	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$pesquisa.$filtro_status.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.(!$selecao ? $imprimir : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
...
if (!$dialogo || $selecao){
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/plano_acao_itens_idx', 'Em execução',null,null,'Em execução','Visualizar as ações em execução d'.$config['genero_acao'].'s '.$config['acoes'].'.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/plano_acao_itens_idx', 'Completadas',null,null,'Completadas','Visualizar as ações completadas d'.$config['genero_acao'].'s '.$config['acoes'].'.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/plano_acao_itens_idx', 'Inativas',null,null,'Inativas','Visualizar todas as ações inativas d'.$config['genero_acao'].'s '.$config['acoes'].'.');
	$caixaTab->mostrar('','','','',true);
	echo estiloFundoCaixa('','', $tab);
	}
else {
	include_once(BASE_DIR.'/modulos/praticas/plano_acao_itens_idx.php');
