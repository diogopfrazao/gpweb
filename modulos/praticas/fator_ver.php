...
if (isset($_REQUEST['fator_id'])) $Aplic->setEstado('fator_id', getParam($_REQUEST, 'fator_id', null), $m, $a, $u);
$fator_id = $Aplic->getEstado('fator_id', null, $m, $a, $u);
...
if(!(permiteAcessarFator($obj->fator_acesso,$fator_id) && $Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'fator'))) $Aplic->redirecionar('m=publico&a=acesso_negado');

$podeEditar=$Aplic->checarModulo('praticas', 'editar', $Aplic->usuario_id, 'fator');
$podeAdicionar=$Aplic->checarModulo('praticas', 'adicionar', $Aplic->usuario_id, 'fator');
$podeExcluir=$Aplic->checarModulo('praticas', 'excluir', $Aplic->usuario_id, 'fator');


$editar=permiteEditarFator($obj->fator_acesso,$fator_id);

$podeEditarTudo=($obj->fator_acesso>=5 ? $editar && (in_array($obj->fator_usuario, $Aplic->usuario_lista_grupo_vetor) || $Aplic->usuario_super_admin || $Aplic->usuario_admin) : $editar);
...
if (!$dialogo){	
	...
	if ($podeAdicionar)	$km->Add("inserir","inserir_objeto",dica('Nov'.$config['genero_fator'].' '.ucfirst($config['fator']), 'Criar um nov'.$config['genero_fator'].' '.$config['fator'].'.').'Nov'.$config['genero_fator'].' '.ucfirst($config['fator']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=fator_editar\");");
	if ($podeEditar && $editar) {
		$km->Add("inserir","inserir_registro",dica('Registro de Ocorrência','Inserir um novo registro de ocorrência.').'Registro de ocorrência'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=log_editar&fator_id=".$fator_id."\");");
		if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('eventos', 'adicionar')) $km->Add("inserir","inserir_evento",dica('Novo Evento', 'Criar um novo evento relacionado.').'Evento'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=calendario&a=editar&evento_fator=".$fator_id."\");");
		...
		if ($Aplic->checarModulo('projetos', 'adicionar', null, 'demanda')) $km->Add("inserir","inserir_demanda",dica('Nova Demanda', 'Inserir uma nova demanda relacionada.').'Demanda'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=demanda_editar&demanda_fator=".$fator_id."\");");
		...
		if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'adicionar')) $km->Add("diverso","inserir_link",dica('Novo Link', 'Inserir um novo link relacionado.').'Link'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=links&a=editar&link_fator=".$fator_id."\");");
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("diverso","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_fator=".$fator_id."\");");
	...
	if ($podeEditarTudo && $podeEditar && !$bloquear) $km->Add("acao","acao_editar",dica('Editar '.ucfirst($config['fator']),'Editar os detalhes d'.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'Editar '.ucfirst($config['fator']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=fator_editar&fator_id=".$fator_id."\");");
	if ($podeEditarTudo && $podeExcluir && !$bloquear) $km->Add("acao","acao_excluir",dica('Excluir','Excluir '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'Excluir '.ucfirst($config['fator']).dicaF(), "javascript: void(0);' onclick='excluir()");
	...
	if ($Aplic->profissional) $km->Add("acao_imprimir","dashboard_geral",dica('Grau de Consecução','Dashboard do grau de consecução d'.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'Grau de Consecução'.dicaF(), "javascript: void(0);' onclick='url_passar(\"dashboard_geral_".$fator_id."\", \"m=praticas&a=fator_deshboard_geral_pro&jquery=1&dialogo=1&fator_id=".$fator_id."\");");
...
echo '<tr><td style="border: outset #d1d1cd 1px;background-color:#'.$obj->fator_cor.'" colspan="2"><font color="'.melhorCor($obj->fator_cor).'"><b>'.$obj->fator_nome.'<b></font>'.$cor_indicador.'</td></tr>';
...
if ($obj->fator_cia) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', ucfirst($config['genero_organizacao']).' '.$config['organizacao'].' responsável.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_cia($obj->fator_cia).'</td></tr>';
if ($Aplic->profissional){
...
if ($obj->fator_dept) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']).' Responsável', ucfirst($config['genero_dept']).' '.$config['departamento'].' responsável por '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_dept($obj->fator_dept).'</td></tr>';
...
if ($saida_depts) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica(ucfirst($config['departamentos']), 'Qual '.strtolower($config['departamento']).' está envolvid'.$config['genero_dept'].' com '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').ucfirst($config['departamentos']).' envolvid'.$config['genero_dept'].'s:'.dicaF().'</td><td width="100%" colspan="2" class="realce">'.$saida_depts.'</td></tr>';




if ($obj->fator_usuario) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Responsável pel'.$config['genero_fator'].' '.ucfirst($config['fator']).'', ucfirst($config['usuario']).' responsável por gerenciar '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'Responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($obj->fator_usuario, '','','esquerda').'</td></tr>';		
...
if ($saida_quem) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Designados', 'Quais '.strtolower($config['usuarios']).' estão envolvid'.$config['genero_usuario'].'s.').'Designados:'.dicaF().'</td><td class="realce">'.$saida_quem.'</td></tr>';

if ($obj->fator_descricao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Descrição', 'Descrição d'.$config['genero_fator'].' '.$config['fator'].'.').'Descrição:'.dicaF().'</td><td class="realce">'.$obj->fator_descricao.'</td></tr>';
...
if ($Aplic->profissional){
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Sistema de Percentagem', 'Qual forma a percentagem d'.$config['genero_fator'].' '.$config['fator'].' é calculada.').'Sistema de percentagem:'.dicaF().'</td><td class="realce">'.(isset($tipo_pontuacao[$obj->fator_tipo_pontuacao]) ? $tipo_pontuacao[$obj->fator_tipo_pontuacao] : '').'</td></tr>';
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Progresso', 'O progresso pode ir de 0% (não iniciado) até 100% (completado).').'Progresso:'.dicaF().'</td><td class="realce">'.number_format((float)$obj->fator_percentagem, 2, ',', '.').'%</td></tr>';
	
	if($obj->fator_tipo_pontuacao){
		...
		$velocidade_fisico=($fisico_previsto > 0 ? $obj->fator_percentagem/$fisico_previsto : 0);
...
else echo '<tr><td align="right" style="white-space: nowrap">'.dica('Progresso', 'O progresso pode ir de 0% (não iniciado) até 100% (completado).').'Progresso:'.dicaF().'</td><td class="realce">'.number_format((float)$obj->fator_percentagem, 2, ',', '.').'%</td></tr>';
	

$sql->adTabela('fator_gestao');
$sql->adCampo('fator_gestao.*');
$sql->adOnde('fator_gestao_fator ='.(int)$fator_id);	
...
if (count($lista)) {
	echo '<tr><td align="right" style="white-space: nowrap" valign="middle">'.dica('Relacionad'.$config['genero_fator'], 'A que área '.$config['genero_fator'].' '.$config['fator'].' está relacionad'.$config['genero_fator'].'.').'Relacionad'.$config['genero_fator'].':'.dicaF().'</td></td><td class="realce">';	
...
if ($obj->fator_principal_indicador) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Indicador Principal', 'O indicador d'.$config['genero_fator'].' '.$config['fator'].' mais representativo da situação geral d'.$config['genero_fator'].' mesm'.$config['genero_fator'].'.').'Indicador principal:'.dicaF().'</td><td width="100%" class="realce">'.link_indicador($obj->fator_principal_indicador).'</td></tr>';



if ($obj->fator_oque) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('O Que Fazer', 'Sumário sobre o que se trata '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'O Que:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_oque.'</td></tr>';
if ($obj->fator_porque) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Por Que Fazer', 'Por que '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' será executad'.$config['genero_fator'].'.').'Por que:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_porque.'</td></tr>';
if ($obj->fator_onde) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Onde Fazer', 'Onde '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' é executad'.$config['genero_fator'].'.').'Onde:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_onde.'</td></tr>';
if ($obj->fator_quando) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Quando Fazer', 'Quando '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' é executad'.$config['genero_fator'].'.').'Quando:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_quando.'</td></tr>';
if ($obj->fator_quem) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Quem', 'Quais '.$config['usuarios'].' estarão executando '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'Quem:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_quem.'</td></tr>';



if ($obj->fator_como) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Como Fazer', 'Como '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' é executad'.$config['genero_fator'].'.').'Como:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_como.'</td></tr>';
if ($obj->fator_quanto) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Quanto Custa', 'Custo para executar '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'Quanto:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_quanto.'</td></tr>';


if ($obj->fator_desde_quando) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Desde Quando é Feita', 'Desde quando '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' é executad'.$config['genero_fator'].'.').'Desde quando:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_desde_quando.'</td></tr>';
if ($obj->fator_controle) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Método de Controle', 'Como '.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' é controlad'.$config['genero_fator'].'.').'Controle:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_controle.'</td></tr>';
if ($obj->fator_metodo_aprendizado) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Método de Aprendizado', 'Como é realizado o aprendizado d'.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].'.').'Aprendizado:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_metodo_aprendizado.'</td></tr>';
if ($obj->fator_melhorias) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Melhorias Efetuadas n'.$config['genero_fator'].' '.ucfirst($config['fator']).'', 'Quais as melhorias realizadas n'.($config['genero_fator']=='a' ? 'esta' : 'este').' '.$config['fator'].' após girar o círculo PDCA.').'Melhorias:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->fator_melhorias.'</td></tr>';
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	if (isset($moedas[$obj->fator_moeda])) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Moeda', 'A moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->fator_moeda].'</td></tr>';	
	}
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Aprovad'.$config['genero_fator'], 'Se '.$config['genero_fator'].' '.$config['fator'].' se encontra aprovad'.$config['genero_fator'].'.').'Aprovad'.$config['genero_fator'].':'.dicaF().'</td><td  class="realce" width="100%">'.($obj->fator_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Ativ'.$config['genero_fator'], 'Se '.$config['genero_fator'].' '.$config['fator'].' se encontra ativ'.$config['genero_fator'].'.').'Ativ'.$config['genero_fator'].':'.dicaF().'</td><td  class="realce" width="100%">'.($obj->fator_ativo ? 'Sim' : 'Não').'</td></tr>';
...
if (!$dialogo) {
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao_item')) {
		...
		$sql->adOnde('plano_acao_item_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'demanda')) {
		...
		$sql->adOnde('demanda_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco_resposta')) {		
		...
		$sql->adOnde('risco_resposta_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('recursos', 'acesso', null, null)) {
		...
		$sql->adOnde('recurso_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->modulo_ativo('patrocinadores') && $Aplic->checarModulo('patrocinadores', 'acesso', null, null)) {
		...
		$sql->adOnde('patrocinador_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'programa')) {
		...
		$sql->adOnde('programa_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'beneficio')) {
		...
		$sql->adOnde('beneficio_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('projeto', 'acesso', 'licao')) {
		...
		$sql->adOnde('licao_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'pratica')) {
		...
		$sql->adOnde('pratica_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('tr') && $Aplic->checarModulo('tr', 'acesso', null, null)) {
		...
		$sql->adOnde('tr_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'tgn')) {
		...
		$sql->adOnde('tgn_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'canvas')) {
		...
		$sql->adOnde('canvas_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('swot') && $Aplic->checarModulo('swot', 'acesso', null, null)) {
		...
		$sql->adOnde('mswot_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('swot') && $Aplic->checarModulo('swot', 'acesso', null, null)) {
		...
		$sql->adOnde('swot_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('operativo') && $Aplic->checarModulo('operativo', 'acesso', null, null)) {
		...
		$sql->adOnde('operativo_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'monitoramento')) {
		...
		$sql->adOnde('monitoramento_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'avaliacao_indicador')) {
		...
		$sql->adOnde('avaliacao_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'checklist')) {
		...
		$sql->adOnde('checklist_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional) {
		...
		$sql->adOnde('agenda_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('agrupamento') && $Aplic->checarModulo('agrupamento', 'acesso', null, null)) {
		...
		$sql->adOnde('agrupamento_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'modelo')) {
		...
		$sql->adOnde('template_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'painel_indicador')) {
		...
		$sql->adOnde('painel_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'odometro_indicador')) {
		...
		$sql->adOnde('painel_odometro_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'composicao_painel')) {
		...
		$sql->adOnde('painel_composicao_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'slideshow_painel')) {
		...
		$sql->adOnde('painel_slideshow_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('agenda', 'acesso', null, null)) {
		...
		$sql->adOnde('calendario_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'perspectiva')) {
		...
		$sql->adOnde('perspectiva_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'tema')) {
		...
		$sql->adOnde('tema_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'objetivo')) {
		...
		$sql->adOnde('objetivo_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'me')) {
		...
		$sql->adOnde('me_gestao_fator = '.(int)$fator_id);
	...
	if ($config['exibe_fator'] && $Aplic->checarModulo('praticas', 'acesso', null, 'fator')) {
		$sql->adTabela('fator_gestao');
		$sql->adOnde('fator_gestao_semelhante = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'iniciativa')) {
		...
		$sql->adOnde('estrategia_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'meta')) {
		...
		$sql->adOnde('meta_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'planejamento')) {
		...
		$sql->adOnde('plano_gestao_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'abertura')) {
		...
		$sql->adOnde('projeto_abertura_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'viabilidade')) {
		...
		$sql->adOnde('projeto_viabilidade_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso')) {
		...
		$sql->adOnde('ssti_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso',null, 'laudo')) {
		...
		$sql->adOnde('laudo_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'acesso')) {
		...
		$sql->adOnde('trelo_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'acesso')) {
		...
		$sql->adOnde('trelo_cartao_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso')) {
		...
		$sql->adOnde('pdcl_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso')) {
		...
		$sql->adOnde('pdcl_item_gestao_fator = '.(int)$fator_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('os') && $Aplic->checarModulo('os', 'acesso')) {
		...
		$sql->adOnde('os_gestao_fator = '.(int)$fator_id);
