...
if(!$Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'iniciativa')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if (!$envolvimento && $ver_subordinadas) $sql->adOnde('pg_cia IN ('.$lista_cias.') OR plano_gestao_cia_cia IN ('.$lista_cias.')');
elseif (!$envolvimento) $sql->adOnde('pg_cia ='.(int)$cia_id.' OR plano_gestao_cia_cia='.(int)$cia_id);
elseif ($ver_subordinadas) $sql->adOnde('pg_cia IN ('.$lista_cias.')');
...
if (!$dialogo || $selecao){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionad'.$config['genero_iniciativa'], 'A que área '.$config['genero_iniciativa'].' '.$config['iniciativa'].' está relacionad'.$config['genero_iniciativa'].'.').'Relacionad'.$config['genero_iniciativa'].':'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	$nova_estrategia=($Aplic->checarModulo('praticas', 'adicionar', null, 'iniciativa') ? '<tr><td style="white-space: nowrap">'.dica('Nov'.$config['genero_iniciativa'].' '.ucfirst($config['iniciativa']), 'Criar nov'.$config['genero_iniciativa'].' '.$config['iniciativa'].'.').'<a href="javascript: void(0)" onclick="url_passar(0,\'m=praticas&a=estrategia_editar\');"><img src="'.acharImagem('estrategia_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr><tr><td style="white-space: nowrap"></td></tr>' : '');
	$imprimir='<tr><td align="right" style="white-space: nowrap"><a href="javascript: void(0);" onclick ="url_passar(1, \'m=praticas&a=estrategia_lista&dialogo=1\');">'.imagem('imprimir_p.png', 'Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['iniciativas'].'.').'</a></td></tr>';
	$botao_favorito=($Aplic->profissional ? '<tr><td style="white-space: nowrap">'.dica('Favoritos', 'Criar ou editar um grupo de '.$config['iniciativas'].' favorit'.$config['genero_iniciativa'].'s, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&estrategia=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$selecionar_planejamento.$procuraBuffer.$favoritos.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.(!$selecao ? $nova_estrategia.$imprimir.$botao_prioridade.$botao_favorito.$botao_trava.$botao_campos : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
...
if ($dialogo && !$selecao && !($Aplic->usuario_nomeguerra=='Visitante' && $Aplic->usuario_id=1)) echo '<script language=Javascript>self.print();</script>';
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=estrategias', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=praticas&a=estrategia_lista');
	}		
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&estrategia=1&filtro_prioridade='+env.filtro_prioridade_estrategia.value, window.setFiltroPriorizacao, window);
	}
