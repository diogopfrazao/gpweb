...
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=pg_perspectiva_nome&ordem='.($ordem ? '0' : '1').$extra.'\');">'.($ordenar=='pg_perspectiva_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome d'.$config['genero_perspectiva'].' '.ucfirst($config['perspectiva']).'', 'Neste campo fica um nome para identificação d'.$config['genero_perspectiva'].' '.$config['perspectiva'].'.').'Nome'.dicaF().'</a></th>';
...
if ($exibir['pg_perspectiva_metodo_aprendizado']) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=pg_perspectiva_metodo_aprendizado&ordem='.($ordem ? '0' : '1').$extra.'\');">'.($ordenar=='pg_perspectiva_metodo_aprendizado' ? imagem('icones/'.$seta[$ordem]) : '').dica('Aprendizado', 'Neste campo fica o método de aprendizado n'.$config['genero_perspectiva'].' '.$config['perspectiva'].'.').'Aprendizado'.dicaF().'</a></th>';
if ($exibir['pg_perspectiva_desde_quando']) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=pg_perspectiva_desde_quando&ordem='.($ordem ? '0' : '1').$extra.'\');">'.($ordenar=='pg_perspectiva_desde_quando' ? imagem('icones/'.$seta[$ordem]) : '').dica('Desde Quando', 'Neste campo fica desde quando é realizad'.$config['genero_perspectiva'].' '.$config['genero_perspectiva'].' '.$config['perspectiva'].'.').'Desde Quando'.dicaF().'</a></th>';
if ($exibir['pg_perspectiva_tipo_pontuacao']) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=pg_perspectiva_tipo_pontuacao&ordem='.($ordem ? '0' : '1').$extra.'\');">'.($ordenar=='pg_perspectiva_tipo_pontuacao' ? imagem('icones/'.$seta[$ordem]) : '').dica('Sistema de Pontuação', 'Neste campo fica o sistema de pontuação d'.$config['genero_perspectiva'].' '.$config['perspectiva'].'.').'Sistema'.dicaF().'</a></th>';

if ($exibir['pg_perspectiva_relacionado']) echo '<th style="white-space: nowrap">'.dica('Relacionad'.$config['genero_perspectiva'], 'A que área '.$config['genero_perspectiva'].' '.$config['perspectiva'].' está relacionad'.$config['genero_perspectiva'].'.').'Relacionad'.$config['genero_perspectiva'].dicaF().'</th>';



if ($exibir['pg_perspectiva_cia']) echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=pg_perspectiva_cia&ordem='.($ordem ? '0' : '1').$extra.'\');" class="hdr">'.dica(ucfirst($config['organizacao']), 'Clique para ordenar pel'.$config['genero_organizacao'].' '.$config['organizacao'].' responsável.').($ordenar=='pg_perspectiva_cia' ? imagem('icones/'.$seta[$ordem]) : '').ucfirst($config['organizacao']).dicaF().'</a></th>';
...
if ($exibir['pg_perspectiva_dept']) echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=pg_perspectiva_dept&ordem='.($ordem ? '0' : '1').$extra.'\');" class="hdr">'.dica(ucfirst($config['departamento']), strtoupper($config['genero_dept']).' '.strtolower($config['departamento']).' responsável.').($ordenar=='pg_perspectiva_dept' ? imagem('icones/'.$seta[$ordem]) : '').ucfirst($config['departamento']).dicaF().'</a></th>';
...
foreach ($perspectivas as $linha) {
	if (permiteAcessarPerspectiva($linha['pg_perspectiva_acesso'],$linha['pg_perspectiva_id'])){
		...
		echo '<tr>';
		if (!$impressao && !$dialogo) echo '<td style="white-space: nowrap" width="16">'.($podeEditar && $editar && !$bloquear ? dica('Editar '.ucfirst($config['perspectiva']).'', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar '.$config['genero_perspectiva'].' '.$config['perspectiva'].'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=perspectiva_editar&pg_perspectiva_id='.$linha['pg_perspectiva_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
		...
		if ($exibir['pg_perspectiva_aprovado'] && $Aplic->profissional) echo '<td style="width: 30px; text-align: center">'.($linha['pg_perspectiva_aprovado'] && $linha['tem_aprovacao'] ? 'Sim' : ($linha['tem_aprovacao'] ? 'Não' : 'N/A')).'</td>';
        ...
        if (!$from_lista) echo '<td style="width: 30px; text-align: center">'.($linha['pg_perspectiva_ativo'] ? 'Sim' : 'Não').'</td>';
		echo '</tr>';
		}
	}
