...
function mudar_nd($nd_id='', $campo='', $posicao='', $script='', $nd_classe=3, $nd_grupo='', $nd_subgrupo='', $nd_elemento_subelemento=''){
...
$xajax->registerFunction("mudar_nd");
...
function incluir_custo(
	...
	if ($custo_id){
	  ...
	  $sql->limpar();
		...
		$sql->adInserir('custo_codigo', $custo_codigo);
		$sql->adInserir('custo_fonte', $custo_fonte);
		$sql->adInserir('custo_regiao', $custo_regiao);
...
function atualizar_custos($log_id=null, $uuid=null, $gasto=0){
	global $config, $unidade, $Aplic, $moedas, $exibir;
	...
	foreach ($linhas as $linha) {
		...
		if (isset($exibir['codigo']) && $exibir['codigo']) $saida.='<td align="center">'.($linha['custo_codigo'] ? $linha['custo_codigo'] : '&nbsp;').'</td>';
		if (isset($exibir['fonte']) && $exibir['fonte']) $saida.='<td align="center">'.($linha['custo_fonte'] ? $linha['custo_fonte'] : '&nbsp;').'</td>';
		if (isset($exibir['regiao']) && $exibir['regiao']) $saida.='<td align="center">'.($linha['custo_regiao'] ? $linha['custo_regiao'] : '&nbsp;').'</td>'; 
		...
		$saida.= '<td width="32">';
		$saida.= dica('Editar Item', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o item '.$linha['custo_nome'].'.').'<a href="javascript:void(0);" onclick="javascript:editar_custo('.$linha['custo_id'].', '.($gasto ? 'true' : 'false').'	);">'.imagem('icones/editar.gif').'</a>'.dicaF();
		$saida.= dica('Excluir Item', 'Clique neste ícone '.imagem('icones/remover.png').' para excluir o item '.$linha['custo_nome'].'.').'<a href="javascript:void(0);" onclick="javascript:excluir_custo('.$linha['custo_id'].', '.($gasto ? 'true' : 'false').');">'.imagem('icones/remover.png').'</a>'.dicaF();
		$saida.= '</td>';	
...
function editar_custo($custo_id){
	...
	$objResposta->assign("custo_quantidade","value", number_format($linha['custo_quantidade'], 2, ',', '.'));	
	$objResposta->assign("custo_bdi","value", ($linha['custo_bdi'] ? number_format($linha['custo_bdi'], 2, ',', '.'):''));	
	$objResposta->assign("custo_custo","value", number_format($linha['custo_custo'], 2, ',', '.'));	
	...
	$objResposta->assign("data2_texto","value", retorna_data($linha['custo_data_moeda'], false));
...
function atualizar_arquivo($log_id=0){
    ...
    foreach ($arquivos as $arquivo) {
        ...
        $saida.= '</td>';
        $saida.= '<td><a href="javascript:void(0);" onclick="javascript:url_passar(0, \'m=perspectivas&a=log_download&sem_cabecalho=1&log_arquivo_id='.$arquivo['log_arquivo_id'].'\');">'.$arquivo['log_arquivo_nome'].'</a></td>';
