...
if(!$Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'objetivo')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if (isset($_REQUEST['tab'])) $Aplic->setEstado('objetivoTab', getParam($_REQUEST, 'tab', null));
$tab = ($Aplic->getEstado('objetivoTab') !== null ? $Aplic->getEstado('objetivoTab') : 0);
...
if (!$dialogo || $selecao){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionad'.$config['genero_objetivo'], 'A que área '.$config['genero_objetivo'].' '.$config['objetivo'].' está relacionad'.$config['genero_objetivo'].'.').'Relacionad'.$config['genero_objetivo'].':'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	$novo_objetivo=($Aplic->checarModulo('praticas', 'adicionar', null, 'objetivo') ? '<tr><td style="white-space: nowrap">'.dica('Nov'.$config['genero_objetivo'].' '.ucfirst($config['objetivo']), 'Criar '.($config['genero_objetivo']=='o' ? 'um' : 'uma').' nov'.$config['genero_objetivo'].' '.$config['objetivos'].'.').'<a href="javascript: void(0)" onclick="url_passar(0,\'m=praticas&a=obj_estrategico_editar\');"><img src="'.acharImagem('obj_estrategicos_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr><tr><td style="white-space: nowrap"></td></tr>' : '');

	$botao_favorito=($Aplic->profissional ? '<tr><td align="right" style="white-space: nowrap">'.dica('Favoritos', 'Criar ou editar um grupo de '.$config['objetivos'].' favorit'.$config['genero_objetivo'].'s, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&objetivo=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	...
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir '.ucfirst($config['objetivos']), 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['objetivos'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&u='.$u.'&tab='.$tab.'&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$selecionar_planejamento.$procuraBuffer.$favoritos.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.(!$selecao ? $novo_objetivo.$imprimir.$botao_prioridade.$botao_favorito.$botao_trava.$botao_campos : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=objetivos', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=praticas&a=obj_estrategico_lista');
	}
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&objetivo=1&filtro_prioridade='+env.filtro_prioridade_objetivo.value, window.setFiltroPriorizacao, window);
	}
