...
global $pratica_indicador_id, $pratica_indicador, $cal_sdf, $ano, $podeEditar; 
...
echo dica('Agrupar Valores', 'Escolha qual a melhor forma de agrupar valores.').'&nbsp;&nbsp;Agrupar por:'.dicaF().selecionaVetor($tipo_agrupamento, 'pratica_indicador_agrupar', 'class="texto"', $pratica_indicador_agrupar);
...
echo dica('Data da Final', 'Última data a ser pesquisada quando da busca dos valores.').'&nbsp;&nbsp;Data final:'.dicaF().'<input type="hidden" name="pratica_indicador_data"  id="pratica_indicador_data" value="'.($data ? $data->format('%Y%m%d') : '').'" /><input type="text" name="data" style="width:70px;" id="data" onchange="setData(\'mudar_grafico\', \'data\',\'pratica_indicador_data\');" value="'.($data ? $data->format('%d/%m/%Y') : '').'" class="texto" />'.dica('Data Final', 'Clique neste ícone '.imagem('icones/calendario.gif').' para abrir um calendário onde poderá selecionar a data do último valor aferido.').'<a href="javascript: void(0);" ><img id="f_btn1" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 /></a>'.dicaF();
...
if ($Aplic->profissional) {
	...
	if ($podeEditar) echo '&nbsp;&nbsp;&nbsp;<a href="javascript: void(0)" onclick="exportar_link1();">'.imagem('icones/indicador_exporta_p.png', 'Exportar Link' , 'Ao clicar neste ícone '.imagem('icones/indicador_exporta_p.png').' será exibido o endereço web para visualização do indicador em ambiente externo.').'</a>'.dicaF();
	}
...
echo dica('Data da Final', 'Última data a ser pesquisada quando da busca dos valores.').'&nbsp;&nbsp;Data final:'.dicaF().'<input type="hidden" name="pratica_indicador_data2"  id="pratica_indicador_data2" value="'.($data2 ? $data2->format('%Y%m%d') : '').'" /><input type="text" name="data2" style="width:70px;" id="data2" onchange="setData(\'mudar_grafico\', \'data2\',\'pratica_indicador_data2\');" value="'.($data2 ? $data2->format('%d/%m/%Y') : '').'" class="texto" />'.dica('Data Final', 'Clique neste ícone '.imagem('icones/calendario.gif').' para abrir um calendário onde poderá selecionar a data do último valor aferido.').'<a href="javascript: void(0);" ><img id="f_btn2" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 /></a>'.dicaF();
echo '</td></tr>';
...
function popIndicador() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Indicador', 1000, 700, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setIndicador&tabela=pratica_indicador&cia_id='+<?php echo $pratica_indicador['pratica_indicador_cia'] ?>, window.setIndicador, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setIndicador&tabela=pratica_indicador&cia_id='+<?php echo $pratica_indicador['pratica_indicador_cia'] ?>, 'Indicador','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
	}
