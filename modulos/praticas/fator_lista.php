...
if(!$Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'fator')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if (!$dialogo || $selecao){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionad'.$config['genero_fator'], 'A que área '.$config['genero_fator'].' '.$config['fator'].' está relacionad'.$config['genero_fator'].'.').'Relacionad'.$config['genero_fator'].':'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	$botao_favorito=($Aplic->profissional ? '<tr><td align="right" style="white-space: nowrap">'.dica('Favoritos', 'Criar ou editar um grupo de '.$config['fatores'].' favorit'.$config['genero_fator'].'s, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&fator=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	$novo_fator=($Aplic->checarModulo('praticas', 'adicionar', null, 'fator') ? '<tr><td style="white-space: nowrap" align=right>'.dica('Nov'.$config['genero_fator'].' '.ucfirst($config['fator']), 'Criar novo '.$config['fator'].'.').'<a href="javascript: void(0)" onclick="url_passar(0,\'m=praticas&a=fator_editar\');"><img src="'.acharImagem('fator_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr><tr><td style="white-space: nowrap"></td></tr>' : '');
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['fatores'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=praticas&a=fator_lista&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=fatores', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=praticas&a=fator_lista');
	}	
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&fator=1&filtro_prioridade='+env.filtro_prioridade_fator.value, window.setFiltroPriorizacao, window);
	}
