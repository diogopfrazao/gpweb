...
if(!$Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'gut')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if (isset($_REQUEST['filtro_prioridade_gut']))	$Aplic->setEstado('filtro_prioridade_gut', getParam($_REQUEST, 'filtro_prioridade_gut', null));
$filtro_prioridade_gut = $Aplic->getEstado('filtro_prioridade_gut') !== null ? $Aplic->getEstado('filtro_prioridade_gut') : null;


$houve_filtro=getParam($_REQUEST, 'houve_filtro', null);
$sql->adTabela('favorito_trava');
$sql->adCampo('favorito_trava_campo');
$sql->adOnde('favorito_trava_gut=1');
...
if (isset($_REQUEST['favorito_id'])) $Aplic->setEstado('gut_favorito', getParam($_REQUEST, 'favorito_id', null));
$favorito_id = $Aplic->getEstado('gut_favorito') !== null ? $Aplic->getEstado('gut_favorito') : 0;
...
if (isset($_REQUEST['guttextobusca'])) $Aplic->setEstado('guttextobusca', getParam($_REQUEST, 'guttextobusca', null));
$pesquisar_texto = ($Aplic->getEstado('guttextobusca') ? $Aplic->getEstado('guttextobusca') : '');
...
echo '<input type="hidden" name="filtro_prioridade_gut" id="filtro_prioridade_gut" value="'.$filtro_prioridade_gut.'" />';
...
if (!$dialogo || $selecao){
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionada', 'A que área a matriz GUT. está relacionada.').'Relacionada:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	$botoesTitulo = new CBlocoTitulo('Lista de Matrizes GUT', 'gut.gif', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$botao_favorito=($Aplic->profissional ? '<tr><td style="white-space: nowrap"><a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&gut=1\');">'.imagem('icones/favorito_p.png','Favoritos', 'Clique neste ícone '.imagem('icones/favorito_p.png').' para criar ou editar um grupo de matrizes GUT favoritas, para uma rápida filtragem.').'</a></td></tr>' : '');
	$novo_gut=($Aplic->checarModulo('praticas', 'adicionar', null, 'gut') ? '<tr><td style="white-space: nowrap"><a href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=praticas&a=gut_editar\');" >'.imagem('icones/gut_novo.png', 'Novo Brainstorm', 'Clique neste ícone '.imagem('icones/gut_novo.png').' para criar uma nova matriz GUT').'</a></td></tr>' : '');
	...
	if($filtro_prioridade_gut) $botao_prioridade=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="priorizacao(0);">'.imagem('icones/priorizacao_nao_p.png', 'Cancelar a Priorização' , 'Clique neste ícone '.imagem('icones/priorizacao_nao_p.png').' para cancelar a priorização.').'</a>'.dicaF().'</td></tr>' : '');
	...
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de matrizes GUT.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=praticas&a='.$a.'&dialogo=1&tab='.$tab.'\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procuraBuffer.$procurar_usuario.$favoritos.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.(!$selecao ? $novo_gut.$imprimir.$botao_prioridade.$botao_favorito.$botao_trava.$botao_campos : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif ($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de Matrizes GUT</center></font>';
	}
if($Aplic->profissional) $Aplic->carregarComboMultiSelecaoJS();

if (!$dialogo|| $selecao){
	$caixaTab = new CTabBox('m='.$m.'&a='.$a.'&u='.$u, '', $tab);
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/gut_ver_idx','Ativos',null,null,'Ativos','Visualizar todos os ativos.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/gut_ver_idx','Inativos',null,null,'Inativos','Visualizar todos os inativos.');
	$caixaTab->adicionar(BASE_DIR.'/modulos/praticas/gut_ver_idx','Todos',null,null,'Inativos','Visualizar todos.');
	$caixaTab->mostrar('','','','',true);
	echo estiloFundoCaixa('','', $tab);
	}
else {
	include_once(BASE_DIR.'/modulos/praticas/gut_ver_idx.php');
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=guts', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=praticas&a=gut_lista');
	}	
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&gut=1&filtro_prioridade='+env.filtro_prioridade_gut.value, window.setFiltroPriorizacao, window);
	}

function setFiltroPriorizacao(filtro_prioridade_gut){
	env.filtro_prioridade_gut.value=filtro_prioridade_gut;
	env.submit();
	}		
