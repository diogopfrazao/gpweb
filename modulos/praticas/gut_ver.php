...
if (isset($_REQUEST['gut_id'])) $Aplic->setEstado('gut_id', getParam($_REQUEST, 'gut_id', null), $m, $a, $u);
$gut_id = $Aplic->getEstado('gut_id', null, $m, $a, $u);
...
if ($Aplic->profissional){
	...
	$sql->adOnde('assinatura_gut='.(int)$gut_id);
	$assinar = $sql->linha();
	$sql->limpar();
...
$sql->adUnir('usuarios','usuarios','usuarios.usuario_id=gut_usuarios.usuario_id');
...
if(!(permiteAcessarGut($obj->gut_acesso,$gut_id) && $Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'gut'))) $Aplic->redirecionar('m=publico&a=acesso_negado');

$podeEditar=$Aplic->checarModulo('praticas', 'editar', $Aplic->usuario_id, 'gut');
$podeAdicionar=$Aplic->checarModulo('praticas', 'adicionar', $Aplic->usuario_id, 'gut');
$podeExcluir=$Aplic->checarModulo('praticas', 'excluir', $Aplic->usuario_id, 'gut');


$editar=permiteEditarGut($obj->gut_acesso,$gut_id);

$podeEditarTudo=($obj->gut_acesso>=5 ? $editar && (in_array($obj->gut_responsavel, $Aplic->usuario_lista_grupo_vetor) || $Aplic->usuario_super_admin || $Aplic->usuario_admin) : $editar);

if (!$gut_id) {
	$Aplic->setMsg('Não foi passado um ID correto ao tentar ver a matriz GUT.', UI_MSG_ERRO);
...
if (!$dialogo){
	$Aplic->salvarPosicao();
	$botoesTitulo = new CBlocoTitulo('Detalhes da Matriz GUT', 'gut.gif', $m, $m.'.'.$a);
	...
	$km->Add("ver","ver_lista",dica('Lista de Matriz GUTs','Clique neste botão para visualizar a lista de matriz GUTs.').'Lista de Matriz GUTs'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=gut_lista\");");
	
	
	if (($podeEditar && $editar) || $podeAdicionar)	$km->Add("root","inserir",dica('Inserir','Menu de opções').'Inserir'.dicaF(), "javascript: void(0);'");
	if ($podeAdicionar)	$km->Add("inserir","inserir_objeto",dica('Nova Matriz GUT', 'Criar um nova matriz GUT.').'Nova Matriz GUT'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=gut_editar\");");
	if ($podeEditar && $editar) {
		$km->Add("inserir","inserir_registro",dica('Registro de Ocorrência','Inserir um novo registro de ocorrência.').'Registro de ocorrência'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=log_editar&gut_id=".$gut_id."\");");
		if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('eventos', 'adicionar')) $km->Add("inserir","inserir_evento",dica('Novo Evento', 'Criar um novo evento relacionado.').'Evento'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=calendario&a=editar&evento_gut=".$gut_id."\");");
		if ($Aplic->modulo_ativo('arquivos') && $Aplic->checarModulo('arquivos', 'adicionar')) $km->Add("inserir","inserir_arquivo",dica('Novo Arquivo', 'Inserir um novo arquivo relacionado.').'Arquivo'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=arquivos&a=editar&arquivo_gut=".$gut_id."\");");
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'adicionar', null, 'indicador')) $km->Add("inserir","inserir_indicador",dica('Novo Indicador', 'Inserir um novo indicador relacionado.').'Indicador'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=indicador_editar&pratica_indicador_gut=".$gut_id."\");");
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'adicionar', null, 'plano_acao')) $km->Add("inserir","inserir_acao",dica('Nov'.$config['genero_acao'].' '.ucfirst($config['acao']), 'Criar nov'.$config['genero_acao'].' '.$config['acao'].' relacionad'.$config['genero_acao'].'.').ucfirst($config['acao']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=plano_acao_editar&plano_acao_gut=".$gut_id."\");");
		if ($Aplic->modulo_ativo('projetos') && $Aplic->checarModulo('projetos', 'adicionar')) $km->Add("inserir","inserir_projeto", dica('Nov'.$config['genero_projeto'].' '.ucfirst($config['projeto']), 'Inserir nov'.$config['genero_projeto'].' '.$config['projeto'].' relacionad'.$config['genero_projeto'].'.').ucfirst($config['projeto']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=editar&projeto_gut=".$gut_id."\");");	
		if ($Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'adicionar')) $km->Add("inserir","inserir_ata",dica('Nova Ata de Reunião', 'Inserir uma nova ata de reunião relacionada.').'Ata de reunião'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=atas&a=ata_editar&ata_gut=".$gut_id."\");");
		...
		if ($Aplic->checarModulo('projetos', 'adicionar', null, 'demanda')) $km->Add("inserir","inserir_demanda",dica('Nova Demanda', 'Inserir uma nova demanda relacionada.').'Demanda'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=demanda_editar&demanda_gut=".$gut_id."\");");
...
if($dialogo && $Aplic->profissional) {
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Matriz GUT</center></font>';
	}

echo '<table id="tblObjetivos" cellpadding=1 cellspacing=1 width="100%"'.($dialogo ? '' : ' class="std"').'>';


if ($obj->gut_nome) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nome', 'Nome desta da matriz GUT.').'Nome:'.dicaF().'</td><td style="border: outset #d1d1cd 1px;background-color:#'.$obj->gut_cor.'"><font color="'.melhorCor($obj->gut_cor).'"><b>'.$obj->gut_nome.'<b></font></td></tr>';
if ($obj->gut_cia) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', ucfirst($config['genero_organizacao']).' '.$config['organizacao'].' responsável.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_cia($obj->gut_cia).'</td></tr>';
if ($Aplic->profissional){
...
if ($obj->gut_dept) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']).' Responsável', ucfirst($config['genero_dept']).' '.$config['departamento'].' responsável por esta matriz GUT.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_dept($obj->gut_dept).'</td></tr>';
$saida_depts='';
...
if ($saida_depts) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica(ucfirst($config['departamentos']), 'Qual '.strtolower($config['departamento']).' está envolvid'.$config['genero_dept'].' com esta matriz GUT.').ucfirst($config['departamento']).' envolvid'.$config['genero_dept'].':'.dicaF().'</td><td width="100%" colspan="2" class="realce">'.$saida_depts.'</td></tr>';


if ($obj->gut_responsavel) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Responsável', 'Responsável por resolver a matriz GUT.').'Responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($obj->gut_responsavel, '','','esquerda').'</td></tr>';		
...
$sql->adOnde('gut_gestao_gut ='.(int)$gut_id);	
...
if (count($lista)) {
	echo '<tr><td align="right" style="white-space: nowrap" valign="middle">'.dica('Relacionada', 'A que área a matriz GUT. está relacionada.').'Relacionada:'.dicaF().'</td></td><td class="realce">';	
...
if ($obj->gut_data) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data', 'A data de realização da matriz GUT.').'Data:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.retorna_data($obj->gut_data, false).'</td></tr>';
if ($obj->gut_descricao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Descrição', 'A descrição detalhada da matriz GUT.').'Descrição:'.dicaF().'</td><td class="realce" width="100%" >'.$obj->gut_descricao.'</td></tr>';


if ($obj->gut_principal_indicador) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Indicador Principal', 'Dentre os indicadores relacionados, o mais representativo da situação geral.').'Indicador principal:'.dicaF().'</td><td width="100%" class="realce">'.link_indicador($obj->gut_principal_indicador).'</td></tr>';
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	if (isset($moedas[$obj->gut_moeda])) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Moeda', 'A moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->gut_moeda].'</td></tr>';	
	}
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Aprovado', 'Se a matriz GUT se encontra aprovada.').'Aprovada:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->gut_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';


$acesso = getSisValor('NivelAcesso','','','sisvalor_id');
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nível de Acesso', 'Pode ter cinco níveis de acesso:<ul><li><b>Público</b> - Todos podem ver e editar.</li><li><b>Protegido I</b> - Todos podem ver, porem apenas o responsável e os designados podem editar.</li><li><b>Protegido II</b> - Todos podem ver, porem apenas o responsável pode editar tudo e os designados editarem os objetos relacionados.</li><li><b>Protegido III</b> - Todos podem ver, porem apenas o responsável pode editar.</li><li><b>Participante I</b> - Somente o responsável e os designados podem ver e editar</li><li><b>Participantes II</b> - Somente o responsável e os designados podem ver e apenas o responsável pode editar tudo e os designados editarem os objetos relacionados.</li><li><b>Participantes III</b> - Somente o responsável e os designados podem ver, e o responsável editar.</li></ul>').'Nível de acesso:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.(isset($acesso[$obj->gut_acesso]) ? $acesso[$obj->gut_acesso] : '').'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Ativa', 'Se a matriz GUT se encontra ativa.').'Ativa:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->gut_ativo ? 'Sim' : 'Não').'</td></tr>';




echo '<tr><td colspan=20><fieldset><legend class=texto style="color: black;">'.dica('Matriz','A matriz GUT já priorizada.').'&nbsp;<b>Matriz</b>&nbsp'.dicaF().'</legend><table cellspacing=0 cellpadding=0>';
...
if ($gut_id){ 
	...
	$sql->adCampo('gut_linha.*, (gut_g*gut_u*gut_t) AS prioridade');
...
echo '<tr><td width=110>&nbsp;</td><td><div id="guts">';
if (count($gut)) {
	echo '<table cellpadding=0 cellspacing=0 class="tbl1" align=left><tr><th>'.dica('Texto', 'Texto da matriz GUT').'Texto'.dicaF().'</th><th>'.dica('Gravidade','A componente de gravidade da matriz GUT.').'Gravidade'.dicaF().'</th><th>'.dica('Urgência','A componente de urgência da matriz GUT.').'Urgência'.dicaF().'</th><th>'.dica('Tendência','A componente de tendência da matriz GUT.').'Tendência'.dicaF().'</th><th>'.dica('Prioridade','A prioridade é calculada multiplicando a gravidade, urgência e tendência.').'Prioridade'.dicaF().'</th></tr>';
	foreach ($gut as $gut_linha_id => $data) {
		...
		echo '<td align="center">'.$data['prioridade'].'</td>';
		echo '</tr>';
		}
	echo '</table>';
	}
echo '</div></td></tr>';	 
	...
	echo '<tr><td width=110>&nbsp;</td></tr>';

	echo '<tr><td width=110>&nbsp;</td><td align=center><table cellspacing=0 cellpadding=0><tr><td><table cellspacing=0 cellpadding=0>';
	...
	echo '<tr><td style="background-color:#fcd5b4">5 Vai piorar rapidamente</td></tr>';
...
$campos_customizados = new CampoCustomizados('gut', $obj->gut_id, 'ver');
...
if ($Aplic->profissional) include_once BASE_DIR.'/modulos/praticas/gut_ver_pro.php';		
...
if (!$dialogo && $Aplic->profissional) {
	$caixaTab = new CTabBox('m=praticas&a=gut_ver&gut_id='.$gut_id, '', $tab);
	$qnt_aba=0;
	
	if ($Aplic->profissional && $Aplic->checarModulo('log', 'acesso')) {
		$sql->adTabela('log');
		$sql->adCampo('count(log_id)');
		$sql->adOnde('log_gut = '.(int)$gut_id);
	...
	if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('eventos', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('evento_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->modulo_ativo('arquivos') && $Aplic->checarModulo('arquivos', 'acesso')) {
			...
			$sql->adOnde('arquivo_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('plano_acao_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao_item')) {
		...
		$sql->adOnde('plano_acao_item_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->modulo_ativo('projetos') && $Aplic->checarModulo('projetos', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('projeto_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'acesso')) {
		...
		$sql->adOnde('ata_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'demanda')) {
		...
		$sql->adOnde('demanda_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->modulo_ativo('email') && $Aplic->checarModulo('email', 'acesso')) {
		...
		$sql->adOnde('msg_gestao_gut = '.(int)$gut_id);
		...
		if ($config['doc_interno']) {
			...
			$sql->adOnde('modelo_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('link_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('forum_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('problema') && $Aplic->checarModulo('problema', 'acesso')) {
		...
		$sql->adOnde('problema_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco')) {
		...
		$sql->adOnde('risco_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco_resposta')) {		
		...
		$sql->adOnde('risco_resposta_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->modulo_ativo('instrumento')  && $Aplic->checarModulo('instrumento', 'acesso')) {
		...
		$sql->adOnde('instrumento_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('recursos', 'acesso', null, null)) {
		...
		$sql->adOnde('recurso_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->modulo_ativo('patrocinadores') && $Aplic->checarModulo('patrocinadores', 'acesso', null, null)) {
		...
		$sql->adOnde('patrocinador_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'programa')) {
		...
		$sql->adOnde('programa_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'beneficio')) {
		...
		$sql->adOnde('beneficio_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('projeto', 'acesso', 'licao')) {
		...
		$sql->adOnde('licao_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'pratica')) {
		...
		$sql->adOnde('pratica_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('tr') && $Aplic->checarModulo('tr', 'acesso', null, null)) {
		...
		$sql->adOnde('tr_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'brainstorm')) {
		...
		$sql->adOnde('brainstorm_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'gut')) {
		$sql->adTabela('gut_gestao');
		$sql->adOnde('gut_gestao_semelhante = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'causa_efeito')) {
		...
		$sql->adOnde('causa_efeito_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'tgn')) {
		...
		$sql->adOnde('tgn_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'canvas')) {
		...
		$sql->adOnde('canvas_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('swot') && $Aplic->checarModulo('swot', 'acesso', null, null)) {
		...
		$sql->adOnde('mswot_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('swot') && $Aplic->checarModulo('swot', 'acesso', null, null)) {
		...
		$sql->adOnde('swot_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('operativo') && $Aplic->checarModulo('operativo', 'acesso', null, null)) {
		...
		$sql->adOnde('operativo_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'monitoramento')) {
		...
		$sql->adOnde('monitoramento_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'avaliacao_indicador')) {
		...
		$sql->adOnde('avaliacao_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'checklist')) {
		...
		$sql->adOnde('checklist_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional) {
		...
		$sql->adOnde('agenda_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('agrupamento') && $Aplic->checarModulo('agrupamento', 'acesso', null, null)) {
		...
		$sql->adOnde('agrupamento_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'modelo')) {
		...
		$sql->adOnde('template_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'painel_indicador')) {
		...
		$sql->adOnde('painel_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'odometro_indicador')) {
		...
		$sql->adOnde('painel_odometro_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'composicao_painel')) {
		...
		$sql->adOnde('painel_composicao_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'slideshow_painel')) {
		...
		$sql->adOnde('painel_slideshow_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('agenda', 'acesso', null, null)) {
		...
		$sql->adOnde('calendario_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'perspectiva')) {
		...
		$sql->adOnde('perspectiva_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'tema')) {
		...
		$sql->adOnde('tema_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'objetivo')) {
		...
		$sql->adOnde('objetivo_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'me')) {
		...
		$sql->adOnde('me_gestao_gut = '.(int)$gut_id);
	...
	if ($config['exibe_fator'] && $Aplic->checarModulo('praticas', 'acesso', null, 'fator')) {
		...
		$sql->adOnde('fator_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'iniciativa')) {
		...
		$sql->adOnde('estrategia_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'meta')) {
		...
		$sql->adOnde('meta_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'planejamento')) {
		...
		$sql->adOnde('plano_gestao_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'abertura')) {
		...
		$sql->adOnde('projeto_abertura_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'viabilidade')) {
		...
		$sql->adOnde('projeto_viabilidade_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso')) {
		...
		$sql->adOnde('ssti_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso',null, 'laudo')) {
		...
		$sql->adOnde('laudo_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'acesso')) {
		...
		$sql->adOnde('trelo_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'acesso')) {
		...
		$sql->adOnde('trelo_cartao_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso')) {
		...
		$sql->adOnde('pdcl_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso')) {
		...
		$sql->adOnde('pdcl_item_gestao_gut = '.(int)$gut_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('os') && $Aplic->checarModulo('os', 'acesso')) {
		...
		$sql->adOnde('os_gestao_gut = '.(int)$gut_id);
...
function excluir() {
	if (confirm('Tem certeza que deseja excluir?')) {
		var f = document.env;
		f.excluir.value=1;
		f.fazerSQL.value='gut_fazer_sql';
		f.a.value='vazio';
		f.dialogo.value=1;
		f.submit();
		}
