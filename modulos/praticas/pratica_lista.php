...
if(!$Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'pratica')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
elseif ($projeto_id){
	$sql->esqUnir('tarefas','tarefas2', 'tarefas2.tarefa_id=pratica_gestao_tarefa');
	$sql->adOnde('pratica_gestao_projeto IN ('.$projeto_id.') OR tarefas2.tarefa_projeto IN ('.$projeto_id.')');
	}
elseif ($projeto_id) $sql->adOnde('pratica_gestao_projeto IN ('.$projeto_id.')');
...
if (!$dialogo || $selecao){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionad'.$config['genero_pratica'], 'A que área '.$config['genero_pratica'].' '.$config['pratica'].' está relacionad'.$config['genero_pratica'].'.').'Relacionad'.$config['genero_pratica'].':'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	$nova_pratica=($Aplic->checarModulo('praticas', 'adicionar', null, 'pratica') ? '<tr><td align="right">'.dica('Nov'.$config['genero_pratica'].' '.ucfirst($config['pratica']), 'Criar '.($config['genero_pratica']=='a' ? 'uma nova ': 'um novo ').$config['pratica'].'.').'<a href="javascript: void(0)" onclick="url_passar(0,\'m=praticas&a=pratica_editar\');"><img src="'.acharImagem('pratica_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	$botao_favorito=($Aplic->profissional ? '<tr><td align="right" style="white-space: nowrap">'.dica('Favorit'.$config['genero_pratica'].'s', 'Criar ou editar um grupo de '.$config['praticas'].' favorit'.$config['genero_pratica'].'s, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&pratica=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprir '.ucfirst($config['praticas']), 'Imprimir a lista de '.$config['praticas'].'.').'<a href="javascript: void(0);" onclick ="imprimir_praticas('.$tab.')">'.imagem('imprimir_p.png').'</a>'. dicaF().'</td></tr>';
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$filtro_pontuacao.$procurar_usuario.$procuraBuffer.$procurar_ano.$favoritos.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.(!$selecao ? $nova_pratica.$imprimir.$botao_prioridade.$botao_favorito.$botao_trava.$botao_campos : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=praticas', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=praticas&a=pratica_lista');
	}	
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&pratica=1&filtro_prioridade='+env.filtro_prioridade_pratica.value, window.setFiltroPriorizacao, window);
	}
