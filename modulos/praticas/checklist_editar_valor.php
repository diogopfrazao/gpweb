...
$sql->adCampo('pratica_indicador_acumulacao, pratica_indicador_agrupar, pratica_indicador.pratica_indicador_acesso, pratica_indicador.pratica_indicador_nome, pratica_indicador_checklist_valor, pratica_indicador_id');
...
$podeEditar=$Aplic->checarModulo('praticas', ($pratica_indicador_id ? 'editar' : 'adicionar'), $Aplic->usuario_id, 'indicador');
$podeValor=$Aplic->checarModulo('praticas', ($pratica_indicador_id ? 'editar' : 'adicionar'), $Aplic->usuario_id, 'indicador_valor');
...
if (!(($podeEditar || $podeValor) && $podeEditarTudo)) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
$ttl = $checklist_dados_id ? 'Editar Checklist' : 'Preencher Checklist';
...
echo '<input type="hidden" name="inicio" id="inicio" value="'.$faixa['inicio'].'" />';
...
if ($checklist_lista && count($checklist_lista)){
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Responsável pela Inserção', 'Todo indicador ao ter valor inserido deve ter um responsável.').'Responsável:'.dicaF().'</td><td colspan="2"><input type="hidden" id="checklist_dados_responsavel" name="checklist_dados_responsavel" value="'.(isset($pratica_indicado_valor['checklist_dados_responsavel']) && $pratica_indicado_valor['checklist_dados_responsavel'] ? $pratica_indicado_valor['checklist_dados_responsavel'] : $Aplic->usuario_id).'" /><input type="text" id="nome_gerente" name="nome_gerente" value="'.nome_om((isset($pratica_indicado_valor['checklist_dados_responsavel']) && $pratica_indicado_valor['checklist_dados_responsavel'] ? $pratica_indicado_valor['checklist_dados_responsavel'] : $Aplic->usuario_id),$Aplic->getPref('om_usuario')).'" style="width:284px;" class="texto" READONLY /><a href="javascript: void(0);" onclick="popGerente();">'.imagem('icones/usuarios.gif','Selecionar '.ucfirst($config['usuario']),'Clique neste ícone '.imagem('icones/usuarios.gif').' para selecionar '.($config['genero_usuario']=='o' ? 'um' : 'uma').' '.$config['usuario'].'.').'</a></td></tr>';
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data da Aferição', 'Quando foi aferido o valor.').'Data da Aferição:'.dicaF().'</td><td width="100%" colspan="2"><input type="hidden" name="pratica_indicador_valor_data" id="pratica_indicador_valor_data" value="'.($data ? $data->format('%Y%m%d') : '').'" /><input type="text" name="data" style="width:70px;" id="data" onchange="setData(\'env\', \'data\', \'pratica_indicador_valor_data\');" value="'.($data ? $data->format('%d/%m/%Y') : '').'" class="texto" />'.dica('Data Inicial', 'Clique neste ícone '.imagem('icones/calendario.gif').' para abrir um calendário onde poderá selecionar a data em que foi aferido o valor.').'<a href="javascript: void(0);" ><img id="f_btn1" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 /></a>'.dicaF().'</td></tr>';
	...
	$cabecalho.='<th style="width:400px;">'.dica('Evidência/Justificativa','Neste campo poderá constar informações pertinentes que justifiquem a opção marcada.').'Evidência/Justificativa'.dicaF().'</th></tr>';
	...
	foreach($checklist_lista as $linha) {
		...
		if (isset($linha['checklist_lista_legenda']) && $linha['checklist_lista_legenda']) {
			...
			foreach($campos as $campo) {
				echo '<td align=center><input type="checkbox" value="1" name="'.$campo['checklist_campo_campo'].'_'.$linha['checklist_lista_id'].'" '.( isset($linha['checklist_lista_'.$campo['checklist_campo_campo']]) && $linha['checklist_lista_'.$campo['checklist_campo_campo']] ? 'checked="checked"' : '').' onchange="if (env.'.$campo['checklist_campo_campo'].'_'.$linha['checklist_lista_id'].'.checked) {document.getElementById(\'checklist_'.$campo['checklist_campo_campo'].'_'.$linha['checklist_lista_id'].'\').value=1;';
			...
			echo '<td><textarea name="justificativa[]" class="texto" style="width:400px;" rows="1" class="textarea">'.$linha['checklist_lista_justificativa'].'</textarea><input type="hidden" name="legenda[]" value="0"></td></tr>';
			}
...
if (count($valores)){
	...
	foreach($valores as $valor){
		...
		echo '<td width="60" style="white-space: nowrap" align=right><a href="javascript:void(0);" onclick="popChecklist('.$pratica_indicador_id.','.$valor['checklist_dados_id'].');">'.number_format($valor['pratica_indicador_valor_valor'], $config['casas_decimais'], ',', '.').'</a></td>';
