...
if (!$dialogo && $Aplic->profissional){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$botao_link=($podeEditar ? '<tr><td><a href="javascript: void(0)" onclick="javascript:exportar_link();">'.imagem('icones/arvore_exporta.png', 'Exportar Link', 'Clique neste ícone '.imagem('icones/arvore_exporta.png').' para criar um endereço web para visualização do mapa estratégico em ambiente externo.').'</a></td></tr>' : '');
...
elseif (!$dialogo && !$Aplic->profissional){
	...
	$botoesTitulo->adicionaCelula('<td align="right" style="white-space: nowrap">'.dica('Imprimir '.$config['genero_plano_gestao'].' '.ucfirst($config['plano_gestao']), 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir '.$config['genero_plano_gestao'].' '.$config['plano_gestao'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=praticas&a=mapa_estrategico&dialogo=1&percentagem='.$percentagem.'&cia_id='.$cia_id.'&pg_id='.$pg_id.'&por_dept='.$por_dept.'&por_designados='.$por_designados.'&por_responsavel='.$por_responsavel.'&cia_id='.$cia_id.'\');">'.imagem('imprimir_p.png').'</a>'.dicaF());
	$botoesTitulo->mostrar();
	}
elseif($dialogo && $Aplic->profissional) {
