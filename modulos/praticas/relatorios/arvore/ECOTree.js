...
ECONode = function (id, pid, dsc, w, h, c, bc, target, meta, extra) {
	this.id = id;
	this.pid = pid;
	this.dsc = dsc;
	this.w = w;
...
ECOTree = function (obj, elm) {
	this.config = {
		...
		expandedImage : './estilo/rondon/imagens/icones/less.gif',
		collapsedImage : './estilo/rondon/imagens/icones/plus.gif',
		transImage : './estilo/rondon/imagens/icones/trans.gif'
	}
		...
		if (!node._isAncestorCollapsed())
		{
			switch (this.render)
			{
				case "CANVAS":
					...
					{
						s.push('<a id="t' + node.id + '" href="'+node.target+'" '+node.extra+'>');
						s.push(node.dsc);
						s.push('</a>');
					}				
					else
...
ECOTree.prototype.add = function (id, pid, dsc, w, h, c, bc, target, meta, extra) {	
	...
	var node = new ECONode(id, pid, dsc, nw, nh, color, border, tg, metadata, extra);	//New node creation...
