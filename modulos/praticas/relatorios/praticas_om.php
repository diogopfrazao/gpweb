...
if (!$dialogo) echo '<td>'.dica('Seleção de Critério', 'Utilize esta opção para filtrar pelo critério selecionado.').'&nbsp;Critério:'.dicaF().'</td><td align="left" style="white-space: nowrap">'.selecionaVetor($lista_criterio, 'filtro_criterio', 'onchange="document.env.submit()" class="texto"', $filtro_criterio).'</td></form>';
...
if ($filtro_criterio) {
	...
	elseif(!($Aplic->usuario_nomeguerra=='Visitante' && $Aplic->usuario_id=1)) echo '<script>self.print();</script>';	
	}
