...
foreach($marcadores as $dado){
	if ($dado['pratica_criterio_id']!=$criterio_atual){
		...
		$dentro .= '</table>';
		echo '<tr><td align="left" style="white-space: nowrap;font-size:200%; font-weight:bold">'.dica('Dados Sobre o Critério', $dentro).$criterios[$dado['pratica_criterio_id']]['pratica_criterio_numero'].'.'.$criterios[$dado['pratica_criterio_id']]['pratica_criterio_nome'].dicaF().'</td><td colspan=20>&nbsp;</td></tr>';
		}
...
function somatorio($vetor_indicadores){
	...
	foreach ($vetor_indicadores as $pratica_indicador_id){
		...
		if (isset($indicadores[$pratica_indicador_id]['pode_ver']) && $indicadores[$pratica_indicador_id]['pode_ver']) echo '<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'.(!$dialogo ? '<a href="javascript: void(0);" onclick="ver_indicador('.$pratica_indicador_id.')">' : '').$indicadores[$pratica_indicador_id]['pratica_indicador_nome'].(!$dialogo ? '</a>':'').'</td>';	
