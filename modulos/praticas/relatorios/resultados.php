...
foreach($marcadores as $dado){
	if ($dado['pratica_criterio_id']!=$criterio_atual){
		...
		$grupo=$sql->Lista();
		$sql->limpar();
		echo '<tr><td align="left" style="white-space: nowrap">'.dica('Dados Sobre o Critério', $dentro).'<b>'.$criterios[$dado['pratica_criterio_id']]['pratica_criterio_numero'].'.'.$criterios[$dado['pratica_criterio_id']]['pratica_criterio_nome'].'</b>'.dicaF().'</td>'.somatorio($grupo).'</tr>';
		}
	...
	if ($dado['pratica_item_id']!=$item_atual){
		...
		if (!$itens[$dado['pratica_item_id']]['pratica_item_oculto']) echo '<tr><td align="left" style="white-space: nowrap">&nbsp;'.dica('Dados Sobre o Ítem', $dentro).$criterios[$dado['pratica_criterio_id']]['pratica_criterio_numero'].'.'.$itens[$dado['pratica_item_id']]['pratica_item_numero'].'.'.$itens[$dado['pratica_item_id']]['pratica_item_nome'].dicaF().'</td>'.somatorio($grupo).'</tr>';
		}
...
function formata_nomes($nomes=array()){
	...
	if ($quantidade){
		...
		$saida='<table align="center"><tr><td style="white-space: nowrap"><a href="javascript: void(0);" onclick="exibir(\'n_'.$n.'\')">'.dica('Indicadores', 'Clique para visualizar os indicadores').$quantidade.dicaF().'</a></td></tr><tr id="n_'.$n.'" style="display:none"><td style="white-space: nowrap">'.$saida.'</td></tr></table>';
...
function formata_soma($total, $quantidade, $nomes=array()){
	...
	if (count($nomes)){
		...
		$saida='<table align="center"><tr><td style="white-space: nowrap"><a href="javascript: void(0);" onclick="exibir(\'c_'.$i.'\')">'.dica('Percentagem', $percentagem.'%<br><br>Clique para visualizar '.($config['genero_pratica']=='a' ? 'as ': 'os ').$config['praticas']).$quantidade.dicaF().'</a></td></tr><tr id="c_'.$i.'" style="display:none"><td style="white-space: nowrap">'.$saida.'</td></tr></table>';
