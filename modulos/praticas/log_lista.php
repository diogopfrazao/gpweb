...
if(!$Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'resposta_risco')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if (isset($_REQUEST['filtro_prioridade_risco_resposta']))	$Aplic->setEstado('filtro_prioridade_risco_resposta', getParam($_REQUEST, 'filtro_prioridade_risco_resposta', null));
$filtro_prioridade_risco_resposta = $Aplic->getEstado('filtro_prioridade_risco_resposta') !== null ? $Aplic->getEstado('filtro_prioridade_risco_resposta') : null;


$houve_filtro=getParam($_REQUEST, 'houve_filtro', null);
$sql->adTabela('favorito_trava');
$sql->adCampo('favorito_trava_campo');
$sql->adOnde('favorito_trava_risco_resposta=1');
...
if (isset($_REQUEST['logtextobusca'])) $Aplic->setEstado('logtextobusca', getParam($_REQUEST, 'logtextobusca', null));
$pesquisar_texto = ($Aplic->getEstado('logtextobusca') ? $Aplic->getEstado('logtextobusca') : '');
...
if (isset($_REQUEST['tab'])) $Aplic->setEstado('logTab', getParam($_REQUEST, 'tab', null));
$tab = ($Aplic->getEstado('logTab') !== null ? $Aplic->getEstado('logTab') : 0);
...
if (isset($_REQUEST['favorito_id'])) $Aplic->setEstado('risco_resposta_favorito', getParam($_REQUEST, 'favorito_id', null));
$favorito_id = $Aplic->getEstado('risco_resposta_favorito') !== null ? $Aplic->getEstado('risco_resposta_favorito') : 0;

$sql->adTabela('favorito');
$sql->adCampo('favorito_id, favorito_nome');
$sql->adOnde('favorito_geral!=1');
$sql->adOnde('favorito_ativo=1');
$sql->adOnde('favorito_risco_resposta=1');
...
echo '<input type="hidden" name="filtro_prioridade_risco_resposta" id="filtro_prioridade_risco_resposta" value="'.$filtro_prioridade_risco_resposta.'" />';
...
if (!$dialogo){
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionado','A qual parte do sistema os registros de ocorrência estão relacionados.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
...
if (!$dialogo && $Aplic->profissional){
	$botoesTitulo = new CBlocoTitulo('Lista de Registros de Ocorrências', 'log.png', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	//$novo_risco=($Aplic->checarModulo('praticas', 'adicionar', null, 'risco_respostas') ? '<tr><td style="white-space: nowrap">'.dica('Novo '.ucfirst($config['risco_resposta']), 'Criar '.($config['genero_risco_resposta']=='o' ? 'um' : 'uma').' novo '.$config['risco_resposta'].'.').'<a href="javascript: void(0)" onclick="url_passar(0,\'m=praticas&a=risco_resposta_pro_editar\');"><img src="'.acharImagem('risco_resposta_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr><tr><td style="white-space: nowrap"></td></tr>' : '');
	
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de registros de ocorrência.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&u='.$u.'&tab='.$tab.'&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
	//$botao_favorito='<tr><td style="white-space: nowrap"><a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&risco_resposta=1\');">'.imagem('icones/favorito_p.png','Favoritos', 'Clique neste ícone '.imagem('icones/favorito_p.png').' para criar ou editar um grupo de favoritos, para uma rápida filtragem.').'</a></td></tr>';
	...
	//if($filtro_prioridade_risco_resposta) $botao_prioridade=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="priorizacao(0);">'.imagem('icones/priorizacao_nao_p.png', 'Cancelar a Priorização' , 'Clique neste ícone '.imagem('icones/priorizacao_nao_p.png').' para cancelar a priorização.').'</a>'.dicaF().'</td></tr>' : '');
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$procuraBuffer.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$imprimir.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de Registros de Ocorrências</center></font>';
	}
echo '</form>';
...
include_once(BASE_DIR.'/modulos/praticas/log_lista_idx.php');
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=risco_respostas', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=praticas&a=risco_resposta_pro_lista');
	}	
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&risco_resposta=1&filtro_prioridade='+env.filtro_prioridade_risco_resposta.value, window.setFiltroPriorizacao, window);
	}

function setFiltroPriorizacao(filtro_prioridade_risco_resposta){
	env.filtro_prioridade_risco_resposta.value=filtro_prioridade_risco_resposta;
	env.submit();
	}
