...
if(!$Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'perspectiva')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if (!$dialogo|| $selecao){
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionad'.$config['genero_perspectiva'], 'A que área '.$config['genero_perspectiva'].' '.$config['perspectiva'].' está relacionad'.$config['genero_perspectiva'].'.').'Relacionad'.$config['genero_perspectiva'].':'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$nova_perspectiva=($Aplic->checarModulo('praticas', 'adicionar', null, 'perspectiva') ? '<tr><td style="white-space: nowrap" align=right>'.dica('Nov'.$config['genero_perspectiva'].' '.ucfirst($config['perspectiva']).'', 'Criar um nov'.$config['genero_perspectiva'].' '.$config['perspectiva'].'.').'<a href="javascript: void(0)" onclick="url_passar(0,\'m=praticas&a=perspectiva_editar\');"><img src="'.acharImagem('perspectiva_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir '.ucfirst($config['perspectivas']).'', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['perspectivas'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&u='.$u.'&dialogo=1&tab='.$tab.'\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=perspectivas', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=praticas&a=perspectiva_lista');
	}
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&perspectiva=1&filtro_prioridade='+env.filtro_prioridade_perspectiva.value, window.setFiltroPriorizacao, window);
	}
