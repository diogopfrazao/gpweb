...
if (isset($_REQUEST['brainstorm_id'])) $Aplic->setEstado('brainstorm_id', getParam($_REQUEST, 'brainstorm_id', null), $m, $a, $u);
$brainstorm_id = $Aplic->getEstado('brainstorm_id', null, $m, $a, $u);
...
if ($Aplic->profissional){
	...
	$sql->adOnde('assinatura_brainstorm='.(int)$brainstorm_id);
	$assinar = $sql->linha();
	$sql->limpar();
...
$sql->adUnir('usuarios','usuarios','usuarios.usuario_id=brainstorm_usuarios.usuario_id');
...
if(!(permiteAcessarBrainstorm($obj->brainstorm_acesso,$brainstorm_id) && $Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'brainstorm'))) $Aplic->redirecionar('m=publico&a=acesso_negado');

$podeEditar=$Aplic->checarModulo('praticas', 'editar', $Aplic->usuario_id, 'brainstorm');
$podeAdicionar=$Aplic->checarModulo('praticas', 'adicionar', $Aplic->usuario_id, 'brainstorm');
$podeExcluir=$Aplic->checarModulo('praticas', 'excluir', $Aplic->usuario_id, 'brainstorm');


$editar=permiteEditarBrainstorm($obj->brainstorm_acesso,$brainstorm_id);

$podeEditarTudo=($obj->brainstorm_acesso>=5 ? $editar && (in_array($obj->brainstorm_responsavel, $Aplic->usuario_lista_grupo_vetor) || $Aplic->usuario_super_admin || $Aplic->usuario_admin) : $editar);


if (!$brainstorm_id) {
	$Aplic->setMsg('Não foi passado um ID correto ao tentar ver o brainstorm.', UI_MSG_ERRO);
...
if (!$dialogo){
	$Aplic->salvarPosicao();
	$botoesTitulo = new CBlocoTitulo('Detalhes do Brainstorm', 'brainstorm.gif', $m, $m.'.'.$a);
	...
	$km->Add("ver","ver_lista",dica('Lista de Brainstorms','Clique neste botão para visualizar a lista de brainstorms.').'Lista de Brainstorms'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=brainstorm_lista\");");
		
	if (($podeEditar && $editar) || $podeAdicionar)	$km->Add("root","inserir",dica('Inserir','Menu de opções').'Inserir'.dicaF(), "javascript: void(0);'");
	if ($podeAdicionar)	$km->Add("inserir","inserir_objeto",dica('Novo Brainstorm', 'Criar um novo brainstorm.').'Novo Brainstorm'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=brainstorm_editar\");");
	if ($podeEditar && $editar) {	
		$km->Add("inserir","inserir_registro",dica('Registro de Ocorrência','Inserir um novo registro de ocorrência.').'Registro de ocorrência'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=log_editar&brainstorm_id=".$brainstorm_id."\");");
		if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('eventos', 'adicionar')) $km->Add("inserir","inserir_evento",dica('Novo Evento', 'Criar um novo evento relacionado.').'Evento'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=calendario&a=editar&evento_brainstorm=".$brainstorm_id."\");");
		if ($Aplic->modulo_ativo('arquivos') && $Aplic->checarModulo('arquivos', 'adicionar')) $km->Add("inserir","inserir_arquivo",dica('Novo Arquivo', 'Inserir um novo arquivo relacionado.').'Arquivo'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=arquivos&a=editar&arquivo_brainstorm=".$brainstorm_id."\");");
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'adicionar', null, 'indicador')) $km->Add("inserir","inserir_indicador",dica('Novo Indicador', 'Inserir um novo indicador relacionado.').'Indicador'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=indicador_editar&pratica_indicador_brainstorm=".$brainstorm_id."\");");
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'adicionar', null, 'plano_acao')) $km->Add("inserir","inserir_acao",dica('Nov'.$config['genero_acao'].' '.ucfirst($config['acao']), 'Criar nov'.$config['genero_acao'].' '.$config['acao'].' relacionad'.$config['genero_acao'].'.').ucfirst($config['acao']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=plano_acao_editar&plano_acao_brainstorm=".$brainstorm_id."\");");
		if ($Aplic->modulo_ativo('projetos') && $Aplic->checarModulo('projetos', 'adicionar')) $km->Add("inserir","inserir_projeto", dica('Nov'.$config['genero_projeto'].' '.ucfirst($config['projeto']), 'Inserir nov'.$config['genero_projeto'].' '.$config['projeto'].' relacionad'.$config['genero_projeto'].'.').ucfirst($config['projeto']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=editar&projeto_brainstorm=".$brainstorm_id."\");");	
		if ($Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'adicionar')) $km->Add("inserir","inserir_ata",dica('Nova Ata de Reunião', 'Inserir uma nova ata de reunião relacionada.').'Ata de reunião'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=atas&a=ata_editar&ata_brainstorm=".$brainstorm_id."\");");
		if ($Aplic->modulo_ativo('email') && $Aplic->checarModulo('email', 'adicionar')) $km->Add("inserir","inserir_mensagem",dica('Nov'.$config['genero_mensagem'].' '.ucfirst($config['mensagem']), 'Inserir '.($config['genero_mensagem']=='a' ? 'uma' : 'um').' nov'.$config['genero_mensagem'].' '.$config['mensagem'].' relacionad'.$config['genero_mensagem'].'.').ucfirst($config['mensagem']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=email&a=nova_mensagem&msg_brainstorm=".$brainstorm_id."\");");
		if ($Aplic->checarModulo('projetos', 'adicionar', null, 'demanda')) $km->Add("inserir","inserir_demanda",dica('Nova Demanda', 'Inserir uma nova demanda relacionada.').'Demanda'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=demanda_editar&demanda_brainstorm=".$brainstorm_id."\");");
...
if($dialogo && $Aplic->profissional) {
	...
	$dados=array();
	$dados['projeto_cia'] = $obj->brainstorm_cia;
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Brainstorm</center></font>';
	}
...
if ($obj->brainstorm_nome) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nome', 'Número desta entegra').'Nome:'.dicaF().'</td><td style="border: outset #d1d1cd 1px;background-color:#'.$obj->brainstorm_cor.'"><font color="'.melhorCor($obj->brainstorm_cor).'"><b>'.$obj->brainstorm_nome.'<b></font></td></tr>';
if ($obj->brainstorm_cia) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', ucfirst($config['genero_organizacao']).' '.$config['organizacao'].' responsável.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_cia($obj->brainstorm_cia).'</td></tr>';
if ($Aplic->profissional){
...
if ($obj->brainstorm_dept) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']).' Responsável', ucfirst($config['genero_dept']).' '.$config['departamento'].' responsável por este brainstorm.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_dept($obj->brainstorm_dept).'</td></tr>';
$saida_depts='';
...
if ($saida_depts) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica(ucfirst($config['departamentos']), 'Qual '.strtolower($config['departamento']).' está envolvid'.$config['genero_dept'].' com este brainstorm.').ucfirst($config['departamento']).' envolvid'.$config['genero_dept'].':'.dicaF().'</td><td width="100%" colspan="2" class="realce">'.$saida_depts.'</td></tr>';


if ($obj->brainstorm_responsavel) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Responsável', 'Responsável por resolver o brainstorm.').'Responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($obj->brainstorm_responsavel, '','','esquerda').'</td></tr>';
...
$sql->adOnde('brainstorm_gestao_brainstorm ='.(int)$brainstorm_id);	
...
if (count($lista)) {
	echo '<tr><td align="right" style="white-space: nowrap" valign="middle">'.dica('Relacionado', 'A que área o brainstorm está relacionado.').'Relacionado:'.dicaF().'</td></td><td class="realce">';	
	foreach($lista as $gestao_data){
		if ($gestao_data['brainstorm_gestao_tarefa']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/tarefa_p.gif').link_tarefa($gestao_data['brainstorm_gestao_tarefa']);
		elseif ($gestao_data['brainstorm_gestao_projeto']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/projeto_p.gif').link_projeto($gestao_data['brainstorm_gestao_projeto']);
		elseif ($gestao_data['brainstorm_gestao_pratica']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/pratica_p.gif').link_pratica($gestao_data['brainstorm_gestao_pratica']);
		elseif ($gestao_data['brainstorm_gestao_acao']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/plano_acao_p.gif').link_acao($gestao_data['brainstorm_gestao_acao']);
		elseif ($gestao_data['brainstorm_gestao_perspectiva']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/perspectiva_p.png').link_perspectiva($gestao_data['brainstorm_gestao_perspectiva']);
		elseif ($gestao_data['brainstorm_gestao_tema']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/tema_p.png').link_tema($gestao_data['brainstorm_gestao_tema']);
		elseif ($gestao_data['brainstorm_gestao_objetivo']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/obj_estrategicos_p.gif').link_objetivo($gestao_data['brainstorm_gestao_objetivo']);
		elseif ($gestao_data['brainstorm_gestao_fator']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/fator_p.gif').link_fator($gestao_data['brainstorm_gestao_fator']);
		elseif ($gestao_data['brainstorm_gestao_estrategia']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/estrategia_p.gif').link_estrategia($gestao_data['brainstorm_gestao_estrategia']);
		elseif ($gestao_data['brainstorm_gestao_meta']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/meta_p.gif').link_meta($gestao_data['brainstorm_gestao_meta']);
		elseif ($gestao_data['brainstorm_gestao_canvas']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/canvas_p.png').link_canvas($gestao_data['brainstorm_gestao_canvas']);
		elseif ($gestao_data['brainstorm_gestao_risco']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/risco_p.png').link_risco($gestao_data['brainstorm_gestao_risco']);
		elseif ($gestao_data['brainstorm_gestao_risco_resposta']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/risco_resposta_p.png').link_risco_resposta($gestao_data['brainstorm_gestao_risco_resposta']);
		elseif ($gestao_data['brainstorm_gestao_indicador']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/indicador_p.gif').link_indicador($gestao_data['brainstorm_gestao_indicador']);
		elseif ($gestao_data['brainstorm_gestao_calendario']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/agenda_p.png').link_calendario($gestao_data['brainstorm_gestao_calendario']);
		elseif ($gestao_data['brainstorm_gestao_monitoramento']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/monitoramento_p.gif').link_monitoramento($gestao_data['brainstorm_gestao_monitoramento']);
		elseif ($gestao_data['brainstorm_gestao_ata']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/ata_p.png').link_ata_pro($gestao_data['brainstorm_gestao_ata']);
		elseif ($gestao_data['brainstorm_gestao_mswot']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/mswot_p.png').link_mswot($gestao_data['brainstorm_gestao_mswot']);
		elseif ($gestao_data['brainstorm_gestao_swot']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/swot_p.png').link_swot($gestao_data['brainstorm_gestao_swot']);
		elseif ($gestao_data['brainstorm_gestao_operativo']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/operativo_p.png').link_operativo($gestao_data['brainstorm_gestao_operativo']);
		elseif ($gestao_data['brainstorm_gestao_instrumento']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/instrumento_p.png').link_instrumento($gestao_data['brainstorm_gestao_instrumento']);
		elseif ($gestao_data['brainstorm_gestao_recurso']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/recursos_p.gif').link_recurso($gestao_data['brainstorm_gestao_recurso']);
		elseif ($gestao_data['brainstorm_gestao_problema']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/problema_p.png').link_problema($gestao_data['brainstorm_gestao_problema']);
		elseif ($gestao_data['brainstorm_gestao_demanda']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/demanda_p.gif').link_demanda($gestao_data['brainstorm_gestao_demanda']);	
		elseif ($gestao_data['brainstorm_gestao_programa']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/programa_p.png').link_programa($gestao_data['brainstorm_gestao_programa']);
		elseif ($gestao_data['brainstorm_gestao_licao']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/licoes_p.gif').link_licao($gestao_data['brainstorm_gestao_licao']);
		elseif ($gestao_data['brainstorm_gestao_evento']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/calendario_p.png').link_evento($gestao_data['brainstorm_gestao_evento']);
		elseif ($gestao_data['brainstorm_gestao_link']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/links_p.gif').link_link($gestao_data['brainstorm_gestao_link']);
		elseif ($gestao_data['brainstorm_gestao_avaliacao']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/avaliacao_p.gif').link_avaliacao($gestao_data['brainstorm_gestao_avaliacao']);
		elseif ($gestao_data['brainstorm_gestao_tgn']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/tgn_p.png').link_tgn($gestao_data['brainstorm_gestao_tgn']);
		
		elseif ($gestao_data['brainstorm_gestao_semelhante']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/brainstorm_p.gif').link_brainstorm($gestao_data['brainstorm_gestao_semelhante']);
		
		elseif ($gestao_data['brainstorm_gestao_gut']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/gut_p.gif').link_gut($gestao_data['brainstorm_gestao_gut']);
		elseif ($gestao_data['brainstorm_gestao_causa_efeito']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/causaefeito_p.png').link_causa_efeito($gestao_data['brainstorm_gestao_causa_efeito']);
		elseif ($gestao_data['brainstorm_gestao_arquivo']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/arquivo_p.png').link_arquivo($gestao_data['brainstorm_gestao_arquivo']);
		elseif ($gestao_data['brainstorm_gestao_forum']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/forum_p.gif').link_forum($gestao_data['brainstorm_gestao_forum']);
		elseif ($gestao_data['brainstorm_gestao_checklist']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/todo_list_p.png').link_checklist($gestao_data['brainstorm_gestao_checklist']);
		elseif ($gestao_data['brainstorm_gestao_agenda']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/compromisso_p.png').link_agenda($gestao_data['brainstorm_gestao_agenda']);
		elseif ($gestao_data['brainstorm_gestao_agrupamento']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/agrupamento_p.png').link_agrupamento($gestao_data['brainstorm_gestao_agrupamento']);
		elseif ($gestao_data['brainstorm_gestao_patrocinador']) echo ($qnt_gestao++ ? '<br>' : '').imagem('../../../modulos/patrocinadores/imagens/patrocinador_p.gif').link_patrocinador($gestao_data['brainstorm_gestao_patrocinador']);
		elseif ($gestao_data['brainstorm_gestao_template']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/template_p.gif').link_template($gestao_data['brainstorm_gestao_template']);
		elseif ($gestao_data['brainstorm_gestao_painel']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/painel_p.png').link_painel($gestao_data['brainstorm_gestao_painel']);
		elseif ($gestao_data['brainstorm_gestao_painel_odometro']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/odometro_p.png').link_painel_odometro($gestao_data['brainstorm_gestao_painel_odometro']);
		elseif ($gestao_data['brainstorm_gestao_painel_composicao']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/composicao_p.gif').link_painel_composicao($gestao_data['brainstorm_gestao_painel_composicao']);		
		elseif ($gestao_data['brainstorm_gestao_tr']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/tr_p.png').link_tr($gestao_data['brainstorm_gestao_tr']);	
		elseif ($gestao_data['brainstorm_gestao_me']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/me_p.png').link_me($gestao_data['brainstorm_gestao_me']);	
		elseif ($gestao_data['brainstorm_gestao_acao_item']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/acao_item_p.png').link_acao_item($gestao_data['brainstorm_gestao_acao_item']);	
		elseif ($gestao_data['brainstorm_gestao_beneficio']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/beneficio_p.png').link_beneficio($gestao_data['brainstorm_gestao_beneficio']);	
		elseif ($gestao_data['brainstorm_gestao_painel_slideshow']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/slideshow_p.gif').link_painel_slideshow($gestao_data['brainstorm_gestao_painel_slideshow']);	
		elseif ($gestao_data['brainstorm_gestao_projeto_viabilidade']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/viabilidade_p.gif').link_viabilidade($gestao_data['brainstorm_gestao_projeto_viabilidade']);	
		elseif ($gestao_data['brainstorm_gestao_projeto_abertura']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/anexo_projeto_p.png').link_termo_abertura($gestao_data['brainstorm_gestao_projeto_abertura']);	
		elseif ($gestao_data['brainstorm_gestao_plano_gestao']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/planogestao_p.png').link_plano_gestao($gestao_data['brainstorm_gestao_plano_gestao']);	
		elseif ($gestao_data['brainstorm_gestao_ssti']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/ssti_p.png').link_ssti($gestao_data['brainstorm_gestao_ssti']);
		elseif ($gestao_data['brainstorm_gestao_laudo']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/laudo_p.png').link_laudo($gestao_data['brainstorm_gestao_laudo']);
		elseif ($gestao_data['brainstorm_gestao_trelo']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/trelo_p.png').link_trelo($gestao_data['brainstorm_gestao_trelo']);
		elseif ($gestao_data['brainstorm_gestao_trelo_cartao']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/trelo_cartao_p.png').link_trelo_cartao($gestao_data['brainstorm_gestao_trelo_cartao']);
		elseif ($gestao_data['brainstorm_gestao_pdcl']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/pdcl_p.png').link_pdcl($gestao_data['brainstorm_gestao_pdcl']);
		elseif ($gestao_data['brainstorm_gestao_pdcl_item']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/pdcl_item_p.png').link_pdcl_item($gestao_data['brainstorm_gestao_pdcl_item']);
		elseif ($gestao_data['brainstorm_gestao_os']) echo ($qnt_gestao++ ? '<br>' : '').imagem('icones/os_p.png').link_os($gestao_data['brainstorm_gestao_os']);
...
if ($obj->brainstorm_data) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data', 'A data de realização do brainstorm.').'Data:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.retorna_data($obj->brainstorm_data, false).'</td></tr>';
if ($obj->brainstorm_descricao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Descrição', 'A descrição detalhada do brainstorm.').'Descrição:'.dicaF().'</td><td class="realce" width="100%" >'.$obj->brainstorm_descricao.'</td></tr>';

if ($obj->brainstorm_principal_indicador) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Indicador Principal', 'Dentre os indicadores relacionados, o mais representativo da situação geral.').'Indicador principal:'.dicaF().'</td><td width="100%" class="realce">'.link_indicador($obj->brainstorm_principal_indicador).'</td></tr>';
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	if (isset($moedas[$obj->brainstorm_moeda])) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Moeda', 'A moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->brainstorm_moeda].'</td></tr>';	
	}
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Aprovado', 'Se o brainstorm se encontra aprovado.').'Aprovado:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->brainstorm_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';


require_once ($Aplic->getClasseSistema('CampoCustomizados'));
$campos_customizados = new CampoCustomizados('brainstorm', $obj->brainstorm_id, 'ver');
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Brainstorm', 'O gráfico do brainstorm.').'Brainstorm:'.dicaF().'</td><td class="realce" width="100%" >'.$arvore->Render().'</td></tr>';
...
if ($Aplic->profissional) include_once BASE_DIR.'/modulos/praticas/brainstorm_ver_pro.php';	
...
if (!$dialogo) {
	$caixaTab = new CTabBox('m=praticas&a=brainstorm_ver&brainstorm_id='.$brainstorm_id, '', $tab);
	$qnt_aba=0;
	
	if ($Aplic->checarModulo('log', 'acesso')) {
		$sql->adTabela('log');
		$sql->adCampo('count(log_id)');
		$sql->adOnde('log_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('eventos', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('evento_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->modulo_ativo('arquivos') && $Aplic->checarModulo('arquivos', 'acesso')) {
			...
			$sql->adOnde('arquivo_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('plano_acao_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao_item')) {
		...
		$sql->adOnde('plano_acao_item_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->modulo_ativo('projetos') && $Aplic->checarModulo('projetos', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('projeto_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('atas') && $Aplic->checarModulo('atas', 'acesso')) {
		...
		$sql->adOnde('ata_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'demanda')) {
		...
		$sql->adOnde('demanda_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->modulo_ativo('email') && $Aplic->checarModulo('email', 'acesso')) {
		...
		$sql->adOnde('msg_gestao_brainstorm = '.(int)$brainstorm_id);
		...
		if ($config['doc_interno']) {
			...
			$sql->adOnde('modelo_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('link_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'acesso')) {
		if ($Aplic->profissional) {
			...
			$sql->adOnde('forum_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('problema') && $Aplic->checarModulo('problema', 'acesso')) {
		...
		$sql->adOnde('problema_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco')) {
		...
		$sql->adOnde('risco_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco_resposta')) {		
		...
		$sql->adOnde('risco_resposta_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->modulo_ativo('instrumento')  && $Aplic->checarModulo('instrumento', 'acesso')) {
		...
		$sql->adOnde('instrumento_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('recursos', 'acesso', null, null)) {
		...
		$sql->adOnde('recurso_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->modulo_ativo('patrocinadores') && $Aplic->checarModulo('patrocinadores', 'acesso', null, null)) {
		...
		$sql->adOnde('patrocinador_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'programa')) {
		...
		$sql->adOnde('programa_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'beneficio')) {
		...
		$sql->adOnde('beneficio_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('projeto', 'acesso', 'licao')) {
		...
		$sql->adOnde('licao_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'pratica')) {
		...
		$sql->adOnde('pratica_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('tr') && $Aplic->checarModulo('tr', 'acesso', null, null)) {
		...
		$sql->adOnde('tr_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'brainstorm')) {
		$sql->adTabela('brainstorm_gestao');
		$sql->adOnde('brainstorm_gestao_semelhante = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'gut')) {
		...
		$sql->adOnde('gut_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'causa_efeito')) {
		...
		$sql->adOnde('causa_efeito_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'tgn')) {
		...
		$sql->adOnde('tgn_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'canvas')) {
		...
		$sql->adOnde('canvas_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('swot') && $Aplic->checarModulo('swot', 'acesso', null, null)) {
		...
		$sql->adOnde('mswot_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('swot') && $Aplic->checarModulo('swot', 'acesso', null, null)) {
		...
		$sql->adOnde('swot_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('operativo') && $Aplic->checarModulo('operativo', 'acesso', null, null)) {
		...
		$sql->adOnde('operativo_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'monitoramento')) {
		...
		$sql->adOnde('monitoramento_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'avaliacao_indicador')) {
		...
		$sql->adOnde('avaliacao_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'checklist')) {
		...
		$sql->adOnde('checklist_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional) {
		...
		$sql->adOnde('agenda_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('agrupamento') && $Aplic->checarModulo('agrupamento', 'acesso', null, null)) {
		...
		$sql->adOnde('agrupamento_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'modelo')) {
		...
		$sql->adOnde('template_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'painel_indicador')) {
		...
		$sql->adOnde('painel_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'odometro_indicador')) {
		...
		$sql->adOnde('painel_odometro_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'composicao_painel')) {
		...
		$sql->adOnde('painel_composicao_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'slideshow_painel')) {
		...
		$sql->adOnde('painel_slideshow_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('agenda', 'acesso', null, null)) {
		...
		$sql->adOnde('calendario_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'perspectiva')) {
		...
		$sql->adOnde('perspectiva_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'tema')) {
		...
		$sql->adOnde('tema_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'objetivo')) {
		...
		$sql->adOnde('objetivo_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'me')) {
		...
		$sql->adOnde('me_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($config['exibe_fator'] && $Aplic->checarModulo('praticas', 'acesso', null, 'fator')) {
		...
		$sql->adOnde('fator_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'iniciativa')) {
		...
		$sql->adOnde('estrategia_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'meta')) {
		...
		$sql->adOnde('meta_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'planejamento')) {
		...
		$sql->adOnde('plano_gestao_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'abertura')) {
		...
		$sql->adOnde('projeto_abertura_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'viabilidade')) {
		...
		$sql->adOnde('projeto_viabilidade_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso')) {
		...
		$sql->adOnde('ssti_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso',null, 'laudo')) {
		...
		$sql->adOnde('laudo_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'acesso')) {
		...
		$sql->adOnde('trelo_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'acesso')) {
		...
		$sql->adOnde('trelo_cartao_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso')) {
		...
		$sql->adOnde('pdcl_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso')) {
		...
		$sql->adOnde('pdcl_item_gestao_brainstorm = '.(int)$brainstorm_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('os') && $Aplic->checarModulo('os', 'acesso')) {
		...
		$sql->adOnde('os_gestao_brainstorm = '.(int)$brainstorm_id);
...
function excluir() {
	if (confirm('Tem certeza que deseja excluir?')) {
		var f = document.env;
		f.excluir.value=1;
		f.fazerSQL.value='brainstorm_fazer_sql';
		f.a.value='vazio';
		f.dialogo.value=1;
		f.submit();
		}
