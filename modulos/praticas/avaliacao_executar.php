...
if ($pratica_indicador_id){
	...
	if ($checklist_id){
		...
		if ($valor['pratica_indicador_valor_id']){
			...
			echo '<tr><td align="right" style="white-space: nowrap">'.dica('Observação', 'Observação sobre esta inserção.').'Observação:'.dicaF().'</td><td colspan=2 align="left"><textarea style="width:400px;" rows="3" name="avaliacao_indicador_lista_observacao" id="avaliacao_indicador_lista_observacao">'.($valor['pratica_indicador_valor_obs']).'</textarea></td></tr>';
			echo '<tr><td colspan=20>'.botao('salvar', ($dialogo ? '' : 'Salvar'), ($dialogo ? '' : 'Salvar os dados.'),'','enviarDados()').'</td></tr>';
			}
