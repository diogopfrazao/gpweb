...
if ($editarPG){
	if ($swot_ativo) {
		echo '<tr><td align="right" width="10" style="white-space: nowrap">'.dica('Oportunidade', 'Selecione uma oportunidade.').'Oportunidade:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="text" id="nome_swot" name="nome_swot" value="" style="width:400px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popSWOT2();">'.imagem('icones/swot_p.png','Selecionar','Clique neste ícone '.imagem('icones/swot_p.png').' para selecionar uma força.').'</a></td></tr></table></td></tr>';
...
if (count($swots)) echo '<table class="tbl1" cellspacing=0 cellpadding=0><tr>'.($editarPG ? '<th></th>' : '').'<th style="white-space: nowrap">Oportunidade</th>'.($editarPG ? '<th></th>' : '').'</tr>';
...
function popSWOT2() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Oportunidade', 1000, 700, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setSWOT&swot_tipo=o&tabela=swot&cia_id='+document.getElementById('cia_id').value, window.setSWOT2, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setSWOT2&swot_tipo=o&tabela=swot&cia_id='+document.getElementById('cia_id').value, 'Oportunidade','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
	}
...
function mudar_ordem_swot(ordem, pg_swot_id, direcao){
	xajax_mudar_ordem_swot(ordem, pg_swot_id, direcao, document.getElementById('pg_id').value, 'o');
	__buildTooltip();
	}

function excluir_swot(pg_swot_id){
	xajax_excluir_swot(pg_swot_id, document.getElementById('pg_id').value, 'o');
	__buildTooltip();
	}
