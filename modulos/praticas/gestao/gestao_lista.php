...
if(!$Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'planejamento')) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if (isset($_REQUEST['filtro_prioridade_plano_gestao']))	$Aplic->setEstado('filtro_prioridade_plano_gestao', getParam($_REQUEST, 'filtro_prioridade_plano_gestao', null));
$filtro_prioridade_plano_gestao = $Aplic->getEstado('filtro_prioridade_plano_gestao') !== null ? $Aplic->getEstado('filtro_prioridade_plano_gestao') : null;
...
echo '<input type="hidden" name="filtro_prioridade_plano_gestao" id="filtro_prioridade_plano_gestao" value="'.$filtro_prioridade_plano_gestao.'" />';
...
if (!$dialogo || $selecao){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionad'.$config['genero_plano_gestao'], 'A que área '.$config['genero_plano_gestao'].' '.$config['plano_gestao'].' está relacionad'.$config['genero_plano_gestao'].'.').'Relacionad'.$config['genero_plano_gestao'].':'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	$novo=($Aplic->checarModulo('praticas', 'adicionar', $Aplic->usuario_id, 'planejamento') ? '<tr><td style="white-space: nowrap">'.dica('Novo Planejamento Estratégico', 'Criar um novo planejamento estratégico.').'<a href="javascript: void(0)" onclick="javascript:url_passar(0, \'m=praticas&u=gestao&a=gestao_editar\');" >'.imagem('icones/planogestao_novo.png').'</a>'.dicaF().'</td></tr>' : '');

	$imprimir='<tr><td align="right" style="white-space: nowrap"><a href="javascript: void(0);" onclick ="url_passar(1, \'m='.$m.'&a='.$a.'&u='.$u.'&dialogo=1\');">'.imagem('imprimir_p.png', 'Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista.').'</a></td></tr>';
	...
	if ($filtro_prioridade_plano_gestao) $botao_prioridade=($Aplic->profissional ? '<tr><td><a href="javascript: void(0)" onclick="priorizacao(0);">'.imagem('icones/priorizacao_nao_p.png', 'Cancelar a Priorização' , 'Clique neste ícone '.imagem('icones/priorizacao_nao_p.png').' para cancelar a priorização.').'</a>'.dicaF().'</td></tr>' : '');
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.(!$selecao ? $novo.$imprimir.$botao_prioridade.$botao_campos : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=planos_gestao', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=praticas&u=gestao&a=gestao_lista');
	}	
	
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&plano_gestao=1&filtro_prioridade='+env.filtro_prioridade_plano_gestao.value, window.setFiltroPriorizacao, window);
	}

function setFiltroPriorizacao(filtro_prioridade_plano_gestao){
	env.filtro_prioridade_plano_gestao.value=filtro_prioridade_plano_gestao;
	env.submit();
	}		
