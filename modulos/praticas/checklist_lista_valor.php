...
global $Aplic, $estilo_interface, $pratica_indicador_id, $pratica_indicador, $editar, $podeEditarTudo, $podeValor, $podeEditar;
...
for ($i = ($pagina - 1) * $xpg_tamanhoPagina; $i < $pagina * $xpg_tamanhoPagina && $i < $xpg_totalregistros; $i++) {
    ...
    if (($podeEditar || $podeValor) && $podeEditarTudo) echo '<td width="16"><a href="javascript:void(0);" onclick="url_passar(0, \'m=praticas&a=checklist_editar_valor&checklist_dados_id='.$valor['checklist_dados_id'].'\');">'.imagem('icones/editar.gif','Editar','Clique neste ícone '.imagem('icones/editar.gif').' para editar o checklist.').'</a></td>';
...
if($Aplic->profissional) {
    echo '<tr><td align="center" colspan="100">'.botao('Exportar para Excel', 'Exportar para Excel', 'Exportar a lista de valores para uma planilha do excel.','','exportar_valores_excel('.$pratica_indicador_id.',\''.$ordenar.'\',\''.$ordem.'\');').'</td></tr>';
    }
...
function exportar_valores_excel(indicador_id, campo_ordem, valor_ordem) {
    url_passar('exportar_valores_indicador', 'm=praticas&a=checklist_lista_valor_exportar_excel_pro&sem_cabecalho=1&pratica_indicador_id='+indicador_id+'&ordenar='+campo_ordem+'&ordem='+(valor_ordem ? '0' : '1'));
}
