...
if (isset($_REQUEST['objetivo_id'])) $Aplic->setEstado('objetivo_id', getParam($_REQUEST, 'objetivo_id', null), $m, $a, $u);
$objetivo_id = $Aplic->getEstado('objetivo_id', null, $m, $a, $u);
...
if(!(permiteAcessarObjetivo($obj->objetivo_acesso,$objetivo_id) && $Aplic->checarModulo('praticas', 'acesso', $Aplic->usuario_id, 'objetivo'))) $Aplic->redirecionar('m=publico&a=acesso_negado');

$podeEditar=$Aplic->checarModulo('praticas', 'editar', $Aplic->usuario_id, 'objetivo');
$podeAdicionar=$Aplic->checarModulo('praticas', 'adicionar', $Aplic->usuario_id, 'objetivo');
$podeExcluir=$Aplic->checarModulo('praticas', 'excluir', $Aplic->usuario_id, 'objetivo');

$editar=permiteEditarObjetivo($obj->objetivo_acesso,$objetivo_id);

$podeEditarTudo=($obj->objetivo_acesso>=5 ? $editar && (in_array($obj->objetivo_usuario, $Aplic->usuario_lista_grupo_vetor) || $Aplic->usuario_super_admin || $Aplic->usuario_admin) : $editar);
...
if (!$dialogo){	
	...
	if ($podeAdicionar)	$km->Add("inserir","inserir_objeto",dica('Nov'.$config['genero_objetivo'].' '.ucfirst($config['objetivo']), 'Criar um nov'.$config['genero_objetivo'].' '.$config['objetivo'].'.').'Nov'.$config['genero_objetivo'].' '.ucfirst($config['objetivo']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=obj_estrategico_editar\");");
	if ($podeEditar && $editar) {
		$km->Add("inserir","inserir_registro",dica('Registro de Ocorrência','Inserir um novo registro de ocorrência.').'Registro de ocorrência'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=log_editar&objetivo_id=".$objetivo_id."\");");
		if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('eventos', 'adicionar')) $km->Add("inserir","inserir_evento",dica('Novo Evento', 'Criar um novo evento relacionado.').'Evento'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=calendario&a=editar&evento_objetivo=".$objetivo_id."\");");
		...
		if ($Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'adicionar', null, 'indicador')) $km->Add("inserir","inserir_indicador",dica('Novo Indicador', 'Inserir um novo indicador relacionado.').'Indicador'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=indicador_editar&pratica_indicador_objetivo=".$objetivo_id."\");");
		...
		if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'adicionar')) $km->Add("diverso","inserir_link",dica('Novo Link', 'Inserir um novo link relacionado.').'Link'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=links&a=editar&link_objetivo=".$objetivo_id."\");");
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("diverso","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_objetivo=".$objetivo_id."\");");
	...
	if ($podeEditarTudo && $podeEditar && !$bloquear)  $km->Add("acao","acao_editar",dica('Editar '.ucfirst($config['objetivo']),'Editar os detalhes d'.($config['genero_objetivo']=='a' ? 'esta' : 'este').' '.$config['objetivo'].'.').'Editar '.ucfirst($config['objetivo']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=obj_estrategico_editar&objetivo_id=".$objetivo_id."\");");
	if ($podeEditarTudo && $podeExcluir && !$bloquear) $km->Add("acao","acao_excluir",dica('Excluir','Excluir '.($config['genero_objetivo']=='a' ? 'esta' : 'este').' '.$config['objetivo'].'.').'Excluir '.ucfirst($config['objetivo']).dicaF(), "javascript: void(0);' onclick='excluir()");
	...
	if ($Aplic->profissional) {
		$km->Add("acao_imprimir","dashboard_geral",dica('Grau de Consecução','Dashboard do grau de consecução d'.($config['genero_objetivo']=='a' ? 'esta' : 'este').' '.$config['objetivo'].'.').'Grau de consecução'.dicaF(), "javascript: void(0);' onclick='url_passar(\"dashboard_geral_".$objetivo_id."\", \"m=praticas&a=objetivo_deshboard_geral_pro&jquery=1&dialogo=1&objetivo_id=".$objetivo_id."\");");
		$km->Add("acao_imprimir","resumo_tarefas",dica('Quadro Resumo d'.$config['genero_tarefa'].'s '.ucfirst($config['tarefas']),'Quadro Resumo d'.$config['genero_tarefa'].'s '.$config['tarefas'].' d'.($config['genero_objetivo']=='a' ? 'esta' : 'este').' '.$config['objetivo'].'.').'Quadro resumo d'.$config['genero_tarefa'].'s '.$config['tarefas'].dicaF(), "javascript: void(0);' onclick='url_passar(\"resumo_tarefas_".$objetivo_id."\", \"m=praticas&a=objetivo_resumo_tarefas_pro&dialogo=1&objetivo_id=".$objetivo_id."\");");
		}
...
echo '<tr><td style="border: outset #d1d1cd 1px;background-color:#'.$obj->objetivo_cor.'" colspan="2"><font color="'.melhorCor($obj->objetivo_cor).'"><b>'.$obj->objetivo_nome.'<b></font>'.$cor_indicador.'</td></tr>';
...
if ($obj->objetivo_cia) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', ucfirst($config['genero_organizacao']).' '.$config['organizacao'].' responsável.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_cia($obj->objetivo_cia).'</td></tr>';
...
if ($obj->objetivo_dept) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']).' Responsável', ucfirst($config['genero_dept']).' '.$config['departamento'].' responsável por '.($config['genero_objetivo']=='a' ? 'esta' : 'este').' '.$config['objetivo'].'.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_dept($obj->objetivo_dept).'</td></tr>';
$saida_depts='';
...
if ($Aplic->profissional){
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Sistema de Percentagem', 'Qual forma a percentagem d'.$config['genero_objetivo'].' '.$config['objetivo'].' é calculada.').'Sistema de percentagem:'.dicaF().'</td><td class="realce">'.(isset($tipo_pontuacao[$obj->objetivo_tipo_pontuacao]) ? $tipo_pontuacao[$obj->objetivo_tipo_pontuacao] : '').'</td></tr>';
	if ($obj->objetivo_tipo_pontuacao && $obj->objetivo_tipo_pontuacao!='indicador'){
		...
		if (is_array($medias) && count($medias)){
			...
			foreach ($medias as $contato_id => $media) {
				...
				if ($media['objetivo_media_projeto']) echo '<td align="left">'.imagem('icones/projeto_p.gif').link_projeto($media['objetivo_media_projeto']).'</td>';
				elseif ($media['objetivo_media_acao']) echo '<td align="left">'.imagem('icones/plano_acao_p.gif').link_acao($media['objetivo_media_acao']).'</td>';
				elseif ($media['objetivo_media_fator']) echo '<td align="left">'.imagem('icones/fator_p.gif').link_fator($media['objetivo_media_fator']).'</td>';
				elseif ($media['objetivo_media_estrategia']) echo '<td align="left">'.imagem('icones/estrategia_p.gif').link_estrategia($media['objetivo_media_estrategia']).'</td>';
				elseif ($media['objetivo_media_me']) echo '<td align="left">'.imagem('icones/me_p.png').link_me($media['objetivo_media_me']).'</td>';
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Progresso', 'O progresso pode ir de 0% (não iniciado) até 100% (completado).').'Progresso:'.dicaF().'</td><td class="realce" width="300">'.number_format((float)$obj->objetivo_percentagem, 2, ',', '.').'%</td></tr>';

	
	if($obj->objetivo_tipo_pontuacao){
		...
		$velocidade_fisico=($fisico_previsto > 0 ? $obj->objetivo_percentagem/$fisico_previsto : 0);
...
else echo '<tr><td align="right" style="white-space: nowrap">'.dica('Progresso', 'O progresso pode ir de 0% (não iniciado) até 100% (completado).').'Progresso:'.dicaF().'</td><td class="realce" width="300">'.number_format((float)$obj->objetivo_percentagem, 2, ',', '.').'%</td></tr>';
...
if (count($lista)) {
	echo '<tr><td align="right" style="white-space: nowrap" valign="middle">'.dica('Relacionad'.$config['genero_objetivo'], 'A que área '.$config['genero_objetivo'].' '.$config['objetivo'].' está relacionad'.$config['genero_objetivo'].'.').'Relacionad'.$config['genero_objetivo'].':'.dicaF().'</td></td><td class="realce">';	
...
if ($obj->objetivo_indicador) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Indicador Principal', 'O indicador d'.$config['genero_objetivo'].' '.$config['objetivo'].' mais representativo da situação geral d'.$config['genero_objetivo'].' mesm'.$config['genero_objetivo'].'.').'Indicador principal:'.dicaF().'</td><td width="100%" class="realce">'.link_indicador($obj->objetivo_indicador).'</td></tr>';
if ($obj->objetivo_oque) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('O Que Fazer', 'Sumário sobre o que se trata '.($config['genero_objetivo']=='o' ? 'este' : 'esta').' '.$config['objetivo'].'.').'O Que:'.dicaF().'</td><td class="realce" width="100%">'.$obj->objetivo_oque.'</td></tr>';
if ($obj->objetivo_porque) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Por Que Fazer', 'Por que '.$config['genero_objetivo'].' '.$config['objetivo'].' será executad'.$config['genero_objetivo'].'.').'Por que:'.dicaF().'</td><td class="realce" width="100%">'.$obj->objetivo_porque.'</td></tr>';
if ($obj->objetivo_onde) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Onde Fazer', 'Onde '.$config['genero_objetivo'].' '.$config['objetivo'].' é executad'.$config['genero_objetivo'].'.').'Onde:'.dicaF().'</td><td class="realce" width="100%">'.$obj->objetivo_onde.'</td></tr>';
if ($obj->objetivo_quando) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Quando Fazer', 'Quando '.$config['genero_objetivo'].' '.$config['objetivo'].' é executad'.$config['genero_objetivo'].'.').'Quando:'.dicaF().'</td><td class="realce" width="100%">'.$obj->objetivo_quando.'</td></tr>';
if ($obj->objetivo_como) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Como Fazer', 'Como '.$config['genero_objetivo'].' '.$config['objetivo'].' é executad'.$config['genero_objetivo'].'.').'Como:'.dicaF().'</td><td class="realce" width="100%">'.$obj->objetivo_como.'</td></tr>';
if ($obj->objetivo_quanto) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Quanto Custa', 'Custo para executar '.$config['genero_objetivo'].' '.$config['objetivo'].'.').'Quanto:'.dicaF().'</td><td class="realce" width="100%">'.$obj->objetivo_quanto.'</td></tr>';
if ($obj->objetivo_quem) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Quem', 'Quais '.$config['usuarios'].' estarão executando '.($config['genero_objetivo']=='o' ? 'este' : 'esta').' '.$config['objetivo'].'.').'Quem:'.dicaF().'</td><td width="100%" class="realce">'.$obj->objetivo_quem.'</td></tr>';
...
if ($saida_projeto) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Projeto', 'Qual projeto está relacionado a '.($config['genero_objetivo']=='o' ? 'este' : 'esta').' '.$config['objetivo'].'.').'Projeto:'.dicaF().'</td><td width="100%" colspan="2" class="realce">'.$saida_projeto.'</td></tr>';
		
		


if ($obj->objetivo_desde_quando) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Desde Quando é Feita', 'Desde quando '.'o'.' '.$config['objetivo'].' é executad'.'o'.'.').'Desde quando:'.dicaF().'</td><td class="realce" width="100%">'.$obj->objetivo_desde_quando.'</td></tr>';
if ($obj->objetivo_controle) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Método de Controle', 'Como '.'o'.' '.$config['objetivo'].' é controlad'.'o'.'.').'Controle:'.dicaF().'</td><td class="realce" width="100%">'.$obj->objetivo_controle.'</td></tr>';
if ($obj->objetivo_metodo_aprendizado) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Método de Aprendizado', 'Como é realizado o aprendizado d'.$config['genero_objetivo'].' '.$config['objetivo'].'.').'Aprendizado:'.dicaF().'</td><td class="realce" width="100%">'.$obj->objetivo_metodo_aprendizado.'</td></tr>';
if ($obj->objetivo_melhorias) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Melhorias Efetuadas n'.$config['genero_objetivo'].' '.ucfirst($config['objetivo']).'', 'Quais as melhorias realizadas n'.$config['genero_objetivo'].' '.$config['objetivo'].' após girar o círculo PDCA.').'Melhorias:'.dicaF().'</td><td class="realce" width="100%">'.$obj->objetivo_melhorias.'</td></tr>';
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	if (isset($moedas[$obj->objetivo_moeda])) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Moeda', 'A moeda padrão utilizada.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->objetivo_moeda].'</td></tr>';	
	}
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Aprovado', 'Se '.$config['genero_objetivo'].' '.$config['objetivo'].' se encontra aprovad'.$config['genero_objetivo'].'.').'Aprovad'.$config['genero_objetivo'].':'.dicaF().'</td><td  class="realce" width="100%">'.($obj->objetivo_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';
...
echo '<tr><td align="right" width="100">'.dica('Ativo', 'Se '.($config['genero_objetivo']=='o' ? 'este' : 'esta').' '.$config['objetivo'].' está ativ'.$config['genero_objetivo'].'.').'Ativ'.$config['genero_objetivo'].':'.dicaF().'</td><td style="white-space: nowrap" class="realce">'.($obj->objetivo_ativo ? 'Sim' : 'Não').'</td></tr>';
...
if (!$dialogo) {
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao_item')) {
		...
		$sql->adOnde('plano_acao_item_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'demanda')) {
		...
		$sql->adOnde('demanda_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco_resposta')) {		
		...
		$sql->adOnde('risco_resposta_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('recursos', 'acesso', null, null)) {
		...
		$sql->adOnde('recurso_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->modulo_ativo('patrocinadores') && $Aplic->checarModulo('patrocinadores', 'acesso', null, null)) {
		...
		$sql->adOnde('patrocinador_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'programa')) {
		...
		$sql->adOnde('programa_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'beneficio')) {
		...
		$sql->adOnde('beneficio_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('projeto', 'acesso', 'licao')) {
		...
		$sql->adOnde('licao_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'pratica')) {
		...
		$sql->adOnde('pratica_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('tr') && $Aplic->checarModulo('tr', 'acesso', null, null)) {
		...
		$sql->adOnde('tr_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'tgn')) {
		...
		$sql->adOnde('tgn_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'canvas')) {
		...
		$sql->adOnde('canvas_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('swot') && $Aplic->checarModulo('swot', 'acesso', null, null)) {
		...
		$sql->adOnde('mswot_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('swot') && $Aplic->checarModulo('swot', 'acesso', null, null)) {
		...
		$sql->adOnde('swot_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('operativo') && $Aplic->checarModulo('operativo', 'acesso', null, null)) {
		...
		$sql->adOnde('operativo_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'monitoramento')) {
		...
		$sql->adOnde('monitoramento_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'avaliacao_indicador')) {
		...
		$sql->adOnde('avaliacao_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'checklist')) {
		...
		$sql->adOnde('checklist_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional) {
		...
		$sql->adOnde('agenda_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('agrupamento') && $Aplic->checarModulo('agrupamento', 'acesso', null, null)) {
		...
		$sql->adOnde('agrupamento_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'modelo')) {
		...
		$sql->adOnde('template_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'painel_indicador')) {
		...
		$sql->adOnde('painel_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'odometro_indicador')) {
		...
		$sql->adOnde('painel_odometro_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'composicao_painel')) {
		...
		$sql->adOnde('painel_composicao_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'slideshow_painel')) {
		...
		$sql->adOnde('painel_slideshow_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('agenda', 'acesso', null, null)) {
		...
		$sql->adOnde('calendario_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'perspectiva')) {
		...
		$sql->adOnde('perspectiva_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'tema')) {
		...
		$sql->adOnde('tema_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'objetivo')) {
		$sql->adTabela('objetivo_gestao');
		$sql->adOnde('objetivo_gestao_semelhante = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'me')) {
		...
		$sql->adOnde('me_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($config['exibe_fator'] && $Aplic->checarModulo('praticas', 'acesso', null, 'fator')) {
		...
		$sql->adOnde('fator_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'iniciativa')) {
		...
		$sql->adOnde('estrategia_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'meta')) {
		...
		$sql->adOnde('meta_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'planejamento')) {
		...
		$sql->adOnde('plano_gestao_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'abertura')) {
		...
		$sql->adOnde('projeto_abertura_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'viabilidade')) {
		...
		$sql->adOnde('projeto_viabilidade_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso')) {
		...
		$sql->adOnde('ssti_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso',null, 'laudo')) {
		...
		$sql->adOnde('laudo_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'acesso')) {
		...
		$sql->adOnde('trelo_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'acesso')) {
		...
		$sql->adOnde('trelo_cartao_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso')) {
		...
		$sql->adOnde('pdcl_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso')) {
		...
		$sql->adOnde('pdcl_item_gestao_objetivo = '.(int)$objetivo_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('os') && $Aplic->checarModulo('os', 'acesso')) {
		...
		$sql->adOnde('os_gestao_objetivo = '.(int)$objetivo_id);
