...
echo '<tr><td width="50%"><fieldset><legend class=texto style="color: black;">'.dica(ucfirst($config['iniciativas']).' Disponíveis', 'Lista de '.$config['iniciativas'].' que poderão ser acrescentad'.$config['genero_iniciativa'].'s à composição. Dê um clique duplo em uma das iniciativas nesta lista de seleção para adiciona-la à lista de composição.<BR><BR>Outra opção é selecionar a iniciativa e clicar no botão Adicionar.<BR><BR>Para selecionar múltiplas iniciativas, clique nas mesmas mantendo a tecla CTRL apertada.').'&nbsp;<b>'.ucfirst($config['iniciativas']).' Disponíveis</b>&nbsp</legend>'.dica().'<div id="combo_lista_estrategias">'.selecionaVetor($estrategias, 'lista', 'style="width:100%;" size="15" class="texto" multiple="multiple" ondblclick="Mover()"').'</div></fieldset></td>';
echo '<td width="50%"><fieldset><legend class=texto style="color: black;">&nbsp;'.dica(ucfirst($config['iniciativas']).' Selecionadas','Lista de '.$config['iniciativas'].' selecionad'.$config['genero_iniciativa'].'s que poderão ser removidas da composição. Dê um clique duplo em uma das iniciativas nesta lista de seleção para remove-la.<BR><BR>Outra opção é selecionar a iniciativa e clicar no botão Remover.<BR><BR>Para selecionar múltiplas iniciativas, clique nas mesmas mantendo a tecla CTRL apertada.').'<b>'.ucfirst($config['iniciativas']).' Selecionad'.$config['genero_iniciativa'].'s</b>&nbsp;</legend>'.selecionaVetor($estrategias_selecionadas, 'selecionadas', 'style="width:100%;" size="15" class="texto" multiple="multiple" ondblclick="Remover()"').'</fieldset></td></tr>';
echo '<tr><td colspan="2" align="center"><table width="100%">';
echo '<tr><td align="left"><table cellspacing=0 cellpadding=0><tr><td>'.botao('adicionar', 'Adicionar', 'Utilize este botão para adicionar um '.$config['iniciativa'].' à lista das selecionadas</p>Caso deseje inserir multipl'.$config['genero_iniciativa'].'s '.$config['iniciativas'].' de uma única vez, mantenha o botão <i>CTRL</i> pressionado enquanto clica com o botão esquerdo do mouse nas iniciativas da lista acima.','','Mover()','','',0).'</td></tr></table></td><td>&nbsp;</td><td align="right">'.botao('remover', 'Remover', 'Utilize este botão para retirar um '.$config['iniciativa'].' da lista das selecionadas. </p>Caso deseje remover multiplas iniciativas estratégicas de uma única vez, mantenha o botão <i>CTRL</i> pressionado enquanto clica com o botão esquerdo do mouse nas iniciativas da lista acima.','','Remover()','','',0).'</td></tr>';
