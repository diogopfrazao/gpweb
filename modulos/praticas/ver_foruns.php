...
echo '<th style="white-space: nowrap">'.dica('Assunto', 'Assunto do fórum.').'Assunto'.dicaF().'</th><th>'.dica('Descrição', 'Descrição do fórum.').'Descrição'.dicaF().'</th><th style="white-space: nowrap">'.dica('Número de Mensagens', 'Número de mensagens contidas neste fórum.').'Msg'.dicaF().'</th><th style="white-space: nowrap">'.dica('Última Postagem', 'Última postagem de mensagem neste fórum.').'Última Postagem'.dicaF().'</th></tr>';
...
foreach ($rc as $linha)	{  
	...
	echo '<tr><td style="white-space: nowrap" align="center">'.( $linha["forum_dono"] == $Aplic->usuario_id ? '<a href="javascript:void(0);" onclick="url_passar(0, \'m=foruns&a=editar&forum_id='.$linha['forum_id'].'\');">'.imagem('icones/editar.gif', 'Editar', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o fórum.').'</a>' : '').'</td>';
