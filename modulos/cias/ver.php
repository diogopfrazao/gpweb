...
$cia_id = ($Aplic->getEstado('cia_id') !== null ? $Aplic->getEstado('cia_id') : null);
...
$editar=permiteEditarCia($obj->cia_acesso, $cia_id);

$podeEditarTudo=($obj->cia_acesso>=5 ? $editar && (in_array($obj->cia_responsavel, $Aplic->usuario_lista_grupo_vetor) || $Aplic->usuario_super_admin || $Aplic->usuario_admin) : $editar);


if (getParam($_REQUEST, 'superior', 0) && $podeEditar && $podeEditarTudo && ($Aplic->usuario_super_admin || ($cia_id==$Aplic->usuario_cia && $Aplic->usuario_admin))){
	$sql->adTabela('cias');
...
if (!$dialogo && !$Aplic->profissional) {
	...
	if (($podeEditar && $podeEditarTudo) || $Aplic->usuario_super_admin || ($cia_id==$Aplic->usuario_cia && $Aplic->usuario_admin)) {
...
if (!$dialogo && $Aplic->profissional){	
		...
		if (($podeEditar && $editar) || $podeAdicionar || $Aplic->usuario_super_admin || ($cia_id==$Aplic->usuario_cia && $Aplic->usuario_admin))	$km->Add("root","inserir",dica('Inserir','Menu de opções').'Inserir'.dicaF(), "javascript: void(0);'");
		...
		if ($podeEditar && ($podeEditarTudo || $Aplic->usuario_super_admin || $Aplic->usuario_admin)) {
			...
			if (!(isset($config['restrito']) && $config['restrito']) && $podeExcluir && $Aplic->usuario_super_admin && ($obj->cia_id!=$Aplic->usuario_cia) && ($obj->cia_superior!=$obj->cia_id)) $km->Add("acao","acao_excluir",dica('Excluir','Excluir '.($config['genero_organizacao']=='a' ? 'esta' : 'este').' '.$config['organizacao'].'.').'Excluir '.ucfirst($config['organizacao']).dicaF(), "javascript: void(0);' onclick='excluir()");
			if (!(isset($config['restrito']) && $config['restrito']) && $podeExcluir && $Aplic->usuario_super_admin && ($obj->cia_superior!=$obj->cia_id)) $km->Add("acao","acao_superior",dica('Tornar Superior','Clique neste botão para que est'.($config['genero_organizacao']=='o' ? 'e' : 'a').' '.$config['organizacao'].' se torna '.$config['genero_organizacao'].' primeir'.$config['genero_organizacao'].' no organograma. '.ucfirst($config['genero_organizacao']).' que atualmente ocupa '.$config['genero_organizacao'].' primeir'.$config['genero_organizacao'].' posição será imediatamente subordinad'.$config['genero_organizacao'].' a '.($config['genero_organizacao']=='a' ? 'esta' : 'este').'.').'Tornar '.($config['genero_organizacao']=='a' ? 'esta' : 'este').' '.ucfirst($config['organizacao']).' Superior'.dicaF(), "javascript: void(0);' onclick='url_passar(1, \"m=cias&a=ver&superior=1&cia_id=".(int)$cia_id."\");");
			$km->Add("acao","acao_brasao",dica('Logotipo d'.$config['genero_organizacao'].' '.ucfirst($config['organizacao']), 'Clique neste link para incluir ou alterar o logotipo d'.$config['genero_organizacao'].' '.$config['organizacao'].'.').'Logotipo d'.$config['genero_organizacao'].' '.ucfirst($config['organizacao']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=cias&a=logo&cia_id=".$cia_id."\");");
			}
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nome d'.$config['genero_organizacao'].' '.$config['organizacao'], 'Tod'.$config['genero_organizacao'].' '.$config['organizacao'].' tem um nome exclusivo e obrigatório.').'Nome:'.dicaF() .'</td><td class="realce" width="100%">'.$obj->cia_nome_completo.'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Abreviatura d'.$config['genero_organizacao'].' '.$config['organizacao'], 'Tod'.$config['genero_organizacao'].' '.$config['organizacao'].' tem uma abreviatura.').'Abreviatura:'.dicaF() .'</td><td class="realce" width="100%">'.$obj->cia_nome.'</td></tr>';
if ($obj->cia_superior) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['organizacao']).' Superior', ucfirst($config['genero_organizacao']).' '.$config['organizacao'].' a qual esta é subordinada.').ucfirst($config['organizacao']).' superior:'.dicaF() .'</td><td class="realce" width="100%">'.link_cia($obj->cia_superior).'</td></tr>';
if ($obj->cia_responsavel) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Responsável pel'.$config['genero_organizacao'].' '.$config['organizacao'], 'Tod'.$config['genero_organizacao'].' '.$config['organizacao'].' deve ter um responsável.<br><br>Embora não tenha impacto no funcionamento do Sistema, facilita a organização e na eventual necessidade de entrar em contato.').'Responsável:'.dicaF() .'</td><td class="realce" width="100%">'.link_contato($obj->cia_responsavel, '','','esquerda').'</td></tr>';
...
if ($obj->cia_cnpj) echo '<tr><td align="right" style="white-space: nowrap">'.dica('CNPJ', 'CNPJ d'.$config['genero_organizacao'].' '.$config['organizacao'].'.').'CNPJ:'.dicaF().'</td><td class="realce" width="100%">'.$obj->cia_cnpj.'</td></tr>';
if ($obj->cia_codigo) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Código', 'Código d'.$config['genero_organizacao'].' '.$config['organizacao'].'.').'Código:'.dicaF().'</td><td class="realce" width="100%">'.$obj->cia_codigo.'</td></tr>';
if ($obj->cia_email) echo '<tr><td align="right" style="white-space: nowrap">'.dica('E-mail', 'E-mail d'.$config['genero_organizacao'].' '.$config['organizacao'].'.<br><br>Embora não tenha impacto no funcionamento do Sistema, facilita a organização e na eventual necessidade de entrar em contato.').'E-mail:'.dicaF().'</td><td class="realce" width="100%">'.$obj->cia_email.'</td></tr>';
if ($obj->cia_tel1) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Telefone', 'Telefone d'.$config['genero_organizacao'].' '.$config['organizacao'].'.<br><br>Embora não tenha impacto no funcionamento do Sistema, facilita a organização e na eventual necessidade de entrar em contato.').'Telefone:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->cia_tel1.'</td></tr>';
if ($obj->cia_tel2) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Telefone 2', 'Telefone alternativo d'.$config['genero_organizacao'].' '.$config['organizacao'].'.<br><br>Embora não tenha impacto no funcionamento do Sistema, facilita a organização e na eventual necessidade de entrar em contato.').'Telefone 2:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->cia_tel2.'</td></tr>';
if ($obj->cia_fax) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Fax', 'Fax d'.$config['genero_organizacao'].' '.$config['organizacao'].'.<br><br>Embora não tenha impacto no funcionamento do Sistema, facilita a organização e na eventual necessidade de entrar em contato.').'Fax:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->cia_fax.'</td></tr>';
if ($obj->cia_endereco1) echo '<tr valign="top"><td align="right" style="white-space: nowrap">'.dica('Endereço', 'O enderço d'.$config['genero_organizacao'].' '.$config['organizacao'].'.<br><br>Embora não tenha impacto no funcionamento do Sistema, facilita a organização e na eventual necessidade de entrar em contato.').'Endereço:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.dica('Google Maps', 'Clique esta imagem para visualizar no Google Maps, aberto em uma nova janela, o endereço d'.$config['genero_organizacao'].' '.$config['organizacao'].'.').'<a href="'.get_protocol().'maps.google.com/maps?key='.$config['google_map_chave'].'&q='.utf8_encode($obj->cia_endereco1).'+'.utf8_encode($obj->cia_endereco2).'+'.utf8_encode($endereco['municipio_nome']).'+'.utf8_encode($obj->cia_estado).'+'.utf8_encode($obj->cia_cep).'+'.utf8_encode($obj->cia_pais).'" target="_blank"><img align="right" border=0 src="'.acharImagem('google_map.png').'" alt="Achar no Google Maps" /></a>'.dicaF().$obj->cia_endereco1.(($obj->cia_endereco2) ? '<br />'.$obj->cia_endereco2 : '').($obj->cia_pais ? ' - '.$paises[$obj->cia_pais] : '').(($obj->cia_cep) ? '<br />'.$obj->cia_cep : '').'</td></tr>';
...
if ($obj->cia_url) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Página Web d'.$config['genero_organizacao'].' '.$config['organizacao'], 'A página na internet d'.$config['genero_organizacao'].' '.$config['organizacao'].'.<br><br>Embora não tenha impacto no funcionamento do Sistema, facilita a organização e na eventual necessidade de entrar em contato.').'Página:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.dica('Página Web', 'Clique neste endereço para visualizar a página web dest'.($config['genero_organizacao']=='o' ? 'e' : 'a').' '.$config['organizacao'].'.').'<a href="'.get_protocol().$obj->cia_url.'" target="Cia">'.$obj->cia_url.'</a>'.dicaF().'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Tipo de '.$config['organizacao'], 'Embora não tenha impacto no funcionamento do Sistema, facilita a organização ao separar as OM por tipo.').'Tipo:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$tipos[$obj->cia_tipo].'</td></tr>';
//echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nível de Acesso', ucfirst($config['genero_organizacao']).' '.$config['organizacao'].' pode ter três níveis de acesso:<ul><li><b>Público</b> - Todos podem ver e editar.</li><li><b>Protegido I</b> - Todos podem ver e o responsável junto com os integrantes editar.</li><li><b>Protegido II</b> - Todos podem ver, porem apenas o responsável pode editar tudo e os designados editarem os objetos relacionados.</li><li><b>Protegido III</b> - Todos podem ver e o responsável editar.</li><li><b>Participantes II</b> - Somente o responsável e os designados podem ver e apenas o responsável pode editar tudo e os designados editarem os objetos relacionados.</li><li><b>Participantes III</b> - Todos d'.$config['genero_organizacao'].' '.$config['organizacao'].' podem ver e o responsável pel'.$config['genero_organizacao'].' mesm'.$config['genero_organizacao'].' ver e editar.</li></ul>').'Nível de acesso:'.dicaF().'</td><td width="100%" class="realce">'.(isset($acesso[$obj->cia_acesso]) ? $acesso[$obj->cia_acesso] : '').'</td></tr>';
if ($obj->cia_ug) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Código Principal da Unidade Gestora', 'Para os órgãos do Governo Federal é um código de 6 algarismos.').'UASG Principal'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->cia_ug.'</td></tr>';
if ($obj->cia_ug2) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Código Secundário da Unidade Gestora', 'Para os órgãos do Governo Federal é um código de 6 algarismos.').'UASG Secundário'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->cia_ug2.'</td></tr>';
if ($obj->cia_nup) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Identificador d'.$config['genero_organizacao'].' '.$config['organizacao'].' para NUP', 'Caso utilize o sistema único e processos faz-se necessário informar o número identificador d'.$config['genero_organizacao'].' '.$config['organizacao'].' de 5 algarismos.').'Identificador de NUP'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->cia_nup.'</td></tr>';
if ($obj->cia_prefixo) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Prefixo','O prefixo à numeração sequencial crescente, nos protocolos diversos de NUP.').'Prefixo'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->cia_prefixo.'</td></tr>';
if ($obj->cia_sufixo) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Sufixo','O sufixo à numeração sequencial crescente, nos protocolos diversos de NUP.').'Sufixo'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->cia_sufixo.'</td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Ativ'.$config['genero_organizacao'], ucfirst($config['genero_organizacao']).' '.$config['organizacao'].' se encontra ativ'.$config['genero_organizacao'].'.').'Ativ'.$config['genero_organizacao'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.($obj->cia_ativo ? 'Sim' : 'Não').'</td></tr>';
