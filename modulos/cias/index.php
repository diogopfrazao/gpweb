...
if (!$dialogo){
	$selecionar_tipos='<tr><td align=right>'.dica('Tipo', 'Pesquisar as '.$config['organizacoes'].' pelo tipo das mesmas.').'Tipo:'.dicaF().'</td><td>'.selecionaVetor($tipos, 'tipoCia', 'size="1" class="texto" style="width:250px;" onChange="enviar();"', $tipoCia).'</td></tr>';
	$pesquisar='<tr><td align=right>'.dica('Pesquisar', 'Pesquisar as '.$config['organizacoes'].' pelo campo texto à direita.').'Pesquisar:'.dicaF().'</td><td>'.'<input class="texto" type="text" style="width:250px;" name="procurar_string" value="'.$procurar_string.'" /></td><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=cias&a=index&procurar_string=-1\');">'.imagem('icones/limpar_p.gif','Limpar Pesquisa', 'Clique neste ícone '.imagem('icones/limpar_p.gif').' para limpar a caixa texto de pesquisa.').'</a></td></tr>';
	$procurar_om='<tr><td align=right>'.dica('Filtrar pel'.$config['genero_organizacao'].' '.ucfirst($config['organizacao']), 'Filtrar pel'.$config['genero_organizacao'].' '.$config['organizacao'].' selecionada.').ucfirst($config['organizacao']).':'.dicaF().'</td><td><div id="combo_cia">'.selecionar_om($cia_id, 'cia_id', 'class=texto size=1 style="width:250px;" onchange="javascript:mudar_om();"', '&nbsp;').'</div></td><td><a href="javascript:void(0);" onclick="enviar();">'.imagem('icones/filtrar_p.png','Filtrar','Clique neste ícone '.imagem('icones/filtrar_p.png').' para aplicar o filtro.').'</a></td></tr>';
	}
...
if (!$dialogo && $Aplic->profissional){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$novo= ($podeEditar && $Aplic->usuario_super_admin ? '<tr><td style="white-space: nowrap">'.dica('Nov'.$config['genero_organizacao'].' '.$config['organizacao'], 'Criar uma nov'.$config['genero_organizacao'].' '.$config['organizacao'].' no Sistema.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=cias&a=editar\');" ><img src="'.acharImagem('organizacao_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	$imprimir='<tr><td>'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para abrir uma nova janela onde poderá imprimir a lista de '.$config['organizacoes'].'.').'<a href="javascript: void(0)" onclick="url_passar(1, \'m='.$m.'&a='.$a.'&u='.$u.'&dialogo=1&tab='.$tab.'\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';

	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.($tab!=3 ? $pesquisar.$selecionar_tipos : '').'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$novo.$imprimir.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif (!$dialogo && !$Aplic->profissional){
	...
	if ($podeEditar && $Aplic->usuario_super_admin) $botoesTitulo->adicionaCelula('<table width=75><tr><td style="white-space: nowrap">'.dica('Nov'.$config['genero_organizacao'].' '.$config['organizacao'], 'Criar uma nov'.$config['genero_organizacao'].' '.$config['organizacao'].' no Sistema.').'<a class="botao" href="javascript:void(0);" onclick="url_passar(0, \'m=cias&a=editar\');" ><span>nov'.$config['genero_organizacao'].'</span></a>'.dicaF().'</td></tr></table>');
	$botoesTitulo->adicionaCelula(dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para abrir uma nova janela onde poderá imprimir a lista de '.$config['organizacoes'].'.').'<a href="javascript: void(0)" onclick="url_passar(1, \'m='.$m.'&a='.$a.'&u='.$u.'&dialogo=1&tab='.$tab.'\');">'.imagem('imprimir_p.png').'</a>'.dicaF());
	$botoesTitulo->mostrar();
	}
