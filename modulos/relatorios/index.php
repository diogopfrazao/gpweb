...
if (isset($_REQUEST['projeto_plano_gestao'])) $Aplic->setEstado('projeto_plano_gestao', getParam($_REQUEST, 'projeto_plano_gestao', null));
$projeto_plano_gestao = $Aplic->getEstado('projeto_plano_gestao', null);
...
if (count($vetor_favoritos)) {
	...
	$favoritos='<tr><td align="right" style="white-space: nowrap">'.dica('Favorito', 'Escolha um favorito.').'Favorito:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($vetor_favoritos, 'favorito_id', 'class="texto"'.($Aplic->profissional ? ' multiple' :'').' style="width:250px;"', $favorito_id).'</td></tr>';
	}
...
if ((!$dialogo) && $Aplic->profissional){
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'projetos_lista')) $tipos['popProjeto2']=ucfirst($config['projeto']);
...
if(!$dialogo){
	...
	$planejamento='<tr ><td align="right" style="white-space: nowrap">'.dica('Planejamento Estratégico', 'O planejamento estratégico que contenha '.$config['genero_projeto'].'s '.$config['projetos'].'.').'Planejamento estratégico:'.dicaF().'</td><td><table cellspacing=0 cellpadding=0><tr><td><input type="hidden" name="projeto_plano_gestao" value="'.$projeto_plano_gestao.'" /><input type="text" id="plano_gestao_nome" name="plano_gestao_nome" value="'.nome_plano_gestao($projeto_plano_gestao).'" style="width:250px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popPlanejamento2();">'.imagem('icones/planogestao_p.png','Selecionar Planejamento Estratégico','Clique neste ícone '.imagem('icones/planogestao_p.png').' para selecionar um planejamento estratégico.').'</a></td></tr></table></td></tr>';
	...
	$mudar_projeto='<tr><td align=right>'.dica('Filtrar pel'.$config['genero_projeto'].' '.ucfirst($config['projeto']), 'Filtrar arquivos pel'.$config['genero_projeto'].' '.$config['projeto'].' que estão relacionados.').ucfirst($config['projeto']).':'.dicaF().'</td><td><input type="text" id="nome" name="nome" value="'.nome_projeto($projeto_id).'" style="width:250px;" class="texto" READONLY /></td><td><a href="javascript: void(0);" onclick="popProjeto();">'.imagem('icones/projeto_p.gif','Selecionar '.ucfirst($config['projeto']),'Clique neste ícone '.imagem('icones/projeto_p.gif').' para selecionar um'.($config['genero_projeto']=='a' ? 'a' : '').' '.$config['projeto'].'.').'</a></td></tr>';
	...
	if ($Aplic->profissional){
	  ...
	  $saida.='<table cellspacing=0 cellpadding=0>';
		...
		$saida.='<tr><td><table cellspacing=0 cellpadding=0>'.$procura_setor.$procura_segmento.$procura_intervencao.$procura_tipo_intervencao.$procurar_estado.$procurar_municipio.$procurar_status.$popFiltro.$filtros.$planejamento.'</table></td>
		<td><table cellspacing=0 cellpadding=0>'.$procurar_om.$mudar_projeto.$procurar_responsavel.$procurar_supervisor.$procurar_autoridade.$procurar_designado.$procura_categoria.$procura_pesquisa.$botao_periodo.$favoritos.'</table></td>
...
if ($dialogo && $self_print && !$Aplic->pdf_print && !($Aplic->usuario_nomeguerra=='Visitante' && $Aplic->usuario_id=1)) echo '<script>self.print();</script>';
...
if ($relatorio_tipo) {
	...
	if ($Aplic->profissional){
		...
		if ($Aplic->modulo_ativo('financeiro')) echo '<tr><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=relatorios&a=index&projeto_id='.$projeto_id.'&relatorio_tipo=financeiro_siafi_pro\');">'.dica('Estágios da Despesa vs Anexos d'.$config['genero_siafi'].' '.$config['siafi'],'Planilha correlacionando as fases da despesa com os arquivos anexados de busca n'.$config['genero_siafi'].' '.$config['siafi'].'.').'Estágios da despesa vs anexos d'.$config['genero_siafi'].' '.$config['siafi'].'</a></td><td>Planilha correlacionando as fases da despesa com os arquivos anexados de busca n'.$config['genero_siafi'].' '.$config['siafi'].'</td></tr>';
		if ($Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso',null,'ssti_rap')) {
			echo '<tr><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=relatorios&a=rap_ssti_pro\');">'.dica('Relatório de Acompanhamento de Projetos - RAP','Relatório de Acompanhamento de Projetos.').'Relatório de Acompanhamento de Projetos - RAP</a></td><td>Planilha informando o detalhamento dos projetos advindos de '.strtoupper($config['ssti']).'.</td></tr>';
			//echo '<tr><td><a href="javascript:void(0);" onclick="url_passar(0, \'m=relatorios&a=ssti1_pro\');">'.dica('Relatório de Indicadores de Desempenho e Qualidade','Relatório de indicadores de desempenho e qualidade.').'Relatório de indicadores de desempenho e qualidade</a></td><td>Planilha informando os indicadores de desempenho e qualidade advindos de '.strtoupper($config['ssti']).'.</td></tr>';
...
function popPlanejamento2() {
		if (window.parent.gpwebApp) parent.gpwebApp.popUp('Planejamento Estratégico', 1000, 700, 'm=praticas&u=gestao&a=gestao_lista&dialogo=1&selecao=2&chamarVolta=setPlanejamento&tabela=plano_gestao&cia_id='+document.getElementById('cia_id').value, window.setPlanejamento2, window);
		else window.open('./index.php?m=publico&a=selecionar_multiplo&dialogo=1&chamar_volta=setPlanejamento2&tabela=plano_gestao&cia_id='+document.getElementById('cia_id').value, 'Planejamento Estratégico','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
		}

	function setPlanejamento2(chave, valor){
...
function popProjeto2() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["projeto"])?>', 1000, 700, 'm=projetos&a=index&dialogo=1&selecao=2&chamarVolta=setProjeto&aceita_portfolio=1&tabela=projetos&cia_id='+document.getElementById('cia_id').value, window.setProjeto2, window);
	else window.open('./index.php?m=publico&a=selecionar_multiplo&dialogo=1&chamar_volta=setProjeto&tabela=projetos&aceita_portfolio=1&cia_id='+document.getElementById('cia_id').value, '<?php echo ucfirst($config["projeto"])?>','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
	}
