...
$data = new CData();	
$titulo=$data->format('%d/%m/%Y').' - '.ucfirst($config['tarefas']).' a serem concluíd'.$config['genero_tarefa'].'s nos próximos sete dias'.($projeto_id  ? ' n'.$config['genero_projeto'].' '.$config['projeto'].' '.$mostrarNomeProjeto : ' em tod'.$config['genero_projeto'].'s '.$config['genero_projeto'].'s '.$config['projetos']);
if (!$dialogo){
	$Aplic->salvarPosicao();
