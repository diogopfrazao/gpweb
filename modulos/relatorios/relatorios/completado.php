...
$data = new CData();	
$titulo=ucfirst($config['tarefas']).' concluíd'.$config['genero_tarefa'].'s nos últimos sete dias'.($projeto_id  ? ' n'.$config['genero_projeto'].' '.$config['projeto'].' '.$mostrarNomeProjeto : ' em tod'.$config['genero_projeto'].'s '.$config['genero_projeto'].'s '.$config['projetos']).' - '.$data->format('%d/%m/%Y');
if (!$dialogo){
	$Aplic->salvarPosicao();
