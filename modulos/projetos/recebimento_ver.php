...
$podeEditarTudo=($obj->projeto_recebimento_acesso>=5 ? $editar && (in_array($obj->projeto_recebimento_responsavel, $Aplic->usuario_lista_grupo_vetor) || $Aplic->usuario_super_admin || $Aplic->usuario_admin) : $editar);

if (!($podeAcessar && $acessar)) $Aplic->redirecionar('m=publico&a=acesso_negado'); 
...
if ($Aplic->profissional){
	...
	if ($podeAdicionar && $editar)	$km->Add("root","inserir",dica('Inserir','Menu de opções').'Inserir'.dicaF(), "javascript: void(0);'");
	if ($podeAdicionar && $editar)	$km->Add("inserir","inserir_objeto",dica('Inserir Produtos/Serviços','Inserir produtos ou serviços.').'Inserir Produtos/Serviços'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=recebimento_produtos&projeto_recebimento_id=".$projeto_recebimento_id."\");");
	
	
	$km->Add("root","acao",dica('Ação','Menu de ações.').'Ação'.dicaF(), "javascript: void(0);'");
	if ($podeEditarTudo && $podeEditar) $km->Add("acao","editar_recebimento",dica('Editar Recebimento de Produtos/Serviços','Editar os detalhes do recebimento de produtos/serviços.').'Editar Recebimento de Produtos/Serviços'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=recebimento_editar&projeto_recebimento_id=".$obj->projeto_recebimento_id."&projeto_id=".$obj->projeto_recebimento_projeto."\");");
	if ($podeEditarTudo && $podeExcluir) $km->Add("acao","acao_excluir",dica('Excluir','Excluir este recebimento de produtos/serviços.').'Excluir Recebimento de Produtos/Serviços'.dicaF(), "javascript: void(0);' onclick='excluir()");
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Número', 'Número desta entegra de produtos/serviços').'Número:'.dicaF().'</td><td style="border: outset #d1d1cd 1px;background-color:#'.$obj->projeto_recebimento_cor.'"><font color="'.melhorCor($obj->projeto_recebimento_cor).'"><b>'.($obj->projeto_recebimento_numero<100 ? '0' : '').($obj->projeto_recebimento_numero<10 ? '0' : '').$obj->projeto_recebimento_numero.'<b></font></td></tr>';
...
if ($obj->projeto_recebimento_tarefa) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['tarefa']).' Relacionada', 'Esta entrega de produtos/serviços é específica de um'.($config['genero_tarefa']=='a' ?  'a' : '').' '.$config['tarefa'].' d'.$config['genero_projeto'].' '.$config['projeto'].'.').ucfirst($config['tarefa']).':'.dicaF().'</td><td align="left" class="realce" width="100%">'.link_tarefa($obj->projeto_recebimento_tarefa).'</td></tr>';
...
if ($obj->projeto_recebimento_responsavel) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Responsável pela Entrega', 'Todo produtos/serviços deve ter um responsável pela entrega.').'Quem entrega:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($obj->projeto_recebimento_responsavel, '','','esquerda').'</td></tr>';		
if ($obj->projeto_recebimento_cliente) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Responsável pelo Recebimento', 'Todo produtos/serviços deve ter um responsável pelo recebimento.').'Quem recebe:'.dicaF().'</td><td class="realce" width="100%">'.link_contato($obj->projeto_recebimento_cliente, '','','esquerda').'</td></tr>';	
if ($obj->projeto_recebimento_autoridade) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Aprovou', 'Quem aprovou esta entrega.').'Quem aprovou:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($obj->projeto_recebimento_autoridade, '','','esquerda').'</td></tr>';		
...
if ($saida_quem) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Equipe', 'Quais '.$config['usuarios'].' estarão na equipe desta entrega de produtos/serviço.').'Equipe:'.dicaF().'</td><td width="100%" colspan="2" class="realce">'.$saida_quem.'</td></tr>';


				
if ($obj->projeto_recebimento_data_prevista) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data Prevista', 'A data prevista para a entrega dos produtos/serviços').'Data prevista:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.retorna_data($obj->projeto_recebimento_data_prevista, false).'</td></tr>';
if ($obj->projeto_recebimento_data_entrega) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data da Entrega', 'A data da entrega dos produtos/serviços').'Data da entrega:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.retorna_data($obj->projeto_recebimento_data_entrega, false).'</td></tr>';
if ($obj->projeto_recebimento_data_aprovacao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data da Aprovação', 'A data em que esta entrega foi aprovada.').'Data da aprovação:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.retorna_data($obj->projeto_recebimento_data_entrega, false).'</td></tr>';
