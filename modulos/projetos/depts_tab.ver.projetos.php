...
echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.(isset($a) ? '&a='.$a : '').(isset($extraGet) ? $extraGet : '').'&ordemPor=projeto_percentagem\');" class="hdr">'.dica('Físico Executado', 'Clique para ordenar '.$config['genero_projeto'].'s '.$config['projetos'].' físico executado.').'%'.dicaF().'</a></th>';
...
echo '<th style="white-space: nowrap">'.dica(ucfirst($config['departamento']), strtoupper($config['genero_dept']).' '.strtolower($config['departamento']).' responsável pel'.$config['genero_projeto'].' '.$config['projeto'].'.').$config['departamento'].dicaF().'</th>';
...
foreach ($projetos as $linha) {
	if ($proFiltro == -1 || $linha['projeto_status'] == $proFiltro || ($proFiltro == -2 && $linha['projeto_status'] != 3) || ($proFiltro == -3 && $linha['projeto_ativo'] != 0)) {
		...
		$s .= '<td width="45" align="right">'.number_format($linha['projeto_percentagem'], 2, ',', '.').'</td>';
		...
		$q->limpar();
		$s .= '<td style="white-space: nowrap" align="center">';
		...
		foreach ($depts as $dept_id => $dept_info) {
			...
			$data_fim_atual = intval($linha['projeto_fim_atualizado']) ? new CData($linha['projeto_fim_atualizado']) : null;
			$estilo = (($data_fim_atual > $data_fim) && !empty($data_fim)) ? 'style="color:red; font-weight:bold"' : '';
			$s .= '<div>'.link_dept($dept_info['dept_id']).'</div>';
			}
		$s .= '</td>';
		...
		$s .= '</td>';
		$s .= '<td style="white-space: nowrap" align="center">'.($data_fim_atual ? dica('Data Calculada', 'Clique para visualizar quais '.$config['tarefas'].' estão alterando a data final.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m=tarefas&a=ver&tarefa_id='.$linha['critica_tarefa'].'\');"><span '.$estilo.'>'.$data_fim_atual->format('%d/%m/%Y').'</span></a>'.dicaF() : '').'</td>';
		$s .= '<td align="center">'.($linha['log_corrigir'] ? '<a href="javascript:void(0);" onclick="url_passar(0, \'m=tarefas&a=index&f=all&projeto_id='.$linha['projeto_id'].'\');">'.imagem('icones/aviso.gif', imagem('icones/aviso.gif').' Problema', 'Foi registrado um problema. Clique para visualizar '.$config['genero_projeto'].' '.$config['projeto'].'.').'</a>' : '').'</td>';
		$s .= '<td style="white-space: nowrap">'.link_usuario($linha['projeto_responsavel'],'','','esquerda').'</td>';
		$s .= '<td align="center" style="white-space: nowrap">'.$linha['total_tarefas'].($linha['minhas_tarefas'] ? ' ('.$linha['minhas_tarefas'].')' : '').'</td>';
		$s .= '<td align="left" style="white-space: nowrap">'.$projStatus[$linha['projeto_status']].'</td></tr>';
		echo $s;
		}
	}
