...
$sql->esqUnir('projeto_abertura', 'projeto_abertura', 'projeto_abertura.projeto_abertura_id=projeto_abertura_custo_projeto_abertura');
$sql->adCampo('projeto_abertura_nome');
$sql->adCampo('projeto_abertura_custo.*,((projeto_abertura_custo_quantidade*projeto_abertura_custo_custo)*((100+projeto_abertura_custo_bdi)/100)) AS valor');
$sql->adOnde('projeto_abertura_custo.projeto_abertura_custo_projeto_abertura='.(int)$obj->projeto_abertura_id);
if ($Aplic->profissional && $config['aprova_custo']) $sql->adOnde('projeto_abertura_custo_aprovado = 1');
$sql->adOrdem('projeto_abertura_custo_ordem');	
$linhas=$sql->Lista();
...
echo '<tr>
	...
	<th>'.dica('Quantidade', 'A quantidade projeto_aberturada do ítem').'Qnt.'.dicaF().'</th>
...
$total=array();
$custo=array();
$projeto_abertura_atual=0;
foreach ($linhas as $linha) {
	if ($linha['projeto_abertura_custo_projeto_abertura']!=$projeto_abertura_atual) {
			echo '<tr><td colspan=20>'.$linha['projeto_abertura_nome'].'</td></tr>';
			$projeto_abertura_atual=$linha['projeto_abertura_custo_projeto_abertura'];	
			}
	$nd=($linha['projeto_abertura_custo_categoria_economica'] && $linha['projeto_abertura_custo_grupo_despesa'] && $linha['projeto_abertura_custo_modalidade_aplicacao'] ? $linha['projeto_abertura_custo_categoria_economica'].'.'.$linha['projeto_abertura_custo_grupo_despesa'].'.'.$linha['projeto_abertura_custo_modalidade_aplicacao'].'.' : '').$linha['projeto_abertura_custo_nd'];
	echo '<tr align="center">';
	echo '<td align="left">'.++$qnt.' - '.$linha['projeto_abertura_custo_nome'].'</td>';
	echo '<td align="left">'.($linha['projeto_abertura_custo_descricao'] ? $linha['projeto_abertura_custo_descricao'] : '&nbsp;').'</td>';
	echo '<td style="white-space: nowrap">'.$unidade[$linha['projeto_abertura_custo_tipo']].'</td>';
	echo '<td style="white-space: nowrap">'.number_format($linha['projeto_abertura_custo_quantidade'], 2, ',', '.').'</td>';
	echo '<td align="right" style="white-space: nowrap">'.$moedas[$linha['projeto_abertura_custo_moeda']].' '.number_format($linha['projeto_abertura_custo_custo'], 2, ',', '.').'</td>';
	if ($config['bdi']) echo '<td align="right">'.number_format($linha['projeto_abertura_custo_bdi'], 2, ',', '.').'</td>';

	echo '<td style="white-space: nowrap">'.$nd.'</td>';
	echo '<td align="right" style="white-space: nowrap">'.$moedas[$linha['projeto_abertura_custo_moeda']].' '.number_format($linha['valor'], 2, ',', '.').'</td>';
	
	if (isset($exibir['codigo']) && $exibir['codigo']) echo '<td align="center">'.($linha['projeto_abertura_custo_codigo'] ? $linha['projeto_abertura_custo_codigo'] : '&nbsp;').'</td>';
	if (isset($exibir['fonte']) && $exibir['fonte']) echo '<td align="center">'.($linha['projeto_abertura_custo_fonte'] ? $linha['projeto_abertura_custo_fonte'] : '&nbsp;').'</td>';
	if (isset($exibir['regiao']) && $exibir['regiao']) echo '<td align="center">'.($linha['projeto_abertura_custo_regiao'] ? $linha['projeto_abertura_custo_regiao'] : '&nbsp;').'</td>'; 
	
	
	echo '<td align="left" style="white-space: nowrap">'.link_usuario($linha['projeto_abertura_custo_usuario'],'','','esquerda').'</td>';
	echo '<tr>';
	
	if (isset($custo[$linha['projeto_abertura_custo_moeda']][$nd])) $custo[$linha['projeto_abertura_custo_moeda']][$nd] += (float)$linha['valor'];
	else $custo[$linha['projeto_abertura_custo_moeda']][$nd]=(float)$linha['valor'];
	
	if (isset($total[$linha['projeto_abertura_custo_moeda']])) $total[$linha['projeto_abertura_custo_moeda']]+=$linha['valor'];
	else $total[$linha['projeto_abertura_custo_moeda']]=$linha['valor']; 
