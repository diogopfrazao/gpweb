...
if ($Aplic->profissional) {
	echo '<tr><td colspan=20><table><tr><td>'.dica('Tela', 'Exibir o relatório na tela do navegador.').'<input type="radio" id="tela" name="exibicao" value="tela" checked />Tela'.dicaF().dica('PDF','Enviar relatório para arquivo PDF.').'</td><td><input type="radio" id="pdf" name="exibicao" value="pdf" />PDF'.dicaF().'</td>'.($pode_exportar ? '<td>'.dica('Exportar Link','Cria o hiperlink para acesso externo ao relatório.').'<input type="radio" id="exportar" name="exibicao" value="exportar" />Exportar Link'.dicaF().'</td>' : '').'</tr></table></td></tr>';	
	
	
	
	echo '<tr><td colspan=20><table><tr><td>Rodapé:</td>
	<td>'.dica(ucfirst($config['gerente']), 'Exibir '.$config['gerente'].' para assinar no rodapé do documento.').'<input type="checkbox" id="gerente" name="gerente" value="1" />'.ucfirst($config['gerente']).dicaF().'</td>
	<td>'.dica(ucfirst($config['supervisor']), 'Exibir '.$config['supervisor'].' para assinar no rodapé do documento.').'<input type="checkbox" id="supervisor" name="supervisor" value="1" />'.ucfirst($config['supervisor']).dicaF().'</td>
	<td>'.dica(ucfirst($config['autoridade']), 'Exibir '.$config['autoridade'].' para assinar no rodapé do documento.').'<input type="checkbox" id="autoridade" name="autoridade" value="1" />'.ucfirst($config['autoridade']).dicaF().'</td>
	<td>'.dica(ucfirst($config['cliente']), 'Exibir '.$config['cliente'].' para assinar no rodapé do documento.').'<input type="checkbox" id="cliente" name="cliente" value="1" />'.ucfirst($config['cliente']).dicaF().'</td>
	<td>'.dica('Código de Barras', 'Exibir o cõdigo de barras no rodapé do documento.').'<input type="checkbox" id="barra" name="barra" value="1" />Código de Barra'.dicaF().'</td>
	</tr></table></td></tr>';	
	}
else {
...
if ($Aplic->profissional) {
	...
	$sql->limpar();
	if ($existe) {
		echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=financeiro_pizza_pro&dialogo=1&jquery=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Estágios da despesa (empenho, liquidação e pagamento)</a></td></tr>';
		if ($Aplic->modulo_ativo('financeiro')) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=financeiro_siafi_pro&dialogo=1&jquery=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Estágios da despesa vs anexos d'.$config['genero_siafi'].' '.$config['siafi'].'</a></td></tr>';
		...
		if (!$obj->projeto_portfolio) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=imprimir_inconsistencia_status_pro&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Inconsistência do status d'.$config['genero_tarefa'].'s '.$config['tarefas'].'</a></td></tr>';
		if (!$obj->projeto_portfolio) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=imprimir_execucao_tarefa_pro&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Execução d'.$config['genero_tarefa'].'s '.$config['tarefas'].'</a></td></tr>';
		...
		echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&u=eb&a=cronograma_financeiro_ver&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Cronograma Físico Financeiro</a></td></tr>';
		echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&u=eb&a=dicionario_eap_ver&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Dicionário da EAP</a></td></tr>';
		echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&u=eb&a=estrutura_analitica_ver&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Estrutura Analítica</a></td></tr>';
		}
		
	if ($Aplic->modulo_ativo('financeiro')) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=financeiro_resumo_siafi_pro&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Resumo das informações importadas d'.$config['genero_siafi'].' '.$config['siafi'].'</a></td></tr>';
...
if ($config['anexo_mpog']){
	...
	$linha1=$sql->linha();
	$sql->limpar();
	if (isset($linha1['demanda_id']) && $linha1['demanda_id']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=demanda_imprimir&dialogo=1&demanda_id='.$linha1['demanda_id'].'\');">Documento de Oficialização da Demanda (DOD)</a></td></tr>';
	if (isset($linha1['demanda_caracteristica_projeto']) && $linha1['demanda_caracteristica_projeto']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=demanda_analise_imprimir&dialogo=1&demanda_id='.$linha1['demanda_id'].'\');">Mensuração do Tamanho d'.$config['genero_projeto'].' '.$config['projeto'].' (MTP)</a></td></tr>';
	if (isset($linha1['demanda_viabilidade']) && $linha1['demanda_viabilidade']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=viabilidade_imprimir&dialogo=1&projeto_viabilidade_id='.$linha1['demanda_viabilidade'].'\');">Análise de Viabilidade d'.$config['genero_projeto'].' '.$config['projeto'].' (AVP)</a></td></tr>';
	...
	$linha4=$sql->linha();
	$sql->limpar();
	if (isset($linha4['projeto_comunicacao_usuario']) && $linha4['projeto_comunicacao_usuario']) echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=projetos&a=comunicacao_imprimir&dialogo=1&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Plano de Comunicação (PC)</a></td></tr>';
	...
	echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=relatorios&a=index&dialogo=1&jquery=1&self_print=1&veio_projeto=1&relatorio_tipo=grafico_fisico_pro&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Curva S do físico</a></td></tr>';
	echo '<tr><td colspan=20><a href="javascript: void(0);" onclick="ir_para2(\'m=relatorios&a=index&dialogo=1&jquery=1&self_print=0&veio_projeto=1&relatorio_tipo=grafico_fisico_financeiro_pro&baseline_id='.$baseline_id.'&projeto_id='.$projeto_id.'\');">Curva S do físico e financeiro</a></td></tr>';
