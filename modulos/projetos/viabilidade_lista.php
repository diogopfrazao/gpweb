...
if (isset($_REQUEST['viabilidadetextobusca']))	$Aplic->setEstado('viabilidade_segmento',getParam($_REQUEST, 'viabilidade_segmento', null));
$viabilidade_segmento = $Aplic->getEstado('viabilidade_segmento') !== null ? $Aplic->getEstado('viabilidade_segmento') : '';

if (isset($_REQUEST['viabilidadetextobusca']))	$Aplic->setEstado('viabilidade_intervencao', getParam($_REQUEST, 'viabilidade_intervencao', null));
$viabilidade_intervencao = $Aplic->getEstado('viabilidade_intervencao') !== null ? $Aplic->getEstado('viabilidade_intervencao') : '';

if (isset($_REQUEST['viabilidadetextobusca']))	$Aplic->setEstado('viabilidade_tipo_intervencao', getParam($_REQUEST, 'viabilidade_tipo_intervencao', null));
...
if (!$dialogo || $selecao){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	if ($exibir['projeto_viabilidade_setor']){
		...
		$procura_setor='<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['setor']), 'A qual '.$config['setor'].' perternce a viabilidade.').ucfirst($config['setor']).':'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($setor, 'viabilidade_setor', 'style="width:250px;" class="texto" onchange="mudar_segmento();"', $viabilidade_setor).'</td></tr>';
		$procura_segmento='<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['segmento']), 'A qual '.$config['segmento'].' perternce a viabilidade.').ucfirst($config['segmento']).':'.dicaF().'</td><td width="100%" colspan="2"><div id="combo_segmento">'.selecionaVetor($segmento, 'viabilidade_segmento', 'style="width:250px;" class="texto" onchange="mudar_intervencao();"', $viabilidade_segmento).'</div></td></tr>';
	 	$procura_intervencao='<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['intervencao']), 'A qual '.$config['intervencao'].' perternce a viabilidade.').ucfirst($config['intervencao']).':'.dicaF().'</td><td width="100%" colspan="2"><div id="combo_intervencao">'.selecionaVetor($intervencao, 'viabilidade_intervencao', 'style="width:250px;" class="texto" onchange="mudar_tipo_intervencao();"', $viabilidade_intervencao).'</div></td></tr>';
		$procura_tipo_intervencao='<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['tipo']), 'A qual '.$config['tipo'].' pertence a viabilidade.').ucfirst($config['tipo']).':'.dicaF().'</td><td width="100%" colspan="2"><div id="combo_tipo_intervencao">'.selecionaVetor($tipo_intervencao, 'viabilidade_tipo_intervencao', 'style="width:250px;" class="texto"', $viabilidade_tipo_intervencao).'</div></td></tr>';
		}
	...
	$procuraBuffer = '<tr><td align=right style="white-space: nowrap">'.dica('Pesquisar', 'Pesquisar pelo nome e campos de descrição').'Pesquisar:'.dicaF().'</td><td><input type="text" class="texto" style="width:250px;" name="viabilidadetextobusca" onChange="document.env.submit();" value="'.$pesquisar_texto.'"/></td><td><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.'&tab='.$tab.'&viabilidadetextobusca=\');">'.imagem('icones/limpar_p.gif','Limpar Pesquisa', 'Clique neste ícone '.imagem('icones/limpar_p.gif').' para limpar a caixa texto de pesquisa.').'</a></td></tr>';
	
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir Estudos de Viabilidade', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de estudos de viabilidade.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=projetos&a=viabilidade_lista&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';

	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionado','A qual parte do sistema os estudos de viabilidade estão relacionados.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
...
if ($config['mostrar_total']){
	$total0=' ('.(int)viabilidades_quantidade($a, 0, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $pesquisar_texto, $responsavel, $viabilidade_setor, $viabilidade_segmento, $viabilidade_intervencao, $viabilidade_tipo_intervencao,
	$tarefa_id,
	$projeto_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$total1=' ('.(int)viabilidades_quantidade($a, 1, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $pesquisar_texto, $responsavel, $viabilidade_setor, $viabilidade_segmento, $viabilidade_intervencao, $viabilidade_tipo_intervencao,
	$tarefa_id,
	$projeto_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$total2=' ('.(int)viabilidades_quantidade($a, 2, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $pesquisar_texto, $responsavel, $viabilidade_setor, $viabilidade_segmento, $viabilidade_intervencao, $viabilidade_tipo_intervencao,
	$tarefa_id,
	$projeto_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$total3=' ('.(int)viabilidades_quantidade($a, 3, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $pesquisar_texto, $responsavel, $viabilidade_setor, $viabilidade_segmento, $viabilidade_intervencao, $viabilidade_tipo_intervencao,
	$tarefa_id,
	$projeto_id,
	$pg_perspectiva_id,
	$tema_id,
...
if (!$dialogo || $selecao){
	...
	$caixaTab->adicionar('viabilidade_tabela', 'Demandas'.$total0,null,null,'Demandas','Demandas que ainda sem estudo de viabilidade.');
	...
	$caixaTab->adicionar('viabilidade_tabela', 'Viraram '.ucfirst($config['projetos']).$total3,null,null,'Viraram '.ucfirst($config['projetos']),'Estudos de viabilidade que se transformaram em '.$config['projeto'].'.');
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=<?php echo ($tab!=0 ? "projeto_viabilidades" : "demandas")?>', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=projetos&a=viabilidade_lista');
	}	
