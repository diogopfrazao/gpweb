...
class CRisco extends CAplicObjeto {
  public $projeto_risco_projeto = null;
  public $projeto_risco_usuario = null;
  public $projeto_risco_descricao = null;
  public $projeto_risco_data = null;
	...
	public function armazenar( $atualizarNulos = false) {
		...
		$sql = new BDConsulta();
		if ($_REQUEST['antigo']) {
			$ret = $sql->atualizarObjeto('projeto_risco', $this, 'projeto_risco_projeto');
			$sql->limpar();
			}
		else {
	...
	public function notificar( $post=array()){
		...
		foreach($usuarios as $usuario){
			if (!isset($usado[$usuario['usuario_id']]) && !isset($usado[$usuario['contato_email']])){
				...
				if ($tipo!='excluido') {
					$corpo_interno .= '<br><a href="javascript:void(0);" onclick="url_passar(0, \'m=projetos&a=risco_ver&projeto_risco_projeto='.$this->projeto_risco_projeto.'\');"><b>Clique para acessar o gerenciamento de risco do projeto</b></a>';
					if ($Aplic->profissional){
						require_once BASE_DIR.'/incluir/funcoes_principais_pro.php';
						$endereco=link_email_externo($usuario['usuario_id'], 'm=projetos&a=risco_ver&projeto_risco_projeto='.$this->projeto_risco_projeto);
						$corpo_externo.='<br><a href="'.$endereco.'"><b>Clique para acessar o gerenciamento de risco do projeto</b></a>';
						}
					}
