...
$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_projeto.$procurar_usuario.'</table></td></tr></table>';
$saida.= '</div></div>';
...
var use_cached_version = <?php echo $Aplic->profissional ? 'true' : 'false'?>;
var usuario_id = <?php echo (int)$Aplic->usuario_id ?>;
var usuario_cia = <?php echo (int)$Aplic->usuario_cia ?>;

var label_tarefa  = "<?php echo $config[ 'tarefa' ]; ?>",
    label_tarefas = "<?php echo $config[ 'tarefas' ]; ?>",
    genero_tarefa = "<?php echo $config[ 'genero_tarefa' ]; ?>",
    label_projeto  = "<?php echo $config[ 'projeto' ]; ?>",
    label_projetos = "<?php echo $config[ 'projetos' ]; ?>",
    genero_projeto = "<?php echo $config[ 'genero_projeto' ]; ?>";
...
function popProjeto2() {
  if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["projeto"])?>', 1000, 700, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setProjeto&edicao=1&tabela=projetos&usuario_id='+document.getElementById('usuario_id').value+'&cia_id='+document.getElementById('cia_id').value, window.setProjeto2, window);
	else window.open('./index.php?m=publico&a=selecionar&dialogo=1&chamar_volta=setProjeto2&edicao=1&tabela=projetos&usuario_id='+document.getElementById('usuario_id').value+'&cia_id='+document.getElementById('cia_id').value, '<?php echo ucfirst($config["projeto"])?>','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
	}
