...
if (count($vetor_favoritos)) {
	...
	$favoritos='<tr><td align="right" style="white-space: nowrap">'.dica('Favoritos', 'Escolha um favorito para mostrar '.$config['genero_me'].'s '.$config['me'].' pertencentes.').'Favoritos:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($vetor_favoritos, 'favorito_id', 'class="texto" style="width:250px;" onchange="document.env.submit()"', $favorito_id).'</td></tr>';
	}
...
if (!$dialogo || $selecao){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionada','A qual parte do sistema as demandas estão relacionadas.').'Relacionada:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	if ($exibir['demanda_setor']){
		...
		$procura_setor='<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['setor']), 'A qual '.$config['setor'].' perternce a demanda.').ucfirst($config['setor']).':'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($setor, 'demanda_setor', 'style="width:250px;" class="texto" onchange="mudar_segmento();"', $demanda_setor).'</td></tr>';
		$procura_segmento='<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['segmento']), 'A qual '.$config['segmento'].' perternce a demanda.').ucfirst($config['segmento']).':'.dicaF().'</td><td width="100%" colspan="2"><div id="combo_segmento">'.selecionaVetor($segmento, 'demanda_segmento', 'style="width:250px;" class="texto" onchange="mudar_intervencao();"', $demanda_segmento).'</div></td></tr>';
		...
		$procura_tipo_intervencao='<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['tipo']), 'A qual '.$config['tipo'].' pertence a demanda.').ucfirst($config['tipo']).':'.dicaF().'</td><td width="100%" colspan="2"><div id="combo_tipo_intervencao">'.selecionaVetor($tipo_intervencao, 'demanda_tipo_intervencao', 'style="width:250px;" class="texto"', $demanda_tipo_intervencao).'</div></td></tr>';
		}
	...
	$novo=($Aplic->checarModulo('projetos', 'adicionar', $Aplic->usuario_id, 'demanda') ? '<tr><td style="white-space: nowrap">'.dica('Nova Demanda', 'Criar um nova demanda.').'<a href="javascript: void(0)" onclick="url_passar(0,\'m=projetos&a=demanda_editar\');"><img src="'.acharImagem('demanda_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr><tr><td style="white-space: nowrap"></td></tr>' : '');
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir Demandas', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de demandas.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=projetos&a=demanda_lista&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';

	$botao_favorito=($Aplic->profissional ? '<tr><td align="right" style="white-space: nowrap">'.dica('Favoritos', 'Criar ou editar um grupo de demandas favoritas, para uma rápida filtragem.').'<a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&demanda=1\');"><img src="'.acharImagem('favorito_p.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_responsavel.$procurar_supervisor.$procurar_autoridade.$procurar_cliente.$procura_pesquisa.$procura_setor.$procura_segmento.$procura_intervencao.$procura_tipo_intervencao.$popFiltro.$filtros.$favoritos.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$botao_filtrar.(!$selecao ? $novo.$botao_prioridade.$imprimir.$botao_favorito.$botao_trava.$botao_campos : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
...
if ($config['mostrar_total']){
	$total1=' ('.(int)demandas_quantidade($a, 0, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $filtro_prioridade_demanda, $favorito_id,
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$total2=' ('.(int)demandas_quantidade($a, 1, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $filtro_prioridade_demanda, $favorito_id,
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$total3=' ('.(int)demandas_quantidade($a, 2, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $filtro_prioridade_demanda, $favorito_id,
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$total4=' ('.(int)demandas_quantidade($a, 3, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $filtro_prioridade_demanda,  $favorito_id,
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$total5=' ('.(int)demandas_quantidade($a, 4, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $filtro_prioridade_demanda, $favorito_id,
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$total6=' ('.(int)demandas_quantidade($a, 5, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $responsavel, $supervisor, $autoridade, $cliente, $demanda_setor, $demanda_segmento, $demanda_intervencao, $demanda_tipo_intervencao, $pesquisar_texto, $filtro_prioridade_demanda, $favorito_id,
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
...
if (!$dialogo || $selecao){
	...
	include_once BASE_DIR.'/modulos/'.$m.'/demanda_tabela.php';
 	if (!($Aplic->usuario_nomeguerra=='Visitante' && $Aplic->usuario_id=1)) echo '<script type="text/javascript">self.print();</script>';
	}
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=demandas', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=projetos&a=demanda_lista');
	}	
...
function popResponsavel(campo) {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp("<?php echo ucfirst($config['usuario'])?>", 500, 500, 'm=publico&a=selecao_usuario&dialogo=1&chamar_volta=setResponsavel&cia_id='+document.getElementById('cia_id').value+'&usuarios_id_selecionados='+usuarios_gerente, window.setResponsavel, window);
...
function popRecurso() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Recurso', 1000, 700, 'm=recursos&a=index&dialogo=1&edicao=1&selecao=2&chamarVolta=setRecurso&tabela=recursos&cia_id='+document.getElementById('cia_id').value, window.setRecurso, window);
