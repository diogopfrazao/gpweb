...
$podeEditarTudo=($obj->projeto_mudanca_acesso>=5 ? $editar && (in_array($obj->projeto_mudanca_responsavel, $Aplic->usuario_lista_grupo_vetor) || $Aplic->usuario_super_admin || $Aplic->usuario_admin) : $editar);
...
if ($Aplic->profissional){
	...
	if ($editar && $podeAdicionar)	$km->Add("inserir","inserir_objeto",dica('Inserir Solicitação de Mudanças','Inserir os detalhes da solicitação de mudanças.').'Solicitação de Mudanças'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=mudanca_editar&projeto_id=".$obj->projeto_mudanca_projeto."\");");
	
	$km->Add("root","acao",dica('Ação','Menu de ações.').'Ação'.dicaF(), "javascript: void(0);'");
	
	if ($podeEditarTudo && $podeEditar) $km->Add("acao","editar_mudanca",dica('Editar Solicitação de Mudanças','Editar os detalhes da solicitação de mudanças.').'Editar Solicitação de Mudanças'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=mudanca_editar&projeto_mudanca_id=".$obj->projeto_mudanca_id."&projeto_id=".$obj->projeto_mudanca_projeto."\");");
	if ($podeEditarTudo && $podeExcluir) $km->Add("acao","acao_excluir",dica('Excluir','Excluir esta solicitação de mudanças.').'Excluir Solicitação de Mudanças'.dicaF(), "javascript: void(0);' onclick='excluir()");
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Número', 'Número desta entegra').'Número:'.dicaF().'</td><td style="border: outset #d1d1cd 1px;background-color:#'.$obj->projeto_mudanca_cor.'"><font color="'.melhorCor($obj->projeto_mudanca_cor).'"><b>'.($obj->projeto_mudanca_numero<100 ? '0' : '').($obj->projeto_mudanca_numero<10 ? '0' : '').$obj->projeto_mudanca_numero.'<b></font></td></tr>';
if ($obj->projeto_mudanca_tarefa) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['tarefa']).' Relacionada', 'Esta solicitação de mudanças é específica de um'.($config['genero_tarefa']=='a' ?  'a' : '').' '.$config['tarefa'].' d'.$config['genero_projeto'].' '.$config['projeto'].'.').ucfirst($config['tarefa']).':'.dicaF().'</td><td align="left" class="realce" width="100%">'.link_tarefa($obj->projeto_mudanca_tarefa).'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Decisão do Requisitante', 'A requisição de mudança foi aprovada ou não pelo requisitante.').'Decisão do Requisitante:'.dicaF().'</td><td class="realce" width="100%">'.($obj->projeto_mudanca_requisitante_aprovada ? 'Aprovada' : 'Reprovada').'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Decisão da Administração', 'A requisição de mudança foi aprovada ou não pela administração.').'Decisão da administração:'.dicaF().'</td><td class="realce" width="100%">'.($obj->projeto_mudanca_administracao_aprovada ? 'Aprovada' : 'Reprovada').'</td></tr>';
if ($obj->projeto_mudanca_cliente) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Demandante', 'O contato que solicitou a mudança.').'Demandante:'.dicaF().'</td><td class="realce" width="100%">'.link_contato($obj->projeto_mudanca_cliente, '','','esquerda').'</td></tr>';	

if ($obj->projeto_mudanca_responsavel) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Responsável', 'Responsável por executar a mudança.').'Responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($obj->projeto_mudanca_responsavel, '','','esquerda').'</td></tr>';		
if ($obj->projeto_mudanca_autoridade) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Autoridade', 'Autoridade que aprova a solicitação de mudança').'Autoridade:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($obj->projeto_mudanca_autoridade, '','','esquerda').'</td></tr>';		
...
if ($saida_quem) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Equipe', 'Quais '.$config['usuarios'].' estarão na equipe desta solicitação de mudança.').'Equipe:'.dicaF().'</td><td width="100%" colspan="2" class="realce">'.$saida_quem.'</td></tr>';

if ($obj->projeto_mudanca_data) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data da Solicitação', 'A data desta solicitação de mudança').'Data da solicitação:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.retorna_data($obj->projeto_mudanca_data, false).'</td></tr>';
if ($obj->projeto_mudanca_data_aprovacao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data da Aprovação', 'A data em que a solicitaçaõ de mudança foi aprovada pela autoridade.').'Data da aprovação:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.retorna_data($obj->projeto_mudanca_data_aprovacao
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Necessidade/Justificativa', 'Descrição de forma clara a necessidade, a motivação, custo e prazo estimado da mudança no projeto.').'Necessidade/justificativa:'.dicaF().'</td><td class="realce" width="100%" >'.$obj->projeto_mudanca_justificativa.'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Parecer Técnico', 'Avaliar tecnicamente se a mudança é pertinente').'Parecer técnico:'.dicaF().'</td><td class="realce" width="100%" >'.$obj->projeto_mudanca_parecer_tecnico.'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Soluções Possíveis', 'Avaliar todas as possíveis soluções para solução da mudança proposta.').'Soluções possíveis:'.dicaF().'</td><td class="realce" width="100%" >'.$obj->projeto_mudanca_solucoes.'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Impacto no Cronograma', 'Descrever o impacto da mudança no tempo, custo e riscos.').'Impacto no cronograma:'.dicaF().'</td><td class="realce" width="100%" >'.$obj->projeto_mudanca_impacto_cronograma.'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Impactos no Custo', 'Descrever o impacto da mudança no tempo, custo e riscos.').'Impactos no custo:'.dicaF().'</td><td class="realce" width="100%" >'.$obj->projeto_mudanca_impacto_custo.'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Novos Riscos', 'Descrever o impacto da mudança no tempo, custo e riscos.').'Novos riscos:'.dicaF().'</td><td class="realce" width="100%" >'.$obj->projeto_mudanca_novo_risco.'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Outros Impactos', 'Descrever o impacto da mudança no tempo, custo e riscos.').'Outros impactos:'.dicaF().'</td><td class="realce" width="100%" >'.$obj->projeto_mudanca_outros_impactos.'</td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Parecer', 'Deliberar sobre a aprovação da mudança.').'Parecer:'.dicaF().'</td><td class="realce" width="100%" >'.$obj->projeto_mudanca_parecer.'</td></tr>';
