...
if (isset($_REQUEST['aberturatextobusca'])) $Aplic->setEstado('aberturatextobusca', getParam($_REQUEST, 'aberturatextobusca', null));
$pesquisar_texto = ($Aplic->getEstado('aberturatextobusca') ? $Aplic->getEstado('aberturatextobusca') : '');
...
if (isset($_REQUEST['projeto_abertura_setor']))	$Aplic->setEstado('projeto_abertura_setor',getParam($_REQUEST, 'projeto_abertura_setor', null));
$projeto_abertura_setor = $Aplic->getEstado('projeto_abertura_setor') !== null ? $Aplic->getEstado('projeto_abertura_setor') : '';

if (isset($_REQUEST['aberturatextobusca']))	$Aplic->setEstado('projeto_abertura_segmento',getParam($_REQUEST, 'projeto_abertura_segmento', null));
$projeto_abertura_segmento = $Aplic->getEstado('projeto_abertura_segmento') !== null ? $Aplic->getEstado('projeto_abertura_segmento') : '';

if (isset($_REQUEST['aberturatextobusca']))	$Aplic->setEstado('projeto_abertura_intervencao', getParam($_REQUEST, 'projeto_abertura_intervencao', null));
$projeto_abertura_intervencao = $Aplic->getEstado('projeto_abertura_intervencao') !== null ? $Aplic->getEstado('projeto_abertura_intervencao') : '';

if (isset($_REQUEST['aberturatextobusca']))	$Aplic->setEstado('projeto_abertura_tipo_intervencao', getParam($_REQUEST, 'projeto_abertura_tipo_intervencao', null));
$projeto_abertura_tipo_intervencao = $Aplic->getEstado('projeto_abertura_tipo_intervencao') !== null ? $Aplic->getEstado('projeto_abertura_tipo_intervencao') : '';
...
if (!$dialogo || $selecao){
	...
	$botoesTitulo = new CBlocoTitulo('Lista de Termos de Abertura', 'anexo_projeto.png', $m, $m.'.'.$a);
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionado','A qual parte do sistema os termos de abertura estão relacionados.').'Relacionado:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	if ($exibir['projeto_abertura_setor']){
		$setor = array(0 => '&nbsp;') + getSisValor('Setor');
		$segmento=array(0 => '&nbsp;');
		if ($projeto_abertura_setor){
			$sql->adTabela('sisvalores');
			$sql->adCampo('sisvalor_valor_id, sisvalor_valor');
			$sql->adOnde('sisvalor_titulo="Segmento"');
			$sql->adOnde('sisvalor_chave_id_pai="'.$projeto_abertura_setor.'"');
			$sql->adOrdem('sisvalor_valor');
			$segmento+=$sql->listaVetorChave('sisvalor_valor_id', 'sisvalor_valor');
			$sql->limpar();
			}
		$intervencao=array(0 => '&nbsp;');
		if ($projeto_abertura_segmento){
			$sql->adTabela('sisvalores');
			$sql->adCampo('sisvalor_valor_id, sisvalor_valor');
			$sql->adOnde('sisvalor_titulo="Intervencao"');
			$sql->adOnde('sisvalor_chave_id_pai="'.$projeto_abertura_segmento.'"');
			$sql->adOrdem('sisvalor_valor');
			$intervencao+=$sql->listaVetorChave('sisvalor_valor_id', 'sisvalor_valor');
			$sql->limpar();
			}
		$tipo_intervencao=array(0 => '&nbsp;');
		if ($projeto_abertura_intervencao){
			$sql->adTabela('sisvalores');
			$sql->adCampo('sisvalor_valor_id, sisvalor_valor');
			$sql->adOnde('sisvalor_titulo="TipoIntervencao"');
			$sql->adOnde('sisvalor_chave_id_pai="'.$projeto_abertura_intervencao.'"');
			$sql->adOrdem('sisvalor_valor');
			$tipo_intervencao+= $sql->listaVetorChave('sisvalor_valor_id', 'sisvalor_valor');
			$sql->limpar();
			}
		$procura_setor='<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['setor']), 'A qual '.$config['setor'].' perternce o termo de abertura.').ucfirst($config['setor']).':'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($setor, 'projeto_abertura_setor', 'style="width:250px;" class="texto" onchange="mudar_segmento();"', $projeto_abertura_setor).'</td></tr>';
		$procura_segmento='<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['segmento']), 'A qual '.$config['segmento'].' perternce o termo de abertura.').ucfirst($config['segmento']).':'.dicaF().'</td><td width="100%" colspan="2"><div id="combo_segmento">'.selecionaVetor($segmento, 'projeto_abertura_segmento', 'style="width:250px;" class="texto" onchange="mudar_intervencao();"', $projeto_abertura_segmento).'</div></td></tr>';
	 	$procura_intervencao='<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['intervencao']), 'A qual '.$config['intervencao'].' perternce o termo de abertura.').ucfirst($config['intervencao']).':'.dicaF().'</td><td width="100%" colspan="2"><div id="combo_intervencao">'.selecionaVetor($intervencao, 'projeto_abertura_intervencao', 'style="width:250px;" class="texto" onchange="mudar_tipo_intervencao();"', $projeto_abertura_intervencao).'</div></td></tr>';
		$procura_tipo_intervencao='<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['tipo']), 'A qual '.$config['tipo'].' pertence o termo de abertura.').ucfirst($config['tipo']).':'.dicaF().'</td><td width="100%" colspan="2"><div id="combo_tipo_intervencao">'.selecionaVetor($tipo_intervencao, 'projeto_abertura_tipo_intervencao', 'style="width:250px;" class="texto"', $projeto_abertura_tipo_intervencao).'</div></td></tr>';
		}
	...
	$procuraBuffer = '<tr><td align=right style="white-space: nowrap">'.dica('Pesquisar', 'Pesquisar pelo nome e campos de descrição').'Pesquisar:'.dicaF().'</td><td><input type="text" class="texto" style="width:250px;" name="aberturatextobusca" onChange="document.env.submit();" value="'.$pesquisar_texto.'"/></td><td><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.'&tab='.$tab.'&aberturatextobusca=\');">'.imagem('icones/limpar_p.gif','Limpar Pesquisa', 'Clique neste ícone '.imagem('icones/limpar_p.gif').' para limpar a caixa texto de pesquisa.').'</a></td></tr>';
	
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de termos de abertura.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=projetos&a=termo_abertura_lista&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_responsavel.$procura_setor.$procura_segmento.$procura_intervencao.$procura_tipo_intervencao.$procuraBuffer.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$botao_filtrar.(!$selecao ? $imprimir.$botao_campos : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
...
elseif ($Aplic->profissional){
	...
	echo '</td></tr></table>';
	echo 	'<font size="4"><center>Lista de Termos de Abertura</center></font>';
	}	
...
if ($config['mostrar_total']){
	$total0=' ('.(int)termo_abertura_quantidade(0, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $pesquisar_texto, $responsavel, $projeto_abertura_setor, $projeto_abertura_segmento, $projeto_abertura_intervencao, $projeto_abertura_tipo_intervencao,
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$total1=' ('.(int)termo_abertura_quantidade(1, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $pesquisar_texto, $responsavel, $projeto_abertura_setor, $projeto_abertura_segmento, $projeto_abertura_intervencao, $projeto_abertura_tipo_intervencao,
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$total2=' ('.(int)termo_abertura_quantidade(2, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $pesquisar_texto, $responsavel, $projeto_abertura_setor, $projeto_abertura_segmento, $projeto_abertura_intervencao, $projeto_abertura_tipo_intervencao,
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$total3=' ('.(int)termo_abertura_quantidade(3, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $pesquisar_texto, $responsavel, $projeto_abertura_setor, $projeto_abertura_segmento, $projeto_abertura_intervencao, $projeto_abertura_tipo_intervencao,
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$total4=' ('.(int)termo_abertura_quantidade(4, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $pesquisar_texto, $responsavel, $projeto_abertura_setor, $projeto_abertura_segmento, $projeto_abertura_intervencao, $projeto_abertura_tipo_intervencao,
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$total5=' ('.(int)termo_abertura_quantidade(5, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $pesquisar_texto, $responsavel, $projeto_abertura_setor, $projeto_abertura_segmento, $projeto_abertura_intervencao, $projeto_abertura_tipo_intervencao,
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
	...
	$total6=' ('.(int)termo_abertura_quantidade(6, $envolvimento, $cia_id, $lista_cias, $dept_id, $lista_depts, $pesquisar_texto, $responsavel, $projeto_abertura_setor, $projeto_abertura_segmento, $projeto_abertura_intervencao, $projeto_abertura_tipo_intervencao,
	$projeto_id,
	$tarefa_id,
	$pg_perspectiva_id,
	$tema_id,
...
if (!$dialogo || $selecao)  {
	$caixaTab = new CTabBox('m=projetos&a=termo_abertura_lista', BASE_DIR.'/modulos/projetos/', $tab);
	$caixaTab->adicionar('termo_abertura_tabela', 'Pendentes'.$total0,null,null,'Termos de Abertura Pendentes','Termos de abertura que ainda não foram aprovadas.');
	$caixaTab->adicionar('termo_abertura_tabela', 'Aprovados'.$total1,null,null,'Termos de Abertura Aprovados','Termos de abertura aprovados.');
	$caixaTab->adicionar('termo_abertura_tabela', 'Reprovados'.$total2,null,null,'Termo de Abertura Reprovado','Termos de abertura que foram reprovados.');
	$caixaTab->adicionar('termo_abertura_tabela', 'Viraram '.ucfirst($config['projetos']).$total3,null,null,'Termo de Abertura que Viraram '.ucfirst($config['projetos']),'Termo de abertura que se transformaram em '.$config['projeto'].'.');
	$caixaTab->adicionar('termo_abertura_tabela', 'Ativos'.$total4,null,null,'Termo de Abertura Ativos','Termos de abertura que se encontram ativos.');
	$caixaTab->adicionar('termo_abertura_tabela', 'Inativos'.$total5,null,null,'Termo de Abertura Inativos','Termos de abertura que se encontram inativos.');
	$caixaTab->adicionar('termo_abertura_tabela', 'Todos'.$total6,null,null,'Todos os Termo de Abertura','Todos os termos de abertura.');
	$caixaTab->mostrar('','','','',true); 
	echo estiloFundoCaixa('','', $tab);
	}
else {
	require_once (BASE_DIR.'/modulos/projetos/termo_abertura_tabela.php');
...
if($Aplic->profissional){
	...
	if ($exibir['projeto_abertura_setor']){
		echo 'function criarComboSegmento(){$jq("#projeto_abertura_segmento").multiSelect({multiple:false, onCheck: function(){mudar_intervencao();}});}';
		echo 'function criarComboIntervencao(){$jq("#projeto_abertura_intervencao").multiSelect({multiple:false, onCheck: function(){mudar_tipo_intervencao();}});}';
		echo 'function criarComboTipoIntervencao(){$jq("#projeto_abertura_tipo_intervencao").multiSelect({multiple:false});}';
		}
	echo '$jq(function(){';
	if ($exibir['projeto_abertura_setor']) echo '  $jq("#projeto_abertura_setor").multiSelect({multiple:false, onCheck: function(){mudar_segmento();}});';
...
function mudar_segmento(){
	<?php
	if($Aplic->profissional){
		echo '$jq.fn.multiSelect.clear("#projeto_abertura_tipo_intervencao");';
		echo '$jq.fn.multiSelect.clear("#projeto_abertura_intervencao");';
		}
	else{
		echo 'document.getElementById("projeto_abertura_intervencao").length=0;';
		echo 'document.getElementById("projeto_abertura_tipo_intervencao").length=0;';
		}
	?>
	xajax_mudar_ajax(document.getElementById('projeto_abertura_setor').value, 'Segmento', 'projeto_abertura_segmento','combo_segmento', 'style="width:250px;" class="texto" size=1 onchange="mudar_intervencao();"');
	}

function mudar_intervencao(){
	<?php
	if($Aplic->profissional) echo '$jq.fn.multiSelect.clear("#projeto_abertura_tipo_intervencao");';
	else echo 'document.getElementById("projeto_abertura_tipo_intervencao").length=0;';
	?>
	xajax_mudar_ajax(document.getElementById('projeto_abertura_segmento').value, 'Intervencao', 'projeto_abertura_intervencao','combo_intervencao', 'style="width:250px;" class="texto" size=1 onchange="mudar_tipo_intervencao();"');

	}

function mudar_tipo_intervencao(){
	xajax_mudar_ajax(document.getElementById('projeto_abertura_intervencao').value, 'TipoIntervencao', 'projeto_abertura_tipo_intervencao','combo_tipo_intervencao', 'style="width:250px;" class="texto" size=1');
	}	
	
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=projeto_aberturas', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=projetos&a=termo_abertura_lista');
	}	
