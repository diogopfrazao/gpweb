...
if ($novo) {
  ...
  echo '<tr><td align="right" style="white-space: nowrap">'.dica('Cor', 'Cor selecionada dentre as 16 milhões possíveis. Pode-se escrever diretamente o hexadecinal na cor ou utilizar a interface que se abre ao clicar na caixa de inserção do valor.').'Cor:'.dicaF().'</td><td align="left" style="white-space: nowrap"><input class="jscolor" name="projeto_area_cor" value="FFFFFF" '.($config['selecao_cor_restrita'] ? 'readonly="readonly" ' : '').'size="6" maxlength="6" style="width:57px;" /></td></tr>';
...
if ($mudar) {
  ...
  echo '<tr><td align="right">Nome:</td><td><input type="text" class="texto" style="width:300px;" name="projeto_area_nome" value="'.$rs['projeto_area_nome'].'"></td></tr>';
 	...
 	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Cor', 'Cor selecionada dentre as 16 milhões possíveis. Pode-se escrever diretamente o hexadecinal na cor ou utilizar a interface que se abre ao clicar na caixa de inserção do valor.').'Cor:'.dicaF().'</td><td align="left" style="white-space: nowrap"><input class="jscolor" name="projeto_area_cor" value="'.($rs['projeto_area_cor'] ? $rs['projeto_area_cor'] : 'FFFFFF').'" '.($config['selecao_cor_restrita'] ? 'readonly="readonly" ' : '').'size="6" maxlength="6" style="width:57px;" /></td></tr>';
  echo '<tr><td colspan=20 align="center">Observações</td></tr>';
...
if (!$novo  && $projeto_area_id && !$mudar) {
  ...
  $sql->adOnde('projeto_area_id='.$projeto_area_id);
	...
	mostrarBarraNav2($xpg_totalregistros, $xpg_tamanhoPagina, $xpg_total_paginas, $pagina, 'ponto', 'pontos');
