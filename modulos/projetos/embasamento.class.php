	...
	public function notificar( $post=array()){
		...
		foreach($usuarios as $usuario){
			if (!isset($usado[$usuario['usuario_id']]) && !isset($usado[$usuario['contato_email']])){
				...
				if ($tipo == 'excluido') {
					$email->Assunto('Excluído embasamento do projeto', $localidade_tipo_caract);
					$titulo='Excluído embasamento do projeto';
					}
				...
				elseif ($tipo=='excluido') $corpo = 'Excluído embasamento do projeto: '.$projeto_nome.'<br>';
				else $corpo = 'Inserido embasamento do projeto: '.$projeto_nome.'<br>';

				if ($tipo=='excluido') $corpo .= '<br><br><b>Responsável pela exclusão do embasamento do projeto:</b> '.$Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra;
				elseif ($tipo=='atualizada') $corpo .= '<br><br><b>Responsável pela edição do embasamento do projeto:</b> '.$Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra;
