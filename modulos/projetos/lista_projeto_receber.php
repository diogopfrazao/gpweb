...
global $m, $a, $u, $estilo_interface, $tab, $dialogo;
$sql = new BDConsulta;
...
if ($tab==0){
	...
	$sql->adOnde('cia_de !='.(int)$Aplic->usuario_cia);
	}
elseif ($tab==1){
	$sql->adOnde('aprovado != 0');
	$sql->adOnde('cia_para ='.(int)$Aplic->usuario_cia);
...
echo '<tr>';
//projeto
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=projeto_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='projeto_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome d'.$config['genero_projeto'].' '.ucfirst($config['projeto']), 'Neste campo fica o nome d'.$config['genero_projeto'].'s '.$config['projetos']).'Nome'.dicaF().'</th>';
//om
if ($tab < 2) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=cia_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='cia_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica(ucfirst($config['organizacao']).' Remetente', 'Clique para ordenar pelas '.$config['organizacao'].' remetentes d'.$config['genero_projeto'].'s '.$config['projetos']).$config['organizacao'].dicaF().'</th>';
else echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=cia_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='cia_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica(ucfirst($config['organizacao']).' Remetente', 'Clique para ordenar pelas '.$config['organizacao'].' destinatárias d'.$config['genero_projeto'].'s '.$config['projetos']).$config['organizacao'].dicaF().'</th>';
//Remetente'
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=remetente&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='remetente' ? imagem('icones/'.$seta[$ordem]) : '').dica(ucfirst($config['usuario']).' Remetente', 'Clique para ordenar pel'.$config['genero_usuario'].'s '.$config['usuarios'].' remetentes d'.$config['genero_projeto'].'s '.$config['projetos']).'Remetente'.dicaF().'</th>';
//destinatario
if ($tab ==1 || $tab ==3) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=usuario_aprovou&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='usuario_aprovou' ? imagem('icones/'.$seta[$ordem]) : '').dica(ucfirst($config['usuario']).' Que Recebeu', 'Clique para ordenar pel'.$config['genero_usuario'].'s '.$config['usuarios'].' que receberam '.$config['genero_projeto'].'s '.$config['projetos']).'Recebeu'.dicaF().'</th>';

//data envio
echo '<th style="white-space: nowrap;width:60px;"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=data_envio&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='data_envio' ? imagem('icones/'.$seta[$ordem]) : '').dica('Data do Envio', 'Clique para ordenar pelas datas de envio  d'.$config['genero_projeto'].'s '.$config['projetos']).'Envio'.dicaF().'</th>';
//data recebimento
if ($tab ==1 || $tab == 3) echo '<th style="white-space: nowrap;width:60px;"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=data_envio&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='data_envio' ? imagem('icones/'.$seta[$ordem]) : '').dica('Data do Recebimento', 'Clique para ordenar pelas datas de recebimento d'.$config['genero_projeto'].'s '.$config['projetos']).'Receb.'.dicaF().'</th>';

//obs
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=obs_remetente&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='obs_remetente' ? imagem('icones/'.$seta[$ordem]) : '').dica('Observação no Envio', 'Clique para ordenar pelas observações no envio d'.$config['genero_projeto'].'s '.$config['projetos']).'Obs. envio'.dicaF().'</th>';
if ($tab==1 || $tab==3) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=obs_destinatario&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='obs_destinatario' ? imagem('icones/'.$seta[$ordem]) : '').dica('Observação no Recebimento', 'Clique para ordenar pelas observações no recebimento d'.$config['genero_projeto'].'s '.$config['projetos']).'Obs. receb.'.dicaF().'</th>';

if ($tab==0) echo '<th style="white-space: nowrap" width="16">'.dica('Selecionar', 'Clique nas caixas para selecionar os '.$config['projeto'].' abaixo.').'&nbsp;'.dicaF().'</th>';
echo '</tr>';
$fp = -1;
$id = 0;
$qnt=0;
...
elseif ($tab==0){
	...
	echo '<tr><td align="right" style="white-space: nowrap">'.dica('Observações', 'Texto para acompanhar o recebimeneto '.(count($lista)>1 ? 'd'.$config['genero_projeto'].'s '.$config['projetos'] : 'd'.$config['genero_projeto'].' '.$config['projeto'])).'Observação:'.dicaF().'</td><td width="100%" colspan="2"><textarea name="obs_destinatario"  data-gpweb-cmp="ckeditor" style="width:284px;" rows="2" class="textarea"></textarea></td><tr>';
	
	echo '<tr><td colspan=20><table width="100%" cellpadding=0 cellspacing=0><tr><td width=99%>'.botao('aceitar', 'Aceitar', 'Clique neste botão para aceitar '.$config['genero_projeto'].'s '.$config['projetos'].' recebidos marcados acima.','','if (verifica_selecao()){frm.aprovado.value=1; frm.submit();}').'</td>
	<td>'.botao('recusar', 'Recusar', 'Clique neste botão para recusar '.$config['genero_projeto'].'s '.$config['projetos'].' recebidos marcados acima.','','if (verifica_selecao()){frm.aprovado.value=-1; frm.submit();}').'</td>
	<td>'.dica('Voltar','Clique neste botão para voltar à tela anterior.').'<a class="botao" href="javascript:void(0);" onclick="javascript:frm.a.value=\'index\'; frm.m.value=\'projetos\'; frm.submit();"><span><b>voltar</b></span></a></td>
