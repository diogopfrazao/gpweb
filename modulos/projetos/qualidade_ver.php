...
if ($Aplic->profissional){
	...
	if ($obj->projeto_qualidade_usuario){
		...
		if ($podeEditarTudo && $podeEditar)  $km->Add("acao","editar_qualidade",dica('Editar Plano de Qualidade','Editar os detalhes do plano de qualidade.').'Editar Plano de Qualidade'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=qualidade_editar&projeto_id=".$projeto_id."\");");
		if ($podeEditarTudo && $podeExcluir) $km->Add("acao","acao_excluir",dica('Excluir','Excluir este plano de qualidade.').'Excluir Plano de Qualidade'.dicaF(), "javascript: void(0);' onclick='excluir()");
...
if ($obj->projeto_qualidade_usuario) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Responsável', 'O responsável pelo plano de qualidade.').'Responsável:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.link_usuario($obj->projeto_qualidade_usuario, '','','esquerda').'</td></tr>';
...
if ($obj->projeto_qualidade_data) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data', 'A data em que o plano de qualidade foi criado ou editado').'Data:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.retorna_data($obj->projeto_qualidade_data).'</td></tr>';
