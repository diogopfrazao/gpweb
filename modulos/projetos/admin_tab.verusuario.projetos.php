...
echo '<tr>';
echo '<td align="center" width="100%" style="white-space: nowrap" colspan="7">&nbsp;</td>';
echo '<form method="post" name="checkPwT"><input type="hidden" name="m" value="admin" /><input type="hidden" name="a" value="ver_usuario" /><input type="hidden" name="usuario_id" value="'.$usuario_id.'" /><input type="hidden" name="tab" value="'.$tab.'" /><td align="right" style="white-space: nowrap"><label for="adProjComTarefas"><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['projetos']).' com '.ucfirst($config['tarefa']).' Designada', 'Alem de pesquisar '.$config['genero_projeto'].'s '.$config['projetos'].' em que seja o responsável, serão verificados aqueles em que tenha sido designado para um'.($config['genero_tarefa']=='a' ?  'a' : '').' '.$config['tarefa'].'.').ucfirst($config['projetos']).' com '.$config['tarefa'].' designad'.$config['genero_tarefa'].dicaF().'</label><input type="checkbox" name="adProjComTarefas" id="adProjComTarefas" onclick="document.checkPwT.submit()" '.($adPcT ? 'checked="checked"' : '').' /></td><td align="right" style="white-space: nowrap"><input type="hidden" name="mostrar_form" value="1" /></td></form>';
echo '<form method="post" name="escolherCia"><input type="hidden" name="m" value="admin" /><input type="hidden" name="a" value="ver_usuario" /><input type="hidden" name="usuario_id" value="'.$usuario_id.'" /><input type="hidden" name="tab" value="'.$tab.'" /><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']), 'Selecione na caixa de opção à direita, de qual '.strtolower($config['departamento']).' deseja visualizar '.$config['genero_projeto'].'s '.$config['projetos'].'.').$config['departamento'].':'.dicaF().$buffer.'</td></form>';
echo '<form method="post" name="escolherProjeto"><input type="hidden" name="m" value="admin" /><input type="hidden" name="a" value="ver_usuario" /><input type="hidden" name="usuario_id" value="'.$usuario_id.'" /><input type="hidden" name="tab" value="'.$tab.'" /><td align="right" style="white-space: nowrap">'.dica('Status', 'Selecione na caixa de opção à direita, por qual status de '.$config['projeto'].' deseja visualizar o resultado.').'Status:'.dicaF().selecionaVetor($projFiltro, 'proFiltro', 'size=1 class=texto onChange="document.escolherProjeto.submit()"', $proFiltro).'</td></form>';
...
echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.(isset($a) ? '&a='.$a : '').(isset($extraGet) ? $extraGet : '').'&ordemPor=projeto_prioridade\');" class="hdr">'.dica('Prioridade', 'Clique para ordenar '.$config['genero_projeto'].'s '.$config['projetos'].' por prioridade.').'P'.dicaF().'</a></th>';
...
echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.(isset($a) ? '&a='.$a : '').(isset($extraGet) ? $extraGet : '').'&ordemPor=projeto_data_inicio\');" class="hdr">'.dica('Início', 'Clique para ordenar '.$config['genero_projeto'].'s '.$config['projetos'].' pela data de início.').'Início'.dicaF().'</a></th>';
...
echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.(isset($a) ? '&a='.$a : '').(isset($extraGet) ? $extraGet : '').'&ordemPor=projeto_data_fim\');" class="hdr">'.dica('Término', 'Clique para ordenar '.$config['genero_projeto'].'s '.$config['projetos'].' pela data de término.').'Término'.dicaF().'</a></th>';
...
echo '<th style="white-space: nowrap"><a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.(isset($a) ? '&a='.$a : '').(isset($extraGet) ? $extraGet : '').'&ordemPor=log_corrigir\');" class="hdr">'.dica('Registros de Problemas', 'Clique para ordenar '.$config['genero_projeto'].'s '.$config['projetos'].' pelos registros de problemas.').'RP'.dicaF().'</a></th>';
...
foreach ($projetos as $linha) {
	if ($proFiltro == -1 || $linha['projeto_status'] == $proFiltro || ($proFiltro == -2 && $linha['projeto_status'] != 3) || ($proFiltro == -3 && $linha['projeto_ativo'] != 0)) {
		...
		$s = '<tr><td width="15" align="right" style="border: outset #eeeeee 1px;background-color:#'.$linha['projeto_cor'].'"><font color="'.melhorCor($linha['projeto_cor']).'">'. number_format($linha['projeto_percentagem'], 2, ',', '.').'</font></td><td align="center">';
		...
		$s .= '<td style="white-space: nowrap" align="center">'.($data_inicio ? $data_inicio->format('%d/%m/%Y') : '').'</td>';
		$s .= '<td align="right" style="white-space: nowrap">'.($linha['projeto_duracao'] > 0 ? round($linha['projeto_duracao'], 0).'h' : '').'</td>';
		$s .= '<td style="white-space: nowrap" align="center" style="white-space: nowrap;background-color:'.$prioridade[$linha['projeto_prioridade']]['cor'].'">';
		$s .= ($data_fim ? $data_fim->format('%d/%m/%Y') : '');
		$s .= '</td><td style="white-space: nowrap" align="center">';
		...
		$s .= ($linha['log_corrigir'] ? '<a href="javascript:void(0);" onclick="url_passar(0, \'m=tarefas&a=index&f=all&projeto_id='.$linha['projeto_id'].'\');">'.imagem('icones/aviso.gif', imagem('icones/aviso.gif').' Problema', 'Foi registrado um problema. Clique para visualizar '.$config['genero_projeto'].' '.$config['projeto'].'.').'</a>' : '&nbsp;');
		$s .= '</td><td align="center" style="white-space: nowrap">'.htmlspecialchars($linha['nome_responsavel'], ENT_QUOTES, $localidade_tipo_caract).'</td><td align="center" style="white-space: nowrap">';
		$s .= $linha['total_tarefas'].($linha['minhas_tarefas'] ? ' ('.$linha['minhas_tarefas'].')' : '');
		$s .= '</td><td align="left" style="white-space: nowrap">'.$projStatus[$linha['projeto_status']].'</td></tr>';
		echo $s;
		}
	}
