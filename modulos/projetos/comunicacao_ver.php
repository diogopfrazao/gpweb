...
if ($Aplic->profissional){
	...
	if ($obj->projeto_comunicacao_usuario){
		...
		if ($podeEditarTudo && $podeEditar) $km->Add("acao","editar_comunicacao",dica('Editar Plano de Comunicação','Editar os detalhes do plano de comunicação.').'Editar Plano de Comunicação'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=comunicacao_editar&projeto_id=".$projeto_id."\");");
		if ($podeEditarTudo && $podeExcluir) $km->Add("acao","acao_excluir",dica('Excluir','Excluir este plano de comunicação.').'Excluir Plano de Comunicação'.dicaF(), "javascript: void(0);' onclick='excluir()");
...
if ($obj->projeto_comunicacao_usuario) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Responsável', 'O responsável pelo plano de comunicação.').'Responsável:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.link_usuario($obj->projeto_comunicacao_usuario, '','','esquerda').'</td></tr>';
...
if ($obj->projeto_comunicacao_data) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Data', 'A data em que o plano de comunicação foi criado ou editado').'Data:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.retorna_data($obj->projeto_comunicacao_data).'</td></tr>';
