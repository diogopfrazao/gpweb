...
echo '<tr><td align="right" style="white-space: nowrap;width:154px;">'.dica('Tamanho d'.$config['genero_projeto'].' '.ucfirst($config['projeto']),'Baseado nos campos preenchidos qual o tamanho d'.$config['genero_projeto'].' '.$config['projeto'].'.').'<b>Tamanho:</b>'.dicaF().'</td><td><b><div id="tamanho_projeto"></div></b></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap;width:154px;">'.dica('Complexidade','Qual o grau de complexidade d'.$config['genero_projeto'].' '.$config['projeto'].'.').'Complexidade:'.dicaF().'</td><td>'.selecionaVetor($ProjetoComplexidade, 'demanda_complexidade', 'size="1" class="texto" onchange="tamanho();"', $obj->demanda_complexidade).'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap;width:154px;">'.dica('Custo','Qual o custo d'.$config['genero_projeto'].' '.$config['projeto'].'.').'Custo:'.dicaF().'</td><td>'.selecionaVetor($ProjetoCusto, 'demanda_custo', 'size="1" class="texto" onchange="tamanho();"', $obj->demanda_custo).'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap;width:154px;">'.dica('Tempo','Qual o tempo para execução d'.$config['genero_projeto'].' '.$config['projeto'].'.').'Tempo:'.dicaF().'</td><td>'.selecionaVetor($ProjetoTempo, 'demanda_tempo', 'size="1" class="texto" onchange="tamanho();"', $obj->demanda_tempo).'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap;width:154px;">'.dica('Servidores','Qual o número de servidores para a execução d'.$config['genero_projeto'].' '.$config['projeto'].'.').'Servidores:'.dicaF().'</td><td>'.selecionaVetor($ProjetoServidores, 'demanda_servidores', 'size="1" class="texto" onchange="tamanho();"', $obj->demanda_servidores).'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap;width:154px;">'.dica('Recurso Externo','Haverá aporte de recurso de outr'.$config['genero_organizacao'].' '.$config['organizacao'].' n'.$config['genero_projeto'].' '.$config['projeto'].'.').'Recurso externo:'.dicaF().'</td><td>'.selecionaVetor($ProjetoRecursoExterno, 'demanda_recurso_externo', 'size="1" class="texto" onchange="tamanho();"', $obj->demanda_recurso_externo).'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap;width:154px;">'.dica('Interligação',ucfirst($config['genero_projeto']).' '.$config['projeto'].' possui interligação com outr'.$config['genero_projeto'].' '.$config['projeto'].'.').'Interligação:'.dicaF().'</td><td>'.selecionaVetor($ProjetoInterligacao, 'demanda_interligacao', 'size="1" class="texto" onchange="tamanho();"', $obj->demanda_interligacao).'</td></tr>';
echo '</table></td></tr>';	
	
echo '<tr><td align="right" style="white-space: nowrap;width:154px;">'.botao('salvar', 'Salvar', 'Salvar a análise da demanda','','env.submit()').'</td></tr>';
echo '</table></td>';
...
if ($obj->demanda_cia) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', $config['organizacao'].' da demanda.').ucfirst($config['organizacao']).':'.dicaF().'</td><td class="realce" width="100%">'.link_cia($obj->demanda_cia).'</td></tr>';
...
if ($obj->demanda_fonte_recurso) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Fonte de Recurso', 'Indicação da fonte de recursos para as despesas da demanda.>').'Recurso:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->demanda_fonte_recurso.'</td></tr>';
if ($obj->demanda_usuario) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Responsável', ucfirst($config['usuario']).' responsável por gerenciar.').'Responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($obj->demanda_usuario, '','','esquerda').'</td></tr>';		
