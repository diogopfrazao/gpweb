	...
	public function notificar( $post=array()){
		...
		foreach($usuarios as $usuario){
			if (!isset($usado[$usuario['usuario_id']]) && !isset($usado[$usuario['contato_email']])){
				...
				if ($tipo == 'excluido') {
					$email->Assunto('Excluído o plano de qualidade do projeto', $localidade_tipo_caract);
					$titulo='Excluído plano de qualidade do projeto';
					}
				...
				elseif ($tipo=='excluido') $corpo = 'Excluído o plano de qualidade do projeto: '.$projeto_nome.'<br>';
				else $corpo = 'Inserido o plano de qualidade do projeto: '.$projeto_nome.'<br>';

				if ($tipo=='excluido') $corpo .= '<br><br><b>Responsável pela exclusão do o plano de qualidade do projeto:</b> '.$Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra;
				elseif ($tipo=='atualizado') $corpo .= '<br><br><b>Responsável pela edição do o plano de qualidade do projeto:</b> '.$Aplic->usuario_posto.' '.$Aplic->usuario_nomeguerra;
