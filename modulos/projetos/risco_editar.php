...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Descrição', 'Descrever de forma clara a justificativa como se processa o gerenciamento de risco.').'Descrição:'.dicaF().'</td><td><textarea data-gpweb-cmp="ckeditor" name="projeto_risco_descricao" style="width:800px;" class="textarea">'.$obj->projeto_risco_descricao.'</textarea></td></tr>';

echo '<tr><td align="right" style="white-space: nowrap">'.dica('Responsável', 'O responsável pelo gerenciamento de risco.').'Responsável:'.dicaF().'</td><td colspan="2"><input type="hidden" id="projeto_risco_usuario" name="projeto_risco_usuario" value="'.($obj->projeto_risco_usuario ? $obj->projeto_risco_usuario : $Aplic->usuario_id).'" /><input type="text" id="nome_gerente" name="nome_gerente" value="'.nome_om(($obj->projeto_risco_usuario ? $obj->projeto_risco_usuario : $Aplic->usuario_id),$Aplic->getPref('om_usuario')).'" style="width:284px;" class="texto" READONLY /><a href="javascript: void(0);" onclick="popResponsavel();">'.imagem('icones/usuarios.gif','Selecionar '.ucfirst($config['usuario']),'Clique neste ícone '.imagem('icones/usuarios.gif').' para selecionar '.($config['genero_usuario']=='o' ? 'um' : 'uma').' '.$config['usuario'].'.').'</a></td></tr>';


$campos_customizados = new CampoCustomizados('projeto_risco', $projeto_id, 'editar');
$campos_customizados->imprimirHTML();

echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Notificar', 'Marque esta caixa para avisar sobre o gerenciamento de risco.').'Notificar:'.dicaF().'</td>';
echo '<td>';
