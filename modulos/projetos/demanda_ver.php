...
if (isset($_REQUEST['demanda_id'])) $Aplic->setEstado('demanda_id', getParam($_REQUEST, 'demanda_id', null), $m, $a, $u);
$demanda_id = $Aplic->getEstado('demanda_id', null, $m, $a, $u);
...
$podeEditarTudo=($obj->demanda_acesso>=5 ? $editar && (in_array($obj->demanda_usuario, $Aplic->usuario_lista_grupo_vetor) || $Aplic->usuario_super_admin || $Aplic->usuario_admin) : $editar);
...
if (!$dialogo){	
	...
	$bloquear=($obj->demanda_aprovado && $config['trava_aprovacao'] && $tem_aprovacao && !$Aplic->usuario_super_admin && !$Aplic->checarModulo('todos', 'editar', null, 'editar_aprovado'));
	
	
	if (($podeEditar && $editar) || $podeAdicionar)	$km->Add("root","inserir",dica('Inserir','Menu de opções').'Inserir'.dicaF(), "javascript: void(0);'");
	if ($podeAdicionar)	$km->Add("inserir","inserir_objeto",dica('Nova Demanda', 'Criar um nova demanda.').'Nova Demanda'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=demanda_editar\");");
	if ($podeEditar && $editar) {
		if ($Aplic->profissional && !$bloquear && ($Aplic->checarModulo('projetos', 'adicionar', null, 'demanda_custo') || $Aplic->checarModulo('projetos', 'editar', null, 'demanda_custo'))) $km->Add("inserir","inserir_custo",dica('Planilha de Custos', 'Inserir a planilha de custos da demanda.').'Planilha de Custos'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=projetos&a=demanda_estimado_pro&demanda_id=".$demanda_id."\");");
		
		$km->Add("inserir","inserir_registro",dica('Registro de Ocorrência','Inserir um novo registro de ocorrência.').'Registro de ocorrência'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=praticas&a=log_editar&demanda_id=".$demanda_id."\");");
		if ($Aplic->modulo_ativo('calendario') && $Aplic->checarModulo('eventos', 'adicionar')) $km->Add("inserir","inserir_evento",dica('Novo Evento', 'Criar um novo evento relacionado.').'Evento'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=calendario&a=editar&evento_demanda=".$demanda_id."\");");
		...
		if ($Aplic->modulo_ativo('links') && $Aplic->checarModulo('links', 'adicionar')) $km->Add("diverso","inserir_link",dica('Novo Link', 'Inserir um novo link relacionado.').'Link'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=links&a=editar&link_demanda=".$demanda_id."\");");
		if ($Aplic->modulo_ativo('foruns') && $Aplic->checarModulo('foruns', 'adicionar')) $km->Add("diverso","inserir_forum",dica('Novo Fórum', 'Inserir um novo fórum relacionado.').'Fórum'.dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=foruns&a=editar&forum_demanda=".$demanda_id."\");");
	...
	if ($podeEditarTudo && $podeExcluir && !$bloquear) $km->Add("acao","acao_excluir",dica('Excluir','Excluir esta demanda.').'Excluir Demanda'.dicaF(), "javascript: void(0);' onclick='excluir()");
	...
	$km->Add("acao_imprimir","acao_imprimir1",dica('Demanda', 'Visualize o artefato da demanda.').'Demanda'.dicaF(), "javascript: void(0);' onclick='url_passar(1, \"m=".$m."&a=demanda_imprimir&dialogo=1&demanda_id=".$demanda_id."\");");
	$km->Add("acao_imprimir","acao_imprimir2",dica('Detalhes desta Demanda', 'Visualize os detalhes desta demanda.').'Detalhes desta demanda'.dicaF(), "javascript: void(0);' onclick='url_passar(1, \"m=".$m."&a=".$a."&dialogo=1&demanda_id=".$demanda_id."\");");
...
if ($obj->demanda_cia) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', $config['organizacao'].' da demanda.').ucfirst($config['organizacao']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_cia($obj->demanda_cia).'</td></tr>';
...
if ($obj->demanda_dept) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']).' Responsável', ucfirst($config['genero_dept']).' '.$config['departamento'].' responsável pela demanda.').ucfirst($config['departamento']).' responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_dept($obj->demanda_dept).'</td></tr>';
...
if ($obj->demanda_usuario) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Responsável pela Demanda', ucfirst($config['usuario']).' responsável por gerenciar a demanda.').'Responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($obj->demanda_usuario, '','','esquerda').'</td></tr>';		
...
if ($obj->demanda_supervisor) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['supervisor']), ucfirst($config['genero_supervisor']).' '.$config['supervisor'].' da demanda.').ucfirst($config['supervisor']).':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.link_usuario($obj->demanda_supervisor, '','','esquerda').'</td></tr>';

if ($obj->demanda_supervisor_data) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Aprovado pel'.$config['genero_supervisor'].' '.ucfirst($config['supervisor']), ucfirst($config['genero_supervisor']).' '.$config['supervisor'].' da demanda aprovou ou reprovou a mesma.').'Aprovado pel'.$config['genero_supervisor'].' '.$config['supervisor'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.($obj->demanda_supervisor_aprovado > 0 ? 'Sim - ': 'Não - ').retorna_data($obj->demanda_supervisor_data).'</td></tr>';
if ($obj->demanda_supervisor_obs) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Observação d'.$config['genero_supervisor'].' '.ucfirst($config['supervisor']), 'A observação redigida pel'.$config['genero_supervisor'].' '.$config['supervisor'].' da demanda.').'Observação d'.$config['genero_supervisor'].' '.$config['supervisor'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->demanda_supervisor_obs.'</td></tr>';

if ($obj->demanda_autoridade) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['autoridade']), ucfirst($config['genero_autoridade']).' '.$config['autoridade'].' da demanda.').ucfirst($config['autoridade']).':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.link_usuario($obj->demanda_autoridade, '','','esquerda').'</td></tr>';
if ($obj->demanda_autoridade_data) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Aprovado pel'.$config['genero_autoridade'].' '.ucfirst($config['autoridade']), ucfirst($config['genero_autoridade']).' '.$config['autoridade'].' da demanda aprovou ou reprovou a mesma.').'Aprovado pel'.$config['genero_autoridade'].' '.$config['autoridade'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.($obj->demanda_autoridade_aprovado > 0 ? 'Sim - ': 'Não - ').retorna_data($obj->demanda_autoridade_data).'</td></tr>';
if ($obj->demanda_autoridade_obs) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Observação d'.$config['genero_autoridade'].' '.ucfirst($config['autoridade']), 'A observação redigida pel'.$config['genero_autoridade'].' '.$config['autoridade'].' da demanda.').'Observação d'.$config['genero_autoridade'].' '.$config['autoridade'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->demanda_autoridade_obs.'</td></tr>';

if ($obj->demanda_cliente) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['cliente']), ucfirst($config['genero_cliente']).' '.$config['cliente'].' da demanda.').ucfirst($config['cliente']).':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.link_usuario($obj->demanda_cliente, '','','esquerda').'</td></tr>';
if ($obj->demanda_cliente_data) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Aprovado pel'.$config['genero_cliente'].' '.ucfirst($config['cliente']), ucfirst($config['genero_cliente']).' '.$config['cliente'].' da demanda aprovou ou reprovou a mesma.').'Aprovado pel'.$config['genero_cliente'].' '.$config['cliente'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.($obj->demanda_cliente_aprovado > 0 ? 'Sim - ': 'Não - ').retorna_data($obj->demanda_cliente_data).'</td></tr>';
if ($obj->demanda_cliente_obs) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Observação d'.$config['genero_cliente'].' '.ucfirst($config['cliente']), 'A observação redigida pel'.$config['genero_cliente'].' '.$config['cliente'].' da demanda.').'Observação d'.$config['genero_cliente'].' '.$config['cliente'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->demanda_cliente_obs.'</td></tr>';

if ($obj->demanda_codigo) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Código', 'O código da demanda.').'Código:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->demanda_codigo.'</td></tr>';
if ($obj->demanda_setor) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['setor']), 'A qual '.$config['setor'].' perternce a demanda.').ucfirst($config['setor']).':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->getSetor().'</td></tr>';
if ($obj->demanda_segmento) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['segmento']), 'A qual '.$config['segmento'].' perternce a demanda.').ucfirst($config['segmento']).':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->getSegmento().'</td></tr>';
if ($obj->demanda_intervencao) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['intervencao']), 'A qual '.$config['intervencao'].' perternce a demanda.').ucfirst($config['intervencao']).':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->getIntervencao().'</td></tr>';
if ($obj->demanda_tipo_intervencao) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['tipo']), 'A qual '.$config['tipo'].' pertence a demanda.').ucfirst($config['tipo']).':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->getTipoIntervencao().'</td></tr>';
if ($obj->demanda_identificacao) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Identificação', 'Descrição da demanda, contendo as informações necessárias para entendimento da necessidade.').'Identificação:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->demanda_identificacao.'</td></tr>';
if ($obj->demanda_justificativa) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Justificativa', 'Descrição da justificativa contendo um breve histórico e as motivações da demanda.').'Justificativa:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->demanda_justificativa.'</td></tr>';
if ($obj->demanda_resultados) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Resultados a Serem Alcançados', 'Descrição dos resultados a serem alcançadas com o atendimento da demanda.').'Resultados:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->demanda_resultados.'</td></tr>';
if ($obj->demanda_alinhamento) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Alinhamento Estratégico', 'Descrição do alinhamento da demanda com os instrumentos de planejamento institucional.').'Alinhamento:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->demanda_alinhamento.'</td></tr>';
if ($obj->demanda_fonte_recurso) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Fonte de Recurso', 'Indicação da fonte de recursos para as despesas da demanda.').'Fonte de recursos:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->demanda_fonte_recurso.'</td></tr>';
if ($obj->demanda_prazo) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Prazo', 'Informações sobre o prazo de execução.').'Prazo:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->demanda_prazo.'</td></tr>';
if ($obj->demanda_custos) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Custos', 'Informações sobre o custo de execução.').'Custos:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->demanda_custos.'</td></tr>';
if ($obj->demanda_observacao) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Observações', 'Informações gerais sobre a demanda').'Observações:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.$obj->demanda_observacao.'</td></tr>';
...
if (count($lista)) {
	echo '<tr><td align="right" style="white-space: nowrap" valign="middle">'.dica('Relacionada', 'A que área este demanda está relacionada.').'Relacionada:'.dicaF().'</td></td><td class="realce">';	
...
if ($obj->demanda_caracteristica_projeto==-1 || $obj->demanda_caracteristica_projeto==1) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Tem Característica de '.ucfirst($config['projeto']), 'Após análise da demanda, a mesma tem característica de '.$config['projeto'].'.').'Característica de '.$config['projeto'].':'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.(($editar && $Aplic->checarModulo('projetos', 'adicionar', $Aplic->usuario_id, 'demanda'))||$Aplic->usuario_super_admin ? '<a href="javascript:void(0);" onclick="url_passar(0, \'m=projetos&a=demanda_analise&demanda_id='.$demanda_id.'\');">'.dica('Editar','Clique neste link para ver ou modificar a análise de demanda.') : '').($obj->demanda_caracteristica_projeto > 0 ? 'Sim' : 'Não').(($editar && $Aplic->checarModulo('projetos', 'adicionar', $Aplic->usuario_id, 'demanda'))||$Aplic->usuario_super_admin ? dicaF() : '').'</td></tr>';				
if ($obj->demanda_viabilidade) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Estudo de Viabilidade', 'Visualização dos detalhes do estudo de viabilidade desta demanda.').'Estudo de viabilidade:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.link_viabilidade($obj->demanda_viabilidade).'</td></tr>';				
if ($obj->demanda_termo_abertura) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Termo de Abertura', 'Visualização dos detalhes do termo de abertura.').'Termo de abertura:'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.link_termo_abertura($obj->demanda_termo_abertura).'</td></tr>';				
if ($obj->demanda_projeto) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica(ucfirst($config['projeto']), 'Visualização dos detalhes do '.$config['projeto'].' que foi criado basead'.$config['genero_projeto'].' nesta demanda.').ucfirst($config['projeto']).':'.dicaF().'</td><td class="realce" width="100%" style="margin-bottom:0cm; margin-top:0cm;">'.link_projeto($obj->demanda_projeto).'</td></tr>';				
...
if ($Aplic->profissional){
	...
	$sql->limpar();
	if (count($ptres)) echo '<tr><td align="right" style="white-space: nowrap">'.dica('PTRES', 'Os PTRES relacionados com '.($portfolio ? $config['genero_portfolio'].' '.$config['portfolio'].' ' : '').'da demanda.').'PTRES:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.implode('<br>', $ptres).'</td></tr>';
	
	
	if ($lista_portfolio){
		$saida_fiho=array();
		foreach($lista_portfolio as $demanda_filho) $saida_fiho[]=link_demanda($demanda_filho);
		echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['portfolio']).' de Demandas', ucfirst($config['genero_portfolio']).' '.$config['portfolio'].' de demandas incluídas').ucfirst($config['portfolio']).':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.implode('<br>', $saida_fiho).'</td></tr>';
		}
	}	

if ($obj->demanda_principal_indicador) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Indicador Principal', 'Dentre os indicadores relacionados, o mais representativo da situação geral.').'Indicador principal:'.dicaF().'</td><td width="100%" class="realce">'.link_indicador($obj->demanda_principal_indicador).'</td></tr>';

if ($Aplic->profissional && $exibir['moeda'] && isset($moedas[$obj->demanda_moeda])) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Moeda', 'A moeda padrão utilizada na demanda.').'Moeda:'.dicaF().'</td><td class="realce" width="100%">'.$moedas[$obj->demanda_moeda].'</td></tr>';
	
		
if ($Aplic->profissional && $tem_aprovacao) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Aprovada', 'Se a demanda se encontra aprovada.').'Aprovada:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->demanda_aprovado ? 'Sim' : '<span style="color:red; font-weight:bold">Não</span>').'</td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Ativa', 'Se a demanda se encontra ativa.').'Ativa:'.dicaF().'</td><td  class="realce" width="100%">'.($obj->demanda_ativa ? 'Sim' : 'Não').'</td></tr>';
...
if (!$dialogo){
	$caixaTab = new CTabBox('m=projetos&a=demanda_ver&demanda_id='.$obj->demanda_id, '', $tab);
	$qnt_aba=0;
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'plano_acao_item')) {
		...
		$sql->adOnde('plano_acao_item_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'demanda')) {
		$sql->adTabela('demanda_gestao');
		$sql->adOnde('demanda_gestao_semelhante = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('praticas') && $Aplic->checarModulo('praticas', 'acesso', null, 'risco_resposta')) {		
		...
		$sql->adOnde('risco_resposta_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('recursos', 'acesso', null, null)) {
		...
		$sql->adOnde('recurso_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->modulo_ativo('patrocinadores') && $Aplic->checarModulo('patrocinadores', 'acesso', null, null)) {
		...
		$sql->adOnde('patrocinador_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'programa')) {
		...
		$sql->adOnde('programa_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'beneficio')) {
		...
		$sql->adOnde('beneficio_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('projeto', 'acesso', 'licao')) {
		...
		$sql->adOnde('licao_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'pratica')) {
		...
		$sql->adOnde('pratica_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('tr') && $Aplic->checarModulo('tr', 'acesso', null, null)) {
		...
		$sql->adOnde('tr_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'brainstorm')) {
		...
		$sql->adOnde('brainstorm_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'gut')) {
		...
		$sql->adOnde('gut_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'causa_efeito')) {
		...
		$sql->adOnde('causa_efeito_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'tgn')) {
		...
		$sql->adOnde('tgn_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'canvas')) {
		...
		$sql->adOnde('canvas_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('swot') && $Aplic->checarModulo('swot', 'acesso', null, null)) {
		...
		$sql->adOnde('mswot_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('swot') && $Aplic->checarModulo('swot', 'acesso', null, null)) {
		...
		$sql->adOnde('swot_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('operativo') && $Aplic->checarModulo('operativo', 'acesso', null, null)) {
		...
		$sql->adOnde('operativo_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'monitoramento')) {
		...
		$sql->adOnde('monitoramento_gestao_demanda = '.(int)$demanda_id);
		...
		$sql->adOnde('avaliacao_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'checklist')) {
		...
		$sql->adOnde('checklist_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional) {
		...
		$sql->adOnde('agenda_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('agrupamento') && $Aplic->checarModulo('agrupamento', 'acesso', null, null)) {
		...
		$sql->adOnde('agrupamento_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', null, 'modelo')) {
		...
		$sql->adOnde('template_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'painel_indicador')) {
		...
		$sql->adOnde('painel_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'odometro_indicador')) {
		...
		$sql->adOnde('painel_odometro_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'composicao_painel')) {
		...
		$sql->adOnde('painel_composicao_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('praticas', 'acesso', null, 'slideshow_painel')) {
		...
		$sql->adOnde('painel_slideshow_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->checarModulo('agenda', 'acesso', null, null)) {
		...
		$sql->adOnde('calendario_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'perspectiva')) {
		...
		$sql->adOnde('perspectiva_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'tema')) {
		...
		$sql->adOnde('tema_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'objetivo')) {
		...
		$sql->adOnde('objetivo_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'me')) {
		...
		$sql->adOnde('me_gestao_demanda = '.(int)$demanda_id);
	...
	if ($config['exibe_fator'] && $Aplic->checarModulo('praticas', 'acesso', null, 'fator')) {
		...
		$sql->adOnde('fator_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'iniciativa')) {
		...
		$sql->adOnde('estrategia_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'meta')) {
		...
		$sql->adOnde('meta_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('praticas', 'acesso', null, 'planejamento')) {
		...
		$sql->adOnde('plano_gestao_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'abertura')) {
		...
		$sql->adOnde('projeto_abertura_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->checarModulo('projetos', 'acesso', null, 'viabilidade')) {
		...
		$sql->adOnde('projeto_viabilidade_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso')) {
		...
		$sql->adOnde('ssti_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('ssti') && $Aplic->checarModulo('ssti', 'acesso',null, 'laudo')) {
		...
		$sql->adOnde('laudo_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'acesso')) {
		...
		$sql->adOnde('trelo_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('trelo') && $Aplic->checarModulo('trelo', 'acesso')) {
		...
		$sql->adOnde('trelo_cartao_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso')) {
		...
		$sql->adOnde('pdcl_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('pdcl') && $Aplic->checarModulo('pdcl', 'acesso')) {
		...
		$sql->adOnde('pdcl_item_gestao_demanda = '.(int)$demanda_id);
	...
	if ($Aplic->profissional && $Aplic->modulo_ativo('os') && $Aplic->checarModulo('os', 'acesso')) {
		...
		$sql->adOnde('os_gestao_demanda = '.(int)$demanda_id);
