...
class CEncerramento extends CAplicObjeto {
  public $projeto_encerramento_projeto = null;
  public $projeto_encerramento_responsavel = null;
  public $projeto_encerramento_justificativa = null;
  public $projeto_encerramento_encerrado = null;
  public $projeto_encerramento_encerrado_ressalvas = null;
  public $projeto_encerramento_nao_encerrado = null;
  public $projeto_encerramento_data = null;
  public $projeto_encerramento_cor = null;
  public $projeto_encerramento_acesso = null;
	...
	public function armazenar( $atualizarNulos = false) {
		...
		$sql = new BDConsulta();
		if ($_REQUEST['antigo']) {
			$ret = $sql->atualizarObjeto('projeto_encerramento', $this, 'projeto_encerramento_projeto');
			$sql->limpar();
			}
		else {
	...
	public function notificar( $post=array()){
		...
		foreach($usuarios as $usuario){
			if (!isset($usado[$usuario['usuario_id']]) && !isset($usado[$usuario['contato_email']])){
				...
				if ($tipo!='excluido') {
					$corpo_interno .= '<br><a href="javascript:void(0);" onclick="url_passar(0, \'m=projetos&a=viabilidade_ver&projeto_encerramento_projeto='.$this->projeto_encerramento_projeto.'\');"><b>Clique para acessar o termo de encerramento do projeto</b></a>';
					if ($Aplic->profissional){
						require_once BASE_DIR.'/incluir/funcoes_principais_pro.php';
						$endereco=link_email_externo($usuario['usuario_id'], 'm=projetos&a=viabilidade_ver&projeto_encerramento_projeto='.$this->projeto_encerramento_projeto);
						$corpo_externo.='<br><a href="'.$endereco.'"><b>Clique para acessar o termo de encerramento do projeto</b></a>';
						}
					}
