...
$data_inicio = intval($licaoinicio) ? new CData($licaoinicio) :  new CData(($ano-1).'-'.date("m-d"));
$data_fim = intval($licaofim) ? new CData($licaofim) : new CData(date("Y-m-d"));
...
if($Aplic->profissional){
  ...
  $sql->adCampo('*');
  $sql->adOnde("campo_customizado_modulo = 'licao_aprendida'");
...
if($Aplic->profissional && $campos_extras){
  ...
  foreach($campos_extras as $filtro){
    if($filtro['campo_customizado_valor_atual']){
      ...
      else $sql->adOnde('campo_customizado_valor_campo = '.$filtro['campo_customizado_id'].' AND campo_customizado_valor_caractere = \''.$filtro['campo_customizado_valor_atual'].'\'');
      $lista = $sql->listaVetorChave('campo_customizado_valor_objeto','campo_customizado_valor_objeto');
      $sql->limpar();
      if(!$first) $filtro_extra_lista = array_intersect_key($filtro_extra_lista, $lista);
      else $filtro_extra_lista = $lista;
      $first = false;
      }
    }
  if(!$first)  $filtro_extra_lista = implode(',', $filtro_extra_lista);
  }
...
if (!$dialogo || $selecao){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionad'.$config['genero_licao'], 'A que área '.$config['licao'].' está relacionad'.$config['genero_licao'].'.').'Relacionad'.$config['genero_licao'].':'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
	...
	$procurar_status='<tr><td align="right" style="white-space: nowrap">'.dica('Status', 'Filtre pelo status.').'Status:'.dicaF().'</td><td align="left" style="white-space: nowrap">'. selecionaVetor($licao_status, 'licaostatus', 'size="1" style="width:250px;"'.($Aplic->profissional ? ' multiple' :'').' class="texto"', $licaostatus) .'</td></tr>';
	$procurar_categoria='<tr><td align="right" style="white-space: nowrap">'.dica('Categoria', 'Filtre pela categoria.').'Categoria:'.dicaF().'</td><td align="left" style="white-space: nowrap">'. selecionaVetor($licao_categoria, 'licaocategoria', 'size="1" style="width:250px;"'.($Aplic->profissional ? ' multiple' :'').' class="texto"', $licaocategoria) .'</td></tr>';
	$procurar_tipo='<tr><td align="right" style="white-space: nowrap">'.dica('Tipo', 'Filtre pelo tipo.').'Tipo:'.dicaF().'</td><td align="left" style="white-space: nowrap">'. selecionaVetor($licao_tipo, 'licaotipo', 'size="1" style="width:250px;"'.($Aplic->profissional ? ' multiple' :'').' class="texto"', $licaotipo) .'</td></tr>';
	$procurar_periodo='<tr><td align="right" style="white-space: nowrap">'.dica('Período', 'Digite ou escolha no calendário a data provável de início.').'Período:'.dicaF().'</td><td style="white-space: nowrap"><table cellspacing=0 cellpadding=0><tr><td>'.dica('Usar Período', 'Marque caso deseje filtrar pela faixa de tempo.').'<input type="checkbox" value="1" name="usar_periodo" '.($usar_periodo ? 'checked="checked"' : '').' />'.dicaF().'<input type="hidden" name="licaoinicio" id="licaoinicio" value="'.($data_inicio ? $data_inicio->format("%Y-%m-%d") : '').'" /><input type="text" name="data_inicio" style="width:70px;" id="data_inicio" onchange="setData(\'env\', \'data_inicio\', \'licaoinicio\');" value="'.($data_inicio ? $data_inicio->format('%d/%m/%Y') : '').'" class="texto" />'.dica('Data de Início', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data provável de início.').'<a href="javascript: void(0);" ><img src="'.acharImagem('calendario.gif').'" id="f_btn1" style="vertical-align:middle" width="18" height="12" alt="Calendário" />'.dicaF().'</a><input type="hidden" name="licaofim" id="licaofim" value="'.($data_fim ? $data_fim->format("%Y-%m-%d") : '').'" /><input type="text" name="data_fim" id="data_fim" style="width:70px;" onchange="setData(\'env\', \'data_fim\', \'licaofim\');" value="'.($data_fim ? $data_fim->format('%d/%m/%Y') : '').'" class="texto" /><a href="javascript: void(0);" >'.dica('Meta de Término', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data provável de término.').'<img id="f_btn2" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" />'.dicaF().'</a></td></tr></table></td></tr>';
	$nova_licao=($Aplic->checarModulo('projetos', 'adicionar', null, 'licao') ? '<tr><td align="right" style="white-space: nowrap">'.dica('Nov'.$config['genero_licao'].' '.ucfirst($config['licao']), 'Criar nov'.$config['genero_licao'].' '.$config['licao'].'.').'<a href="javascript: void(0)" onclick="url_passar(0,\'m=projetos&a=licao_editar\');"><img src="'.acharImagem('licoes_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr><tr><td style="white-space: nowrap"></td></tr>' : '');
	$imprimir='<tr><td align="right" style="white-space: nowrap">'.dica('Imprimir', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de '.$config['licoes'].'.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=projetos&a=licao_lista&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF().'</td></tr>';
  $btnCustomizados = '<tr><td align="right" style="white-space: nowrap">'.dica('Filtar por campos customizados', 'Clique neste ícone '.imagem('custom_field_search.png').' para visualizar uma janela com filtros para campos customizados.').'<a href="javascript: void(0)" onclick="javascript:popFiltroCamposCustomizados();"><img src="'.acharImagem('custom_field_search.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>';
	
	$botao_favorito=($Aplic->profissional ? '<tr><td style="white-space: nowrap"><a href="javascript: void(0)" onclick="url_passar(0, \'m=publico&a=favoritos&licao=1\');">'.imagem('icones/favorito_p.png','Favoritos', 'Clique neste ícone '.imagem('icones/favorito_p.png').' para criar ou editar um grupo de favoritos, para uma rápida filtragem.').'</a></td></tr>' : '');
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_usuario.$procurar_status.$procurar_categoria.$procurar_tipo.$procuraBuffer.$popFiltro.$filtros.$favoritos.$procurar_periodo.'</table></td><td valign=top><table cellspacing=0 cellpadding=0 align=right>'.$botao_filtrar.(!$selecao ? $nova_licao.$botao_prioridade.$imprimir.$btnCustomizados.$botao_favorito.$botao_trava.$botao_campos : '').'</table></td></tr></table>';
	$saida.= '</div></div>';
...
if(!$dialogo || $selecao){
    ...
    $caixaTab->adicionar('licao_tabela', 'Ativ'.$config['genero_licao'].'s',null,null,'Ativ'.$config['genero_licao'].'s','Tod'.$config['genero_licao'].'s '.$config['genero_licao'].'s '.$config['licoes'].' ativas.');
    $caixaTab->adicionar('licao_tabela', 'Inativ'.$config['genero_licao'].'s',null,null,'Inativ'.$config['genero_licao'].'s','Tod'.$config['genero_licao'].'s '.$config['genero_licao'].'s '.$config['licoes'].' inativas.');
    $caixaTab->adicionar('licao_tabela', 'Tod'.$config['genero_licao'].'s',null,null,'Tod'.$config['genero_licao'].'s','Tod'.$config['genero_licao'].'s '.$config['genero_licao'].'s '.$config['licoes'].'.');
...
else if ($dialogo && !$selecao && !($Aplic->usuario_nomeguerra=='Visitante' && $Aplic->usuario_id=1)) echo '<script language=Javascript>self.print();</script>';	
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=licoes', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=projetos&a=licao_lista');
	}	
...
function priorizacao() {
	parent.gpwebApp.popUp("Priorização", 400, 300, 'm=publico&a=filtro_priorizacao_pro&dialogo=1&licao=1&filtro_prioridade='+env.filtro_prioridade_licao.value, window.setFiltroPriorizacao, window);
	}
