	...
	public function notificar( $post=array()){
		...
		foreach($usuarios as $usuario){
			if (!isset($usado[$usuario['usuario_id']]) && !isset($usado[$usuario['contato_email']])){
				...
				if ($tipo!='excluido') {
					$corpo_interno .= '<br><a href="javascript:void(0);" onclick="url_passar(0, \'m=projetos&a=mudanca_ver&projeto_mudanca_id='.$this->projeto_mudanca_id.'\');"><b>Clique para acessar a solicitação de mudanças</b></a>';
					if ($Aplic->profissional){
						...
						$corpo_externo.='<br><a href="'.$endereco.'"><b>Clique para acessar a solicitação de mudanças</b></a>';
						}
					}
