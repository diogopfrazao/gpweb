...
if (!$dialogo){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_projeto.$procurar_usuario.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$imprimir.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
if ($projeto_id){
    ...
    $sql->adOnde('tarefa_tarefaex_id IS NULL AND tarefa_projetoex_id IS NULL');
...
if ($projeto_id){
    echo '<table id="geral" width="100%" cellspacing=0 cellpadding=0>';
      ...
      if ($Aplic->profissional && $Aplic->checarModulo('projetos', 'acesso', $Aplic->usuario_id, 'projetos_wbsgrafico')) $km->Add("ver","wbs_grafico",dica('EAP Gráfica', 'Abre a interface de estrutura analítica de projeto (EAP) gráfica.').'EAP Gráfica'.dicaF(), "javascript: void(0);' onclick='ver_wbs_grafico();");
      ...
      if ($podeEditar) {
        ...
        if (!$portfolio) {
            $km->Add("inserir","adicionar",dica(ucfirst($config['tarefa']),'Adicionar '.$config['tarefa'].' a'.($config['genero_projeto']=='o' ? 'o' : '').' '.$config['projeto'].'. Pode-se utilizar teclas de atalho (Alt+T).').ucfirst($config['tarefa']).' (Alt+T)'.dicaF(), "javascript: void(0);' onclick='addTreeNode();");
            ...
            if ($Aplic->checarModulo('eventos', 'adicionar')) $km->Add("inserir","inserir_evento",dica('Novo Evento', 'Criar um novo evento.<br><br>Os eventos são atividades com data e hora específicas podendo estar relacionados com '.$config['projetos'].','.$config['tarefas'].' e '.$config['usuarios'].' específicos.').'Evento'.dicaF(),  "javascript: void(0);' onclick='adicionarEvento();");
...
function acrescentar_subordinada($tarefa_pai=0){
    ...
    $sql->adTabela('tarefas');
    $sql->adCampo('tarefa_id, tarefa_nome');
    $sql->adOnde('tarefa_superior ='.$tarefa_pai.' AND tarefa_id!='.$tarefa_pai);
    $sql->adOnde('tarefa_tarefaex_id IS NULL AND tarefa_projetoex_id IS NULL');
...
jQuery(document).on('keydown', function(e) {
  if ((e.altKey) && (e.which === 84)) addTreeNode(); // (Alt+T)
});
...
function adicionarOcorrencia(){
    if (nodeselect=='treeview.root' || nodeselect==null) alert("Selecione <?php echo $config['genero_tarefa'].' '.$config['tarefa'] ?>");
    else parent.gpwebApp.popUp('Registro de Ocorrência', 800, 500, 'm=tarefas&a=ver_log_atualizar&dialogo=1&projeto_id='+projeto_id+'&tarefa_id='+nodeselect, null, window);
    }
...
function adicionarMsg(){
    if (nodeselect==null) alert("Selecione <?php echo $config['genero_projeto'].' '.$config['projeto'].' ou '.$config['tarefa'] ?>");
    else parent.gpwebApp.popUp("<?php echo ucfirst($config['mensagem']) ?>", 800, 500, 'm=email&a=nova_mensagem&dialogo=1&msg_projeto='+projeto_id+'&msg_tarefa='+(nodeselect!='treeview.root' ? nodeselect : ''), null, window);
    }
...
function duplicar_tarefa(){
    if (nodeselect=='treeview.root' || nodeselect==null) alert("Selecione <?php echo $config['genero_tarefa'].' '.$config['tarefa'] ?>");
        ...
        if (nome_tarefa!=null && nome_tarefa!='')    {
            url_passar(0, 'm=projetos&a=wbs_vertical&jquery=1&projeto_id='+projeto_id+'&duplicar='+nodeselect+'&nome_tarefa='+nome_tarefa);
            }
...
function popProjeto() {
    if (window.parent.gpwebApp) parent.gpwebApp.popUp('<?php echo ucfirst($config["projeto"])?>', 630, 700, 'm=publico&a=selecionar&dialogo=1&chamar_volta=setProjeto&edicao=1&edicaoCompleta=1&tabela=projetos&usuario_id='+document.getElementById('usuario_id').value+'&cia_id='+document.getElementById('cia_id').value, window.setProjeto, window);
    else window.open('./index.php?m=publico&a=selecionar&dialogo=1&edicao=1&edicaoCompleta=1&chamar_volta=setProjeto&tabela=projetos&usuario_id='+document.getElementById('usuario_id').value+'&cia_id='+document.getElementById('cia_id').value, 'Projetos','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
    }
