...
if (isset($_REQUEST['dept_id'])) $Aplic->setEstado('dept_id', getParam($_REQUEST, 'dept_id', null));
...
$editar=permiteEditarDept($obj->dept_acesso, $dept_id);

$podeEditarTudo=($obj->dept_acesso>=5 ? $editar && (in_array($obj->dept_responsavel, $Aplic->usuario_lista_grupo_vetor) || $Aplic->usuario_super_admin || $Aplic->usuario_admin) : $editar);
...
if (!$dialogo && !$Aplic->profissional) {
	...
	if ($podeEditar && ($podeEditarTudo || $Aplic->usuario_super_admin || $Aplic->usuario_admin)) $botoesTitulo->adicionaBotao('m=depts&a=editar&dept_id='.$dept_id, 'editar '.($config['genero_dept']=='o' ? 'este' : 'esta').' '.strtolower($config['departamento']),'','Editar '.($config['genero_dept']=='o' ? 'este' : 'esta').' '.$config['departamento'],'Editar os detalhes relativos a '.($config['genero_dept']=='o' ? 'este' : 'esta').' '.strtolower($config['departamento']).'.');
	if ($podeExcluir && ($podeEditarTudo || $Aplic->usuario_super_admin || $Aplic->usuario_admin))	$botoesTitulo->adicionaBotaoExcluir('excluir '.strtolower($config['dept']), $podeExcluir, (isset($msg) ? $msg : ''), 'Excluir '.$config['departamento'],'Excluir '.($config['genero_dept']=='o' ? 'este' : 'esta').' '.strtolower($config['departamento']).'.');	
...
if (!$dialogo && $Aplic->profissional){	
	...
	if ($podeAdicionar)	$km->Add("inserir","inserir_objeto",dica('Nov'.$config['genero_dept'].' '.ucfirst($config['departamento']), 'Criar um nov'.$config['genero_dept'].' '.$config['departamento'].'.').'Nov'.$config['genero_dept'].' '.ucfirst($config['departamento']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=depts&a=editar&cia_id=".(int)$cia_id."\");");


	$km->Add("root","acao",dica('Ação','Menu de ações.').'Ação'.dicaF(), "javascript: void(0);'");

	if ($podeEditarTudo && $podeEditar)	$km->Add("acao","acao_editar",dica('Editar '.ucfirst($config['departamento']),'Editar os detalhes d'.($config['genero_dept']=='a' ? 'esta' : 'este').' '.$config['departamento'].'.').'Editar '.ucfirst($config['departamento']).dicaF(), "javascript: void(0);' onclick='url_passar(0, \"m=depts&a=editar&dept_id=".$dept_id."\");");
	if ($podeEditarTudo && $podeExcluir && ($Aplic->usuario_super_admin || $Aplic->usuario_admin)) $km->Add("acao","acao_excluir",dica('Excluir','Excluir '.($config['genero_dept']=='a' ? 'esta' : 'este').' '.$config['departamento'].'.').'Excluir '.ucfirst($config['departamento']).dicaF(), "javascript: void(0);' onclick='excluir()");
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['organizacao']), 'Tod'.$config['genero_dept'].' '.strtolower($config['departamento']).' deve pertencer à um'.$config['genero_organizacao'].' '.$config['organizacao']).ucfirst($config['organizacao']).':'.dicaF().'</td><td class="realce" width="100%">'.link_cia($obj->dept_cia).'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nome d'.$config['genero_dept'].' '.$config['departamento'], 'Tod'.$config['genero_dept'].' '.strtolower($config['departamento']).' deve ter um nome exclusivo e obrigatório.').ucfirst($config['dept']).':'.dicaF().'</td><td class="realce" width="100%">'.$obj->dept_nome.'</td></tr>';
if ($obj->dept_superior) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamento']).' Superior', ucfirst($config['genero_dept']).' '.strtolower($config['departamento']).' superior a est'.($config['genero_dept']=='a' ? 'a' : 'e').'.').ucfirst($config['departamento']).' superior:'.dicaF().'</td><td align="left" class="realce" width="100%">'.nome_dept($obj->dept_superior).'</td></td></tr>';
if ($obj->dept_responsavel) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Responsável', 'Tod'.$config['genero_dept'].' '.strtolower($config['departamento']).' deve ter um responsável.<br><br>Embora não tenha impacto no funcionamento do Sistema, facilita a organização e na eventual necessidade de entrar em contato.').'Responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_contato($obj->dept_responsavel,'','','esquerda').'</td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Tipo de '.$config['departamento'], 'Embora não tenha impacto no funcionamento do Sistema, facilita a organização ao separar '.$config['genero_dept'].'s '.strtolower($config['departamentos']).' por tipo.').'Tipo:'.dicaF().'</td><td class="realce" width="100%">'.$tipos[$obj->dept_tipo].'</td></tr>';
...
if ($obj->dept_codigo) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Código', 'Código d'.$config['genero_dept'].' '.strtolower($config['departamento']).'.').'Código:'.dicaF().'</td><td class="realce" width="100%">'.$obj->dept_codigo.'</td></tr>';
if ($obj->dept_email) echo '<tr><td align="right" style="white-space: nowrap">'.dica('e-mail', 'e-mail d'.$config['genero_dept'].' '.strtolower($config['departamento']).'.<br><br>Embora não tenha impacto no funcionamento do Sistema, facilita a organização e na eventual necessidade de entrar em contato.').'e-mail:'.dicaF().'</td><td class="realce" width="100%">'.$obj->dept_email.'</td></tr>';
if ($obj->dept_tel) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Telefone', 'Telefone d'.$config['genero_dept'].' '.strtolower($config['departamento']).'.<br><br>Embora não tenha impacto no funcionamento do Sistema, facilita a organização e na eventual necessidade de entrar em contato.').'Telefone:'.dicaF().'</td><td class="realce" width="100%">'.$obj->dept_tel.'</td></tr>';
if ($obj->dept_fax) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Fax', 'Fax d'.$config['genero_dept'].' '.strtolower($config['departamento']).'.<br><br>Embora não tenha impacto no funcionamento do Sistema, facilita a organização e na eventual necessidade de entrar em contato.').'Fax:'.dicaF().'</td><td class="realce" width="100%">'.$obj->dept_fax.'</td></tr>';
if ($obj->dept_endereco1) echo '<tr valign="top"><td align="right" style="white-space: nowrap">'.dica('Endereço', 'O enderço d'.$config['genero_dept'].' '.strtolower($config['departamento']).'.<br><br>Embora não tenha impacto no funcionamento do Sistema, facilita a organização e na eventual necessidade de entrar em contato.').'Endereço:'.dicaF().'</td><td class="realce">'.dica('Google Maps', 'Clique esta imagem para visualizar no Google Maps, aberto em uma nova janela, o endereço d'.$config['genero_dept'].' '.strtolower($config['departamento']).'.').'<a href="'.get_protocol().'maps.google.com/maps?key=AIzaSyAsFbkGMNJdcsHBSQySo8jpA7zqBhlg1Pg&q='.utf8_encode($obj->dept_endereco1).'+'.utf8_encode($obj->dept_endereco2).'+'.utf8_encode($obj->dept_cidade).'+'.utf8_encode($obj->dept_estado).'+'.utf8_encode($obj->dept_cep).'+'.utf8_encode($obj->dept_pais).'" target="_blank"><img align="right" border=0 src="'.acharImagem('google_map.png').'" alt="Achar no Google Maps" /></a>'.dicaF().$obj->dept_endereco1.(($obj->dept_endereco2) ? '<br />'.$obj->dept_endereco2 : '').'<br />'.$obj->dept_cidade.'&nbsp;&nbsp;'.$obj->dept_estado.'&nbsp;&nbsp;'.$obj->dept_cep.(($obj->dept_pais) ? '<br />'.$paises[$obj->dept_pais] : '').'</td></tr>';
if ($obj->dept_nup) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Identificador de NUP', 'Caso utilize o sistema único e processos faz-se necessário informar o número identificador d'.$config['genero_dept'].' '.$config['departamento'].' de 5 algarismos.').'Identificador de NUP:'.dicaF().'</td><td width="100%" class="realce">'.$obj->dept_nup.'</td></tr>';
if ($obj->dept_qnt_nr) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Quantos Protocolos', 'Caso utilize utilize um sistema de protocolo, faz-se necessário informar quantos protocolos já foram emitidos, neste ano, para que aqueles emitidos pelo '.$config['gpweb'].' sigam a sequuência numérica crescente.').'Quantos protocolos:'.dicaF().'</td><td width="100%" class="realce" >'.$obj->dept_qnt_nr.'</td></tr>';
if ($obj->dept_prefixo) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Prefixo','Preencha, caso exista, o prefixo à numeração sequencial crescente, nos protocolos diversos de NUP.').'Prefixo:'.dicaF().'</td><td class="realce"  width="100%" colspan="2">'.$obj->dept_prefixo.'</td></tr>';
if ($obj->dept_sufixo) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Sufixo','Preencha, caso exista, o sufixo à numeração sequencial crescente, nos protocolos diversos de NUP.').'Sufixo:'.dicaF().'</td><td class="realce" width="100%">'.$obj->dept_sufixo.'</td></tr>';
if ($obj->dept_ordem) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Ordem', 'Caso deseje que '.$config['genero_dept'].'s '.strtolower($config['departamentos']).' sejam ordenad'.$config['genero_dept'].'s em uma ordem diferente da alfabética este campo deverá ser preenchido. A lista é ordenada por este campo de forma ascendente, ou seja numerações de ordem mais baixas aparecem primeiro.').'Ordem:'.dicaF().'</td><td width="100%" class="realce" >'.$obj->dept_ordem.'</td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Ativ'.$config['genero_dept'], ucfirst($config['genero_dept']).' '.$config['dept'].' se encontra ativ'.$config['genero_dept'].'.').'Ativ'.$config['genero_dept'].':'.dicaF().'</td><td class="realce" style="text-align: justify;">'.($obj->dept_ativo ? 'Sim' : 'Não').'</td></tr>';
