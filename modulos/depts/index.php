...
if (!$dialogo){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$novo=($cia_id && $Aplic->checarModulo('depts', 'adicionar') && ($Aplic->usuario_super_admin || $Aplic->usuario_admin) ? '<tr><td style="white-space: nowrap">'.dica('Nov'.$config['genero_dept'].' '.$config['departamento'], 'Criar um'.$config['genero_dept'].' nov'.$config['genero_dept'].' '.$config['departamento'].' dentro d'.$config['genero_organizacao'].' '.$config['organizacao'].' atual.').'<a href="javascript: void(0)" onclick="javascript:frm_dept.a.value=\'editar\'; frm_dept.submit();" ><img src="'.acharImagem('depts_novo.png').'" border=0 width="16" heigth="16" /></a>'.dicaF().'</td></tr>' : '');

	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_usuario.$pesquisar.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$novo.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
$caixaTab = new CTabBox('m=depts', BASE_DIR.'/modulos/depts/', $tab);
