...
echo '<input type="hidden" name="arquivo_pasta_usuario" id="arquivo_pasta_usuario" value="'.$arquivo_pasta_usuario.'" />';
...
if (!$dialogo){
	...
	elseif($arquivo_pasta_usuario){
		$legenda_filtro=dica('Filtrar pelas Particulares', 'Filtrar pelas suas pastas particulares.').'Particular:'.dicaF();
		$nome=nome_usuario($arquivo_pasta_usuario);
		}		
		
	elseif($arquivo_pasta_id){
		$legenda_filtro=dica('Filtrar pela Pasta', 'Filtrar pela pasta relacionada.').'Pasta:'.dicaF();
		$nome=nome_pasta($arquivo_pasta_id);
		}		
	...
	$popFiltro='<tr><td align="right" style="white-space: nowrap">'.dica('Relacionada', 'A que área a pasta está relacionada.').'Relacionada:'.dicaF().'</td><td align="left">'.selecionaVetor($tipos, 'tipo_relacao', 'style="width:250px;" class="texto" onchange="popRelacao(this.value)"').'</td></tr>';
...
if (!$dialogo){
  ...
  $saida.='<table cellspacing=0 cellpadding=0>';
	...
	$saida.='<tr><td valign=top><table cellspacing=0 cellpadding=0>'.$procurar_om.$procurar_envolvido.$procurar_dono.$procuraBuffer.$popFiltro.$filtros.'</table></td><td valign=top><table cellspacing=0 cellpadding=0>'.$novo_arquivo_pasta.$botao_campos.'</table></td></tr></table>';
	$saida.= '</div></div>';
...
function popCamposExibir(){
	parent.gpwebApp.popUp('Campos', 500, 500, 'm=publico&a=campos&dialogo=1&campo_formulario_tipo=arquivo_pastas', window.setCamposExibir, window);
	}
	
function setCamposExibir(){
	url_passar(0, 'm=arquivos&a=pasta_lista');
	}		
...
function limpar_tudo(){
	...
	document.env.arquivo_pasta_id.value = null;		
	}


function popPasta(){
	if (window.parent.gpwebApp)parent.gpwebApp.popUp('Pasta', 800, 600, 'm=publico&a=selecao_unica_pasta&dialogo=1&arquivo=1&chamar_volta=setPasta&cia_id='+document.getElementById('cia_id').value, window.setPasta, window);
	else window.open('./index.php?m=publico&a=selecao_unica_pasta&dialogo=1&arquivo=1&chamar_volta=setPasta&cia_id='+document.getElementById('cia_id').value, 'Pasta','left=0,top=0,height=700,width=700,scrollbars=yes, resizable=yes');
	}

function setPasta(chave, valor){
	limpar_tudo();
	document.env.arquivo_pasta_id.value = chave;
	env.submit();
	}
