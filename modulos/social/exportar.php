...
echo '<th width="80"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.'&ordenar=social_familia_envio_data&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_envio_data' ? imagem('icones/'.$seta[$ordem]) : '').dica('Data', 'Neste campo fica a data da geração do arqivo.').'Data'.dicaF().'</a></th>';
echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.'&ordenar=social_familia_envio_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_envio_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Responsável', 'Neste campo fica o nome do responsável pela geração do arquivo.').'Responsável'.dicaF().'</a></th>';
echo '<th><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.'&ordenar=social_familia_envio_uuid&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_envio_uuid' ? imagem('icones/'.$seta[$ordem]) : '').dica('Arquivo', 'Neste campo fica o arquivo gerado.').'Arquivo'.dicaF().'</a></th>';


echo '<tr><td style="white-space: nowrap" width="20">'.dica('Marcar para Exportar', 'Clique nesta caixa para exportar novos beneficiários').'<input type="checkbox" value="novos" name="marcado[]" '.(!$qnt ? 'DISABLED' : '').' /></td><td style="white-space: nowrap" >'.retorna_data(date('Y-m-d H:i:s')).'</td><td>'.$qnt.' novos beneficiários</td><td>&nbsp</td></tr>';
foreach($lista as $linha){
	echo '<tr>';
	echo '<td style="white-space: nowrap" width="20">'.dica('Marcar para Exportar', 'Clique nesta caixa para marcar este lote para nova exportação.').'<input type="checkbox" value="'.$linha['social_familia_envio_uuid'].'" name="marcado[]" /></td>';
	echo '<td style="white-space: nowrap" width="20">'.retorna_data($linha['social_familia_envio_data']).'</td>';
