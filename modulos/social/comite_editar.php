...
if ($social_comite_id && !$podeEditar) $Aplic->redirecionar('m=publico&a=acesso_negado');
elseif (!$social_comite_id && !$podeAdicionar) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Telefone Principal', 'O telefone principal do comitê.').'Telefone Principal:'.dicaF().'</td><td><input type="text" class="texto" name="social_comite_tel" value="'.$obj->social_comite_tel.'" maxlength="30" size="25" /></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Telefone Reserva', 'O telefone residencial do comitê.').'Telefone Reserva:'.dicaF().'</td><td><input type="text" class="texto" name="social_comite_tel2" value="'.$obj->social_comite_tel2.'" maxlength="30" size="25" /></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Celular', 'O celular do comitê.').'Celular:'.dicaF().'</td><td><input type="text" class="texto" name="social_comite_cel" value="'.$obj->social_comite_cel.'" maxlength="30" size="25" /></td></tr>';
echo '<tr><td align="right">'.dica('e-mail', 'O e-mail do comitê.').'e-mail:'.dicaF().'</td><td style="white-space: nowrap"><input type="text" class="texto" name="social_comite_email" value="'.$obj->social_comite_email.'" maxlength="255" size="25" /></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Cor', 'Cor selecionada dentre as 16 milhões possíveis. Pode-se escrever diretamente o hexadecinal na cor ou utilizar a interface que se abre ao clicar na caixa de inserção do valor.').'Cor:'.dicaF().'</td><td align="left" style="white-space: nowrap"><input class="jscolor" name="social_comite_cor" value="'.($obj->social_comite_cor ? $obj->social_comite_cor : 'FFFFFF').'" '.($config['selecao_cor_restrita'] ? 'readonly="readonly" ' : '').'size="6" maxlength="6" style="width:57px;" /></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Responsável pelo Comitê', 'Todo comitê deve ter um responsável.').'Responsável:'.dicaF().'</td><td colspan="2"><input type="hidden" id="social_comite_responsavel" name="social_comite_responsavel" value="'.$obj->social_comite_responsavel.'" /><input type="text" id="nome_gerente" name="nome_gerente" value="'.nome_om($obj->social_comite_responsavel, $Aplic->getPref('om_usuario')).'" style="width:284px;" class="texto" READONLY /><a href="javascript: void(0);" onclick="popGerente();">'.imagem('icones/usuarios.gif','Selecionar '.ucfirst($config['usuario']),'Clique neste ícone '.imagem('icones/usuarios.gif').' para selecionar '.($config['genero_usuario']=='o' ? 'um' : 'uma').' '.$config['usuario'].'.').'</a></td></tr>';
...
if ($saida_quem) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Membros', 'Quais '.$config['usuarios'].' são membros deste comitê.').'Membros:'.dicaF().'</td><td width="100%" colspan="2"><table><tr><td>'.$saida_quem.'</td></tr></table></td></tr>';

echo '<tr><td align="right" style="white-space: nowrap"></td><td width="100%" colspan="2"><table><tr><td>'.botao('membros', 'Membros','Abrir uma janela onde poderá selecionar quais serão os membros deste comitê.','','popUsuarios()').'</td></tr></table></td></tr>';
