...
foreach($quantidade as $chave => $linha){
	...
	foreach($familias as $familia){
		...
		echo '<tr><td><table cellpadding=0 cellspacing=0 class="tbl1"><tr><th width="250"  style="font-weight: bold">'.ucfirst($config['beneficiario']).'</th><th style="font-weight: bold">Atividades previstas</th><th width="50" style="font-weight: bold">Realizadas</th><th width="50" style="font-weight: bold">Problemas</th></tr>';
		$col2='<table cellpadding=0 cellspacing=0 align="center">';
		foreach ($lista as $linha) $col2.='<tr><td align="center">'.(isset($lista_marcados[$linha['social_acao_lista_id']])? 'OK' : 'Não').'</td></tr>';
		$col2.= '<tr><td align="center">'.(($familia['social_familia_acao_concluido'])? 'OK' : 'Não').'</td></tr>';
		$col2.='</table>';
		echo '<tr><td>'.$familia['social_familia_nome'].'</td><td>'.$col_descricao.'</td><td>'.$col2.'</td><td align="center">'.($problema ? $problema : 'Nenhum').'</td></tr>';
		echo '</table></td></tr>';
		}
