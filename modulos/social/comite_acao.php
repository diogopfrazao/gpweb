...
if ($excluiranexo){
	...
	if ($arquivo['social_acao_arquivo_local']) @unlink($base_dir.'/arquivos/'.$arquivo['social_acao_arquivo_local'].$arquivo['social_acao_arquivo_nome_real']);
	else @unlink($base_dir.'/arquivos/acoes_comites/'.$arquivo['social_acao_arquivo_endereco']);
...
echo '<tr><td colspan=20><table cellpadding=0 cellspacing=2><tr><td align="right">'.dica('Programa Social', 'Escolha o programa social no qual há uma ação social à inserir neste comitê.').'Programa:'.dicaF().'</td><td align="left" style="white-space: nowrap">'.selecionaVetor($lista_programas, 'social_id', 'size="1" style="width:160px;" class="texto" onchange="mudar_acao()"', $social_id).'</td><td align="right">&nbsp;&nbsp;'.dica('Ação Social', 'Escolha a ação social à inserir neste comitê.').'Ação:'.dicaF().'</td><td align="left" style="white-space: nowrap"><div id="acao_combo">'.selecionar_acao_para_ajax($social_id, 'acao_id', 'size="1" style="width:160px;" class="texto"', '', $acao_id, false).'</div></td><td><a href="javascript:void(0);" onclick="javascript:inserir_acao();">'.imagem('icones/adicionar.png', 'Inserir', 'Clique neste ícone '.imagem('icones/adicionar.png').' para adicionar a ação social à esquerda no comitê.').'</a></td></tr></table></td></tr>';
...
if ($obj->social_comite_tel) echo '<tr><td align="right" style="white-space: nowrap" width="110">'.dica('Telefone Principal', 'O telefone principal do comitê.').'Telefone principal:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->social_comite_tel.'</td></tr>';
if ($obj->social_comite_tel2) echo '<tr><td align="right" style="white-space: nowrap" width="110">'.dica('Telefone Reserva', 'O telefone residencial do comitê.').'Telefone reserva:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->social_comite_tel2.'</td></tr>';
if ($obj->social_comite_cel) echo '<tr><td align="right" style="white-space: nowrap" width="110">'.dica('Celular', 'O celular do comitê.').'Celular:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->social_comite_cel.'</td></tr>';
if ($obj->social_comite_email) echo '<tr><td align="right">'.dica('e-mail', 'O e-mail do comitê.').'e-mail:'.dicaF().'</td><td style="white-space: nowrap" class="realce">'.$obj->social_comite_email.'</td></tr>';
if ($obj->social_comite_responsavel) echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Responsável pelo Comitê', ucfirst($config['usuario']).' responsável pelo comitê.').'Responsável:'.dicaF().'</td><td class="realce" width="100%">'.link_usuario($obj->social_comite_responsavel, '','','esquerda').'</td></tr>';		
echo '</table></fieldset></td></tr>';
