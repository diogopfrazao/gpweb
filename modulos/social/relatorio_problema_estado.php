...
echo '<tr><td align=center><h1>Lista dos Problemas '.($tipo==0 ? 'n'.$config['genero_beneficiario'].'s '.ucfirst($config['beneficiarios']) : '').($tipo==4 ? 'nas Comissões Comunitárias' : '').($tipo==3 ? 'nos Comitês Municipais' : '').($tipo==2 ? 'nas Coordenações Regionais' : '').'</h1><br></td></tr>';
