...
echo '<tr><td align="right">'.dica('Data', 'Escolha qual a data deste registro de ocorrência.').'Data:'.dicaF().'</td><td style="white-space: nowrap"><input type="hidden" name="social_comunidade_log_data" id="social_comunidade_log_data" value="'.$log_data->format('%Y%m%d').'" /><input type="text" name="log_date" id="log_date" onchange="setData(\'frmEditar\', \'log_date\');" value="'.$log_data->format('%d/%m/%Y').'" class="texto" />'.dica('Data do Registro', 'Clique neste ícone '.imagem('icones/calendario.gif').'  para abrir um calendário onde poderá selecionar a data deste registro.').'<a href="javascript: void(0);" ><img id="f_btn1" src="'.acharImagem('calendario.gif').'" style="vertical-align:middle" width="18" height="12" alt="Calendário" border=0 /></a>'.dicaF().'</td></tr>';
echo '<tr><td align="right">'.dica('Horas Trabalhadas', 'Horas trabalhadas na social.<br><br>Ex: Para inserir 1h30min digite 1.5').'Horas Trab.:'.dicaF().'</td><td style="white-space: nowrap"><input type="text" style="text-align:right;" class="texto" name="social_comunidade_log_horas" value="'.$log->social_comunidade_log_horas.'" maxlength="8" size="4" /></td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nível de Acesso', 'O registro de ocorrência da social pode ter cinco níveis de acesso:<ul><li><b>Público</b> - Todos podem ver e editar.</li><li><b>Protegido I</b> - Todos podem ver, porem apenas o responsável pela social e os designados podem editar.</li><li><b>Protegido II</b> - Todos podem ver, porem apenas o responsável pode editar tudo e os designados editarem os objetos relacionados.</li><li><b>Protegido III</b> - Todos podem ver, porem apenas o responsável pode editar.</li><li><b>Participante I</b> - Somente o responsável pela social e os designados podem ver e editar</li><li><b>Participantes II</b> - Somente o responsável e os designados podem ver e apenas o responsável pode editar tudo e os designados editarem os objetos relacionados.</li><li><b>Participantes III</b> - Somente o responsável pela social e os designados podem ver, e o responsável editar.</li></ul>').'Nível de Acesso'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($niveis_acesso, 'social_comunidade_log_acesso', 'class="texto"', ($social_comunidade_log_id ? $log->social_comunidade_log_acesso : $config['nivel_acesso_padrao'])).'</td></tr>';	
echo '</table></td>';
...
echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Notificar', 'Marque esta caixa para avisar sobre a '.($social_comunidade_log_id > 0 ? 'modificação' : 'criação').' do registro.').'Notificar:'.dicaF().'</td>';
echo '<td>';
...
function updateTarefa() {
	...
	if (f.social_comunidade_log_descricao.value.length < 3) {
