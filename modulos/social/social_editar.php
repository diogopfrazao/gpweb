...
if ($social_id && !$podeEditar) $Aplic->redirecionar('m=publico&a=acesso_negado');
elseif (!$social_id && !$podeAdicionar) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if ($saida_quem) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Quem', 'Quais '.$config['usuarios'].' estarão trabalhando junto a este programa social.').'Quem:'.dicaF().'</td><td width="100%" colspan="2"><table><tr><td>'.$saida_quem.'</td></tr></table></td></tr>';
...
if ($saida_depts) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamentos']), 'Qual '.strtolower($config['departamento']).' está relacionad'.$config['genero_dept'].' à este programa social.').ucfirst($config['departamento']).':'.dicaF().'</td><td width="100%" colspan="2">'.$saida_depts.'</td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Responsável pelo Programa Social', 'Todo programa social deve ter um responsável.').'Responsável:'.dicaF().'</td><td colspan="2"><input type="hidden" id="social_responsavel" name="social_responsavel" value="'.($obj->social_responsavel ? $obj->social_responsavel : $Aplic->usuario_id).'" /><input type="text" id="nome_gerente" name="nome_gerente" value="'.nome_om(($obj->social_responsavel? $obj->social_responsavel : $Aplic->usuario_id),$Aplic->getPref('om_usuario')).'" style="width:284px;" class="texto" READONLY /><a href="javascript: void(0);" onclick="popGerente();">'.imagem('icones/usuarios.gif','Selecionar '.ucfirst($config['usuario']),'Clique neste ícone '.imagem('icones/usuarios.gif').' para selecionar '.($config['genero_usuario']=='o' ? 'um' : 'uma').' '.$config['usuario'].'.').'</a></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Cor', 'Cor selecionada dentre as 16 milhões possíveis. Pode-se escrever diretamente o hexadecinal na cor ou utilizar a interface que se abre ao clicar na caixa de inserção do valor.').'Cor:'.dicaF().'</td><td align="left" style="white-space: nowrap"><input class="jscolor" name="social_cor" value="'.($obj->social_cor ? $obj->social_cor : 'FFFFFF').'" '.($config['selecao_cor_restrita'] ? 'readonly="readonly" ' : '').'size="6" maxlength="6" style="width:57px;" /></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Nível de Acesso', 'O programa social pode ter cinco níveis de acesso:<ul><li><b>Público</b> - Todos podem ver e editar o programa social.</li><li><b>Protegido I</b> - Todos podem ver, porem apenas o responsável e os designados para o programa social podem editar.</li><li><b>Protegido II</b> - Todos podem ver, porem apenas o responsável pode editar tudo e os designados editarem os objetos relacionados.</li><li><b>Protegido III</b> - Todos podem ver, porem apenas o responsável pode editar.</li><li><b>Participante I</b> - Somente o responsável e os designados para o programa social ver e editar o programa social</li><li><b>Participantes II</b> - Somente o responsável e os designados podem ver e apenas o responsável pode editar tudo e os designados editarem os objetos relacionados.</li><li><b>Participantes III</b> - Somente o responsável e os designados para o programa social podem ver a mesma, e o responsável editar.</li></ul>').'Nível de acesso:'.dicaF().'</td><td width="100%" colspan="2">'.selecionaVetor($social_acesso, 'social_acesso', 'class="texto"', ($social_id ? $obj->social_acesso : $config['nivel_acesso_padrao'])).'</td></tr>';


$campos_customizados = new CampoCustomizados('social', $social_id, 'editar');
$campos_customizados->imprimirHTML();
echo '<tr><td align="right" valign="top" style="white-space: nowrap">'.dica('Notificar', 'Marque esta caixa para avisar sobre a '.($social_id > 0 ? 'modificação' : 'criação').' do programa social.').'Notificar:'.dicaF().'</td>';
