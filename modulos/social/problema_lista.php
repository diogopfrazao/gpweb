...
$sql = new BDConsulta;
$status=array('' => '')+getSisValor('StatusProblemaSocial');
...
if (!$dialogo){
	...
	$tipos='<tr><td align="right" style="white-space: nowrap">'.dica('Grupo', 'Filtre os problemas pelo grupo ao qual pertencem.').'Grupo:'.dicaF().'</td><td align="left" style="white-space: nowrap">'. selecionaVetor($lista_tipo, 'problema_tipo', 'size="1" style="width:160px;" class="texto"', $problema_tipo) .'</td></tr>';
	...
	$programas='<tr><td align="right" style="white-space: nowrap">'.dica('Programa Social', 'Filtre os problemas pelo programa social em que estão inseridos.').'Programa:'.dicaF().'</td><td align="left" style="white-space: nowrap">'. selecionaVetor($lista_programas, 'social_id', 'size="1" style="width:160px;" class="texto" onchange="mudar_acao()"', $social_id) .'</td></tr>';
	$acoes='<tr><td align="right" style="white-space: nowrap">'.dica('Ação Social', 'Filtre os problemas pela ação social à qual estão relacionados.').'Ação:'.dicaF().'</td><td align="left" style="white-space: nowrap"><div id="acao_combo">'.selecionar_acao_para_ajax($social_id, 'acao_id', 'size="1" style="width:160px;" class="texto" onchange="mudar_problema()"', '', $acao_id, false).'</div></td></tr>';
	$pesquisar='<tr><td align="right" style="white-space: nowrap">'.dica('Pesquisa', 'Pesquisar os problemas pelo campo texto à direita.').'Pesquisar:'.dicaF().'</td><td align="left" style="white-space: nowrap"><input type="text" class="texto" style="width:145px;" name="familiabusca" onChange="document.env.submit();" value="'.$pesquisa.'"/><a href="javascript:void(0);" onclick="url_passar(0, \'m=social&a=familia_lista&familiabusca=\');">'.imagem('icones/limpar_p.gif','Limpar Pesquisa', 'Clique neste ícone '.imagem('icones/limpar_p.gif').' para limpar a caixa texto de pesquisa.').'</a></td></tr>';
	...
	$problemas='<tr><td align="right" style="white-space: nowrap">'.dica('Tipo', 'Filtre os problemas pelo tipo.').'Tipo:'.dicaF().'</td><td align="left" style="white-space: nowrap"><div id="problema_combo">'.selecionar_problema_para_ajax($acao_id, 'tipo_problema', 'size="1" style="width:240px;" class="texto"', '', $tipo_problema, false).'</div></td></tr>';
	$filtro_status='<tr><td align="right" style="white-space: nowrap">'.dica('Status', 'Filtre os problemas pelo status.').'Status:'.dicaF().'</td><td align="left" style="white-space: nowrap">'. selecionaVetor($status, 'status_id', 'size="1" style="width:160px;" class="texto"', $status_id) .'</td></tr>';
	...
	$botoesTitulo->adicionaCelula('<td align="right" style="white-space: nowrap">'.dica('Imprimir Problemas', 'Clique neste ícone '.imagem('imprimir_p.png').' para imprimir a lista de problemas.').'<a href="javascript: void(0);" onclick ="url_passar(1, \'m=social&a='.$a.'&dialogo=1\');">'.imagem('imprimir_p.png').'</a>'.dicaF());
