...
if ($social_familia_id && !$podeEditar) $Aplic->redirecionar('m=publico&a=acesso_negado');
elseif (!$social_familia_id && !$podeAdicionar) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
echo '<tr><td align="right">'.dica('Telefone Principal', 'O telefone principal d'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'Telefone Principal:'.dicaF().'</td><td><input type="text" class="texto" name="social_familia_tel" value="'.$obj->social_familia_tel.'" maxlength="9" size="25" /></div></td></tr>';
...
echo '<tr><td align="right">'.dica('Data de Nascimento', 'A data de nascimento d'.$config['genero_beneficiario'].' '.$config['beneficiario'].' no formato <b>(dd/mm/aaaa)</b>.').'Nascimento:'.dicaF().'</td><td style="white-space: nowrap"><input type="text" class="texto" name="social_familia_nascimento" onchange="setData(\'env\', \'social_familia_nascimento\');" value="'.($obj->social_familia_nascimento && $obj->social_familia_nascimento !='0000-00-00' ? $data->format('%d/%m/%Y') : '').'" maxlength="10" size="14" onkeyup="barra(this)" />(dd/mm/aaaa)</td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Telefone Reserva', 'O telefone residencial d'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'Telefone Reserva:'.dicaF().'</td><td><input type="text" class="texto" name="social_familia_tel2" value="'.$obj->social_familia_tel2.'" maxlength="9" size="25" /></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Celular', 'O celular d'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'Celular:'.dicaF().'</td><td><input type="text" class="texto" name="social_familia_cel" value="'.$obj->social_familia_cel.'" maxlength="9" size="25" /></td></tr>';
echo '<tr><td align="right">'.dica('e-mail', 'O e-mail d'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'e-mail:'.dicaF().'</td><td style="white-space: nowrap"><input type="text" class="texto" name="social_familia_email" value="'.$obj->social_familia_email.'" maxlength="255" size="25" /></td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Necessita do Bolsa Família', 'O beneficiário necessita do Bolsa Família.').'Necessita do Bolsa Família:'.dicaF().'</td><td><input type="checkbox" value="1" name="social_familia_necessita_bolsa" '.($obj->social_familia_necessita_bolsa ? 'checked="checked"' : '').' /></td></tr>';
...
function enviarDados() {
	...
	else if (document.getElementById('social_familia_municipio').value.length < 3) {
		alert('Preencha o município do beneficiário.');
		document.getElementById('social_familia_municipio').focus();
		}
	else if (document.getElementById('social_familia_comunidade').value.length < 3) {
