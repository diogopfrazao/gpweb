...
global $sql, $perms, $Aplic, $tab, $status_id, $dialogo, $estado_sigla, $estado, $municipios_superintendencia, $relatorio_id, $municipio_id , $social_id, $acao_id, $social_comunidade_id, $social_familia_id, $opcao_id;
...
if ($opcao_id=='problema_nacional_beneficiario') $tipo=0;
elseif ($opcao_id=='problema_nacional_comunidade') $tipo=4;
elseif ($opcao_id=='problema_nacional_municipio') $tipo=3;
elseif ($opcao_id=='problema_nacional_estado') $tipo=2;
elseif ($opcao_id=='problema_nacional_nacional') $tipo=1;

echo '<table cellpadding=0 cellspacing=0 align=center>';
echo '<tr><td align=center><h1>Lista dos Problemas '.($tipo==0 ? 'n'.$config['genero_beneficiario'].'s '.ucfirst($config['beneficiarios']) : '').($tipo==4 ? 'nas Comissões Comunitárias' : '').($tipo==3 ? 'nos Comitês Municipais' : '').($tipo==2 ? 'nas Coordenações Regionais' : '').($tipo==1 ? 'no Comitê Nacional' : '').'</h1><br></td></tr>';
...
foreach($tipos_problema as $problema){
	if ($tipo==0){
		...
		$sql->adCampo('count(social_familia_problema_tipo) AS total');
		...
		$sql->limpar();
		foreach($vetor_comunidade as $linha) $resultado['Nacional'][$problema['social_acao_problema_id']]=$linha['total'];
		}
		
	if ($tipo==4||$tipo==3||$tipo==2||$tipo==1){
		//comitês
		...
		$sql->adCampo('count(social_comite_problema_tipo) AS total');
		$sql->adOnde('social_acao_social='.(int)$social_id);
