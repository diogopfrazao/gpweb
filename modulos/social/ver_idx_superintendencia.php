...
global $m, $a, $u, $estilo_interface, $sql, $perms, $Aplic, $tab, $ordem, $ordenar, $dialogo, $estado_sigla, $municipio_id , $social_id, $acao_id, $social_superintendencia_id, $pesquisa;
...
mostrarBarraNav($xtotalregistros, $xtamanhoPagina, $xtotal_paginas, $pagina, 'Superintendência', 'Superintendências','','&ordenar='.$ordenar.'&ordem='.$ordem,($estilo_interface=='classico' ? 'a6a6a6' : '006fc2'));
echo '<table width="100%" cellpadding=0 cellspacing=0 class="tbl1">';
echo '<tr>';

if (!$impressao && !$dialogo) echo '<th style="white-space: nowrap">&nbsp;</th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_superintendencia_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_superintendencia_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome', 'Neste campo fica o nome d'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'Nome'.dicaF().'</a></th>';
echo '</tr>';
$fp = -1;
$id = 0;
$qnt=0;
...
for ($i = 0; $i < count($social); $i++) {
	...
	$qnt++;
	echo '<tr>';
	if (!$impressao && !$dialogo) echo '<td style="white-space: nowrap" width="20">'.($Aplic->usuario_super_admin || $Aplic->checarModulo('social', 'adicionar', $Aplic->usuario_id, 'cria_comite') ? dica('Editar Superintendência', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar a superintendência.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a=superintendencia_editar&social_superintendencia_id='.(int)$linha['social_superintendencia_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
