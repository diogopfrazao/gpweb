...
if ($excluiranexo){
	$sql->adTabela('social_acao_arquivo');
	$sql->adCampo('indicador_valor_arquivo_local, social_acao_arquivo_endereco');
	...
	else @unlink($base_dir.'/arquivos/acoes_superintendencias/'.$arquivo['social_acao_arquivo_endereco']);
...
echo '<tr><td colspan=20><table cellpadding=0 cellspacing=2><tr><td align="right">'.dica('Programa Social', 'Escolha o programa social no qual há uma ação social à inserir nesta superintendência.').'Programa:'.dicaF().'</td><td align="left" style="white-space: nowrap">'.selecionaVetor($lista_programas, 'social_id', 'size="1" style="width:160px;" class="texto" onchange="mudar_acao()"', $social_id).'</td><td align="right">&nbsp;&nbsp;'.dica('Ação Social', 'Escolha a ação social à inserir nesta superintendência.').'Ação:'.dicaF().'</td><td align="left" style="white-space: nowrap"><div id="acao_combo">'.selecionar_acao_para_ajax($social_id, 'acao_id', 'size="1" style="width:160px;" class="texto"', '', $acao_id, false).'</div></td><td><a href="javascript:void(0);" onclick="javascript:inserir_acao();">'.imagem('icones/adicionar.png', 'Inserir', 'Clique neste ícone '.imagem('icones/adicionar.png').' para adicionar a ação social à esquerda na superintendência.').'</a></td></tr></table></td></tr>';
