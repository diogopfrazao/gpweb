...
global $m, $a, $u, $estilo_interface, $podeEditar, $sql, $perms, $Aplic, $tab, $ordem, $ordenar, $dialogo, $estado_sigla, $municipio_id , $municipios_superintendencia, $social_id, $acao_id, $social_comunidade_id, $social_familia_id, $pesquisa;
...
mostrarBarraNav($xtotalregistros, $xtamanhoPagina, $xtotal_paginas, $pagina, ucfirst($config['beneficiario']), ucfirst($config['beneficiarios']),'','&ordenar='.$ordenar.'&ordem='.$ordem,($estilo_interface=='classico' ? 'a6a6a6' : '006fc2'));
echo '<table width="100%" cellpadding=0 cellspacing=0 class="tbl1">';
echo '<tr>';

if (!$impressao && !$dialogo) echo '<th style="white-space: nowrap">&nbsp;</th>';
echo '<th style="white-space: nowrap">&nbsp;</th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_familia_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome', 'Neste campo fica o nome '.($tab==1 ? 'da pessoa jurídica' : 'd'.$config['genero_beneficiario'].' '.$config['beneficiario']).'.').'Nome'.dicaF().'</a></th>';

if ($tab==1){
	echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_familia_cnpj&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_cnpj' ? imagem('icones/'.$seta[$ordem]) : '').dica('CNPJ', 'Neste campo fica o CNPJ da pessoa jurídica.').'CNPJ'.dicaF().'</a></th>';
	echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_familia_cnes&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_cnes' ? imagem('icones/'.$seta[$ordem]) : '').dica('CNES', 'Neste campo fica o CNES da pessoa jurídica, se for o caso.').'CNES'.dicaF().'</a></th>';
	echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_familia_inep&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_inep' ? imagem('icones/'.$seta[$ordem]) : '').dica('INEP', 'Neste campo fica o INEP da pessoa jurídica, se for o caso.').'INEP'.dicaF().'</a></th>';
	}
else{
	echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_familia_nis&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_nis' ? imagem('icones/'.$seta[$ordem]) : '').dica('NIS', 'Neste campo fica o Número de Inscrição Social (NIS) d'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'NIS'.dicaF().'</a></th>';
	echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_familia_conjuge&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_conjuge' ? imagem('icones/'.$seta[$ordem]) : '').dica('Cônjuge', 'O cônjugue d'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'Cônjuge'.dicaF().'</a></th>';
	}	
echo '</tr>';
...
foreach($social as $linha) {
	$qnt++;
	echo '<tr>';
	if (!$impressao && !$dialogo) echo '<td style="white-space: nowrap" width="20">'.($Aplic->usuario_super_admin || $Aplic->checarModulo('social', 'adicionar', $Aplic->usuario_id, 'cria_familia') ? dica('Editar '.ucfirst($config['beneficiario']), 'Clique neste ícone '.imagem('icones/editar.gif').' para editar '.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a=familia_editar&social_familia_id='.$linha['social_familia_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
