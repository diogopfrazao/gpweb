...
global $Aplic, $social_id, $podeExcluir, $m, $tab;
...
echo '<th width="1%" style="white-space: nowrap">'.dica('Filtro', 'Selecione de qual '.$config['usuario'].' deseja ver os registros de cadastrados.').'Filtro'.dicaF().'</th><th><input type="hidden" id="usuario_id" name="usuario_id" value="'.$usuario_id.'" /><input type="text" id="nome_usuario" name="nome_usuario" value="'.nome_om($usuario_id,$Aplic->getPref('om_usuario')).'" style="width:284px;" class="texto" READONLY /></th><th><a href="javascript: void(0);" onclick="popUsuario();">'.imagem('icones/usuarios.gif','Selecionar '.ucfirst($config['usuario']),'Clique neste ícone '.imagem('icones/usuarios.gif').' para selecionar '.($config['genero_usuario']=='o' ? 'um' : 'uma').' '.$config['usuario'].'.').'</a></th>';
...
echo '<th width="100"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$m.'&tab='.$tab.'&social_id='.$social_id.'&ordenar=social_acao_log_nome&ordem='.($ordem ? '0' : '1').'\');">'.dica('Nome', 'Nome do registro d'.$config['genero_tarefa'].' '.$config['tarefa'].'.').($ordenar=='social_acao_log_nome' ? imagem('icones/'.$seta[$ordem]) : '').'Título'.dicaF().'</a></th>';
...
foreach ($logs as $linha) {
	...
	echo '</td><td style="white-space: nowrap" >'.retorna_data($linha['social_acao_log_data'], false).'</td>';
	$imagem_referencia = '-';
	...
	echo '<td style="white-space: nowrap;'.$estilo.'">'.($linha['social_acao_log_nome'] ? $linha['social_acao_log_nome'] : '&nbsp;').'</td>';
	echo '<td>'.link_usuario($linha['social_acao_log_criador'],'','','esquerda').'</td>';
	echo '<td width="100" align="right">'.($linha['social_acao_log_horas'] ? number_format($linha['social_acao_log_horas'], 2, ',', '.') : '&nbsp;').'</td>';
