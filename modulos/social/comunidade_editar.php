...
if ($social_comunidade_id && !$podeEditar) $Aplic->redirecionar('m=publico&a=acesso_negado');
elseif (!$social_comunidade_id && !$podeAdicionar) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
if ($saida_quem) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Quem', 'Quais '.$config['usuarios'].' estarão trabalhando junto a esta comunidade.').'Quem:'.dicaF().'</td><td width="100%" colspan="2"><table><tr><td>'.$saida_quem.'</td></tr></table></td></tr>';
...
if ($saida_depts) echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['departamentos']), 'Qual '.strtolower($config['departamento']).' está relacionad'.$config['genero_dept'].' à esta comunidade.').ucfirst($config['departamento']).':'.dicaF().'</td><td width="100%" colspan="2">'.$saida_depts.'</td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Cor', 'Cor selecionada dentre as 16 milhões possíveis. Pode-se escrever diretamente o hexadecinal na cor ou utilizar a interface que se abre ao clicar na caixa de inserção do valor.').'Cor:'.dicaF().'</td><td align="left" style="white-space: nowrap"><input class="jscolor" name="social_comunidade_cor" value="'.($obj->social_comunidade_cor ? $obj->social_comunidade_cor : 'FFFFFF').'" '.($config['selecao_cor_restrita'] ? 'readonly="readonly" ' : '').'size="6" maxlength="6" style="width:57px;" /></td></tr>';
