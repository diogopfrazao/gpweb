...
global $m, $a, $u, $estilo_interface, $Aplic, $social_id, $tab;
...
echo '<tr>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($social_id ? '&social_id='.$social_id  : '').($tab ? '&tab='.$tab : '').'&ordenar=projeto_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenarPor=='projeto_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome', 'Neste campo fica um nome para identificação d'.$config['genero_projeto'].' '.$config['projeto'].'.').ucfirst($config['projeto']).dicaF().'</th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($social_id ? '&social_id='.$social_id  : '').($tab ? '&tab='.$tab : '').'&ordenar=tarefa_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenarPor=='tarefa_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome', 'Neste campo fica um nome para identificação d'.$config['genero_tarefa'].' '.$config['tarefa'].'.').ucfirst($config['tarefa']).dicaF().'</th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($social_id ? '&social_id='.$social_id  : '').($tab ? '&tab='.$tab : '').'&ordenar=tarefa_dono&ordem='.($ordem ? '0' : '1').'\');">'.($ordenarPor=='tarefa_dono' ? imagem('icones/'.$seta[$ordem]) : '').dica('Responsável', 'Neste campo fica o responsável pel'.$config['genero_tarefa'].' '.$config['tarefa'].'.').'Responsável'.dicaF().'</th>';
