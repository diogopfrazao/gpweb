...
global $m, $a, $u, $estilo_interface, $sql, $perms, $Aplic, $tab, $ordem, $ordenar, $dialogo, $estado_sigla, $municipio_id , $pesquisa;
...
mostrarBarraNav($xtotalregistros, $xtamanhoPagina, $xtotal_paginas, $pagina, 'Comunidade', 'Comunidades','','&ordenar='.$ordenar.'&ordem='.$ordem,($estilo_interface=='classico' ? 'a6a6a6' : '006fc2'));
...
echo '<th width=16><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_comunidade_cor&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_comunidade_cor' ? imagem('icones/'.$seta[$ordem]) : '').dica('Cor da Comunidade', 'Neste campo fica a cor de identificação da comunidade.').'Cor'.dicaF().'</a></th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_comunidade_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_comunidade_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome da Comunidade', 'Neste campo fica um nome para identificação da comunidade.').'Nome'.dicaF().'</a></th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_comunidade_descricao&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_comunidade_descricao' ? imagem('icones/'.$seta[$ordem]) : '').dica('Descrição da Comunidade', 'Neste campo fica a descrição da comunidade.').'Descrição'.dicaF().'</a></th>';
echo '</tr>';
$fp = -1;
$id = 0;
$qnt=0;
for ($i = 0; $i < count($comunidade); $i++) {
	...
	echo '<tr>';
	if (!$impressao && !$dialogo) echo '<td style="white-space: nowrap" width="20">'.($Aplic->usuario_super_admin || $Aplic->checarModulo('social', 'adicionar', $Aplic->usuario_id, 'cria_comunidade') ? dica('Editar Social', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o programa social.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a=comunidade_editar&social_comunidade_id='.$linha['social_comunidade_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
