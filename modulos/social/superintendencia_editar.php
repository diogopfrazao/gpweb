...
if ($social_superintendencia_id && !$podeEditar) $Aplic->redirecionar('m=publico&a=acesso_negado');
elseif (!$social_superintendencia_id && !$podeAdicionar) $Aplic->redirecionar('m=publico&a=acesso_negado');
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica(ucfirst($config['organizacao']).' Responsável', 'Selecionar '.$config['genero_organizacao'].' '.$config['organizacao'].' que será encarregad'.$config['genero_organizacao'].' da superintendência.').ucfirst($config['organizacao']).':'.dicaF().'</td><td width="100%" style="white-space: nowrap" colspan="2"><div id="combo_cia">'.selecionar_om($obj->social_superintendencia_cia, 'social_superintendencia_cia', 'class=texto size=1 style="width:300px;" onchange="javascript:mudar_om();"').'</div></td></tr>';
...
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Telefone Principal', 'O telefone principal da superintendência.').'Telefone Principal:'.dicaF().'</td><td><input type="text" class="texto" name="social_superintendencia_tel" value="'.$obj->social_superintendencia_tel.'" maxlength="30" size="25" /></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Telefone Reserva', 'O telefone residencial da superintendência.').'Telefone Reserva:'.dicaF().'</td><td><input type="text" class="texto" name="social_superintendencia_tel2" value="'.$obj->social_superintendencia_tel2.'" maxlength="30" size="25" /></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Celular', 'O celular da superintendência.').'Celular:'.dicaF().'</td><td><input type="text" class="texto" name="social_superintendencia_cel" value="'.$obj->social_superintendencia_cel.'" maxlength="30" size="25" /></td></tr>';
echo '<tr><td align="right">'.dica('e-mail', 'O e-mail da superintendência.').'e-mail:'.dicaF().'</td><td style="white-space: nowrap"><input type="text" class="texto" name="social_superintendencia_email" value="'.$obj->social_superintendencia_email.'" maxlength="255" size="25" /></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Cor', 'Cor selecionada dentre as 16 milhões possíveis. Pode-se escrever diretamente o hexadecinal na cor ou utilizar a interface que se abre ao clicar na caixa de inserção do valor.').'Cor:'.dicaF().'</td><td align="left" style="white-space: nowrap"><input class="jscolor" name="social_superintendencia_cor" value="'.($obj->social_superintendencia_cor ? $obj->social_superintendencia_cor : 'FFFFFF').'" '.($config['selecao_cor_restrita'] ? 'readonly="readonly" ' : '').'size="6" maxlength="6" style="width:57px;" /></td></tr>';
echo '<tr><td align="right" style="white-space: nowrap">'.dica('Responsável pela Superintendência', 'Toda superintendência deve ter um responsável.').'Responsável:'.dicaF().'</td><td colspan="2"><input type="hidden" id="social_superintendencia_responsavel" name="social_superintendencia_responsavel" value="'.$obj->social_superintendencia_responsavel.'" /><input type="text" id="nome_gerente" name="nome_gerente" value="'.nome_om($obj->social_superintendencia_responsavel, $Aplic->getPref('om_usuario')).'" style="width:284px;" class="texto" READONLY /><a href="javascript: void(0);" onclick="popGerente();">'.imagem('icones/usuarios.gif','Selecionar '.ucfirst($config['usuario']),'Clique neste ícone '.imagem('icones/usuarios.gif').' para selecionar '.($config['genero_usuario']=='o' ? 'um' : 'uma').' '.$config['usuario'].'.').'</a></td></tr>';
...
if ($saida_quem) echo '<tr><td align="right" style="white-space: nowrap">'.dica('Membros', 'Quais '.$config['usuarios'].' são membros deste superintendência.').'Membros:'.dicaF().'</td><td width="100%" colspan="2"><table><tr><td>'.$saida_quem.'</td></tr></table></td></tr>';

echo '<tr><td align="right" style="white-space: nowrap"></td><td width="100%" colspan="2"><table><tr><td>'.botao('membros', 'Membros','Abrir uma janela onde poderá selecionar quais serão os membros deste superintendência.','','popUsuarios()').'</td></tr></table></td></tr>';


echo '<tr><td align="right" style="white-space: nowrap"></td><td valign="top"><table><tr><td>'.botao('municípios', 'Municípios','Abrir uma janela onde poderá selecionar quais serão os municípios desta superintendência.','','popMunicipios()').'</td></tr></table></td></tr>';
...
function popMunicipios() {
	if (window.parent.gpwebApp) parent.gpwebApp.popUp('Municípios', 1000, 700, 'm=publico&a=selecionar_municipios&dialogo=1&chamar_volta=setMunicipios&valores='+municipios_selecionados, null, window);
