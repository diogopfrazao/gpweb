...
mostrarBarraNav($xtotalregistros, $xtamanhoPagina, $xtotal_paginas, $pagina, 'Comitê', 'Comitês','','&ordenar='.$ordenar.'&ordem='.$ordem,($estilo_interface=='classico' ? 'a6a6a6' : '006fc2'));
...
for ($i = 0; $i < count($social); $i++) {
	...
	$qnt++;
	echo '<tr>';
	if (!$impressao && !$dialogo) echo '<td style="white-space: nowrap" width="20">'.($Aplic->usuario_super_admin || $Aplic->checarModulo('social', 'adicionar', $Aplic->usuario_id, 'cria_comite') ? dica('Editar Comitê', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o comitê.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a=comite_editar&social_comite_id='.$linha['social_comite_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
...
if (!$qnt) echo '<tr><td colspan=20><p>Nenhum comitê encontrado.</p></td></tr>';
echo '</table>';
