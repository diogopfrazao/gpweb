...
if ($excluiranexo){
	...
	$sql->adOnde('social_acao_arquivo_id='.(int)$social_acao_arquivo_id);
	$arquivo=$sql->linha();
	$sql->limpar();
	if ($arquivo['social_acao_arquivo_local']) @unlink($base_dir.'/arquivos/'.$arquivo['indicador_valor_arquivo_local'].$arquivo['indicador_valor_arquivo_nome_real']);
	else @unlink($base_dir.'/arquivos/familias/'.$arquivo['social_acao_arquivo_endereco']);
...
echo '<tr><td colspan=20><table cellpadding=0 cellspacing=2><tr><td align="right">'.dica('Programa Social', 'Escolha o programa social no qual há uma ação social à inserir n'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'Programa:'.dicaF().'</td><td align="left" style="white-space: nowrap">'.selecionaVetor($lista_programas, 'social_id', 'size="1" style="width:160px;" class="texto" onchange="mudar_acao()"', $social_id).'</td><td align="right">&nbsp;&nbsp;'.dica('Ação Social', 'Escolha a ação social à inserir n'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'Ação:'.dicaF().'</td><td align="left" style="white-space: nowrap"><div id="acao_combo">'.selecionar_acao_para_ajax($social_id, 'acao_id', 'size="1" style="width:160px;" class="texto"', '', $acao_id, false).'</div></td><td><a href="javascript:void(0);" onclick="javascript:inserir_acao();">'.imagem('icones/adicionar.png', 'Inserir', 'Clique neste ícone '.imagem('icones/adicionar.png').' para adicionar a ação social à esquerda n'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'</a></td></tr></table></td></tr>';
...
echo '<tr><td colspan=20><table cellpadding=0 cellspacing=2><tr><td align="right">'.dica('Programa Social', 'Escolha o programa social no qual há uma ação social que será negada a '.($config['genero_beneficiario']=='o' ? 'este' : 'esta').' '.$config['beneficiario'].'.').'Programa:'.dicaF().'</td><td align="left" style="white-space: nowrap">'.selecionaVetor($lista_programas, 'social_id_negado', 'size="1" style="width:160px;" class="texto" onchange="mudar_acao_negado();"', $social_id_negado).'</td><td align="right">&nbsp;&nbsp;'.dica('Ação Social Negada', 'Escolha a ação social foi negada a '.($config['genero_beneficiario']=='o' ? 'este' : 'esta').' '.$config['beneficiario'].'.').'Ação:'.dicaF().'</td><td align="left" style="white-space: nowrap"><div id="acao_combo_negado">'.selecionar_acao_para_ajax($social_id_negado, 'acao_id_negado', 'size="1" style="width:160px;" class="texto" onchange="lista_negacao();"', '', $acao_id_negado, false).'</div></td><td><a href="javascript:void(0);" onclick="javascript:inserir_acao_negado();">'.imagem('icones/adicionar.png', 'Inserir', 'Clique neste ícone '.imagem('icones/adicionar.png').' para adicionar a ação social que foi negada à beneficiário.').'</a></td></tr></table></td></tr>';
echo '<tr><td colspan=20><table cellpadding=0 cellspacing=2><tr><td align="right">'.dica('Justificativa', 'Escolha a justificativa para a negação desta ação social à beneficiário.').'Justificativa:'.dicaF().'</td><td align="left" style="white-space: nowrap"><div id="combo_justificativa">'.selecionar_acao_negacao_para_ajax($acao_id_negado, 'negacao_id', 'size="1" style="width:355px;" class="texto"', '', '', false).'</div></td></tr></table></td></tr>';
echo '</table></fieldset></td></tr>';
...
echo '<tr><td align="right" width="100" style="white-space: nowrap">'.dica('Nome Completo', 'Nome completo d'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'Nome completo:'.dicaF().'</td><td class="realce" style="text-align: justify;">'.$obj->social_familia_nome.'</td></tr>';
