...
global $m, $a, $u, $estilo_interface, $sql, $perms, $Aplic, $tab, $ordem, $ordenar, $dialogo, $status, $estado_sigla, $municipio_id , $social_id, $acao_id, $social_familia_comunidade_id, $social_familia_id, $status_id, $tipo_problema, $pesquisa;
...
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_familia_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome', 'Neste campo fica o nome d'.$config['genero_beneficiario'].' '.$config['beneficiario'].'.').'Nome'.dicaF().'</a></th>';
...
if (!$status_id) echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_familia_problema_status&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_problema_status' ? imagem('icones/'.$seta[$ordem]) : '').dica('Status', 'O status do problemao.').'Status'.dicaF().'</a></th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_familia_problema_usuario_status&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_problema_usuario_status' ? imagem('icones/'.$seta[$ordem]) : '').dica('Responsável', 'O responsável por gerenciar p problema.').'Responsável'.dicaF().'</a></th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_familia_problema_data_status&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_problema_data_status' ? imagem('icones/'.$seta[$ordem]) : '').dica('Data', 'O data da última mudança de status.').'Data'.dicaF().'</a></th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_familia_problema_observacao&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_familia_problema_observacao' ? imagem('icones/'.$seta[$ordem]) : '').dica('Observação', 'Observação sobre o problema.').'Observação'.dicaF().'</a></th>';
...
for ($i = 0; $i < count($social); $i++) {
	...
	$qnt++;
	echo '<tr>';
	if (!$impressao && !$dialogo) echo '<td style="white-space: nowrap" width="20">'.($podeEditar ? dica('Marcar Problema', 'Clique nesta caixa para marcar este problema e depois selecione o novo status a ser registrado.').'<input type="checkbox" value="'.$linha['social_familia_problema_id'].'" name="marcado[]" />' : '&nbsp;').'</td>';
