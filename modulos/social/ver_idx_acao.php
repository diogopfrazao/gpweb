...
mostrarBarraNav($xtotalregistros, $xtamanhoPagina, $xtotal_paginas, $pagina, 'Ação', 'Ações','','&ordenar='.$ordenar.'&ordem='.$ordem,($estilo_interface=='classico' ? 'a6a6a6' : '006fc2'));
...
echo '<th width=16><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_acao_cor&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_acao_cor' ? imagem('icones/'.$seta[$ordem]) : '').dica('Cor da Ação Social', 'Neste campo fica a cor de identificação da ação social.').'Cor'.dicaF().'</a></th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_acao_nome&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_acao_nome' ? imagem('icones/'.$seta[$ordem]) : '').dica('Nome da Ação Socia', 'Neste campo fica um nome para identificação da ação social.').'Nome'.dicaF().'</a></th>';
echo '<th style="white-space: nowrap"><a class="hdr" href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a='.$a.'&u='.$u.($tab ? '&tab='.$tab : '').'&ordenar=social_acao_descricao&ordem='.($ordem ? '0' : '1').'\');">'.($ordenar=='social_acao_descricao' ? imagem('icones/'.$seta[$ordem]) : '').dica('Descrição da Ação Socia', 'Neste campo fica a descrição da ação social.').'Descrição'.dicaF().'</a></th>';
echo '</tr>';
$fp = -1;
$id = 0;
$qnt=0;
for ($i = 0; $i < count($acao); $i++) {
	...
	echo '<tr>';
	if (!$impressao && !$dialogo) echo '<td style="white-space: nowrap" width="20">'.($Aplic->usuario_super_admin || $Aplic->checarModulo('social', 'adicionar', $Aplic->usuario_id, 'cria_acao') ? dica('Editar Social', 'Clique neste ícone '.imagem('icones/editar.gif').' para editar o programa social.').'<a href="javascript:void(0);" onclick="url_passar(0, \'m='.$m.'&a=acao_editar&social_acao_id='.$linha['social_acao_id'].'\');">'.imagem('icones/editar.gif').'</a>'.dicaF() : '&nbsp;').'</td>';
